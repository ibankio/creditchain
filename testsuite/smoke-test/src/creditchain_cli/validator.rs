...
use creditchain_crypto::{bls12381, ed25519::Ed25519PrivateKey, x25519, ValidCryptoMaterialStringExt};
use creditchain_forge::{reconfig, wait_for_all_nodes_to_catchup, LocalSwarm, NodeExt, Swarm, SwarmExt};
use creditchain_genesis::config::HostAndPort;
use creditchain_keygen::KeyGen;
use creditchain_logger::info;
use creditchain_rest_client::{Client, State};
...
async fn assert_reordering(swarm: &mut dyn Swarm, expected_reordering: bool) {
    swarm
        ...
        .await;
    let transaction_factory = swarm.creditchain_public_info().transaction_factory();
    ...
    for _ in 0..2 {
        ...
        for _ in 0..5 {
            let txn = account.sign_with_transaction_builder(
                transaction_factory.payload(creditchain_stdlib::creditchain_coin_transfer(dst.address(), 10)),
            );
            txns.push(txn);
        }
    ...
    for txn in committed_order {
        match txn.transaction {
            ...
            creditchain_types::transaction::Transaction::BlockMetadata(b) => {
