...
use crate::creditchain_cli::validator::generate_blob;
...
use creditchain_forge::{reconfig, LocalSwarm, NodeExt, Swarm, SwarmExt};
use creditchain_rest_client::{Client as RestClient, Client};
use creditchain_sdk::{
...
pub async fn transfer_coins_non_blocking(
    ...
    let txn = sender.sign_with_transaction_builder(transaction_factory.payload(
        creditchain_stdlib::creditchain_coin_transfer(receiver.address(), amount),
    ));
...
pub async fn check_create_mint_transfer_node(swarm: &mut LocalSwarm, idx: usize) {
    ...
    let mut info = swarm.creditchain_public_info_for_node(idx);
...
#[cfg(test)]
pub mod swarm_utils {
    use creditchain_config::config::{NodeConfig, SecureBackend, WaypointConfig};
    ...
    pub fn insert_waypoint(node_config: &mut NodeConfig, waypoint: Waypoint) {
        ...
        let f = |backend: &SecureBackend| {
            ...
            storage
                .set(creditchain_global_constants::WAYPOINT, waypoint)
                .expect("Unable to write waypoint");
            storage
                .set(creditchain_global_constants::GENESIS_WAYPOINT, waypoint)
                .expect("Unable to write waypoint");
        };
