...
async fn test_gas_check() {
    ...
    let transfer_txn = account1.sign_with_transaction_builder(
        info.transaction_factory()
            .payload(creditchain_stdlib::creditchain_coin_transfer(account2.address(), 100)),
    );
    ...
    let transfer_too_much = account2.sign_with_transaction_builder(
        // TODO(Gas): double check this
        info.transaction_factory()
            .payload(creditchain_stdlib::creditchain_coin_transfer(account1.address(), 1_000)),
    );
    ...
    let mut gas_params = CreditChainGasParameters::initial();
    ...
    // via creditchain_stdlib.

    let update_txn = info
        .root_account()
        .sign_with_transaction_builder(txn_factory.payload(
            creditchain_stdlib::gas_schedule_set_gas_schedule(gas_schedule_blob),
        ));
