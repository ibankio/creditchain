...
use creditchain_types::CoinType;
...
async fn test_account_auto_creation() {
    ...
    let migrate_txn = account1.sign_with_transaction_builder(info.transaction_factory().payload(
        creditchain_stdlib::coin_migrate_to_fungible_store(creditchain_types::CreditChainCoinType::type_tag()),
    ));
    ...
    let send_fa_txn = account1.sign_with_transaction_builder(info.transaction_factory().payload(
        creditchain_stdlib::creditchain_account_fungible_transfer_only(account2.address(), 10_000_000_000),
    ));
