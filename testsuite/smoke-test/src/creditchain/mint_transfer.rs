...
async fn test_mint_transfer() {
    ...
    let transfer_txn = account1.sign_with_transaction_builder(
        info.transaction_factory()
            .payload(creditchain_stdlib::creditchain_coin_transfer(account2.address(), 40000)),
    );
    ...
    let delegate_txn1 = info
        ...
        .sign_with_transaction_builder(txn_factory.payload(
            creditchain_stdlib::creditchain_coin_delegate_mint_capability(account1.address()),
        ));
    ...
    let delegate_txn2 = info
        .root_account()
        .sign_with_transaction_builder(txn_factory.payload(
            creditchain_stdlib::creditchain_coin_delegate_mint_capability(account2.address()),
        ));
    info.client().submit_and_wait(&delegate_txn2).await.unwrap();

    let claim_txn = account1.sign_with_transaction_builder(
        txn_factory.payload(creditchain_stdlib::creditchain_coin_claim_mint_capability()),
    );
    info.client().submit_and_wait(&claim_txn).await.unwrap();
    let mint_txn = account1.sign_with_transaction_builder(
        txn_factory.payload(creditchain_stdlib::creditchain_coin_mint(account1.address(), 10000)),
    );
    info.client().submit_and_wait(&mint_txn).await.unwrap();

    // Testing the CreditChainDebugger by reexecuting the transaction that has been published.
    println!("Testing....");
    let debugger = CreditChainDebugger::rest_client(info.client().clone()).unwrap();
