...
use creditchain_retrier::fixed_retry_strategy;
use creditchain_sdk::{
...
impl K8sSwarm {
    pub async fn new(
        ...
        let address = creditchain_sdk::types::account_config::creditchain_test_root_address();
        ...
        let query = "container_memory_usage_bytes{pod=\"creditchain-node-0-validator-0\"}";
...
fn parse_service_name_from_stateful_set_name(
    ...
    let re = Regex::new(r"(creditchain-node-\d+)-(validator|fullnode)").unwrap();
...
// e.g. creditchain-node-<idx>-validator
// e.g. creditchain-node-<idx>-fullnode-e<era>
fn parse_node_index(s: &str) -> Result<usize> {
    // first get rid of the prefixes
    let v = s.split("creditchain-node-").collect::<Vec<&str>>();
    if v.len() < 2 {
...
pub async fn nodes_healthcheck(nodes: Vec<&K8sNode>) -> Result<Vec<String>> {
    ...
    for node in nodes {
        ...
        let check = creditchain_retrier::retry_async(k8s_wait_nodes_strategy(), || {
            Box::pin(async move {
...
mod tests {
    ...
    fn test_parse_service_name_from_stateful_set_name() {
        let validator_sts_name = "creditchain-node-19-validator";
        let validator_service_name =
            parse_service_name_from_stateful_set_name(validator_sts_name, false);
        assert_eq!("creditchain-node-19-validator", &validator_service_name);
        // with haproxy
        let validator_service_name =
            parse_service_name_from_stateful_set_name(validator_sts_name, true);
        assert_eq!("creditchain-node-19-validator-lb", &validator_service_name);

        let fullnode_sts_name = "creditchain-node-0-fullnode-eforge195";
        let fullnode_service_name =
            parse_service_name_from_stateful_set_name(fullnode_sts_name, false);
        assert_eq!("creditchain-node-0-fullnode", &fullnode_service_name);
        // with haproxy
        let fullnode_service_name =
            parse_service_name_from_stateful_set_name(fullnode_sts_name, true);
        assert_eq!("creditchain-node-0-fullnode-lb", &fullnode_service_name);
    }
