...
use creditchain_rest_client::{Client as RestClient, PendingTransaction, State, Transaction};
use creditchain_sdk::{
...
impl<'t> CreditChainContext<'t> {
    ...
    pub fn transaction_factory(&self) -> TransactionFactory {
        let unit_price = std::cmp::max(creditchain_global_constants::GAS_UNIT_PRICE, 1);
        TransactionFactory::new(self.chain_id()).with_gas_unit_price(unit_price)
    }

    pub fn creditchain_transaction_factory(&self) -> TransactionFactory {
...
#[derive(Clone)]
pub struct CreditChainPublicInfo {
    chain_id: ChainId,
...
impl CreditChainPublicInfo {
        ...
        let create_account_txn =
            self.root_account
                .sign_with_transaction_builder(self.transaction_factory().payload(
                    creditchain_stdlib::creditchain_account_create_account(auth_key.account_address()),
                ));
        self.rest_client
    ...
    pub async fn mint(&mut self, addr: AccountAddress, amount: u64) -> Result<()> {
        let mint_txn = self.root_account.sign_with_transaction_builder(
            self.transaction_factory()
                .payload(creditchain_stdlib::creditchain_coin_mint(addr, amount)),
        );
    ...
    pub async fn transfer_non_blocking(
        ...
        let tx = from_account.sign_with_transaction_builder(self.transaction_factory().payload(
            creditchain_stdlib::creditchain_coin_transfer(to_account.address(), amount),
        ));
    ...
    pub async fn get_approved_execution_hash_at_creditchain_governance(
        &self,
        proposal_id: u64,
    ) -> Vec<u8> {
        let approved_execution_hashes = self
            ...
            .get_account_resource_bcs::<SimpleMap<u64, Vec<u8>>>(
                CORE_CODE_ADDRESS,
                "0x1::creditchain_governance::ApprovedExecutionHashes",
            )
            .await;
    ...
    pub async fn get_balance(&self, address: AccountAddress) -> u64 {
        self.rest_client
            .get_account_balance(address, "0x1::creditchain_coin::CreditChainCoin")
            .await
            .unwrap()
            .into_inner()
    }
...
pub async fn reconfig(
    ...
    let txns = {
        vec![root_account.sign_with_transaction_builder(
            transaction_factory
                .clone()
                .payload(creditchain_stdlib::creditchain_governance_force_end_epoch_test_only()),
        )]
    };
