...
def wipe_node_main(testnet_name: str, node_name: str, kubernetes: Kubernetes):
    ...
    ledger_db_path = path.join(util.CREDITCHAIN_DATA_DIR, "db", util.LEDGER_DB_NAME)
    state_db_path = path.join(util.CREDITCHAIN_DATA_DIR, "db", util.STATE_MERKLE_DB_NAME)
    state_sync_db_path = path.join(util.CREDITCHAIN_DATA_DIR, "db", util.STATE_SYNC_DB_NAME)
