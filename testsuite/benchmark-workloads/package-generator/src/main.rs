...
use anyhow::bail;
use creditchain_transaction_generator_lib::{create_prebuilt_packages_bundle, PrebuiltPackageConfig};
use clap::Parser;
use std::{
    fs, io,
    path::{Path, PathBuf},
...
struct Args {
    ...
    /// If true, uses local creditchain-framework from creditchain-core.
    #[clap(long)]
    use_local_std: bool,
}
