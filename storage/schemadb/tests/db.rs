...
fn open_db(dir: &creditchain_temppath::TempPath) -> DB {
...
fn open_db_read_only(dir: &creditchain_temppath::TempPath) -> DB {
    DB::open_cf_readonly(&rocksdb::Options::default(), dir.path(), "test", get_cfds())
        .expect("Failed to open DB.")
}

fn open_db_as_secondary(dir: &creditchain_temppath::TempPath, dir_sec: &creditchain_temppath::TempPath) -> DB {
...
struct TestDB {
    _tmpdir: creditchain_temppath::TempPath,
    db: DB,
}

impl TestDB {
    fn new() -> Self {
        let tmpdir = creditchain_temppath::TempPath::new();
...
fn collect_values<S: Schema>(db: &TestDB) -> Vec<(S::Key, S::Value)> {
    ...
    iter.seek_to_first();
    iter.collect::<Result<Vec<_>, CreditChainDbError>>().unwrap()
}
...
fn test_open_as_secondary() {
    let tmpdir = creditchain_temppath::TempPath::new();
    let tmpdir_sec = creditchain_temppath::TempPath::new();
...
fn test_checkpoint() {
    let tmpdir = creditchain_temppath::TempPath::new();
    let checkpoint = creditchain_temppath::TempPath::new();
    {
        let db = open_db(&tmpdir);
        db.put::<TestSchema1>(&TestField(0), &TestField(0)).unwrap();
