...
fn test_various_cmd_parsing() {
    ...
    run_cmd(&["creditchain-db-tool", "backup", "verify", "--local-fs-dir", "."]);
    run_cmd(&[
        "creditchain-db-tool",
        "replay-verify",
        "--target-db-dir",
...
mod dbtool_tests {
    ...
    use creditchain_executor_test_helpers::integration_test_impl::{
    ...
    fn db_restore_test_setup(
        ...
        use creditchain_db_indexer::utils::PrefixedStateValueIterator as IndexerPrefixedStateValueIterator;
        ...
        let creditchain_db: Arc<dyn DbReader> = Arc::new(
            CreditChainDB::open(
        ...
        let ledger_version = creditchain_db.get_latest_ledger_info_version().unwrap();
        for ver in start..=ledger_version {
            let old_block_res = db.get_block_info_by_version(ver);
            let new_block_res = creditchain_db.get_block_info_by_version(ver);
