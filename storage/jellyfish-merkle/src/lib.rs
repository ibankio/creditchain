...
use crate::metrics::{CREDITCHAIN_JELLYFISH_LEAF_COUNT, CREDITCHAIN_JELLYFISH_LEAF_DELETION_COUNT, COUNTER};
...
pub trait TreeReader<K> {
    ...
    fn get_node_with_tag(&self, node_key: &NodeKey, tag: &str) -> Result<Node<K>> {
        self.get_node_option(node_key, tag)?
            .ok_or_else(|| CreditChainDbError::NotFound(format!("Missing node at {:?}.", node_key)))
    }
    ...
    K: Key,
{
    ...
    pub fn put_top_levels_nodes(
        ...
        CREDITCHAIN_JELLYFISH_LEAF_COUNT.set(root_node.leaf_count() as i64);
    ...
    pub fn get_with_proof_ext(
        ...
        for nibble_depth in 0..=ROOT_NIBBLE_HEIGHT {
            let next_node = self
                ...
                .map_err(|err| {
                    if nibble_depth == 0 {
                        CreditChainDbError::MissingRootError(version)
                    } else {
                        err
                    }
                })?;
            match next_node {
                Node::Internal(internal_node) => {
                    ...
                    let queried_child_index = nibble_iter
                        .next()
                        .ok_or_else(|| CreditChainDbError::Other("ran out of nibbles".to_string()))?;
    ...
    fn get_root_node(&self, version: Version) -> Result<Node<K>> {
        self.get_root_node_option(version)?.ok_or_else(|| {
            CreditChainDbError::NotFound(format!("Root node not found for version {}.", version))
        })
    }
    ...
    K: Key,
{
    ...
    if kvs.len() == 1 && &kvs[0].0 == existing_leaf_key {
        if let (key, Some((value_hash, state_key))) = kvs[0] {
            ...
            CREDITCHAIN_JELLYFISH_LEAF_DELETION_COUNT.inc();
            Ok(None)
        }
    } else {
...
mod test {
    use super::NibbleExt;
    use creditchain_crypto::hash::{HashValue, TestOnlyHash};
