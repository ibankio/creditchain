...
use creditchain_storage_interface::{DbReader, Order, Result};
...
fn verify_epochs(db: &CreditChainDB, ledger_infos_with_sigs: &[LedgerInfoWithSignatures]) {
    const LIMIT: usize = 2;
...
pub fn test_save_blocks_impl(
    ...
    let db =
        CreditChainDB::new_for_test_with_buffered_state_target_items(&tmp_dir, snapshot_size_threshold);
