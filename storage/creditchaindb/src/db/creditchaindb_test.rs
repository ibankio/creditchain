...
use creditchain_storage_interface::{DbReader, Order};
...
fn test_pruner_config() {
    ...
    for enable in [false, true] {
        let state_merkle_pruner = StateMerklePrunerManager::<StaleNodeIndexSchema>::new(
            Arc::clone(&creditchain_db.state_merkle_db()),
        ...
        let ledger_pruner = LedgerPrunerManager::new(
            Arc::clone(&creditchain_db.ledger_db),
            LedgerPrunerConfig {
                enable,
                prune_window: 100,
                batch_size: 1,
...
fn test_error_if_version_pruned() {
    ...
    assert_eq!(
        db.error_if_state_merkle_pruned("State", 4)
            .unwrap_err()
            .to_string(),
        "CreditChainDB Other Error: Version 4 is not epoch ending."
    );
    ...
    assert_eq!(
        db.error_if_ledger_pruned("Transaction", 9)
            .unwrap_err()
            .to_string(),
        "CreditChainDB Other Error: Transaction at version 9 is pruned, min available version is 10."
    );
