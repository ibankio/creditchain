...
use creditchain_storage_interface::{
    ...
    CreditChainDbError, DbReader, DbWriter, LedgerSummary, MAX_REQUEST_LIMIT,
};
...
/// Alternate implementation of [CreditChainDB] for use with consensus-only-perf-test feature.
/// It stores and serves data from in-memory data structures as opposed to [CreditChainDB],
/// which uses RocksDB. Note that FakeCreditChainDB re-implements only a subset of the
/// features of [CreditChainDB] while passing through remaining features to the wrapped inner
/// [CreditChainDB].
pub struct FakeCreditChainDB {
    inner: CreditChainDB,
...
impl FakeCreditChainDB {
    pub fn new(db: CreditChainDB) -> Self {
        Self {
            inner: db,
    ...
    fn save_and_compute_root_hash(
        ...
        let (root_hash, writes) =
            MerkleAccumulator::<FakeCreditChainDB, TransactionAccumulatorHasher>::append(
                self,
    ...
    fn get_frozen_subtree_hashes(&self, num_transactions: u64) -> Result<Vec<HashValue>> {
        MerkleAccumulator::<FakeCreditChainDB, TransactionAccumulatorHasher>::get_frozen_subtree_hashes(
            self,
            num_transactions,
        )
        .map_err(Into::into)
...
impl DbWriter for FakeCreditChainDB {
    fn get_state_snapshot_receiver(
        &self,
        version: Version,
        expected_root_hash: HashValue,
    ) -> Result<Box<dyn creditchain_storage_interface::StateSnapshotReceiver<StateKey, StateValue>>> {
        self.inner
...
impl DbReader for FakeCreditChainDB {
    fn get_epoch_ending_ledger_infos(
        &self,
        start_epoch: u64,
        end_epoch: u64,
    ) -> Result<creditchain_types::epoch_change::EpochChangeProof> {
    ...
    fn get_events(
        &self,
        event_key: &creditchain_types::event::EventKey,
        start: u64,
        order: creditchain_storage_interface::Order,
        limit: u64,
        ledger_version: Version,
    ...
    fn get_block_timestamp(&self, version: Version) -> Result<u64> {
        gauged_api("get_block_timestamp", || {
            ...
            if timestamp > 0 {
                ...
                Err(CreditChainDbError::NotFound("NewBlockEvent".to_string()).into())
            }
        })
    }
    ...
    fn get_account_ordered_transaction(
        &self,
        address: creditchain_types::PeerId,
        seq_num: u64,
        include_events: bool,
        ledger_version: Version,
    ) -> Result<Option<TransactionWithProof>> {
    ...
    fn get_account_ordered_transactions(
        ...
        include_events: bool,
        ledger_version: Version,
    ) -> Result<creditchain_types::transaction::AccountOrderedTransactionsWithProof> {
    ...
    fn get_state_value_by_version(
        ...
        // Since the genesis write set is persisted with CreditChainDB, we call
        // it to serve state values targeting the framework account
        // (to access CreditChainCoin, for example).
    ...
    fn get_pre_committed_ledger_summary(&self) -> Result<LedgerSummary> {
        // If the genesis is not executed yet, we need to get the executed trees from the inner CreditChainDB
        // This is because when we call save_transactions for the genesis block, we call [CreditChainDB::save_transactions]
        // where there is an expectation that the root of the SMTs are the same pointers. Here,
        // we get from the inner CreditChainDB which ensures that the pointers match when save_transactions is called.
    ...
    fn get_latest_version(&self) -> Result<Version> {
        gauged_api("get_latest_version", || {
            self.latest_version
                .lock()
                .ok_or_else(|| CreditChainDbError::NotFound("No latest version found.".to_string()))
        })
    }
...
/// requests.
impl HashReader for FakeCreditChainDB {
...
fn error_if_too_many_requested(num_requested: u64, max_allowed: u64) -> Result<()> {
    if num_requested > max_allowed {
        Err(CreditChainDbError::TooManyRequested(num_requested, max_allowed).into())
    } else {
        Ok(())
    }
}
...
mod tests {
    ...
    proptest! {
        ...
        fn test_save_transactions(input in arb_blocks_to_commit()) {
            ...
            let db = FakeCreditChainDB::new(CreditChainDB::new_for_test(&tmp_dir));
