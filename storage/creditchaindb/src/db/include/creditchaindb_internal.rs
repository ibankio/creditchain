...
use creditchain_storage_interface::block_info::BlockInfo;
...
impl CreditChainDB {
    ...
    fn open_indexer(
        ...
        if indexer.next_version() < ledger_next_version {
            ...
            while next_version < ledger_next_version {
                info!(next_version = next_version, "CreditChainDB Indexer catching up. ",);
        ...
        info!("CreditChainDB Indexer caught up.");
    ...
    fn new_without_pruner<P: AsRef<Path> + Clone>(
        ...
        .expect("Unable to open CreditChainDB")
    }
    ...
    fn get_raw_block_info_by_height(&self, block_height: u64) -> Result<BlockInfo> {
        if !self.skip_index_and_usage {
            ...
            Ok(self
                ...
                .ok_or_else(|| {
                    CreditChainDbError::NotFound(format!("BlockInfo not found at height {block_height}"))
                })?)
        }
    }
    ...
    fn get_raw_block_info_by_version(
        ...
        if !self.skip_index_and_usage {
            let (first_version, event_index, block_height) = self
                ...
                .ok_or_else(|| CreditChainDbError::NotFound("NewBlockEvent".to_string()))?;
    ...
    fn to_api_block_info(
        ...
        let last_version = match self.get_raw_block_info_by_height(block_height + 1) {
            ...
            Err(CreditChainDbError::NotFound(..)) => committed_version,
...
impl Debug for CreditChainDB {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        f.write_str("{CreditChainDB}")
    }
}

fn error_if_too_many_requested(num_requested: u64, max_allowed: u64) -> Result<()> {
    if num_requested > max_allowed {
        Err(CreditChainDbError::TooManyRequested(num_requested, max_allowed))
    } else {
        Ok(())
    }
}
        ...
        let res_type = match &res {
            ...
            Err(e) => {
                warn!(
                    api_name = api_name,
                    error = ?e,
                    "CreditChainDB API returned error."
                );
                "Err"
            },
        };
