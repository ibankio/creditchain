...
impl DbReader for CreditChainDB {
    ...
    /// Returns the transaction by version, delegates to `CreditChainDB::get_transaction_with_proof`.
    ...
    fn get_table_info(&self, handle: TableHandle) -> Result<TableInfo> {
        gauged_api("get_table_info", || {
            self.get_table_info_option(handle)?
                .ok_or_else(|| CreditChainDbError::NotFound(format!("TableInfo for {:?}", handle)))
        })
    }
