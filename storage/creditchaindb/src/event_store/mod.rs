...
use creditchain_db_indexer_schemas::schema::{
    event_by_key::EventByKeySchema, event_by_version::EventByVersionSchema,
};
use creditchain_schemadb::{batch::SchemaBatch, schema::ValueCodec, DB};
...
impl EventStore {
    ...
    pub fn get_txn_ver_by_seq_num(&self, event_key: &EventKey, seq_num: u64) -> Result<u64> {
        let (ver, _) = self
            ...
            .ok_or_else(|| {
                CreditChainDbError::NotFound(format!("Index entry should exist for seq_num {}", seq_num))
            })?;
        Ok(ver)
    }
    ...
    pub fn get_next_sequence_number(
        ...
        self.get_latest_sequence_number(ledger_version, event_key)?
            .map_or(Ok(0), |seq| {
                seq.checked_add(1)
                    .ok_or_else(|| CreditChainDbError::Other("Seq num overflowed.".to_string()))
            })
    }
    ...
    fn lookup_event_by_key(
        ...
        if indices.is_empty() {
            return Err(CreditChainDbError::NotFound(format!(
                "Event {} of seq num {}.",
                event_key, seq_num
            )));
        }
        ...
        C: FnMut(&ContractEvent) -> Result<bool>,
    {
        ...
        let mut end = match self.get_latest_sequence_number(ledger_version, event_key)? {
            Some(s) => s.checked_add(1).ok_or_else(|| {
                CreditChainDbError::Other("event sequence number overflew.".to_string())
            })?,
            None => return Ok(None),
        };
    ...
    pub(crate) fn get_last_version_before_timestamp(
        ...
        let seq_at_or_after_ts = self.search_for_event_lower_bound(
            ...
            ledger_version,
        )?.ok_or_else(|| CreditChainDbError::NotFound(
        ...
        version.checked_sub(1).ok_or_else(|| {
            CreditChainDbError::Other("A block with non-zero seq num started at version 0.".to_string())
        })
    }
