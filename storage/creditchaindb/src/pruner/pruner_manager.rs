...
/// The `PrunerManager` is meant to be part of a `CreditChainDB` instance and runs in the background to
/// prune old data.
///
...
pub trait PrunerManager: Sync {
    ...
    fn wait_for_pruner(&self) -> Result<()> {
        use creditchain_storage_interface::{db_other_bail, CreditChainDbError};
        use std::{
            thread::sleep,
