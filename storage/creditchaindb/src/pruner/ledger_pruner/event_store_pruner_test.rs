...
use crate::{CreditChainDB, EventStore, LedgerPrunerManager, PrunerManager};
use creditchain_config::config::LedgerPrunerConfig;
use creditchain_proptest_helpers::Index;
...
fn verify_event_store_pruner(events: Vec<Vec<ContractEvent>>) {
    ...
    let event_store = &creditchain_db.event_store;
    ...
    creditchain_db.ledger_db.event_db().write_schemas(batch).unwrap();
