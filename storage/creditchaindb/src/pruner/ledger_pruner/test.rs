...
use crate::{
    schema::version_data::VersionDataSchema, CreditChainDB, LedgerPrunerManager, LedgerStore,
    PrunerManager, TransactionStore,
};
use creditchain_accumulator::HashReader;
use creditchain_config::config::LedgerPrunerConfig;
use creditchain_schemadb::SchemaBatch;
...
fn verify_write_set_pruner(write_sets: Vec<WriteSet>) {
    ...
    let creditchain_db = CreditChainDB::new_for_test(&tmp_dir);
    let transaction_store = &creditchain_db.transaction_store;
    let num_write_sets = write_sets.len();

    let pruner = LedgerPrunerManager::new(Arc::clone(&creditchain_db.ledger_db), LedgerPrunerConfig {
        enable: true,
        prune_window: 0,
        batch_size: 1,
...
fn verify_txn_store_pruner(
    ...
    let creditchain_db = CreditChainDB::new_for_test(&tmp_dir);
    let transaction_store = &creditchain_db.transaction_store;
    let ledger_store = LedgerStore::new(Arc::clone(&creditchain_db.ledger_db));
    ...
    for i in (0..=num_transaction).step_by(step_size) {
        ...
        // logic.
        let pruner =
            LedgerPrunerManager::new(Arc::clone(&creditchain_db.ledger_db), LedgerPrunerConfig {
                enable: true,
                prune_window: 0,
                batch_size: 1,
                user_pruning_window_offset: 0,
        ...
        for j in 0..i {
            ...
            assert!(creditchain_db.state_store.get_usage(Some(j as u64)).is_err());
        }
        // ensure all other are valid in DB
        for j in i..num_transaction {
            ...
            creditchain_db.get_accumulator_summary(j as Version).unwrap();
            assert!(creditchain_db.state_store.get_usage(Some(j as u64)).is_ok());
        }
