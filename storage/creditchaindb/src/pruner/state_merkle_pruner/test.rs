...
use creditchain_config::config::{LedgerPrunerConfig, StateMerklePrunerConfig};
...
fn test_state_store_pruner() {
    ...
    let creditchain_db = CreditChainDB::new_for_test_no_cache(&tmp_dir);
    let state_store = &creditchain_db.state_store;
    ...
    {
        let pruner =
            create_state_merkle_pruner_manager(&creditchain_db.state_merkle_db(), prune_batch_size);
...
fn test_state_store_pruner_partial_version() {
    ...
    let tmp_dir = TempPath::new();
    let creditchain_db = CreditChainDB::new_for_test_with_sharding(&tmp_dir, 0);
    let state_store = &creditchain_db.state_store;
    ...
    if creditchain_db.state_merkle_db().sharding_enabled() {
