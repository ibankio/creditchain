    ...
    fn expect_continuous_versions(
        ...
        Ok(ContinuousVersionIter {
            ...
            end_version: first_version
                ...
                .ok_or(CreditChainDbError::TooManyRequested(first_version, limit as u64))?,
    ...
    fn next_impl(&mut self) -> Result<Option<Vec<ContractEvent>>> {
        ...
        while let Some(res) = self.inner.peek() {
            let ((version, _index), _event) = res
                .as_ref()
                .map_err(|e| CreditChainDbError::Other(format!("Hit error iterating events: {}", e)))?;
        ...
        self.expected_next_version = self
            .expected_next_version
            .checked_add(1)
            .ok_or_else(|| CreditChainDbError::Other("expected version overflowed.".to_string()))?;
        Ok(Some(ret))
    }
}
