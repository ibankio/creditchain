...
use crate::{
    schema::state_value_by_key_hash::StateValueByKeyHashSchema, state_kv_db::StateKvDb, CreditChainDB,
};
use creditchain_config::config::{RocksdbConfig, StorageDirPaths};
use creditchain_crypto::{hash::CryptoHash, HashValue};
use creditchain_db_indexer::db_ops::open_internal_indexer_db;
...
use creditchain_schemadb::{ReadOptions, DB};
...
pub fn validate_db_data(
    ...
    let creditchain_db = CreditChainDB::new_for_test_with_sharding(db_root_path, 1000000);
    let batch_size = 20_000;
    let start_version = creditchain_db.get_first_txn_version()?.unwrap();
    target_ledger_version = std::cmp::min(
        creditchain_db.get_synced_version()?.unwrap(),
