...
use creditchain_config::config::{RocksdbConfigs, StorageDirPaths};
...
impl Cmd {
    pub fn run(self) -> Result<()> {
        ...
        let (ledger_db, state_merkle_db, state_kv_db) = CreditChainDB::open_dbs(
...
mod test {
    use super::*;
    use crate::{
        db::{test_helper::arb_blocks_to_commit_with_block_nums, CreditChainDB},
        schema::{
    ...
    use creditchain_types::state_store::NUM_STATE_SHARDS;
    ...
    proptest! {
        ...
        fn test_truncation(input in arb_blocks_to_commit_with_block_nums(80, 120)) {
            ...
            let db = if input.1 { CreditChainDB::new_for_test_with_sharding(&tmp_dir, DEFAULT_MAX_NUM_NODES_PER_LRU_CACHE_SHARD) } else { CreditChainDB::new_for_test(&tmp_dir) };
            let mut version = 0;
