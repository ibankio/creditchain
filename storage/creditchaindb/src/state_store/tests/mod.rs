...
use creditchain_jellyfish_merkle::{
    node_type::{Node, NodeKey},
    TreeReader,
};
use creditchain_storage_interface::{DbReader, DbWriter, StateSnapshotReceiver};
...
fn test_empty_store() {
    ...
    let db = CreditChainDB::new_for_test(&tmp_dir);
...
fn test_get_values_by_key_prefix() {
    ...
    let key4 = StateKey::resource_typed::<CoinInfoResource<CreditChainCoinType>>(&address).unwrap();
    ...
    let key5 = StateKey::resource_typed::<CoinStoreResource<CreditChainCoinType>>(&address1).unwrap();
...
proptest! {
    ...
    fn test_raw_restore(
        ...
        let db1 = CreditChainDB::new_for_test(&tmp_dir1);
        ...
        let db2 = CreditChainDB::new_for_test(&tmp_dir2);
    ...
    fn test_get_rightmost_leaf_with_sharding(
        ...
        let tmp_dir1 = TempPath::new();
        let db1 = CreditChainDB::new_for_test_with_sharding(&tmp_dir1, 1000);
        let store1 = &db1.state_store;
        init_store(store1, input.clone().into_iter());
        ...
        let tmp_dir2 = TempPath::new();
        let db2 = CreditChainDB::new_for_test_with_sharding(&tmp_dir2, 1000);
    ...
    fn test_get_usage(
        ...
        for batch in input {
            ...
            let tmp_dir = TempPath::new();
            let db2 = CreditChainDB::new_for_test(&tmp_dir);
