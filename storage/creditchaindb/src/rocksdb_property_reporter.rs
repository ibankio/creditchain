...
use creditchain_schemadb::{ColumnFamilyName, DB};
...
static ROCKSDB_PROPERTY_MAP: Lazy<HashMap<&str, String>> = Lazy::new(|| {
    ...
    .iter()
    .map(|x| (*x, format!("creditchain_{}", x.replace('.', "_"))))
    .collect()
});

fn set_property(cf_name: &str, db: &DB) -> Result<()> {
    if !skip_reporting_cf(cf_name) {
        for (rockdb_property_name, creditchain_rocksdb_property_name) in &*ROCKSDB_PROPERTY_MAP {
            ROCKSDB_PROPERTIES
                .with_label_values(&[cf_name, creditchain_rocksdb_property_name])
        ...
        for (rockdb_property_name, creditchain_rocksdb_property_name) in &*ROCKSDB_PROPERTY_MAP {
            ROCKSDB_SHARD_PROPERTIES
                .with_label_values(&[
                    ...
                    cf_name,
                    creditchain_rocksdb_property_name,
                ])
                .set(db.get_property(cf_name, rockdb_property_name)? as i64);
        }
    }
