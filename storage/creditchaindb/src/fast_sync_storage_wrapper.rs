...
use crate::CreditChainDB;
use anyhow::anyhow;
use creditchain_config::config::{NodeConfig, StorageDirPaths};
...
/// This is a wrapper around [CreditChainDB] that is used to bootstrap the node for fast sync mode
pub struct FastSyncStorageWrapper {
    // Used for storing genesis data during fast sync
    temporary_db_with_genesis: Arc<CreditChainDB>,
    // Used for restoring fast sync snapshot and all the read/writes afterwards
    db_for_fast_sync: Arc<CreditChainDB>,
...
impl FastSyncStorageWrapper {
    ...
    /// Otherwise, we return CreditChainDB directly and the FastSyncStorageWrapper is None
    pub fn initialize_dbs(
        config: &NodeConfig,
        internal_indexer_db: Option<InternalIndexerDB>,
        update_sender: Option<Sender<(Instant, Version)>>,
    ) -> Result<Either<CreditChainDB, Self>> {
        let mut db_main = CreditChainDB::open(
                ...
                == 0)
        {
            ...
            let secondary_db = CreditChainDB::open(
    ...
    pub fn get_fast_sync_db(&self) -> Arc<CreditChainDB> {
        self.db_for_fast_sync.clone()
    }

    pub fn get_temporary_db_with_genesis(&self) -> Arc<CreditChainDB> {
    ...
    pub(crate) fn get_creditchain_db_read_ref(&self) -> &CreditChainDB {
    ...
    pub(crate) fn get_creditchain_db_write_ref(&self) -> &CreditChainDB {
...
impl DbWriter for FastSyncStorageWrapper {
    fn get_state_snapshot_receiver(
        ...
        *self.fast_sync_status.write() = FastSyncStatus::STARTED;
        self.get_creditchain_db_write_ref()
    ...
    fn finalize_state_snapshot(
        ...
        self.get_creditchain_db_write_ref().finalize_state_snapshot(
            version,
            output_with_proof,
            ledger_infos,
        )?;
...
impl DbReader for FastSyncStorageWrapper {
    fn get_read_delegatee(&self) -> &dyn DbReader {
        self.get_creditchain_db_read_ref()
    }
}
