...
/// Provides functionalities for CreditChainDB data restore.
#[derive(Clone)]
pub struct RestoreHandler {
    pub creditchaindb: Arc<CreditChainDB>,
    state_store: Arc<StateStore>,
    ledger_db: Arc<LedgerDb>,
}

impl RestoreHandler {
    pub(crate) fn new(creditchaindb: Arc<CreditChainDB>, state_store: Arc<StateStore>) -> Self {
        Self {
            ledger_db: Arc::clone(&creditchaindb.ledger_db),
            creditchaindb,
            state_store,
        }
    }
    ...
    pub fn save_ledger_infos(&self, ledger_infos: &[LedgerInfoWithSignatures]) -> Result<()> {
        restore_utils::save_ledger_infos(self.creditchaindb.ledger_db.metadata_db(), ledger_infos, None)
    }
    ...
    pub fn confirm_or_save_frozen_subtrees(
        ...
        restore_utils::confirm_or_save_frozen_subtrees(
            self.creditchaindb.ledger_db.transaction_accumulator_db_raw(),
            num_leaves,
            frozen_subtrees,
            None,
        )
    ...
    pub fn get_next_expected_transaction_version(&self) -> Result<Version> {
        Ok(self.creditchaindb.get_synced_version()?.map_or(0, |ver| ver + 1))
    }
    ...
    pub fn get_state_snapshot_before(
        ...
        self.creditchaindb.get_state_snapshot_before(version)
    }

    pub fn get_in_progress_state_kv_snapshot_version(&self) -> Result<Option<Version>> {
        let db = self.creditchaindb.state_kv_db.metadata_db_arc();
