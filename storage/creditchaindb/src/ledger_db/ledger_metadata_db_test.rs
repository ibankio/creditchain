...
use crate::{ledger_db::ledger_metadata_db::LedgerMetadataDb, CreditChainDB};
...
fn set_up(path: &impl AsRef<Path>, ledger_infos_with_sigs: &[LedgerInfoWithSignatures]) -> CreditChainDB {
    let db = CreditChainDB::new_for_test(path);
    ...
    ledger_infos_with_sigs
        ...
        .collect::<Result<Vec<_>, CreditChainDbError>>()
        .unwrap();
