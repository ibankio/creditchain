...
use creditchain_db_indexer_schemas::schema::ordered_transaction_by_account::OrderedTransactionByAccountSchema;
...
use creditchain_types::transaction::{IndexedTransactionSummary, ReplayProtector, Transaction, Version};
use rayon::prelude::*;
...
impl TransactionDb {
    ...
    pub(crate) fn get_transaction(&self, version: Version) -> Result<Transaction> {
        self.db
            ...
            .ok_or_else(|| CreditChainDbError::NotFound(format!("Txn {version}")))
    }
