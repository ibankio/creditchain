...
use creditchain_storage_interface::{block_info::BlockInfo, db_ensure as ensure, CreditChainDbError, Result};
...
impl LedgerMetadataDb {
    ...
    pub(crate) fn get_ledger_commit_progress(&self) -> Result<Version> {
        get_progress(&self.db, &DbMetadataKey::LedgerCommitProgress)?
            .ok_or_else(|| CreditChainDbError::NotFound("No LedgerCommitProgress in db.".to_string()))
    }

    pub(crate) fn get_pruner_progress(&self) -> Result<Version> {
        get_progress(&self.db, &DbMetadataKey::LedgerPrunerProgress)?
            .ok_or_else(|| CreditChainDbError::NotFound("No LedgerPrunerProgress in db.".to_string()))
    }
}
...
impl LedgerMetadataDb {
    ...
    pub(crate) fn get_latest_ledger_info(&self) -> Result<LedgerInfoWithSignatures> {
        self.get_latest_ledger_info_option()
            .ok_or_else(|| CreditChainDbError::NotFound(String::from("Genesis LedgerInfo")))
    }
    ...
    pub(crate) fn get_latest_ledger_info_in_epoch(
        ...
        self.db
            ...
            .ok_or_else(|| CreditChainDbError::NotFound(format!("Last LedgerInfo of epoch {epoch}")))
    }
    ...
    pub(crate) fn get_epoch_state(&self, epoch: u64) -> Result<EpochState> {
        ...
        let ledger_info_with_sigs =
            self.db
                ...
                .ok_or_else(|| {
                    CreditChainDbError::NotFound(format!("Last LedgerInfo of epoch {}", epoch - 1))
                })?;
        let latest_epoch_state = ledger_info_with_sigs
            .ledger_info()
            .next_epoch_state()
            .ok_or_else(|| {
                CreditChainDbError::Other(
    ...
    pub(crate) fn get_epoch_ending_ledger_info(
        ...
        let li = self
            .db
            .get::<LedgerInfoSchema>(&epoch)?
            .ok_or_else(|| CreditChainDbError::NotFound(format!("LedgerInfo for epoch {}.", epoch)))?;
        ensure!(
        ...
        li.ledger_info().next_epoch_state().ok_or_else(|| {
            CreditChainDbError::NotFound(format!("Not an epoch change at version {version}"))
        })?;
...
impl LedgerMetadataDb {
    ...
    pub(crate) fn ensure_epoch_ending(&self, version: Version) -> Result<()> {
        self.db
            ...
            .ok_or_else(|| {
                CreditChainDbError::Other(format!("Version {version} is not epoch ending."))
            })?;
...
impl LedgerMetadataDb {
    ...
    pub(crate) fn get_usage_before_or_at(
        ...
        match iter.next().transpose()? {
            ...
            None => Err(CreditChainDbError::NotFound(
