...
use creditchain_db_indexer_schemas::schema::event_sequence_number::EventSequenceNumberSchema;
use creditchain_schemadb::DB;
use creditchain_storage_interface::{
    state_store::state_view::db_state_view::LatestDbStateCheckpointView, CreditChainDbError, DbReader,
    Result,
};
...
impl EventV2Translator for TokenMutationTranslator {
    fn translate_event_v2_to_v1(
        ...
        let (key, sequence_number) = if let Some(state_value_bytes) = engine
            ...
            return Err(CreditChainDbError::from(anyhow::format_err!(
