...
def get_release_number_from_release_tag(release_tag: str) -> str:
    """Get the release number from the release tag. The release tag looks like cc-node-vX.Y.Z."""
...
def main():
    required_envs = ["RELEASE_TAG", "CC_NODE_CARGO_TOML"]
    ...
    CC_NODE_TOML = os.environ.get("CC_NODE_CARGO_TOML")

    new_lines = []  # construct the file again
    with open(CC_NODE_TOML, "r") as f:
        lines = f.readlines()
        for line in lines:
            new_line = line
...
class CheckCCNodeVersionTests(unittest.TestCase):
    def test_envs_not_specified(self):
        with self.assertRaises(SystemExit):
            os.environ["RELEASE_TAG"] = ""
            os.environ["CC_NODE_CARGO_TOML"] = ""
            main()
    ...
    def test_version_match(self):
        ...
        os.environ["CC_NODE_CARGO_TOML"] = tmp.name
