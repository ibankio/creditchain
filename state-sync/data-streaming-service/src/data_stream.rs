...
use creditchain_data_client::{
    ...
    interface::{
        CreditChainDataClientInterface, Response, ResponseContext, ResponseError, ResponsePayload,
...
/// has already been sent along the stream to the client and the in-flight CreditChain
...
impl<T: CreditChainDataClientInterface + Send + Clone + 'static> DataStream<T> {
    pub fn new(
        data_client_config: CreditChainDataClientConfig,
    ...
    /// Notifies the CreditChain data client of a bad client response
    ...
    fn check_subscription_stream_lag(
    ...
    ) -> Result<(), creditchain_data_client::error::Error> {
        ...
        let highest_response_version = match response_payload {
            ResponsePayload::NewTransactionsWithProof((transactions_with_proof, _)) => {
                if let Some(first_version) = transactions_with_proof.get_first_transaction_version()
                {
                    ...
                    return Err(creditchain_data_client::error::Error::UnexpectedErrorEncountered(
        ...
        let highest_advertised_version = global_data_summary
            ...
            .ok_or_else(|| {
                creditchain_data_client::error::Error::UnexpectedErrorEncountered(
        ...
        if let Some(mut subscription_stream_lag) = self.subscription_stream_lag.take() {
            ...
            {
                return Err(
                    creditchain_data_client::error::Error::SubscriptionStreamIsLagging(format!(
    ...
    fn notify_new_data_request_error(
        ...
        error: creditchain_data_client::error::Error,
    ) -> Result<(), Error> {
    ...
    fn handle_data_client_error(
        &mut self,
        data_client_request: &DataClientRequest,
        data_client_error: &creditchain_data_client::error::Error,
    ) -> Result<(), Error> {
        // Log the error
        warn!(LogSchema::new(LogEntry::ReceivedDataResponse)
            .stream_id(self.data_stream_id)
...
fn create_missing_epoch_ending_ledger_infos_request(
    ...
    match response_payload {
        ...
        payload => Err(Error::CreditChainDataClientResponseIsInvalid(format!(
...
fn create_missing_transactions_or_outputs_request(
    ...
    let num_received_data_items = match response_payload {
        ...
        payload => {
            return Err(Error::CreditChainDataClientResponseIsInvalid(format!(
...
/// can be sent to the CreditChain data client.
...
fn spawn_request_task<T: CreditChainDataClientInterface + Send + Clone + 'static>(
    data_stream_id: DataStreamId,
    data_client_request: DataClientRequest,
    creditchain_data_client: T,
    ...
    tokio::spawn(async move {
        ...
        let client_response = match data_client_request {
            DataClientRequest::EpochEndingLedgerInfos(request) => {
                get_epoch_ending_ledger_infos(creditchain_data_client, request, request_timeout_ms).await
            },
            DataClientRequest::NewTransactionsWithProof(request) => {
                get_new_transactions_with_proof(creditchain_data_client, request, request_timeout_ms)
                    .await
            },
            ...
            DataClientRequest::NumberOfStates(request) => {
                get_number_of_states(creditchain_data_client, request, request_timeout_ms).await
            },
            DataClientRequest::StateValuesWithProof(request) => {
                get_states_values_with_proof(creditchain_data_client, request, request_timeout_ms).await
            },
            DataClientRequest::SubscribeTransactionsWithProof(request) => {
                subscribe_to_transactions_with_proof(creditchain_data_client, request, request_timeout_ms)
                    .await
            },
            ...
            DataClientRequest::TransactionOutputsWithProof(request) => {
                get_transaction_outputs_with_proof(creditchain_data_client, request, request_timeout_ms)
                    .await
            },
            DataClientRequest::TransactionsWithProof(request) => {
                get_transactions_with_proof(creditchain_data_client, request, request_timeout_ms).await
            },
            DataClientRequest::TransactionsOrOutputsWithProof(request) => {
...
async fn get_states_values_with_proof<T: CreditChainDataClientInterface + Send + Clone + 'static>(
    creditchain_data_client: T,
    request: StateValuesWithProofRequest,
    request_timeout_ms: u64,
) -> Result<Response<ResponsePayload>, creditchain_data_client::error::Error> {
    let client_response = creditchain_data_client.get_state_values_with_proof(
        request.version,
        request.start_index,
        request.end_index,
        request_timeout_ms,
...
async fn get_epoch_ending_ledger_infos<T: CreditChainDataClientInterface + Send + Clone + 'static>(
    creditchain_data_client: T,
    request: EpochEndingLedgerInfosRequest,
    request_timeout_ms: u64,
) -> Result<Response<ResponsePayload>, creditchain_data_client::error::Error> {
    let client_response = creditchain_data_client.get_epoch_ending_ledger_infos(
        request.start_epoch,
        request.end_epoch,
        request_timeout_ms,
    );
...
async fn get_new_transaction_outputs_with_proof<
    T: CreditChainDataClientInterface + Send + Clone + 'static,
>(
    creditchain_data_client: T,
    request: NewTransactionOutputsWithProofRequest,
    request_timeout_ms: u64,
) -> Result<Response<ResponsePayload>, creditchain_data_client::error::Error> {
    let client_response = creditchain_data_client.get_new_transaction_outputs_with_proof(
...
async fn get_new_transactions_with_proof<T: CreditChainDataClientInterface + Send + Clone + 'static>(
    creditchain_data_client: T,
    request: NewTransactionsWithProofRequest,
    request_timeout_ms: u64,
) -> Result<Response<ResponsePayload>, creditchain_data_client::error::Error> {
    let client_response = creditchain_data_client.get_new_transactions_with_proof(
        request.known_version,
        request.known_epoch,
...
>(
    ...
    let client_response = creditchain_data_client.get_new_transactions_or_outputs_with_proof(
        request.known_version,
        request.known_epoch,
        request.include_events,
        request_timeout_ms,
...
async fn get_number_of_states<T: CreditChainDataClientInterface + Send + Clone + 'static>(
    creditchain_data_client: T,
    request: NumberOfStatesRequest,
    request_timeout_ms: u64,
) -> Result<Response<ResponsePayload>, creditchain_data_client::error::Error> {
    let client_response =
        creditchain_data_client.get_number_of_states(request.version, request_timeout_ms);
    client_response
        .await
        .map(|response| response.map(ResponsePayload::from))
}
...
>(
    ...
    let client_response = creditchain_data_client.get_transaction_outputs_with_proof(
...
async fn get_transactions_with_proof<T: CreditChainDataClientInterface + Send + Clone + 'static>(
    creditchain_data_client: T,
    request: TransactionsWithProofRequest,
    request_timeout_ms: u64,
) -> Result<Response<ResponsePayload>, creditchain_data_client::error::Error> {
    let client_response = creditchain_data_client.get_transactions_with_proof(
        request.proof_version,
        request.start_version,
        request.end_version,
        request.include_events,
...
>(
    ...
    let client_response = creditchain_data_client.get_transactions_or_outputs_with_proof(
        request.proof_version,
        request.start_version,
        request.end_version,
        request.include_events,
...
>(
    ...
    let client_response = creditchain_data_client.subscribe_to_transactions_with_proof(
...
>(
    ...
    let client_response = creditchain_data_client.subscribe_to_transaction_outputs_with_proof(
        subscription_request_metadata,
        request_timeout_ms,
    );
    client_response
...
>(
    ...
    let client_response = creditchain_data_client.subscribe_to_transactions_or_outputs_with_proof(
        subscription_request_metadata,
        request.include_events,
        request_timeout_ms,
    );
...
mod test {
    use super::*;
    use crate::tests::utils::MockCreditChainDataClient;
    use creditchain_channels::message_queues::QueueStyle;
    ...
    async fn completed_request_notifies_streaming_service() {
        ...
        let data_client_config = CreditChainDataClientConfig::default();
        let creditchain_data_client =
            MockCreditChainDataClient::new(data_client_config, true, false, true, true);
