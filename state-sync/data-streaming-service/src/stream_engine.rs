...
pub trait DataStreamEngine {
    ...
    /// CreditChain data client to progress the stream. The number of requests
    ...
    fn notify_new_data_request_error(
        ...
        request_error: creditchain_data_client::error::Error,
    ) -> Result<(), Error> {
...
impl DataStreamEngine for StateStreamEngine {
    fn create_data_client_requests(
        ...
        info!(
            (LogSchema::new(LogEntry::CreditChainDataClient)
...
fn extract_new_versions_and_target(
    ...
    if num_versions == 0 {
        // TODO(joshlind): eventually we want to notify the data client of the bad response
        return Err(Error::CreditChainDataClientResponseIsInvalid(
