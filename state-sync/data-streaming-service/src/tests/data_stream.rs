...
use crate::{
    ...
    tests::utils::{
        ...
        initialize_logger, MockCreditChainDataClient, NoopResponseCallback, MAX_ADVERTISED_EPOCH_END,
...
use creditchain_config::config::{
    CreditChainDataClientConfig, DataStreamingServiceConfig, DynamicPrefetchingConfig,
};
...
async fn test_stream_data_error() {
    ...
    let pending_response = PendingClientResponse::new_with_response(
        client_request.clone(),
        Err(creditchain_data_client::error::Error::DataIsUnavailable(
            "Missing data!".into(),
        )),
    );
    insert_response_into_pending_queue(&mut data_stream, pending_response);
...
async fn test_continuous_stream_optimistic_fetch_timeout() {
    ...
    let data_client_config = CreditChainDataClientConfig {
...
async fn advertise_new_data_and_verify_requests(
    data_stream: &mut DataStream<MockCreditChainDataClient>,
...
fn create_state_value_stream(
    ...
    version: Version,
) -> (DataStream<MockCreditChainDataClient>, DataStreamListener) {
...
fn create_continuous_transaction_output_stream(
    ...
    DataStream<MockCreditChainDataClient>,
    DataStreamListener,
    TimeService,
) {
...
fn create_data_stream(
    ...
    let creditchain_data_client = MockCreditChainDataClient::new(data_client_config, true, false, true, false);
...
fn set_failure_response_in_queue(data_stream: &mut DataStream<MockCreditChainDataClient>, index: usize) {
    set_response_in_queue(
        data_stream,
        index,
        creditchain_data_client::error::Error::UnexpectedErrorEncountered("Oops!".into()),
    );
}

/// Sets the client response at the index in the pending
/// queue to contain a pending response.
fn set_pending_response_in_queue(data_stream: &mut DataStream<MockCreditChainDataClient>, index: usize) {
...
fn set_timeout_response_in_queue(data_stream: &mut DataStream<MockCreditChainDataClient>, index: usize) {
    set_response_in_queue(
        data_stream,
        index,
        creditchain_data_client::error::Error::TimeoutWaitingForResponse("Timed out!".into()),
    );
}
...
fn set_response_in_queue(
    data_stream: &mut DataStream<MockCreditChainDataClient>,
    index: usize,
    error_response: creditchain_data_client::error::Error,
) {
    // Get the pending response at the specified index
    let (sent_requests, _) = data_stream.get_sent_requests_and_notifications();
    let pending_response = sent_requests.as_mut().unwrap().get_mut(index).unwrap();
...
async fn wait_for_data_client_to_respond(
    ...
    loop {
        if let Some(client_response) = &pending_response.lock().client_response {
            if !matches!(
                client_response,
                Err(creditchain_data_client::error::Error::TimeoutWaitingForResponse(
                    _
                ))
            ) {
                return;
...
fn set_transaction_response_at_queue_head(data_stream: &mut DataStream<MockCreditChainDataClient>) {
    let (sent_requests, _) = data_stream.get_sent_requests_and_notifications();
