...
use crate::{
    ...
    interface::{CreditChainDataClientInterface, Response},
...
use creditchain_config::{
    config::{CreditChainDataClientConfig, CreditChainDataPollerConfig},
    network_id::PeerNetworkId,
};
use creditchain_logger::{debug, error, info, sample, sample::SampleRate, warn};
...
pub struct DataSummaryPoller {
    data_client_config: Arc<CreditChainDataClientConfig>, // The configuration for the data client
    data_client: CreditChainDataClient,                   // The data client through which to poll peers
...
pub async fn start_poller(poller: DataSummaryPoller) {
    ...
    info!((LogSchema::new(LogEntry::DataSummaryPoller).message("Starting the CreditChain data poller!")));
    loop {
...
pub(crate) fn calculate_num_peers_to_poll(
    ...
    data_poller_config: CreditChainDataPollerConfig,
) -> u64 {
