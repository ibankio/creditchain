...
use crate::{
    client::CreditChainDataClient,
    error::Result,
    global_summary::GlobalDataSummary,
    interface::{CreditChainDataClientInterface, Response, SubscriptionRequestMetadata},
    poller::DataSummaryPoller,
    priority::PeerPriority,
};
use creditchain_channels::{creditchain_channel, message_queues::QueueStyle};
use creditchain_config::{
    config::{CreditChainDataClientConfig, BaseConfig, RoleType},
...
use creditchain_storage_service_server::network::{NetworkRequest, ResponseSender};
...
pub struct MockNetwork {
    ...
    peer_mgr_reqs_rxs:
        HashMap<NetworkId, creditchain_channel::Receiver<(PeerId, ProtocolId), PeerManagerRequest>>, // The peer manager request receivers
}

impl MockNetwork {
    pub fn new(
        base_config: Option<BaseConfig>,
        data_client_config: Option<CreditChainDataClientConfig>,
        networks: Option<Vec<NetworkId>>,
    ) -> (Self, MockTimeService, CreditChainDataClient, DataSummaryPoller) {
        ...
        for network in &networks {
            ...
            let queue_cfg = creditchain_channel::Config::new(10).queue_style(QueueStyle::FIFO);
        ...
        let (client, poller) = CreditChainDataClient::new(
            data_client_config,
            base_config.clone(),
            mock_time.clone(),
...
pub fn create_mock_data_client() -> Arc<dyn CreditChainDataClientInterface + Send + Sync> {
    Arc::new(MockCreditChainDataClient::new())
}

// This automatically creates a MockCreditChainDataClient
mock! {
    pub CreditChainDataClient {}
...
impl DbReader for MockDatabaseReader {
    fn get_block_timestamp(&self, version: Version) -> creditchain_storage_interface::Result<u64> {
        Ok(version * 100_000)
    }
}
