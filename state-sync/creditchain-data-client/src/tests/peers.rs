...
use creditchain_config::{
    config::{CreditChainDataClientConfig, CreditChainDataMultiFetchConfig},
    network_id::{NetworkId, PeerNetworkId},
};
use creditchain_storage_service_server::network::NetworkRequest;
...
use creditchain_types::transaction::{TransactionListWithProof, TransactionListWithProofV2};
...
async fn all_bad_peers_with_invalid_responses() {
    ...
    for peer_priority in PeerPriority::get_all_ordered_priorities() {
        ...
        let data_client_config = CreditChainDataClientConfig {
            data_multi_fetch_config: CreditChainDataMultiFetchConfig {
...
fn verify_peer_states(client: &CreditChainDataClient, all_peers: HashSet<PeerNetworkId>) {
