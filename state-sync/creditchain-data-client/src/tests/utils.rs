...
use crate::{
    client::CreditChainDataClient, error::Error, interface::CreditChainDataClientInterface,
...
pub fn remove_distance_metadata(client: &CreditChainDataClient, peer: PeerNetworkId) {
...
pub fn remove_latency_metadata(client: &CreditChainDataClient, peer: PeerNetworkId) {
    // Get the peer monitoring metadata
    let peers_and_metadata = client.get_peers_and_metadata();
    let peer_metadata = peers_and_metadata.get_metadata_for_peer(peer).unwrap();
    let mut peer_monitoring_metadata = peer_metadata.get_peer_monitoring_metadata().clone();
