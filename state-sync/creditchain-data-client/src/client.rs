...
use creditchain_config::{
    config::{CreditChainDataClientConfig, BaseConfig},
    network_id::PeerNetworkId,
};
use creditchain_id_generator::{IdGenerator, U64IdGenerator};
use creditchain_infallible::Mutex;
use creditchain_logger::{info, sample, sample::SampleRate, trace, warn};
...
/// An [`CreditChainDataClientInterface`] that fulfills requests from remote peers' Storage Service
/// over CreditChainNet.
///
/// The `CreditChainDataClient`:
///
/// 1. Sends requests to connected CreditChain peers.
...
/// and/or threads.
#[derive(Clone, Debug)]
pub struct CreditChainDataClient {
    /// The base config of the node.
    base_config: Arc<BaseConfig>,
    /// The config for the CreditChainNet data client.
    data_client_config: Arc<CreditChainDataClientConfig>,
    /// The underlying CreditChainNet storage service client.
...
impl CreditChainDataClient {
        ...
        E: Into<Error>,
    {
        ...
        for peer in peers {
            ...
            let creditchain_data_client = self.clone();
    ...
    async fn send_request_to_peer(
        ...
        match result {
            Ok(response) => {
                ...
                // with this RPC.
                let response_callback = CreditChainNetResponseCallback {
            ...
            Err(error) => {
                ...
                let client_error = match error {
                    creditchain_storage_service_client::Error::RpcError(rpc_error) => match rpc_error {
                    ...
                    creditchain_storage_service_client::Error::StorageServiceError(err) => {
...
#[async_trait]
impl CreditChainDataClientInterface for CreditChainDataClient {
...
/// The CreditChainNet-specific request context needed to update a peer's scoring.
struct CreditChainNetResponseCallback {
    data_client: CreditChainDataClient,
    id: ResponseId,
    peer: PeerNetworkId,
    request: StorageServiceRequest,
}

impl ResponseCallback for CreditChainNetResponseCallback {
...
impl fmt::Debug for CreditChainNetResponseCallback {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("CreditChainNetResponseCallback")
            .field("data_client", &"..")
            .field("id", &self.id)
