...
use creditchain_logger::{info, sample, sample::SampleRate, warn};
...
pub struct LatencyMonitor {
    ...
    data_client: Arc<dyn CreditChainDataClientInterface + Send + Sync>, // The data client through which to see advertised data
...
impl LatencyMonitor {
    pub fn new(
        ...
        data_client: Arc<dyn CreditChainDataClientInterface + Send + Sync>,
    ...
    pub async fn start_latency_monitor(mut self) {
        info!(
            (LogSchema::new(LogEntry::LatencyMonitor)
                .message("Starting the CreditChain data client latency monitor!"))
        );
...
mod tests {
    ...
    fn create_latency_monitor() -> (TimeService, LatencyMonitor) {
        let data_client_config = Arc::new(CreditChainDataClientConfig::default());
