...
impl<T: StorageReaderInterface + Send + Sync> StorageServiceServer<T> {
    pub fn new(
        ...
        let creditchain_data_client_config = config.creditchain_data_client;
    ...
    async fn spawn_continuous_storage_summary_tasks(&mut self) {
        ...
        let (cache_update_notifier_optimistic_fetch, cache_update_listener_optimistic_fetch) =
            creditchain_channel::new(QueueStyle::LIFO, CACHED_SUMMARY_UPDATE_CHANNEL_SIZE, None);
    ...
    async fn spawn_storage_summary_refresher(
        &mut self,
        cache_update_notifiers: Vec<creditchain_channel::Sender<(), CachedSummaryUpdateNotification>>,
    ) {
    ...
    async fn spawn_optimistic_fetch_handler(
        &mut self,
        mut cached_summary_update_listener: creditchain_channel::Receiver<
            (),
