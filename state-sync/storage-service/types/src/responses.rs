...
use creditchain_compression::{client::CompressionClient, CompressedData};
use creditchain_config::config::{
    CreditChainDataClientConfig, StorageServiceConfig, MAX_APPLICATION_MESSAGE_SIZE,
};
...
impl From<creditchain_compression::Error> for Error {
    fn from(error: creditchain_compression::Error) -> Self {
...
impl StorageServiceResponse {
    ...
    pub fn new(data_response: DataResponse, perform_compression: bool) -> Result<Self, Error> {
        if perform_compression {
            ...
            let compressed_data = creditchain_compression::compress(
                raw_data,
    ...
    pub fn get_data_response(&self) -> Result<DataResponse, Error> {
        match self {
            StorageServiceResponse::CompressedResponse(_, compressed_data) => {
                let raw_data = creditchain_compression::decompress(
                    compressed_data,
                    CompressionClient::StateSync,
                    MAX_APPLICATION_MESSAGE_SIZE,
                )?;
...
impl StorageServerSummary {
    pub fn can_service(
        ...
        self.protocol_metadata.can_service(request)
            && self
                .data_summary
                .can_service(creditchain_data_client_config, time_service, request)
    }
}
...
fn can_service_optimistic_request(
    ...
    let max_lag_secs = creditchain_data_client_config.max_optimistic_fetch_lag_secs;
...
fn can_service_subscription_request(
    ...
    let max_lag_secs = creditchain_data_client_config.max_subscription_lag_secs;
    check_synced_ledger_lag(synced_ledger_info, time_service, max_lag_secs)
}
