...
struct TestValidator {
    ...
    _db_path: creditchain_temppath::TempPath,
}

impl TestValidator {
    fn new() -> Self {
        let _db_path = creditchain_temppath::TempPath::new();
        _db_path.create_as_dir().unwrap();
        let (db, db_rw) = DbReaderWriter::wrap(CreditChainDB::new_for_test(_db_path.path()));
        creditchain_executor_test_helpers::bootstrap_genesis::<CreditChainVMBlockExecutor>(
            &db_rw,
            &creditchain_vm_genesis::test_genesis_transaction(),
        )
...
//   language e2e tests in: creditchain-core/language/e2e-testsuite/src/tests/verify_txn.rs ->
...
// * SENDING_ACCOUNT_FROZEN: Tested in functional-tests/tests/creditchain_account/freezing.move.
...
fn test_validate_transaction() {
    ...
    let address = account_config::creditchain_test_root_address();
    let program = creditchain_stdlib::creditchain_coin_mint(address, 100);
    let transaction = transaction_test_helpers::get_test_signed_txn(
        address,
        1,
        &creditchain_vm_genesis::GENESIS_KEYPAIR.0,
        creditchain_vm_genesis::GENESIS_KEYPAIR.1.clone(),
        Some(program),
    );
...
fn test_validate_invalid_signature() {
    ...
    let address = account_config::creditchain_test_root_address();
    let program = creditchain_stdlib::creditchain_coin_transfer(address, 100);
...
fn test_get_account_sequence_number() {
    let vm_validator = TestValidator::new();
    let root_address = account_config::creditchain_test_root_address();
