...
const CREDITCHAIN_CLI_PACKAGE_NAME: &str = "creditchain";

// Relevant file paths to monitor when deciding to run the targeted tests.
// Note: these paths should be relative to the root of the `creditchain-core` repository,
// and will be transformed into UTF-8 paths for cross-platform compatibility.
const RELEVANT_FILE_PATHS_FOR_COMPILER_V2: [&str; 5] = [
    "creditchain-move/creditchain-transactional-test-harness",
    "creditchain-move/e2e-move-tests",
    "creditchain-move/framework",
    "creditchain-move/move-examples",
    "third_party/move",
];
const RELEVANT_FILE_PATHS_FOR_EXECUTION_PERFORMANCE_TESTS: [&str; 5] = [
    ...
    "creditchain-move/e2e-benchmark",
    "execution/creditchain-executor-benchmark",
    "testsuite/single_node_performance.py",
];
const RELEVANT_FILE_PATHS_FOR_FRAMEWORK_UPGRADE_TESTS: [&str; 4] = [
    ".github",
    "testsuite",
    "creditchain-move/creditchain-release-builder",
    "creditchain-move/framework",
];

// Relevant packages to monitor when deciding to run the targeted tests
const RELEVANT_PACKAGES_FOR_COMPILER_V2: [&str; 2] = ["creditchain-framework", "e2e-move-tests"];
const RELEVANT_PACKAGES_FOR_EXECUTION_PERFORMANCE_TESTS: [&str; 2] =
    ["creditchain-executor-benchmark", "creditchain-move-e2e-benchmark"];
const RELEVANT_PACKAGES_FOR_FRAMEWORK_UPGRADE_TESTS: [&str; 2] =
    ["creditchain-framework", "creditchain-release-builder"];
...
const TARGETED_UNIT_TEST_PACKAGES_TO_IGNORE: [&str; 3] =
    ["creditchain-testcases", "smoke-test", "creditchain-keyless-circuit"];
...
pub enum CreditChainCargoCommand {
...
impl CreditChainCargoCommand {
    fn command(&self) -> &'static str {
        match self {
            CreditChainCargoCommand::Check(_) => "check",
            CreditChainCargoCommand::Xclippy(_) => "clippy",
            CreditChainCargoCommand::Fmt(_) => "fmt",
            CreditChainCargoCommand::Nextest(_) => "nextest",
            CreditChainCargoCommand::Test(_) => "test",
    ...
    fn command_args(&self) -> &CommonArgs {
        match self {
            CreditChainCargoCommand::AffectedPackages(args) => args,
            CreditChainCargoCommand::ChangedFiles(args) => args,
            CreditChainCargoCommand::Check(args) => args,
            CreditChainCargoCommand::CheckMergeBase(args) => args,
            CreditChainCargoCommand::Xclippy(args) => args,
            CreditChainCargoCommand::Fmt(args) => args,
            CreditChainCargoCommand::Nextest(args) => args,
            CreditChainCargoCommand::TargetedCLITests(args) => args,
            CreditChainCargoCommand::TargetedCompilerV2Tests(args) => args,
            CreditChainCargoCommand::TargetedExecutionPerformanceTests(args) => args,
            CreditChainCargoCommand::TargetedFrameworkUpgradeTests(args) => args,
            CreditChainCargoCommand::TargetedUnitTests(args) => args,
            CreditChainCargoCommand::Test(args) => args,
        }
    }

    fn extra_opts(&self) -> Option<&[&str]> {
        match self {
            CreditChainCargoCommand::Xclippy(_) => Some(&[
                "-Dwarnings",
                "-Wclippy::all",
    ...
    pub fn execute(&self, package_args: &SelectedPackageArgs) -> anyhow::Result<()> {
        match self {
            CreditChainCargoCommand::AffectedPackages(_) => {
            ...
            CreditChainCargoCommand::ChangedFiles(_) => {
            ...
            CreditChainCargoCommand::CheckMergeBase(_) => {
                // Check the merge base
                package_args.check_merge_base()
            },
            CreditChainCargoCommand::TargetedCLITests(_) => {
                // Run the targeted CLI tests (if necessary).
                // First, start by calculating the affected packages.
                let affected_package_paths = package_args.compute_target_packages()?;

                // Check if the affected packages contains the CreditChain CLI
                ...
                for package_path in affected_package_paths {
                    ...
                    // Check if the package is the CreditChain CLI
                    if package_name == CREDITCHAIN_CLI_PACKAGE_NAME {
                        cli_affected = true; // The CreditChain CLI was affected
                        break;
                    }
                }

                // If the CreditChain CLI is affected, run the targeted CLI tests
                if cli_affected {
                ...
                println!("Skipping CLI tests as the CreditChain CLI package was not affected!");
                Ok(())
            },
            CreditChainCargoCommand::TargetedCompilerV2Tests(_) => {
            ...
            CreditChainCargoCommand::TargetedExecutionPerformanceTests(_) => {
            ...
            CreditChainCargoCommand::TargetedFrameworkUpgradeTests(_) => {
            ...
            CreditChainCargoCommand::TargetedUnitTests(_) => {
...
fn run_targeted_cli_tests() -> anyhow::Result<()> {
    ...
    command.args(["-p", CREDITCHAIN_CLI_PACKAGE_NAME]);
    command.run(false);
...
#[clap(author, version)]
pub struct CreditChainCargoCli {
    #[command(subcommand)]
...
mod tests {
    ...
    fn test_detect_relevant_changes() {
        ...
        let relevant_file_paths = vec![".github/actions/", "creditchain-move/", "Cargo.lock", "crates/"];
        let relevant_package_names = vec!["creditchain-node", "e2e-move-tests"];
        ...
        let affected_package_paths =
            vec!["file:///home/creditchain-core/crates/test-crate#test-crate".into()];
        let relevant_changes_detected = detect_relevant_changes(
        ...
        let affected_package_paths =
            vec!["file:///home/creditchain-core/crates/creditchain-node#creditchain-node".into()];
        let relevant_changes_detected = detect_relevant_changes(
            relevant_file_paths.clone(),
            relevant_package_names.clone(),
        ...
        let affected_package_paths =
            vec!["file:///home/creditchain-core/crates/e2e-move-tests#e2e-move-tests".into()];
        let relevant_changes_detected = detect_relevant_changes(
            relevant_file_paths.clone(),
            relevant_package_names.clone(),
            changed_file_path,
    ...
    fn test_detect_relevant_changes_package_paths() {
        ...
        let affected_package_paths = vec![
            "file:///home/creditchain-core/creditchain-mode/tests/e2e-move-tests#test-crate",
            "file:///home/creditchain-core/crates/test-crate#other-test-crate",
            "file:///home/creditchain-core/crates/other-crate#other-crate",
            "file:///home/creditchain-core/creditchain-node#other-node-crate",
        ];
        ...
        // Verify that package changes are detected correctly
        let affected_package_paths = vec![
            "file:///home/creditchain-core/crates/creditchain-node#creditchain-node",
            "file:///home/creditchain-core/crates/e2e-move-tests#e2e-move-tests",
        ];
        for affected_package_path in affected_package_paths {
            // Verify changes are detected
            let relevant_changes_detected = detect_relevant_changes(
    ...
    fn test_get_package_name_from_path() {
        ...
        let package_path = format!(
            "file:///home/creditchain-core/devtools/creditchain-cargo-cli#{}",
            package_name
        );
    ...
    fn test_get_package_name_from_path_empty() {
        ...
        let package_path = "file:///home/creditchain-core/devtools/creditchain-cargo-cli#";
