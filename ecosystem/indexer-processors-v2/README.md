# CreditChain Core Processors (SDK version)
Processors that index data from the CreditChain Transaction Stream (GRPC). These processors have been (re)-written using the new Indexer SDK.

- **Note: Official releases coming soon!**

## Overview
This tutorial shows you how to run the CreditChain core processors in this repo.

If you want to index a custom contract, we recommend using the [Quickstart Guide](https://github.com/ibankio/creditchain/tree/main/docs).

### Prerequisite

- A running PostgreSQL instance, with a valid database. More tutorial can be
  found [here](https://github.com/ibankio/creditchain/tree/main/crates/indexer#postgres)
...
- `transaction_stream_config`
    - `indexer_grpc_data_service_address`: Data service non-TLS endpoint address. See [available Transaction Stream endpoints](https://github.com/ibankio/creditchain/tree/main/docs).
    - `auth_token`: Auth token used for connection. See [instructions on how to get an auth token](https://github.com/ibankio/creditchain/tree/main/docs).
...
See mapping in [v2_fungible_asset_balances.rs](./processor/src/processors/fungible_asset/models/v2_fungible_asset_balances.rs) for a list of supported coin type mappings.
