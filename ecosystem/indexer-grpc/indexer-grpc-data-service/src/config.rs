...
impl RunnableConfig for IndexerGrpcDataServiceConfig {
    ...
    async fn run(&self) -> Result<()> {
        ...
        // InMemoryCache.
        let in_memory_cache =
            creditchain_indexer_grpc_utils::in_memory_cache::InMemoryCache::new_with_redis_connection(
        ...
        let svc = creditchain_protos::indexer::v1::raw_data_server::RawDataServer::new(server)
            .send_compressed(CompressionEncoding::Zstd)
            .accept_compressed(CompressionEncoding::Zstd)
            .accept_compressed(CompressionEncoding::Gzip);
