...
pub fn create_tar_gz(dir_path: PathBuf, backup_file_name: &str) -> Result<PathBuf, anyhow::Error> {
    ...
    creditchain_logger::info!("[Table Info] Directory contents appended to the tar.gz archive");
    ...
    creditchain_logger::info!("[Table Info] Tar.gz archive created successfully");
