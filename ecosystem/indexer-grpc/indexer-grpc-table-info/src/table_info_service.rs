...
use creditchain_api_types::TransactionOnChainData;
use creditchain_db_indexer::db_v2::IndexerAsyncV2;
use creditchain_indexer_grpc_fullnode::stream_coordinator::{
    IndexerStreamCoordinator, TransactionBatchInfo,
};
use creditchain_indexer_grpc_utils::counters::{log_grpc_step, IndexerGrpcStep};
use creditchain_logger::{debug, error, info, sample, sample::SampleRate};
use creditchain_types::write_set::WriteSet;
...
impl TableInfoService {
        ...
        let backup_is_enabled = match self.backup_restore_operator.clone() {
            Some(backup_restore_operator) => {
                ...
                let _task = tokio::spawn(async move {
                    loop {
                        creditchain_logger::info!("[Table Info] Checking for snapshots to backup.");
    ...
    async fn backup_snapshot_if_present(
        ...
        if epochs_to_backup.is_empty() {
            ...
            creditchain_logger::info!("[Table Info] No snapshot to backup. Skipping the backup.");
            return;
        }
        creditchain_logger::info!(
