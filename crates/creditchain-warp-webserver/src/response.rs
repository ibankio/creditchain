...
use creditchain_api_types::{
    mime_types::{BCS, JSON},
    LedgerInfo, X_CREDITCHAIN_BLOCK_HEIGHT, X_CREDITCHAIN_CHAIN_ID, X_CREDITCHAIN_EPOCH,
    X_CREDITCHAIN_LEDGER_OLDEST_VERSION, X_CREDITCHAIN_LEDGER_TIMESTAMP, X_CREDITCHAIN_LEDGER_VERSION,
    X_CREDITCHAIN_OLDEST_BLOCK_HEIGHT,
};
...
impl warp::Reply for Response {
    fn into_response(self) -> warp::reply::Response {
        ...
        headers.insert(X_CREDITCHAIN_CHAIN_ID, (self.ledger_info.chain_id as u16).into());
        headers.insert(
            X_CREDITCHAIN_LEDGER_VERSION,
            self.ledger_info.ledger_version.0.into(),
        );
        headers.insert(
            X_CREDITCHAIN_LEDGER_OLDEST_VERSION,
            self.ledger_info.oldest_ledger_version.0.into(),
        );
        headers.insert(
            X_CREDITCHAIN_LEDGER_TIMESTAMP,
            self.ledger_info.ledger_timestamp.0.into(),
        );
        headers.insert(X_CREDITCHAIN_EPOCH, self.ledger_info.epoch.0.into());
        headers.insert(X_CREDITCHAIN_BLOCK_HEIGHT, self.ledger_info.block_height.0.into());
        headers.insert(
            X_CREDITCHAIN_OLDEST_BLOCK_HEIGHT,
