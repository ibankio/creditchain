...
pub struct Error {
    pub code: u16,
    pub message: String,
    /// CreditChain blockchain latest onchain ledger version.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub creditchain_ledger_version: Option<U64>,
}
...
impl Error {
    pub fn new(code: StatusCode, message: String) -> Self {
        Self {
            code: code.as_u16(),
            message,
            creditchain_ledger_version: None,
        }
    }
    ...
    pub fn not_found<S: Display>(resource: &str, identifier: S, ledger_version: u64) -> Self {
        ...
        .creditchain_ledger_version(ledger_version)
    }
    ...
    pub fn creditchain_ledger_version(mut self, ledger_version: u64) -> Self {
        self.creditchain_ledger_version = Some(ledger_version.into());
        self
    }
}
...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        if let Some(val) = &self.creditchain_ledger_version {
            write!(f, "\nCreditChain ledger version: {}", val)?;
        }
        Ok(())
    }
}
...
mod tests {
    ...
    #[test]
    fn test_to_string_with_creditchain_ledger_version() {
        let err = Error::new(StatusCode::BAD_REQUEST, "invalid address".to_owned())
            .creditchain_ledger_version(123);
        assert_eq!(
            err.to_string(),
            "400 Bad Request: invalid address\nCreditChain ledger version: 123"
        )
    }
