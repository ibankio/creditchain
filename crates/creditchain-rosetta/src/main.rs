...
use creditchain_config::config::{ApiConfig, DEFAULT_MAX_PAGE_SIZE};
use creditchain_logger::prelude::*;
use creditchain_node::CreditChainNodeArgs;
use creditchain_rosetta::{bootstrap, common::native_coin, types::Currency};
...
async fn main() {
    ...
    match args {
        CommandArgs::OnlineRemote(_) => {
            println!("creditchain-rosetta: Starting Rosetta in Online remote (no local full node) mode")
        },
        CommandArgs::Online(_) => {
            println!("creditchain-rosetta: Starting Rosetta in Online (with local full node) mode")
        },
        CommandArgs::Offline(_) => println!("creditchain-rosetta: Starting Rosetta in Offline mode"),
    }
    ...
    {
        println!("creditchain-rosetta: Starting local full node");
        let node_args = node_args.clone();
        let runtime = thread::spawn(move || node_args.run());

        // Wait and ensure the node is running on the URL
        let client = creditchain_rest_client::Client::new(online_args.rest_api_url.clone());
            ...
            match client.get_index_bcs().await {
                ...
                Err(err) => {
                    sample!(
                        ...
                        println!(
                            "creditchain-rosetta: Full node REST API isn't responding yet.  You should check the node logs.  It's been waiting {} seconds.  Error: {:?}",
        ...
        println!("creditchain-rosetta: Local full node started successfully");
        Some(runtime)
    } else {
    ...
    println!("creditchain-rosetta: Starting rosetta");
    ...
    .expect("creditchain-rosetta: Should bootstrap rosetta server");

    println!("creditchain-rosetta: Rosetta started");
...
trait ServerArgs {
    ...
    fn rest_client(&self) -> Option<creditchain_rest_client::Client>;
...
/// CreditChain Rosetta API Server
///
/// Provides an implementation of [Rosetta](https://www.rosetta-api.org/docs/Reference.html) on CreditChain.
#[derive(Debug, Parser)]
#[clap(name = "creditchain-rosetta", author, version, propagate_version = true)]
...
pub struct OnlineRemoteArgs {
    ...
    /// URL for the CreditChain REST API. e.g. https://fullnode.devnet.creditchain.io
...
impl ServerArgs for OnlineRemoteArgs {
    ...
    fn rest_client(&self) -> Option<creditchain_rest_client::Client> {
        Some(creditchain_rest_client::Client::new(self.rest_api_url.clone()))
    }
