...
use creditchain_logger::{debug, trace, warn};
use creditchain_rest_client::{
    creditchain_api_types::{CreditChainError, CreditChainErrorCode, ViewFunction},
    error::{CreditChainErrorResponse, RestError},
    Client,
};
use creditchain_types::{account_address::AccountAddress, account_config::AccountResource};
use move_core_types::{
    ident_str,
    identifier::IdentStr,
...
async fn account_balance(
    ...
    Ok(AccountBalanceResponse {
        ...
        metadata: AccountBalanceMetadata {
            ...
            operators,
            lockup_expiration_time_utc: creditchain_rest_client::creditchain_api_types::U64(lockup_expiration),
        },
    })
}
...
async fn get_sequence_number(
        ...
        .await
    {
        ...
        Err(RestError::Api(CreditChainErrorResponse {
            error:
                CreditChainError {
                    error_code: CreditChainErrorCode::AccountNotFound,
                    ..
                },
            ..
        }))
        | Err(RestError::Api(CreditChainErrorResponse {
            error:
                CreditChainError {
                    error_code: CreditChainErrorCode::ResourceNotFound,
                    ..
                },
            ..
        })) => {
