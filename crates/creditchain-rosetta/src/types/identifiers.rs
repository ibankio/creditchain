...
/// Identifier for a "block".  On CreditChain, we use a transaction model, so the index
...
impl From<creditchain_crypto::HashValue> for TransactionIdentifier {
    fn from(hash: creditchain_crypto::HashValue) -> Self {
        TransactionIdentifier {
