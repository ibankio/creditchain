...
use creditchain_crypto::{ed25519::Ed25519PublicKey, ValidCryptoMaterialStringExt};
use creditchain_logger::warn;
use creditchain_rest_client::creditchain_api_types::{ResourceGroup, TransactionOnChainData, U64};
...
/// A combination of a transaction and the block associated.  In CreditChain, this is just the same
...
pub struct CurrencyMetadata {
    /// Move coin type e.g. 0x1::creditchain_coin::CreditChainCoin
    #[serde(skip_serializing_if = "Option::is_none")]
...
///
/// This is known as a write set change within CreditChain
...
/// Cryptographic signature type used for signing transactions.  CreditChain only uses
    ...
    if let TransactionPayload::EntryFunction(inner) = payload {
        match (
            *inner.module().address(),
            ...
            | (AccountAddress::ONE, CREDITCHAIN_ACCOUNT_MODULE, TRANSFER_COINS_FUNCTION) => {
            ...
            (AccountAddress::ONE, CREDITCHAIN_ACCOUNT_MODULE, TRANSFER_FUNCTION) => {
            ...
            | (AccountAddress::ONE, CREDITCHAIN_ACCOUNT_MODULE, TRANSFER_FUNGIBLE_ASSETS_FUNCTION) => {
    ...
    /*    if let Some(owner_address) = server_context.pool_address_to_owner.get(&pool_address) {
            if let Ok(stakepool) = bcs::from_bytes::<StakePool>(data) {
                ...
                let add_stake_events = filter_events(
                    ...
                    |event_key, event| {
                        if let Ok(event) = bcs::from_bytes::<creditchain_types::stake_pool::AddStakeEvent>(
                            event.event_data(),
                        ) {
                            Some(event)
                        } else {
                ...
                let set_operator_events = filter_events(
                    ...
                    |event_key, event| {
                        if let Ok(event) = bcs::from_bytes::<creditchain_types::stake_pool::SetOperatorEvent>(
                            event.event_data(),
                        ) {
                            Some(event)
                        } else {
...
impl InternalOperation {
    ...
    pub fn payload(
        &self,
    ) -> ApiResult<(creditchain_types::transaction::TransactionPayload, AccountAddress)> {
        Ok(match self {
            InternalOperation::CreateAccount(create_account) => (
                creditchain_stdlib::creditchain_account_create_account(create_account.new_account),
            ...
            InternalOperation::Transfer(transfer) => {
                ...
                if currency == &native_coin() {
                    return Ok((
                        creditchain_stdlib::creditchain_account_transfer(transfer.receiver, transfer.amount.0),
                        transfer.sender,
                    ));
                }
                ...
                if let Some(ref metadata) = currency.metadata {
                    match (&metadata.move_type, &metadata.fa_address) {
                        ...
                        (Some(coin_type), Some(_)) | (Some(coin_type), None) => {
                            ...
                            (
                                creditchain_stdlib::creditchain_account_transfer_coins(
                                    coin_type_tag,
                                    transfer.receiver,
                                    transfer.amount.0,
                                ),
            ...
            InternalOperation::SetOperator(set_operator) => {
                ...
                (
                    creditchain_stdlib::staking_contract_switch_operator_with_same_commission(
            ...
            InternalOperation::SetVoter(set_voter) => {
                ...
                (
                    creditchain_stdlib::staking_contract_update_voter(
            ...
            InternalOperation::InitializeStakePool(init_stake_pool) => (
                creditchain_stdlib::staking_contract_create_staking_contract(
            ...
            InternalOperation::ResetLockup(reset_lockup) => (
                creditchain_stdlib::staking_contract_reset_lockup(reset_lockup.operator),
                reset_lockup.owner,
            ),
            InternalOperation::UnlockStake(unlock_stake) => (
                creditchain_stdlib::staking_contract_unlock_stake(
            ...
            InternalOperation::UpdateCommission(update_commision) => (
                creditchain_stdlib::staking_contract_update_commision(
            ...
            InternalOperation::DistributeStakingRewards(distribute_staking_rewards) => (
                creditchain_stdlib::staking_contract_distribute(
            ...
            InternalOperation::AddDelegatedStake(add_delegated_stake) => (
                creditchain_stdlib::delegation_pool_add_stake(
            ...
            InternalOperation::UnlockDelegatedStake(unlock_delegated_stake) => (
                creditchain_stdlib::delegation_pool_unlock(
            ...
            InternalOperation::WithdrawUndelegated(withdraw_undelegated) => (
                creditchain_stdlib::delegation_pool_withdraw(
