...
The `creditchain-rosetta` binary can run in three modes:
...
All transactions are parsed from the events provided by the CreditChainFramework.  There are a few exceptions to this that use the transaction payload, but only for errors.
...
Additionally, we have our `creditchain-rosetta-cli` crate for local testing.
