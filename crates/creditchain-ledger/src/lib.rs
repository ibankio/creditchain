...
//! # creditchain-ledger
//!
//! `creditchain-ledger` provides convenience methods to communicate with the CreditChain app on ledger

#![deny(missing_docs)]

use creditchain_crypto::ed25519::Ed25519Signature;
pub use creditchain_crypto::{ed25519::Ed25519PublicKey, ValidCryptoMaterialStringExt};
...
/// 637 is the key for CreditChain
/// To allow CreditChain app (with CLA 0x5C) to coexist with the CreditChain app (with CLA 0x5B) on
/// the same Ledger device, you must update both the Ledger firmware and your Rust code
/// if Both CreditChain and legacy app in same ledger, need differ CLA value like ox5C.
pub const DERIVATION_PATH: &str = "m/44'/637'/{index}'/0'/0'";

const CLA_CREDITCHAIN: u8 = 0x5B; // CreditChain CLA Instruction class - same as A-PTOS for now
...
#[derive(Debug, Error)]
/// CreditChain Ledger Error
pub enum CreditChainLedgerError {
    /// Error when trying to open a connection to the Ledger device
    #[error("Device not found")]
    DeviceNotFound,

    /// Error when communicating with CreditChain app on Ledger
    #[error("Error - {0}")]
    CreditChainError(CreditChainLedgerStatusCode),
...
impl From<LedgerHIDError> for CreditChainLedgerError {
    fn from(e: LedgerHIDError) -> Self {
        CreditChainLedgerError::UnexpectedError(e.to_string(), None)
    }
}
...
/// Most CreditChain ones defined here <https://github.com/aptos-labs/ledger-app-aptos/blob/main/doc/COMMANDS.md#status-words>
/// Some of the ledger status code defined here - <https://www.eftlab.com/knowledge-base/complete-list-of-apdu-responses>
pub enum CreditChainLedgerStatusCode {
    /// CreditChain ledger app related status code
    ...
    /// CreditChain ledger app is not opened
    AppNotOpen = 0x6E01,
...
impl CreditChainLedgerStatusCode {
    fn description(&self) -> &str {
        match self {
            CreditChainLedgerStatusCode::Deny => "Request rejected by user",
            CreditChainLedgerStatusCode::WrongPip2 => "Wrong P1 or P2",
            CreditChainLedgerStatusCode::WrongDataLength => "Wrong data length",
            CreditChainLedgerStatusCode::InsNotSupported => "Ins(Instruction) not supported",
            CreditChainLedgerStatusCode::ClaNotSupported => "Cla not supported",
            CreditChainLedgerStatusCode::WrongResponseLength => "Wrong response length",
            CreditChainLedgerStatusCode::DisplayBip32PathFail => "BIP32 path conversion to string failed",
            CreditChainLedgerStatusCode::DisplayAddressFail => "Address conversion to string failed",
            CreditChainLedgerStatusCode::DisplayAmountFail => "Amount conversion to string failed",
            CreditChainLedgerStatusCode::WrongTxnLength => "Wrong raw transaction length",
            CreditChainLedgerStatusCode::TxnParsingFail => "Failed to parse raw transaction",
            CreditChainLedgerStatusCode::TxnHashFail => {
                "Failed to compute hash digest of raw transaction"
            },
            CreditChainLedgerStatusCode::BadState => "Security issue with bad state",
            CreditChainLedgerStatusCode::SignatureFail => "Signature of raw transaction failed",
            CreditChainLedgerStatusCode::Success => "Success",
            CreditChainLedgerStatusCode::LedgerLocked => "Ledger device is locked",
            CreditChainLedgerStatusCode::AppNotOpen => "CreditChain ledger app is not opened",
            CreditChainLedgerStatusCode::Unknown => "Unknown status code",
        }
    }

    fn map_status_code(status_code: u16) -> CreditChainLedgerStatusCode {
        match status_code {
            0x6985 => CreditChainLedgerStatusCode::Deny,
            0x6A86 => CreditChainLedgerStatusCode::WrongPip2,
            0x6A87 => CreditChainLedgerStatusCode::WrongDataLength,
            0x6D00 => CreditChainLedgerStatusCode::InsNotSupported,
            0x6E00 => CreditChainLedgerStatusCode::ClaNotSupported,
            0xB000 => CreditChainLedgerStatusCode::WrongResponseLength,
            0xB001 => CreditChainLedgerStatusCode::DisplayBip32PathFail,
            0xB002 => CreditChainLedgerStatusCode::DisplayAddressFail,
            0xB003 => CreditChainLedgerStatusCode::DisplayAmountFail,
            0xB004 => CreditChainLedgerStatusCode::WrongTxnLength,
            0xB005 => CreditChainLedgerStatusCode::TxnParsingFail,
            0xB006 => CreditChainLedgerStatusCode::TxnHashFail,
            0xB007 => CreditChainLedgerStatusCode::BadState,
            0xB008 => CreditChainLedgerStatusCode::SignatureFail,
            0x9000 => CreditChainLedgerStatusCode::Success,
            0x5515 => CreditChainLedgerStatusCode::LedgerLocked,
            0x6E01 => CreditChainLedgerStatusCode::AppNotOpen,
            _ => CreditChainLedgerStatusCode::Unknown,
        }
    }
}

impl Display for CreditChainLedgerStatusCode {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{} (0x{:x})", self.description(), *self as u16)
    }
}

/// CreditChain version in format major.minor.patch
#[derive(Debug)]
pub struct Version {
    major: u8,
    minor: u8,
...
/// Returns the current version of the CreditChain app on Ledger
pub fn get_app_version() -> Result<Version, CreditChainLedgerError> {
    ...
    match transport.exchange(&APDUCommand {
        ...
        Ok(response) => {
            ...
            if response.retcode() == APDU_CODE_SUCCESS {
                ...
                let error_code = CreditChainLedgerStatusCode::map_status_code(response.retcode());
                Err(CreditChainLedgerError::CreditChainError(error_code))
            }
        },
        Err(err) => Err(CreditChainLedgerError::from(err)),
    }
}

/// Returns the official app name register in Ledger
pub fn get_app_name() -> Result<String, CreditChainLedgerError> {
    // Open connection to ledger
    let transport = open_ledger_transport()?;
        ...
        Ok(response) => {
            if response.retcode() == APDU_CODE_SUCCESS {
                let app_name = match str::from_utf8(response.data()) {
                    Ok(v) => v,
                    Err(e) => return Err(CreditChainLedgerError::UnexpectedError(e.to_string(), None)),
                };
...
pub fn fetch_batch_accounts(
...
) -> Result<BTreeMap<String, AccountAddress>, CreditChainLedgerError> {
    ...
    if range.end - range.start > 10 {
        return Err(CreditChainLedgerError::UnexpectedError(
        ...
        match transport.exchange(&APDUCommand {
            ...
            Ok(response) => {
                ...
                if response.retcode() == APDU_CODE_SUCCESS {
                    ...
                    // Skipping weird 0x04 - because of how the CreditChain Ledger parse works when return pub key
                    offset += 1;
                    ...
                    let public_key = match Ed25519PublicKey::from_encoded_string(&hex_string) {
                        Ok(pk) => Ok(pk),
                        Err(err) => Err(CreditChainLedgerError::UnexpectedError(
                            err.to_string(),
                    ...
                    let error_code = CreditChainLedgerStatusCode::map_status_code(response.retcode());
                    return Err(CreditChainLedgerError::CreditChainError(error_code));
                }
            },
            Err(err) => return Err(CreditChainLedgerError::from(err)),
        }
    }
...
/// Returns the public key of your CreditChain account in Ledger device at index 0
///
/// # Arguments
///
/// * `display` - If true, the public key will be displayed on the Ledger device, and confirmation is needed
pub fn get_public_key(path: &str, display: bool) -> Result<Ed25519PublicKey, CreditChainLedgerError> {
    // Open connection to ledger
    let transport = open_ledger_transport()?;
        ...
        Ok(response) => {
            ...
            if response.retcode() == APDU_CODE_SUCCESS {
                ...
                match Ed25519PublicKey::from_encoded_string(&hex_string) {
                    Ok(pk) => Ok(pk),
                    Err(err) => Err(CreditChainLedgerError::UnexpectedError(err.to_string(), None)),
                }
            } else {
                let error_code = CreditChainLedgerStatusCode::map_status_code(response.retcode());
                Err(CreditChainLedgerError::CreditChainError(error_code))
...
pub fn sign_message(path: &str, raw_message: &[u8]) -> Result<Ed25519Signature, CreditChainLedgerError> {
    // open connection to ledger
    let transport = open_ledger_transport()?;
    ...
    if let Err(err) = sign_start {
        return Err(CreditChainLedgerError::UnexpectedError(err.to_string(), None));
    }
                ...
                if response.retcode() == APDU_CODE_SUCCESS {
                    if is_last_chunk {
                        ...
                        return Ed25519Signature::try_from(signature_buffer).map_err(|err| {
                            CreditChainLedgerError::UnexpectedError(err.to_string(), None)
                        });
                    }
                } else {
                    let error_code = CreditChainLedgerStatusCode::map_status_code(response.retcode());
    ...
    Err(CreditChainLedgerError::UnexpectedError(
...
fn open_ledger_transport() -> Result<TransportNativeHID, CreditChainLedgerError> {
    // open connection to ledger
    // NOTE: ledger has to be unlocked
    let hidapi = match HidApi::new() {
        Ok(hidapi) => hidapi,
        Err(_err) => return Err(CreditChainLedgerError::DeviceNotFound),
    };
