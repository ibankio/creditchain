...
fn random_message(rng: &mut ThreadRng) -> TestCreditChainCrypto {
    ...
    g.bench_function("sig_deserialize", move |b| {
        b.iter_with_setup(
            || {
                ...
                sk.sign(&TestCreditChainCrypto("Hello CreditChain!".to_owned()))
                    .unwrap()
                    .to_bytes()
            },
    ...
    g.bench_function("aggregate_one_sigshare", move |b| {
        b.iter_with_setup(
            || {
                (
                    bls12381::PrivateKey::generate(&mut rng)
                        .sign(&TestCreditChainCrypto("Hello CreditChain!".to_owned()))
                        .unwrap(),
                    bls12381::PrivateKey::generate(&mut rng)
                        .sign(&TestCreditChainCrypto("Hello CreditChain!".to_owned()))
                        .unwrap(),
...
fn verify_aggsig<M: Measurement>(g: &mut BenchmarkGroup<M>, n: usize) {
    ...
    g.bench_with_input(BenchmarkId::new("verify_aggsig", n), &n, |b, &_n| {
        b.iter_batched(
            ...
            |(msgs, pks, aggsig)| {
                let msgs_refs = msgs.iter().collect::<Vec<&TestCreditChainCrypto>>();
