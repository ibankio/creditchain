...
use crate::{
    ...
    test_utils::{random_subset, KeyPair, TestCreditChainCrypto},
...
fn random_message_for_signing(rng: &mut OsRng) -> TestCreditChainCrypto {
    TestCreditChainCrypto(rng.sample_iter(&Alphanumeric).take(256).collect::<String>())
}

/// Returns several 256-character unique strings that can be aggregate-signed by the BLS API.
fn random_messages_for_signing(rng: &mut OsRng, n: usize) -> Vec<TestCreditChainCrypto> {
    (0..n)
        .map(|_| random_message_for_signing(rng))
        .collect::<Vec<TestCreditChainCrypto>>()
}
...
fn bls12381_aggsig_should_verify() {
    ...
    let msgs_refs = messages.iter().collect::<Vec<&TestCreditChainCrypto>>();
    ...
    let msgs_wrong_refs = messages_wrong.iter().collect::<Vec<&TestCreditChainCrypto>>();
...
/// creditchain-move/framework/move-stdlib/sources/signer.move
...
fn bls12381_sample_signature() {
    ...
    let message = b"Hello CreditChain!";
...
fn bls12381_sample_signature_verifies() {
    ...
    assert!(sig.verify_arbitrary_msg(b"Hello CreditChain!", &pk).is_ok());
}
...
fn bls12381_sample_doc_test_for_normal_sigs() {
    ...
    println!("// signature on TestCreditChainCrypto(\"test\".to_owned())");
    println!(
...
fn bls12381_sample_aggregate_pk_and_aggsig() {
    ...
    for i in 1..=num {
        let msg = format!("Hello, CreditChain {}!", i);
        let keypair = KeyPair::<PrivateKey, PublicKey>::generate(&mut rng);
    ...
    println!(
        "// The signed messages are \"Hello, CreditChain <i>!\", where <i> \\in {{1, ..., {}}}",
        num
    );
