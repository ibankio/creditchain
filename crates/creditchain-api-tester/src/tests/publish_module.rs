...
use creditchain_api_types::{HexEncodedBytes, U64};
use creditchain_cached_packages::creditchain_stdlib::EntryFunctionCall;
use creditchain_framework::{BuildOptions, BuiltPackage};
use creditchain_logger::error;
use creditchain_rest_client::Client;
use creditchain_sdk::{bcs, types::LocalAccount};
...
async fn build_module(address: AccountAddress) -> Result<BuiltPackage, TestFailure> {
    // get file to compile
    let move_dir = PathBuf::from("./creditchain-move/move-examples/hello_blockchain");
...
async fn publish_module(
    ...
    // create payload
    let payload: creditchain_types::transaction::TransactionPayload =
