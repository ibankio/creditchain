...
use crate::{
    get_version_path_with_base, Client, DEFAULT_VERSION_PATH_BASE, X_CREDITCHAIN_SDK_HEADER_VALUE,
};
use anyhow::Result;
...
impl CreditChainBaseUrl {
    pub fn to_url(&self) -> Url {
        match self {
            CreditChainBaseUrl::Mainnet => Url::from_str("https://api.mainnet.creditchain.io").unwrap(),
            CreditChainBaseUrl::Devnet => Url::from_str("https://api.devnet.creditchain.io").unwrap(),
            CreditChainBaseUrl::Testnet => Url::from_str("https://api.testnet.creditchain.io").unwrap(),
            CreditChainBaseUrl::Custom(url) => url.to_owned(),
        }
    }
}
...
impl ClientBuilder {
    pub fn new(creditchain_base_url: CreditChainBaseUrl) -> Self {
        let mut headers = HeaderMap::new();
        headers.insert(
            X_CREDITCHAIN_CLIENT,
            HeaderValue::from_static(X_CREDITCHAIN_SDK_HEADER_VALUE),
        );

        let mut client_builder = Self {
            reqwest_builder: ReqwestClient::builder(),
            base_url: creditchain_base_url.to_url(),
