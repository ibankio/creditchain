...
pub use creditchain_metrics_core::{
...
use creditchain_metrics_core::{Encoder, TextEncoder};
use std::{
    env,
    ops::Sub,
    sync::mpsc,
...
impl MetricsPusher {
    fn push(
        ...
        if let Err(e) = TextEncoder::new().encode(&creditchain_metrics_core::gather(), &mut buffer) {
