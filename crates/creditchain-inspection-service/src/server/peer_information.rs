...
use creditchain_data_client::{
    client::CreditChainDataClient, interface::CreditChainDataClientInterface, peer_states,
};
...
pub fn handle_peer_information_request(
    ...
    let (status_code, body) = if node_config.inspection_service.expose_peer_information {
        let peer_information = get_peer_information(creditchain_data_client, peers_and_metadata);
...
fn get_peer_information(
    ...
    display_state_sync_metadata(&mut peer_information_output, &all_peers, creditchain_data_client);
    peer_information_output.push("\n".into());
...
fn display_state_sync_metadata(
    ...
    if let Ok((priority_peers, regular_peers)) = creditchain_data_client.get_priority_and_regular_peers()
    {
    ...
    let peer_to_state = creditchain_data_client.get_peer_states().get_peer_to_states();
    for peer in all_peers {
