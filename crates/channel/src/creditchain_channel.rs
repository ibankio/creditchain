...
//! creditchain_channel provides an mpsc channel which has two ends `creditchain_channel::Receiver`
//! and `creditchain_channel::Sender` similar to existing mpsc data structures.
...
use creditchain_infallible::{Mutex, NonZeroUsize};
...
/// The sending end of the creditchain_channel.
#[derive(Debug)]
pub struct Sender<K: Eq + Hash + Clone, M> {
    shared_state: Arc<Mutex<SharedState<K, M>>>,
}

/// The status of an element inserted into a creditchain_channel. If the element is successfully
...
/// The receiving end of the creditchain_channel.
#[derive(Debug)]
...
/// Configuration for a new creditchain_channel queue.
#[derive(Clone, Copy)]
pub struct Config {
...
impl Config {
    /// The creditchain_channel has a "sub-queue" per key. The `max_capacity` controls
...
pub fn new<K: Eq + Hash + Clone, M>(
    ...
    let max_queue_size_per_key =
        NonZeroUsize!(max_queue_size_per_key, "creditchain_channel cannot be of size 0");
