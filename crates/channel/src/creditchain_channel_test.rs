...
use crate::{creditchain_channel, creditchain_channel::ElementStatus, message_queues::QueueStyle};
...
fn test_send_recv_order() {
    let (sender, mut receiver) = creditchain_channel::new(QueueStyle::FIFO, 10, None);
...
#[test]
fn test_empty() {
    let (_, mut receiver) = creditchain_channel::new::<u8, u8>(QueueStyle::FIFO, 10, None);
    // Ensures that there is no other value which is ready
    assert_eq!(receiver.select_next_some().now_or_never(), None);
}
...
fn test_sender_clone() {
    let (sender, mut receiver) = creditchain_channel::new(QueueStyle::FIFO, 5, None);
    // Ensures that there is no other value which is ready
    assert_eq!(receiver.select_next_some().now_or_never(), None);
...
fn test_multiple_validators_helper(
    ...
    let (sender, mut receiver) = creditchain_channel::new(queue_style, 1, None);
...
fn test_feedback_on_drop() {
    let (sender, mut receiver) = creditchain_channel::new(QueueStyle::FIFO, 3, None);
