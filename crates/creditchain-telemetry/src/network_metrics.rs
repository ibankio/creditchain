...
const CREDITCHAIN_NODE_NETWORK_METRICS: &str = "CREDITCHAIN_NODE_NETWORK_METRICS";
...
pub(crate) async fn create_network_metric_telemetry_event() -> TelemetryEvent {
    ...
    TelemetryEvent {
        name: CREDITCHAIN_NODE_NETWORK_METRICS.into(),
...
fn collect_connection_metrics(network_metrics: &mut BTreeMap<String, String>) {
    ...
    for metric_family in creditchain_network::counters::CREDITCHAIN_CONNECTIONS.collect() {
...
fn collect_message_and_traffic_metrics(network_metrics: &mut BTreeMap<String, String>) {
    ...
    let inbound_metric_families =
        creditchain_network::counters::NETWORK_APPLICATION_INBOUND_METRIC.collect();
    ...
    let outbound_metric_families =
        creditchain_network::counters::NETWORK_APPLICATION_OUTBOUND_METRIC.collect();
