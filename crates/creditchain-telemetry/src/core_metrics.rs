...
use creditchain_state_sync_driver::metrics::StorageSynchronizerOperations;
use creditchain_telemetry_service::types::telemetry::TelemetryEvent;
use prometheus::core::Collector;
use std::collections::BTreeMap;

/// Core metrics event name
const CREDITCHAIN_NODE_CORE_METRICS: &str = "CREDITCHAIN_NODE_CORE_METRICS";
...
pub(crate) async fn create_core_metric_telemetry_event(node_config: &NodeConfig) -> TelemetryEvent {
    ...
    TelemetryEvent {
        name: CREDITCHAIN_NODE_CORE_METRICS.into(),
        params: core_metrics,
    }
}
...
fn collect_consensus_metrics(core_metrics: &mut BTreeMap<String, String>) {
    core_metrics.insert(
        ...
        creditchain_consensus::counters::PROPOSALS_COUNT.get().to_string(),
    );
    core_metrics.insert(
        CONSENSUS_LAST_COMMITTED_ROUND.into(),
        creditchain_consensus::counters::LAST_COMMITTED_ROUND
            .get()
            .to_string(),
    );
    core_metrics.insert(
        CONSENSUS_TIMEOUT_COUNT.into(),
        creditchain_consensus::counters::TIMEOUT_COUNT.get().to_string(),
    );
...
fn collect_mempool_metrics(core_metrics: &mut BTreeMap<String, String>) {
    core_metrics.insert(
        ...
        creditchain_mempool::counters::CORE_MEMPOOL_INDEX_SIZE
...
fn collect_rest_metrics(core_metrics: &mut BTreeMap<String, String>) {
    let rest_response_metrics = creditchain_api::metrics::RESPONSE_STATUS.collect();
...
fn collect_state_sync_metrics(
    ...
    core_metrics.insert(
        ...
        creditchain_state_sync_driver::metrics::STORAGE_SYNCHRONIZER_OPERATIONS
...
fn collect_storage_metrics(core_metrics: &mut BTreeMap<String, String>) {
    core_metrics.insert(
        ...
        creditchain_db::metrics::LEDGER_VERSION.get().to_string(),
    );
    core_metrics.insert(
        STORAGE_MIN_READABLE_LEDGER_VERSION.into(),
        creditchain_db::metrics::PRUNER_VERSIONS
...
fn collect_telemetry_metrics(core_metrics: &mut BTreeMap<String, String>) {
    let telemetry_failure_metrics = crate::metrics::CREDITCHAIN_TELEMETRY_FAILURE.collect();
    ...
    let telemetry_success_metrics = crate::metrics::CREDITCHAIN_TELEMETRY_SUCCESS.collect();
