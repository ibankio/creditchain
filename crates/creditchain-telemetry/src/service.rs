...
use creditchain_logger::{
    creditchain_logger::RUST_LOG_TELEMETRY, prelude::*, telemetry_log_writer::TelemetryLog,
    LoggerFilterUpdater,
};
use creditchain_telemetry_service::types::telemetry::{TelemetryDump, TelemetryEvent};
...
const CREDITCHAIN_NODE_CONFIG_EVENT_NAME: &str = "CREDITCHAIN_NODE_CONFIG";
...
pub fn telemetry_is_disabled() -> bool {
    env::var(ENV_CREDITCHAIN_DISABLE_TELEMETRY).is_ok()
}

/// Flag to force enabling/disabling of telemetry
#[inline]
fn force_enable_telemetry() -> bool {
    env::var(ENV_CREDITCHAIN_FORCE_ENABLE_TELEMETRY).is_ok()
}
...
fn enable_prometheus_push_metrics() -> bool {
    force_enable_telemetry()
        || !(telemetry_is_disabled() || env::var(ENV_CREDITCHAIN_DISABLE_TELEMETRY_PUSH_METRICS).is_ok())
}

#[inline]
fn enable_prometheus_node_metrics() -> bool {
    env::var(ENV_CREDITCHAIN_DISABLE_PROMETHEUS_NODE_METRICS).is_err()
}
...
fn enable_push_logs() -> bool {
    force_enable_telemetry()
        || !(telemetry_is_disabled() || env::var(ENV_CREDITCHAIN_DISABLE_TELEMETRY_PUSH_LOGS).is_ok())
}
...
fn enable_push_custom_events() -> bool {
    force_enable_telemetry()
        || !(telemetry_is_disabled() || env::var(ENV_CREDITCHAIN_DISABLE_TELEMETRY_PUSH_EVENTS).is_ok())
}

#[inline]
fn enable_log_env_polling() -> bool {
    force_enable_telemetry()
        || !(telemetry_is_disabled() || env::var(ENV_CREDITCHAIN_DISABLE_LOG_ENV_POLLING).is_ok())
}
...
pub fn start_telemetry_service(
    ...
    if enable_prometheus_node_metrics() {
        creditchain_node_resource_metrics::register_node_metrics_collector(Some(&build_info));
    }

    // Don't start the service if telemetry has been disabled
    if telemetry_is_disabled() {
        warn!("CreditChain telemetry is disabled!");
        return None;
    }

    // Create the telemetry runtime
    let telemetry_runtime = creditchain_runtimes::spawn_named_runtime("telemetry".into(), None);
...
async fn spawn_telemetry_service(
    ...
    if !force_enable_telemetry() && !telemetry_sender.check_chain_access(chain_id).await {
        warn!(
                "CreditChain telemetry is not sent to the telemetry service because the service is not configured for chain ID {}",
                chain_id
            );
        ...
        loop {
            ...
            if telemetry_sender.check_chain_access(chain_id).await {
                handle.abort();
                info!("CreditChain telemetry service is now configured for Chain ID {}. Starting telemetry service...", chain_id);
                break;
            }
        }
    }
...
async fn send_node_config(
    ...
    let telemetry_event = TelemetryEvent {
        name: CREDITCHAIN_NODE_CONFIG_EVENT_NAME.into(),
        params: node_config,
    };
    send_telemetry_event_with_ip(peer_id, chain_id, telemetry_sender, telemetry_event).await;
}
...
async fn send_telemetry_event(
    ...
    let api_secret =
        env::var(ENV_GA_API_SECRET).unwrap_or_else(|_| CREDITCHAIN_GA_API_SECRET.to_string());
    let measurement_id =
        env::var(ENV_GA_MEASUREMENT_ID).unwrap_or_else(|_| CREDITCHAIN_GA_MEASUREMENT_ID.to_string());
    ...
    if telemetry_sender.is_none() {
        // telemetry_sender is None for CreditChain CLI.
        ...
        // CreditChain nodes send their metrics to creditchain-telemetry-service crate.
