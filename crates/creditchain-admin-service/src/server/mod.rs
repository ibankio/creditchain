...
use creditchain_config::config::{AuthenticationConfig, NodeConfig};
...
use creditchain_system_utils::utils::reply_with_status;
#[cfg(target_os = "linux")]
use creditchain_system_utils::{
...
pub struct Context {
    ...
    creditchain_db: RwLock<Option<Arc<DbReaderWriter>>>,
...
impl Context {
    fn set_creditchain_db(&self, creditchain_db: Arc<DbReaderWriter>) {
        *self.creditchain_db.write() = Some(creditchain_db);
    }
...
impl AdminService {
    ...
    pub fn new(node_config: &NodeConfig) -> Self {
        ...
        let runtime = creditchain_runtimes::spawn_named_runtime("admin".into(), None);
    ...
    pub fn set_creditchain_db(&self, creditchain_db: Arc<DbReaderWriter>) {
        self.context.set_creditchain_db(creditchain_db)
    }
