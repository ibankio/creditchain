...
//!
//! ## A note on `creditchain-crypto` traits
//!
//! We do not implement the `PublicKey` and `PrivateKey` traits from `creditchain-crypto` for our PVSS
...
use anyhow::bail;
use creditchain_crypto::{SigningKey, Uniform, ValidCryptoMaterial, VerifyingKey};
use num_traits::Zero;
