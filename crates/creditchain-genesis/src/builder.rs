...
use creditchain_vm_genesis::default_gas_schedule;
use rand::Rng;
...
fn set_identity_for_network(network: &mut NetworkConfig) -> anyhow::Result<()> {
    if let Identity::None = network.identity {
        ...
        let peer_id = creditchain_types::account_address::from_identity_public_key(key.public_key());
        ...
        R: rand::RngCore + rand::CryptoRng,
    {
        ...
        let fullnode_network_listen_address =
            if let Some(template_fullnode_config) = config.full_node_networks.first() {
                ...
                creditchain_config::utils::get_available_port_in_multiaddr(true)
            };
        ...
        let vfn_network = NetworkConfig {
            listen_address: creditchain_config::utils::get_available_port_in_multiaddr(true),
