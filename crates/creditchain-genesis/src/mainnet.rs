...
use creditchain_vm_genesis::{AccountBalance, EmployeePool, ValidatorWithCommissionRate};
...
impl MainnetGenesisInfo {
    ...
    fn generate_genesis_txn(&self) -> Transaction {
        creditchain_vm_genesis::encode_creditchain_mainnet_genesis_transaction(
            &self.accounts,
            &self.employee_vesting_accounts,
            &self.validators,
            &self.framework,
            self.chain_id,
            &creditchain_vm_genesis::GenesisConfiguration {
    ...
    pub fn generate_waypoint(&mut self) -> anyhow::Result<Waypoint> {
        ...
        let creditchaindb = CreditChainDB::open(
        ...
        let db_rw = DbReaderWriter::new(creditchaindb);
        creditchain_executor::db_bootstrapper::generate_waypoint::<CreditChainVMBlockExecutor>(&db_rw, genesis)
    }
}
