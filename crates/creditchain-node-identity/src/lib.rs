...
use creditchain_types::{chain_id::ChainId, PeerId};
use once_cell::sync::OnceCell;
use std::sync::Arc;

/// The global [CreditChainNodeIdentity]
static CREDITCHAIN_NODE_IDENTITY: OnceCell<Arc<CreditChainNodeIdentity>> = OnceCell::new();

/// Structure that holds information related to a node's identity
pub struct CreditChainNodeIdentity {
...
/// Initializes the [CreditChainNodeIdentity] using the provided [PeerId] and
/// sets it globally exactly once.
pub fn init(peer_id: Option<PeerId>) -> Result<()> {
    let identity = CreditChainNodeIdentity {
        chain_id: OnceCell::new(),
        peer_id,
        peer_id_str: peer_id.map(|id| id.to_string()),
    };

    CREDITCHAIN_NODE_IDENTITY
        .set(Arc::new(identity))
        .map_err(|_| format_err!("CREDITCHAIN_NODE_IDENTITY was already set"))
}

/// Sets the [ChainId] in the global [CreditChainNodeIdentity], returning an error
/// if [init] was not called already.
pub fn set_chain_id(chain_id: ChainId) -> Result<()> {
    match CREDITCHAIN_NODE_IDENTITY.get() {
        Some(identity) => identity
            .chain_id
            .set(chain_id)
            .map_err(|_| format_err!("chain_id was already set.")),
        None => Err(format_err!("CREDITCHAIN_NODE_IDENTITY has not been set yet")),
    }
}

/// Returns the [PeerId] from the global `CREDITCHAIN_NODE_IDENTITY`
...
/// Returns the [PeerId] as [str] from the global `CREDITCHAIN_NODE_IDENTITY`
...
/// Returns the [ChainId] from the global `CREDITCHAIN_NODE_IDENTITY`
...
mod tests {
    ...
    #[test]
    fn test_creditchain_node_identity() {
