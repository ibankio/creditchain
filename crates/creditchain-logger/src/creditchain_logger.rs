...
impl LogEntry {
    fn new(event: &Event, thread_name: Option<&str>, enable_backtrace: bool) -> Self {
        ...
        #[cfg(node_identity)]
        {
            peer_id = creditchain_node_identity::peer_id_as_str();
            chain_id = creditchain_node_identity::chain_id().map(|chain_id| chain_id.id());
        }
...
/// A builder for a `CreditChainData`, configures what, where, and how to write logs.
pub struct CreditChainDataBuilder {
    channel_size: usize,
...
impl CreditChainDataBuilder {
    ...
    fn build_logger(&mut self) -> Arc<CreditChainData> {
        ...
        if self.is_async {
            ...
            let logger = Arc::new(CreditChainData {
    ...
    pub fn build(&mut self) -> Arc<CreditChainData> {
...
impl CreditChainData {
    pub fn builder() -> CreditChainDataBuilder {
        CreditChainDataBuilder::new()
    }

    #[allow(clippy::new_ret_no_self)]
    pub fn new() -> CreditChainDataBuilder {
        Self::builder()
    }

    pub fn init_for_testing() {
        // Create the CreditChain Data Builder
...
impl Logger for CreditChainData {
    fn enabled(&self, metadata: &Metadata) -> bool {
...
/// or to a `CreditChainData` for external logging.
struct LoggerService {
    receiver: sync::mpsc::Receiver<LoggerServiceEvent>,
    printer: Option<Box<dyn Writer>>,
    facade: Arc<CreditChainData>,
...
/// Example:
/// 2020-03-07 05:03:03 INFO [thread_name] common/creditchain-logger/src/lib.rs:261 Hello { "world": true }
...
pub struct LoggerFilterUpdater {
    logger: Arc<CreditChainData>,
    logger_builder: CreditChainDataBuilder,
}

impl LoggerFilterUpdater {
    pub fn new(logger: Arc<CreditChainData>, logger_builder: CreditChainDataBuilder) -> Self {
        Self {
            logger,
            logger_builder,
        }
...
#[cfg(test)]
mod tests {
    use super::{text_format, CreditChainData, LogEntry};
    use crate::{
        creditchain_logger::{json_format, TruncatedLogString, RUST_LOG_TELEMETRY},
        debug, error, info,
        logger::Logger,
        telemetry_log_writer::TelemetryLog,
        trace, warn, CreditChainDataBuilder, Event, Key, KeyValue, Level, LoggerFilterUpdater, Metadata,
    ...
    fn basic() {
        ...
        let expected = format!("{{\"level\":\"INFO\",\"source\":{{\"package\":\"creditchain_logger\",\"file\":\"crates/creditchain-logger/src/creditchain_logger.rs:{line_num}\"}},\"thread_name\":\"{thread_name}\",\"hostname\":\"test-host\",\"timestamp\":\"2022-07-24T23:42:29.540278Z\",\"message\":\"This is a log\",\"data\":{{\"bar\":\"foo_bar\",\"category\":\"name\",\"display\":\"12345\",\"foo\":5,\"test\":true}}}}");
    ...
    fn new_async_logger() -> (CreditChainDataBuilder, Arc<CreditChainData>) {
        let mut logger_builder = CreditChainDataBuilder::new();
    ...
    fn new_text_logger() -> (
        ...
        let logger = CreditChainDataBuilder::new()
            .level(Level::Debug)
