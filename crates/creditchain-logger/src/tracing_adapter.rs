...
/// A layer that translates tracing events into creditchain-logger events.
pub struct TracingToCreditChainDataLayer;
...
impl<S> Layer<S> for TracingToCreditChainDataLayer
where
    S: tracing::Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    fn on_event(&self, event: &Event, ctx: Context<S>) {
        let metadata = match translate_metadata(event.metadata()) {
            ...
            None => {
                dl::warn!(
                    "[tracing-to-creditchain-logger] failed to translate event due to unknown level {:?}",
