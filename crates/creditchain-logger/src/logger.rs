...
pub fn set_global_logger(logger: Arc<dyn Logger>, tokio_console_port: Option<u16>) {
    ...
    // tokio-tracing infrastructure. Otherwise, creditchain-logger intercepts all
    // tracing::log events. In both scenarios *all* creditchain-logger::log events are
    // captured by the creditchain-logger (as usual).
    ...
    if tokio_console_port.is_none() {
        let _ = tracing::subscriber::set_global_default(
            crate::tracing_adapter::TracingToCreditChainDataLayer
