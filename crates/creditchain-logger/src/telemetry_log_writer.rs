...
use crate::counters::{CREDITCHAIN_LOG_INGEST_WRITER_DISCONNECTED, CREDITCHAIN_LOG_INGEST_WRITER_FULL};
use futures::channel;
use std::{
    io::{Error, ErrorKind},
    sync,
...
impl TelemetryLogWriter {
    pub fn write(&mut self, log: String) -> std::io::Result<usize> {
        ...
        match self.tx.try_send(TelemetryLog::Log(log)) {
            ...
            Err(err) => {
                if err.is_full() {
                    CREDITCHAIN_LOG_INGEST_WRITER_FULL.inc_by(len as u64);
                    Err(Error::new(ErrorKind::WouldBlock, "Channel full"))
                } else {
                    CREDITCHAIN_LOG_INGEST_WRITER_DISCONNECTED.inc_by(len as u64);
