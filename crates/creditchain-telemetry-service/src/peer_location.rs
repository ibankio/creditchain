        ...
        tokio::spawn(async move {
            loop {
                match query_peer_locations(&self.client).await {
                    ...
                    Err(e) => {
                        creditchain_logger::error!("Failed to query peer locations: {}", e);
                    },
                }
...
fn get_chain_id() -> ChainId {
    match env::var("GCP_METADATA_PROJECT_ID") {
        Ok(val) if val == "creditchain-telemetry-svc-mainnet" => ChainId::mainnet(),
        Ok(val) if val == "creditchain-telemetry-svc-dev" => ChainId::testnet(),
        _ => {
            creditchain_logger::warn!("Unknown GCP_METADATA_PROJECT_ID, defaulting to test");
            ChainId::test()
        },
    }
}
...
mod tests {
    ...
    #[tokio::test]
    async fn test_query() {
        env::set_var("GCP_METADATA_PROJECT_ID", "creditchain-telemetry-svc-dev");
