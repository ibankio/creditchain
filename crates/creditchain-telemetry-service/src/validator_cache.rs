...
use creditchain_config::config::{Peer, PeerRole, PeerSet};
use creditchain_infallible::RwLock;
use creditchain_rest_client::Response;
...
impl PeerSetCacheUpdater {
    ...
    async fn update_for_chain(
        ...
        let client = creditchain_rest_client::Client::new(Url::parse(url).map_err(|e| {
...
mod tests {
    ...
    use creditchain_infallible::RwLock;
    use creditchain_rest_client::creditchain_api_types::*;
    use creditchain_types::{
    ...
    async fn test_validator_cache_updater_with_invalid_address() {
        ...
        let mock = server.mock(|when, then| {
            ...
            then.status(200)
                ...
                .header(X_CREDITCHAIN_CHAIN_ID, "25")
                .header(X_CREDITCHAIN_EPOCH, "10")
                .header(X_CREDITCHAIN_LEDGER_VERSION, "10")
                .header(X_CREDITCHAIN_LEDGER_OLDEST_VERSION, "2")
                .header(X_CREDITCHAIN_BLOCK_HEIGHT, "25")
                .header(X_CREDITCHAIN_OLDEST_BLOCK_HEIGHT, "10")
                .header(X_CREDITCHAIN_LEDGER_TIMESTAMP, "10");
        });
    ...
    async fn test_validator_cache_updater_with_valid_address() {
        ...
        let validator_info = ValidatorInfo::new(
            ...
            ValidatorConfig::new(
                ...
                bcs::to_bytes(&vec![NetworkAddress::from_str("/dns/fullnode0.testnet.creditchain.io/tcp/6182/noise-ik/0xea19ab47ed9191865f15d85d751ed0663205c0b2f0f465714b1947c023715973/handshake/0").unwrap()]).unwrap(),
                2,
            ),
        );
