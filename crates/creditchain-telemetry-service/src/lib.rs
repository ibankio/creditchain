...
#[clap(name = "CreditChain Telemetry Service", author, version)]
pub struct CreditChainTelemetryServiceArgs {
    #[clap(short = 'f', long, value_parser)]
    config_path: PathBuf,
}

impl CreditChainTelemetryServiceArgs {
    pub async fn run(self) {
        ...
        let validators = Arc::new(creditchain_infallible::RwLock::new(HashMap::new()));
        let validator_fullnodes = Arc::new(creditchain_infallible::RwLock::new(HashMap::new()));
        let peer_locations = Arc::new(creditchain_infallible::RwLock::new(HashMap::new()));
...
fn verify_tool() {
    ...
    CreditChainTelemetryServiceArgs::command().debug_assert()
}
