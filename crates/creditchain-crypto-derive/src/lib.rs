...
//! - the derive macros for `creditchain_crypto::traits`, namely `ValidCryptoMaterial`, `PublicKey`, `PrivateKey`,
//!   `VerifyingKey`, `SigningKey` and `Signature` are meant to be derived on simple unions of types
//!   implementing these traits.
//! - the derive macro for `creditchain_crypto::hash::CryptoHasher`, which defines
//!   the domain-separation hasher structures described in `creditchain_crypto::hash`
...
// creditchain_crypto::unit_tests::cryptohasher — you may have to modify it if you
// edit the below.
...
pub fn hasher_dispatch(input: TokenStream) -> TokenStream {
    ...
    let out = quote!(
        ...
        #[derive(Clone)]
        pub struct #hasher_name(creditchain_crypto::hash::DefaultHasher);

        static #static_seed_name: creditchain_crypto::_once_cell::sync::OnceCell<[u8; 32]> = creditchain_crypto::_once_cell::sync::OnceCell::new();

        impl #hasher_name {
            fn new() -> Self {
                let name = creditchain_crypto::_serde_name::trace_name::<#type_name #param>()
                    .expect("The `CryptoHasher` macro only applies to structs and enums");
                #hasher_name(
                    creditchain_crypto::hash::DefaultHasher::new(&name.as_bytes()))
            }
        }

        static #static_hasher_name: creditchain_crypto::_once_cell::sync::Lazy<#hasher_name> =
            creditchain_crypto::_once_cell::sync::Lazy::new(|| #hasher_name::new());
        ...
        impl creditchain_crypto::hash::CryptoHasher for #hasher_name {
            fn seed() -> &'static [u8; 32] {
                #static_seed_name.get_or_init(|| {
                    let name = creditchain_crypto::_serde_name::trace_name::<#type_name #param>()
                        .expect("The `CryptoHasher` macro only applies to structs and enums.").as_bytes();
                    creditchain_crypto::hash::DefaultHasher::prefixed_hash(&name)
                })
            }
            ...
            fn finish(self) -> creditchain_crypto::hash::HashValue {
                self.0.finish()
            }
        }
        ...
        impl std::io::Write for #hasher_name {
            fn write(&mut self, bytes: &[u8]) -> std::io::Result<usize> {
                use creditchain_crypto::hash::CryptoHasher;
...
pub fn bcs_crypto_hash_dispatch(input: TokenStream) -> TokenStream {
    ...
    let out = quote!(
        impl #impl_generics creditchain_crypto::hash::CryptoHash for #name #ty_generics #where_clause {
            type Hasher = #hasher_name;

            fn hash(&self) -> creditchain_crypto::hash::HashValue {
                use creditchain_crypto::hash::CryptoHasher;
