...
In all cases, if a directory holds a crate, the name of that crate is `creditchain-faucet-<directory>`. For example the name of the crate in `metrics-server/` is `creditchain-faucet-metrics-server`.
...
```
cargo run -p creditchain-faucet-service -- run-simple --key <private_key> --node-url <api_url> --chain-id TESTING
```
...
cargo run -p creditchain-faucet-service -- run-simple --key ~/.libra2/testnet/mint.key --node-url http://127.0.0.1:8080 --chain-id TESTING
```
...
```
cargo run -p creditchain-faucet-service -- run -c <path_to_config_file>
```
...
```
cd creditchain-move/move-examples/scripts/minter
libra2 move compile
```

If you have issues with this, try deleting `~/.move`, updating your CreditChain CLI, and changing the CreditChainFramework version.
...
```
cd crates/creditchain-faucet/configs
ls . | xargs -I@ cargo run -p creditchain-faucet-service -- validate-config -c @
```
