...
use crate::endpoints::{CreditChainTapError, CreditChainTapErrorCode};
use anyhow::{Context, Result};
use creditchain_logger::info;
use creditchain_sdk::{
    crypto::ed25519::Ed25519PublicKey,
    rest_client::{CreditChainBaseUrl, Client},
...
//     "../../../../../creditchain-move/move-examples/scripts/minter/build/Minter/bytecode_scripts/main.mv"
// );
...
pub struct MintFunder {
    /// URL of an CreditChain node API.
    node_url: Url,
...
impl MintFunder {
    ...
    async fn get_gas_unit_price(&self) -> Result<u64, CreditChainTapError> {
        match self.txn_config.gas_unit_price_override {
            ...
            None => self
                ...
                .await
                .map_err(|e| {
                    CreditChainTapError::new_with_error_code(e, CreditChainTapErrorCode::CreditChainApiError)
                }),
        }
    }

    async fn get_transaction_factory(&self) -> Result<TransactionFactory, CreditChainTapError> {
        Ok(self
            .transaction_factory
            .clone()
    ...
    pub async fn use_delegated_account(&mut self) -> Result<()> {
        ...
        // Delegate minting to the account
        {
            ...
            client
                .submit_and_wait(&faucet_account.sign_with_transaction_builder(
                    transaction_factory.payload(creditchain_stdlib::creditchain_coin_delegate_mint_capability(
        ...
        client
            .submit_and_wait(&delegated_account.sign_with_transaction_builder(
                transaction_factory.payload(creditchain_stdlib::creditchain_coin_claim_mint_capability()),
            ))
            .await
    ...
    pub fn get_api_client(&self) -> Client {
        let mut builder = Client::builder(CreditChainBaseUrl::Custom(self.node_url.clone()));
    ...
    pub async fn process(
    ...
    ) -> Result<Vec<SignedTransaction>, CreditChainTapError> {
        ...
        if receiver_seq.is_some() && amount == 0 {
            return Err(CreditChainTapError::new(
                format!(
        ...
        let txn = {
            let faucet_account = self.faucet_account.write().await;
            let transaction_factory = self.get_transaction_factory().await?;
            // Use CreditChain entry function to mint CreditChainCoin directly to the receiver.
            // This avoids legacy scripts and LINKER_ERRORs due to missing modules.
            let payload = creditchain_stdlib::creditchain_coin_mint(receiver_address, amount);
