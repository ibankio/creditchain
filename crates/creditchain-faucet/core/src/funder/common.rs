...
use crate::{
    endpoints::{CreditChainTapError, CreditChainTapErrorCode},
...
const DEFAULT_KEY_FILE_PATH: &str = "/opt/creditchain/etc/mint.key";
...
pub struct ApiConnectionConfig {
    /// CreditChain node (any node type with an open API) server URL.
    /// Include the port in this if not using the default for the scheme.
    #[clap(long, default_value = "https://fullnode.testnet.creditchain.io/")]
    pub node_url: Url,
...
impl ApiConnectionConfig {
    ...
    pub fn get_key(&self) -> Result<Ed25519PrivateKey> {
        ...
        let result = creditchain_sdk::bcs::from_bytes(&key_bytes); //.with_context(|| "bad bcs");
        if let Ok(x) = result {
            return Ok(x);
        }
...
pub async fn update_sequence_numbers(
    ...
    amount: u64,
    wait_for_outstanding_txns_secs: u64,
) -> Result<(u64, Option<u64>), CreditChainTapError> {
    ...
    let (result, event_on_success) = if wait_for_transactions {
        ...
        (
            client
                ...
                .map(|_| ())
                .map_err(|e| {
                    CreditChainTapError::new_with_error_code(e, CreditChainTapErrorCode::TransactionFailed)
                }),
            "transaction_success",
        )
    } else {
...
pub struct GasUnitPriceManager {
    api_client: creditchain_sdk::rest_client::Client,
...
impl GasUnitPriceManager {
    pub fn new(node_url: Url, cache_ttl: Duration) -> Self {
        Self {
            api_client: creditchain_sdk::rest_client::Client::new(node_url),
