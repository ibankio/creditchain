...
use crate::{
    endpoints::{CreditChainTapError, CreditChainTapErrorCode, RejectionReason, RejectionReasonCode},
...
impl TransferFunder {
    ...
    ) -> Result<SignedTransaction, CreditChainTapError> {
    ...
    async fn is_healthy_as_result(&self) -> Result<(), CreditChainTapError> {
    ...
    async fn fund(
        ...
        let transactions = if check_only {
            ...
            let txn = self
                .execute_transaction(
                    &client,
                    creditchain_stdlib::creditchain_account_transfer(receiver_address, amount),
                    &receiver_address,
                )
                .await?;
            info!(
