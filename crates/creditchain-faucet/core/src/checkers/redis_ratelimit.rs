...
impl RatelimitKeyProvider {
    ...
    /// JWT's `sub` field).
    pub async fn ratelimit_key_value(&self, data: &CheckerData) -> Result<String, CreditChainTapError> {
        match self {
...
impl RedisRatelimitChecker {
    ...
    pub async fn get_redis_connection(&self) -> Result<Connection, CreditChainTapError> {
        self.db_pool.get().await.map_err(|e| {
            CreditChainTapError::new_with_error_code(
...
impl CheckerTrait for RedisRatelimitChecker {
    async fn check(
        ...
        data: CheckerData,
        dry_run: bool,
    ) -> Result<Vec<RejectionReason>, CreditChainTapError> {
        let mut conn = self
            .get_redis_connection()
            .await
            .map_err(|e| CreditChainTapError::new_with_error_code(e, CreditChainTapErrorCode::StorageError))?;
    ...
    async fn complete(&self, data: CompleteData) -> Result<(), CreditChainTapError> {
