...
use crate::endpoints::{CreditChainTapError, CreditChainTapErrorCode, RejectionReason, RejectionReasonCode};
use anyhow::Result;
    ...
    async fn check(
        ...
        let captcha_token = match data.headers.get(COMPLETED_CAPTCHA_TOKEN) {
            Some(header_value) => header_value.to_str().map_err(|e| {
                CreditChainTapError::new_with_error_code(e, CreditChainTapErrorCode::InvalidRequest)
            })?,
            None => {
        ...
        let verify_result = reqwest::Client::new()
            ...
            .send()
            .await
            .map_err(|e| CreditChainTapError::new_with_error_code(e, CreditChainTapErrorCode::CheckerError))?;
