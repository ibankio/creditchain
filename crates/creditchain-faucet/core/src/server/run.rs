...
use creditchain_faucet_metrics_server::{run_metrics_server, MetricsServerConfig};
use creditchain_logger::info;
use creditchain_sdk::{
    crypto::ed25519::Ed25519PrivateKey,
    types::{account_config::creditchain_test_root_address, chain_id::ChainId},
};
use clap::Parser;
    ...
    pub fn build_for_cli(
        ...
        Self {
            ...
            funder_config: FunderConfig::MintFunder(MintFunderConfig {
                ...
                mint_account_address: Some(creditchain_test_root_address()),
                do_not_delegate,
            }),
            handler_config: HandlerConfig {
...
mod test {
    ...
    use crate::{
        endpoints::{
            CreditChainTapError, CreditChainTapErrorCode, FundRequest, FundResponse, RejectionReasonCode,
        },
    ...
    async fn start_server(config_content: &'static str) -> Result<(u16, JoinHandle<Result<()>>)> {
        ...
        let port = creditchain_config::utils::get_available_port();
    ...
    async fn test_checkers() -> Result<()> {
        ...
        let response = reqwest::Client::new()
            ...
            .send()
            .await?;
        let creditchain_error = CreditChainTapError::parse_from_json_string(&response.text().await?)
            .expect("Failed to read response as CreditChainError");
        assert!(!creditchain_error.rejection_reasons.is_empty());
        ...
        let creditchain_error = CreditChainTapError::parse_from_json_string(&response.text().await?)
            .expect("Failed to read response as CreditChainError");
        let rejection_reason_codes: HashSet<RejectionReasonCode> = creditchain_error
            .rejection_reasons
            .into_iter()
            .map(|r| r.get_code())
            .collect();
    ...
    async fn test_redis_ratelimiter() -> Result<()> {
        ...
        let creditchain_node_api_client = creditchain_sdk::rest_client::Client::new(
            reqwest::Url::from_str("http://127.0.0.1:8080").unwrap(),
        );
        creditchain_node_api_client
            .get_index_bcs()
            .await
    ...
    async fn test_transfer_health() -> Result<()> {
        ...
        let serialized_keys = creditchain_sdk::bcs::to_bytes(&private_key)?;
        ...
        unwrap_reqwest_result(
            reqwest::Client::new()
                .post("https://faucet.devnet.creditchain.io/fund")
                .body(
                    FundRequest {
        ...
        let creditchain_error = CreditChainTapError::parse_from_json_string(&response.text().await?)
            .expect("Failed to read response as CreditChainError");
        assert_eq!(
            creditchain_error.error_code,
            CreditChainTapErrorCode::FunderAccountProblem
        );
    ...
    async fn test_mint_funder() -> Result<()> {
        ...
        let response = creditchain_node_api_client
