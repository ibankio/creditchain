...
pub struct CreditChainTapError {
    /// A message describing the error
    pub message: String,
    /// A code describing the error for programmatic use.
    pub error_code: CreditChainTapErrorCode,
...
impl CreditChainTapError {
    pub fn new(message: String, error_code: CreditChainTapErrorCode) -> Self {
        if *USE_HELPFUL_ERRORS.get().unwrap_or(&true) {
            ...
            Self {
                ...
                error_code: CreditChainTapErrorCode::YeahNahYeahYeahYeahNahYeahNah,
                rejection_reasons: vec![],
                txn_hashes: vec![],
            }
        }
    ...
    pub fn new_with_error_code<ErrorType: std::fmt::Display>(
        error: ErrorType,
        error_code: CreditChainTapErrorCode,
    ) -> CreditChainTapError {
...
impl std::fmt::Display for CreditChainTapError {
...
impl std::error::Error for CreditChainTapError {}

// This is only really necessary for the type of the error in the Result
// returned by the endpoint handlers.
#[derive(Debug, ApiResponse)]
pub enum CreditChainTapErrorResponse {
    Default(
        StatusCode,
        Json<CreditChainTapError>,
        #[oai(header = "Retry-After")] Option<u64>,
    ),
}

impl From<CreditChainTapError> for CreditChainTapErrorResponse {
    fn from(error: CreditChainTapError) -> Self {
...
#[repr(u32)]
pub enum CreditChainTapErrorCode {
    ...
    /// Failed when making requests to the CreditChain API.
    CreditChainApiError = 50,
...
impl CreditChainTapErrorCode {
    pub fn status(&self) -> StatusCode {
        match self {
            CreditChainTapErrorCode::InvalidRequest
            | CreditChainTapErrorCode::AccountDoesNotExist
            | CreditChainTapErrorCode::EndpointNotEnabled => StatusCode::BAD_REQUEST,
            CreditChainTapErrorCode::Rejected
            | CreditChainTapErrorCode::SourceIpMissing
            | CreditChainTapErrorCode::TransactionFailed
            | CreditChainTapErrorCode::AuthTokenInvalid => StatusCode::FORBIDDEN,
            CreditChainTapErrorCode::CreditChainApiError
            | CreditChainTapErrorCode::TransactionTimedOut
            | CreditChainTapErrorCode::SerializationError
            | CreditChainTapErrorCode::BypasserError
            | CreditChainTapErrorCode::CheckerError
            | CreditChainTapErrorCode::StorageError
            | CreditChainTapErrorCode::InternalError => StatusCode::INTERNAL_SERVER_ERROR,
            CreditChainTapErrorCode::ServerOverloaded | CreditChainTapErrorCode::FunderAccountProblem => {
                StatusCode::SERVICE_UNAVAILABLE
            },
            CreditChainTapErrorCode::YeahNahYeahYeahYeahNahYeahNah => StatusCode::IM_A_TEAPOT,
            // We shouldn't get here, this code is only used in error_converter.rs.
            CreditChainTapErrorCode::WebFrameworkError => StatusCode::INTERNAL_SERVER_ERROR,
        }
    }
}
