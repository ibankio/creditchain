...
use super::{errors::CreditChainTapErrorResponse, ApiTags, CreditChainTapError, CreditChainTapErrorCode};
...
impl CaptchaApi {
    ...
    async fn request_captcha(&self) -> Result<Response<Binary<Vec<u8>>>, CreditChainTapErrorResponse> {
        if !self.enabled {
            return Err(CreditChainTapError::new(
                "The CaptchaChecker is not enabled".to_string(),
                CreditChainTapErrorCode::EndpointNotEnabled,
            )
            .into());
        }
        ...
        let (key, image) = match captcha_manager.create_challenge() {
            ...
            Err(e) => {
                return Err(
                    CreditChainTapError::new_with_error_code(e, CreditChainTapErrorCode::CheckerError).into(),
                );
            },
        };
