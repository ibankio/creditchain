...
use super::errors::CreditChainTapErrorResponse;
...
/// be a JSON representation of CreditChainTapError. For our own errors, this is exactly
...
pub async fn convert_error(error: poem::Error) -> impl poem::IntoResponse {
    ...
    if is_framework_error {
        // Build an CreditChainTapErrorResponse and then reset its status code
...
fn build_error_response(error_string: String) -> Response {
    CreditChainTapErrorResponse::from(CreditChainTapError::new_with_error_code(
        error_string,
        CreditChainTapErrorCode::WebFrameworkError,
    ))
    .into_response()
}
