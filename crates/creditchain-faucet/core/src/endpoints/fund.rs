...
use super::{
    errors::{CreditChainTapError, CreditChainTapErrorResponse},
    ApiTags,
};
use crate::{
    bypasser::{Bypasser, BypasserTrait},
    checkers::{Checker, CheckerData, CheckerTrait, CompleteData},
    endpoints::CreditChainTapErrorCode,
...
impl FundApi {
    ...
    ) -> poem::Result<Json<FundResponse>, CreditChainTapErrorResponse> {
        let txns = self
            .components
    ...
    async fn is_eligible(
        ...
        // Same thing, this uses FromRequest.
        header_map: &HeaderMap,
    ) -> poem::Result<(), CreditChainTapErrorResponse> {
...
impl FundApiComponents {
    ...
    async fn preprocess_request(
        ...
        header_map: &HeaderMap,
        dry_run: bool,
    ) -> poem::Result<(CheckerData, bool, Option<SemaphorePermit>), CreditChainTapError> {
            ...
            Some(semaphore) => match semaphore.try_acquire() {
                ...
                Err(_) => {
                    return Err(CreditChainTapError::new(
                        ...
                        CreditChainTapErrorCode::ServerOverloaded,
                    ))
                },
            },
            None => None,
        ...
        for bypasser in &self.bypassers {
            if bypasser
                ...
                .await
                .map_err(|e| {
                    CreditChainTapError::new_with_error_code(e, CreditChainTapErrorCode::BypasserError)
                })?
            {
                info!(
        ...
        for checker in &self.checkers {
            rejection_reasons.extend(checker.check(checker_data.clone(), dry_run).await.map_err(
                |e| CreditChainTapError::new_with_error_code(e, CreditChainTapErrorCode::CheckerError),
            )?);
    ...
    async fn fund_inner(
        ...
        header_map: &HeaderMap,
        dry_run: bool,
    ) -> poem::Result<Vec<SignedTransaction>, CreditChainTapError> {
        let (checker_data, bypass, _semaphore_permit) = self
        ...
        if !bypass {
            ...
            for checker in &self.checkers {
                checker.complete(complete_data.clone()).await.map_err(|e| {
                    CreditChainTapError::new_with_error_code(e, CreditChainTapErrorCode::CheckerError)
                })?;
            }
        }
...
pub async fn mint(
    ...
    // We take the CreditChainTapError and convert it into an anyhow error with just the
    ...
    if return_txns.unwrap_or(false) {
        let txn_bcs =
            creditchain_sdk::bcs::to_bytes(&txns).map_err(|e| poem::Error::from(anyhow::anyhow!(e)))?;
