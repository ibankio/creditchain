...
use creditchain_config::config::SafetyRulesConfig;
...
pub struct EpochManager<P: OnChainConfigProvider> {
    ...
    jwk_updated_event_txs: Option<creditchain_channel::Sender<(), ObservedJWKsUpdated>>,
    jwk_rpc_msg_tx:
...
impl<P: OnChainConfigProvider> EpochManager<P> {
    ...
    async fn start_new_epoch(&mut self, payload: OnChainConfigPayload<P>) -> Result<()> {
        ...
        if jwk_manager_should_run && my_index.is_some() {
            ...
            let (jwk_event_tx, jwk_event_rx) = creditchain_channel::new(QueueStyle::KLAST, 1, None);
            self.jwk_updated_event_txs = Some(jwk_event_tx);
            let (jwk_rpc_msg_tx, jwk_rpc_msg_rx) = creditchain_channel::new(QueueStyle::FIFO, 100, None);
