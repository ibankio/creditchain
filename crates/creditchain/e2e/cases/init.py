...
@test_case
def test_creditchain_header_included(run_helper: RunHelper, test_name=None):
    # Make sure the creditchain-cli header is included on the original request
    response = requests.get(run_helper.get_metrics_url())

    if 'request_source_client="creditchain-cli' not in response.text:
        raise TestError("Request should contain the correct libra2 header: creditchain-cli")
