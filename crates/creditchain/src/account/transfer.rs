...
use creditchain_rest_client::{
    creditchain_api_types::{HashValue, WriteResource, WriteSetChange},
    Transaction,
};
...
impl CliCommand<TransferSummary> for TransferCoins {
    ...
    async fn execute(self) -> CliTypedResult<TransferSummary> {
        self.txn_options
            .submit_transaction(creditchain_stdlib::creditchain_account_transfer(
                self.account,
                self.amount,
            ))
            .await
...
const SUPPORTED_COINS: [&str; 1] = ["0x1::coin::CoinStore<0x1::creditchain_coin::CreditChainCoin>"];
