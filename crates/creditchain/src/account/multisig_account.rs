...
use creditchain_rest_client::{
    creditchain_api_types::{HexEncodedBytes, WriteResource, WriteSetChange},
    Transaction,
};
...
impl CliCommand<CreateSummary> for Create {
    ...
    async fn execute(self) -> CliTypedResult<CreateSummary> {
        self.txn_options
            .submit_transaction(creditchain_stdlib::multisig_account_create_with_owners(
...
impl CliCommand<TransactionSummary> for CreateTransaction {
    ...
    async fn execute(self) -> CliTypedResult<TransactionSummary> {
        ...
        let transaction_payload = if self.store_hash_only {
            creditchain_stdlib::multisig_account_create_transaction_with_hash(
                self.multisig_account.multisig_address,
                HashValue::sha3_256_of(&multisig_transaction_payload_bytes).to_vec(),
            )
        } else {
            creditchain_stdlib::multisig_account_create_transaction(
                self.multisig_account.multisig_address,
    ...
    async fn execute(self) -> CliTypedResult<serde_json::Value> {
        ...
        if expected_payload_hash.eq(&actual_payload_hash) {
            ...
            Err(CliError::UnexpectedError(format!(
                ...
                https://docs.creditchain.io/move/move-on-aptos/cli#multisig-governance"
            )))
        }
    }
}
...
impl CliCommand<TransactionSummary> for Approve {
    ...
    async fn execute(self) -> CliTypedResult<TransactionSummary> {
        self.txn_options
            .submit_transaction(creditchain_stdlib::multisig_account_approve_transaction(
...
impl CliCommand<TransactionSummary> for Reject {
    ...
    async fn execute(self) -> CliTypedResult<TransactionSummary> {
        self.txn_options
            .submit_transaction(creditchain_stdlib::multisig_account_reject_transaction(
                self.multisig_account_with_sequence_number
                    .multisig_account
                    .multisig_address,
                self.multisig_account_with_sequence_number.sequence_number,
...
impl CliCommand<TransactionSummary> for ExecuteReject {
    ...
    async fn execute(self) -> CliTypedResult<TransactionSummary> {
        self.txn_options
            .submit_transaction(creditchain_stdlib::multisig_account_execute_rejected_transaction(
                self.multisig_account.multisig_address,
            ))
            .await
            .map(|inner| inner.into())
