...
use creditchain_rest_client::{
    creditchain_api_types::{Address, HexEncodedBytes, U128, U64},
    Client, Transaction,
};
use creditchain_sdk::move_types::language_storage::CORE_CODE_ADDRESS;
use creditchain_types::{
    account_address::AccountAddress,
    account_config::is_creditchain_governance_create_proposal_event,
    event::EventHandle,
    ...
    async fn execute(mut self) -> CliTypedResult<Vec<ProposalSummary>> {
        ...
        let events = client
            .get_account_events_bcs(
                AccountAddress::ONE,
                "0x1::creditchain_governance::GovernanceEvents",
...
async fn get_proposal(
    client: &creditchain_rest_client::Client,
...
impl CliCommand<ProposalSubmissionSummary> for SubmitProposal {
    ...
    async fn execute(mut self) -> CliTypedResult<ProposalSubmissionSummary> {
        ...
        let txn: Transaction = if self.args.is_multi_step {
            self.args
                .txn_options
                .submit_transaction(creditchain_stdlib::creditchain_governance_create_proposal_v2(
            ...
            self.args
                .txn_options
                .submit_transaction(creditchain_stdlib::creditchain_governance_create_proposal(
                    self.pool_address_args.pool_address,
                    script_hash.to_vec(),
                    self.args.metadata_url.to_string().as_bytes().to_vec(),
                    metadata_hash.to_hex().as_bytes().to_vec(),
    ...
    if let Transaction::UserTransaction(inner) = txn {
        ...
        let proposal_id =
            if let Some(event) = inner.events.iter().find(|event| {
                is_creditchain_governance_create_proposal_event(event.typ.to_string().as_str())
            }) {
    ...
    async fn vote_before_partial_governance_voting(
        ...
        let voting_records = client
            .get_account_resource_bcs::<VotingRecords>(
                CORE_CODE_ADDRESS,
                "0x1::creditchain_governance::VotingRecords",
            )
            .await
            .unwrap()
            .into_inner()
        ...
        for pool_address in &self.pool_addresses {
            let voting_record = client
                .get_table_item(
                    voting_records,
                    "0x1::creditchain_governance::RecordKey",
                    "bool",
                    VotingRecord {
            ...
            summaries.push(
                self.args
                    .txn_options
                    .submit_transaction(creditchain_stdlib::creditchain_governance_vote(
                        *pool_address,
                        proposal_id,
                        vote,
                    ))
        ...
        for pool_address in &self.pool_addresses {
            let remaining_voting_power = self
                ...
                .view(ViewFunction {
                    module: ModuleId::new(
                        AccountAddress::ONE,
                        ident_str!("creditchain_governance").to_owned(),
                    ),
            ...
            summaries.push(
                self.args
                    .txn_options
                    .submit_transaction(creditchain_stdlib::creditchain_governance_partial_vote(
                        *pool_address,
                        proposal_id,
                        voting_power,
                        vote,
...
/// This is needed for upgrading large packages such as creditchain-framework.
#[derive(Parser)]
...
impl CliCommand<TransactionSummary> for ApproveExecutionHash {
    ...
    async fn execute(mut self) -> CliTypedResult<TransactionSummary> {
        Ok(self
            .txn_options
            .submit_transaction(
                creditchain_stdlib::creditchain_governance_add_approved_script_hash_script(self.proposal_id),
            )
            .await
            .map(TransactionSummary::from)?)
    }
...
struct JsonProposal {
    ...
    execution_hash: creditchain_rest_client::creditchain_api_types::HashValue,
    expiration_secs: U64,
    is_resolved: bool,
    min_vote_threshold: U128,
    no_votes: U128,
