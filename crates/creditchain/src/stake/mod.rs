            ...
            match stake_pool.pool_type {
                StakePoolType::Direct => {
                    transaction_summaries.push(
                        self.txn_options
                            .submit_transaction(creditchain_stdlib::stake_add_stake(amount))
                            .await
                ...
                StakePoolType::StakingContract => {
                    transaction_summaries.push(
                        self.txn_options
                            .submit_transaction(creditchain_stdlib::staking_contract_add_stake(
        ...
        for stake_pool in stake_pool_results {
            match stake_pool.pool_type {
                StakePoolType::Direct => {
                    transaction_summaries.push(
                        self.txn_options
                            .submit_transaction(creditchain_stdlib::stake_unlock(amount))
                            .await
                            .map(|inner| inner.into())?,
                    );
                },
                StakePoolType::StakingContract => {
                    transaction_summaries.push(
                        self.txn_options
                            .submit_transaction(creditchain_stdlib::staking_contract_unlock_stake(
                                stake_pool.operator_address,
                                amount,
                            ))
                            .await
        ...
        for stake_pool in stake_pool_results {
            match stake_pool.pool_type {
                StakePoolType::Direct => {
                    transaction_summaries.push(
                        self.node_op_options
                            .submit_transaction(creditchain_stdlib::stake_withdraw(amount))
                            .await
                            .map(|inner| inner.into())?,
                    );
                },
                StakePoolType::StakingContract => {
                    transaction_summaries.push(
                        self.node_op_options
                            .submit_transaction(creditchain_stdlib::staking_contract_distribute(
                                owner_address,
                                stake_pool.operator_address,
                            ))
                            .await
        ...
        for stake_pool in stake_pool_results {
            match stake_pool.pool_type {
                StakePoolType::Direct => {
                    transaction_summaries.push(
                        self.txn_options
                            .submit_transaction(creditchain_stdlib::stake_increase_lockup())
                            .await
                            .map(|inner| inner.into())?,
                    );
                },
                StakePoolType::StakingContract => {
                    transaction_summaries.push(
                        self.txn_options
                            .submit_transaction(creditchain_stdlib::staking_contract_reset_lockup(
                                stake_pool.operator_address,
                            ))
                            .await
                            .map(|inner| inner.into())?,
                ...
                StakePoolType::Vesting => {
                    transaction_summaries.push(
                        self.txn_options
                            .submit_transaction(creditchain_stdlib::vesting_reset_lockup(
...
impl CliCommand<TransactionSummary> for InitializeStakeOwner {
    ...
    async fn execute(mut self) -> CliTypedResult<TransactionSummary> {
        let owner_address = self.txn_options.sender_address()?;
        self.txn_options
            .submit_transaction(creditchain_stdlib::stake_initialize_stake_owner(
        ...
        for stake_pool in stake_pool_results {
            match stake_pool.pool_type {
                StakePoolType::Direct => {
                    transaction_summaries.push(
                        self.txn_options
                            .submit_transaction(creditchain_stdlib::stake_set_operator(
                ...
                StakePoolType::Vesting => {
                    transaction_summaries.push(
                        self.txn_options
                            .submit_transaction(
                                creditchain_stdlib::vesting_update_operator_with_same_commission(
                                    stake_pool.vesting_contract.unwrap(),
                                    new_operator_address,
                                ),
                            )
        ...
        for stake_pool in stake_pool_results {
            match stake_pool.pool_type {
                StakePoolType::Direct => {
                    transaction_summaries.push(
                        self.txn_options
                            .submit_transaction(creditchain_stdlib::stake_set_delegated_voter(
                                new_voter_address,
                            ))
                            .await
                            .map(|inner| inner.into())?,
                ...
                StakePoolType::StakingContract => {
                    transaction_summaries.push(
                        self.txn_options
                            .submit_transaction(creditchain_stdlib::staking_contract_update_voter(
                                stake_pool.operator_address,
                                new_voter_address,
                            ))
                            .await
                ...
                StakePoolType::Vesting => {
                    transaction_summaries.push(
                        self.txn_options
                            .submit_transaction(creditchain_stdlib::vesting_update_voter(
                                stake_pool.vesting_contract.unwrap(),
                                new_voter_address,
                            ))
                            .await
...
impl CliCommand<TransactionSummary> for CreateStakingContract {
    ...
    async fn execute(mut self) -> CliTypedResult<TransactionSummary> {
        ...
        self.txn_options
            .submit_transaction(creditchain_stdlib::staking_contract_create_staking_contract(
                self.operator,
                self.voter,
                self.amount,
...
impl CliCommand<TransactionSummary> for DistributeVestedCoins {
    ...
    async fn execute(mut self) -> CliTypedResult<TransactionSummary> {
        ...
        self.txn_options
            .submit_transaction(creditchain_stdlib::vesting_distribute(vesting_contract_address))
            .await
            .map(|inner| inner.into())
    }
}
...
impl CliCommand<TransactionSummary> for UnlockVestedCoins {
    ...
    async fn execute(mut self) -> CliTypedResult<TransactionSummary> {
        let vesting_contract_address = create_vesting_contract_address(self.admin_address, 0, &[]);
        self.txn_options
            .submit_transaction(creditchain_stdlib::vesting_vest(vesting_contract_address))
            .await
            .map(|inner| inner.into())
    }
}
...
impl CliCommand<TransactionSummary> for RequestCommission {
    ...
    async fn execute(mut self) -> CliTypedResult<TransactionSummary> {
        ...
        self.txn_options
            .submit_transaction(creditchain_stdlib::staking_contract_request_commission(
                staker_address,
