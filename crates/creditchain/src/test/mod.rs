...
use creditchain_config::config::Peer;
use creditchain_crypto::{
    bls12381,
    ed25519::{Ed25519PrivateKey, Ed25519PublicKey},
    x25519, PrivateKey,
};
use creditchain_framework::chunked_publish::CHUNK_SIZE_IN_BYTES;
...
use creditchain_rest_client::{
    creditchain_api_types::{MoveStructTag, MoveType},
    Transaction,
};
use creditchain_sdk::move_types::{account_address::AccountAddress, language_storage::ModuleId};
use creditchain_temppath::TempPath;
use creditchain_types::on_chain_config::ValidatorSet;
...
module NamedAddress0::store {
    use std::string;
    use creditchain_framework::coin::{Self};
...
impl CliTestFramework {
            ...
            entry_function_args: EntryFunctionArguments {
                ...
                type_arg_vec: TypeArgVec {
                    type_args: vec![MoveType::Struct(MoveStructTag::new(
                        ...
                        ident_str!("creditchain_coin").into(),
                        ident_str!("CreditChainCoin").into(),
                        vec![],
                    ))],
                },
                json_file: None,
    ...
    async fn last_n_transactions_details(&self, count: u16) -> String {
        let result = creditchain_rest_client::Client::new(self.endpoint.clone())
    ...
    pub fn add_move_files(&self) {
        ...
        let hello_blockchain_test_contents = include_str!("../../../../creditchain-move/move-examples/hello_blockchain/sources/hello_blockchain_test.move");
    ...
    /// Runs the given script contents using the local creditchain_framework directory.
    pub async fn run_script(
        ...
        self.run_script_with_framework_package(index, script_contents, FrameworkPackageArgs {
            framework_git_rev: None,
            framework_local_dir: Some(Self::creditchain_framework_dir()),
            skip_fetch_latest_git_deps: false,
        })
        .await
    }
    ...
    /// Runs the given script contents using the creditchain_framework from creditchain-core git repository.
    ...
    pub fn creditchain_framework_dir() -> PathBuf {
