...
use creditchain_rest_client::VersionedNewBlockEvent;
...
impl AnalyzeValidators {
    ...
    pub fn fetch_epoch(epoch: u64, creditchain_db: &dyn DbReader) -> Result<Vec<VersionedNewBlockEvent>> {
        let batch = 100;

        let mut cursor = u64::MAX;
        let mut result: Vec<VersionedNewBlockEvent> = vec![];
        let ledger_version = creditchain_db.get_latest_ledger_info()?.ledger_info().version();

        loop {
            let raw_events = creditchain_db.get_events(
                &new_block_event_key(),
                cursor,
                Order::Descending,
                batch as u64,
