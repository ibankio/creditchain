...
use creditchain_crypto::{bls12381, bls12381::PublicKey, x25519, ValidCryptoMaterialStringExt};
use creditchain_genesis::config::{HostAndPort, OperatorConfiguration};
use creditchain_logger::Level;
use creditchain_network_checker::args::{
    validate_address, CheckEndpointArgs, HandshakeArgs, NodeAddressArgs,
};
use creditchain_rest_client::{creditchain_api_types::VersionedEvent, Client, State};
...
impl CliCommand<TransactionSummary> for InitializeValidator {
    ...
    async fn execute(mut self) -> CliTypedResult<TransactionSummary> {
        ...
        self.txn_options
            .submit_transaction(creditchain_stdlib::stake_initialize_validator(
...
impl CliCommand<TransactionSummary> for JoinValidatorSet {
    ...
    async fn execute(mut self) -> CliTypedResult<TransactionSummary> {
        ...
        self.txn_options
            .submit_transaction(creditchain_stdlib::stake_join_validator_set(address))
            .await
            .map(|inner| inner.into())
    }
}
...
impl CliCommand<TransactionSummary> for LeaveValidatorSet {
    ...
    async fn execute(mut self) -> CliTypedResult<TransactionSummary> {
        ...
        self.txn_options
            .submit_transaction(creditchain_stdlib::stake_leave_validator_set(address))
            .await
            .map(|inner| inner.into())
    }
}
...
impl From<&ValidatorInfoSummary> for ValidatorInfo {
    fn from(summary: &ValidatorInfoSummary) -> Self {
        ...
        ValidatorInfo::new(
            ...
            creditchain_types::validator_config::ValidatorConfig::new(
...
impl CliCommand<TransactionSummary> for UpdateConsensusKey {
    ...
    async fn execute(mut self) -> CliTypedResult<TransactionSummary> {
        ...
        let consensus_proof_of_possession = self
            ...
            .get_consensus_proof_of_possession(&operator_config)?;
        self.txn_options
            .submit_transaction(creditchain_stdlib::stake_rotate_consensus_key(
                address,
                consensus_public_key.to_bytes().to_vec(),
                consensus_proof_of_possession.to_bytes().to_vec(),
            ))
...
impl CliCommand<TransactionSummary> for UpdateValidatorNetworkAddresses {
    ...
    async fn execute(mut self) -> CliTypedResult<TransactionSummary> {
        ...
        self.txn_options
            .submit_transaction(creditchain_stdlib::stake_update_network_and_fullnode_addresses(
                address,
                // BCS encode, so that we can hide the original type
                bcs::to_bytes(&validator_network_addresses)?,
                bcs::to_bytes(&full_node_network_addresses)?,
...
/// Bootstrap CreditChainDB from a backup
///
...
impl CliCommand<String> for CheckNetworkConnectivity {
    ...
    async fn execute(self) -> CliTypedResult<String> {
        ...
        // Check the endpoint
        creditchain_network_checker::check_endpoint(&check_endpoint_args, None)
            .await
...
mod tests {
    ...
    async fn test_check_network_connectivity() {
        ...
        let args = &["creditchain", "node", "check-network-connectivity", "--address", "/ip4/34.70.116.169/tcp/6182/noise-ik/0x249f3301db104705652e0a0c471b46d13172b2baf14e31f007413f3baee46b0c/handshake/0", "--chain-id", "invalid-chain"];
        let error_message = run_tool_with_args(args).await.unwrap_err();
        assert_contains(error_message, "invalid value");

        // Verify that a failure to connect will return a timeout
        let args = &["creditchain", "node", "check-network-connectivity", "--address", "/ip4/31.71.116.169/tcp/0001/noise-ik/0x249f3301db104705652e0a0c471b46d13172b2baf14e31f007413f3baee46b0c/handshake/0", "--chain-id", "testnet"];
        let error_message = run_tool_with_args(args).await.unwrap_err();
