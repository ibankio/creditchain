...
// Copyright © CreditChain Research
...
use creditchain_framework::chunked_publish::{
    default_large_packages_module_address, CHUNK_SIZE_IN_BYTES,
};
use creditchain_global_constants::adjust_gas_headroom;
...
use creditchain_rest_client::{
    creditchain_api_types::{EntryFunctionId, HashValue, MoveType, ViewRequest},
    error::RestError,
    CreditChainBaseUrl, Client, Transaction,
};
use creditchain_sdk::{
...
pub const USER_AGENT: &str = concat!("creditchain-cli/", env!("CARGO_PKG_VERSION"));
...
pub const CREDITCHAIN_FOLDER_GIT_IGNORE: &str = indoc! {"
    *
    testnet/
    config.yaml
"};
...
const X_CREDITCHAIN_CLIENT_VALUE: &str = concat!("creditchain-cli/", env!("CARGO_PKG_VERSION"));
...
impl From<creditchain_config::config::Error> for CliError {
    fn from(e: creditchain_config::config::Error) -> Self {
        CliError::UnexpectedError(e.to_string())
    }
}

impl From<creditchain_github_client::Error> for CliError {
    fn from(e: creditchain_github_client::Error) -> Self {
        CliError::UnexpectedError(e.to_string())
    }
}
...
impl From<creditchain_crypto::CryptoMaterialError> for CliError {
    fn from(e: creditchain_crypto::CryptoMaterialError) -> Self {
        CliError::UnexpectedError(e.to_string())
    }
}
...
impl From<creditchain_ledger::CreditChainLedgerError> for CliError {
    fn from(e: creditchain_ledger::CreditChainLedgerError) -> Self {
        CliError::UnexpectedError(e.to_string())
    }
}
...
pub struct ProfileConfig {
    ...
    /// URL for the CreditChain rest endpoint
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl CliConfig {
    ...
    pub fn config_exists(mode: ConfigSearchMode) -> bool {
        if let Ok(folder) = Self::creditchain_folder(mode) {
    ...
    pub fn load(mode: ConfigSearchMode) -> CliTypedResult<Self> {
        let folder = Self::creditchain_folder(mode)?;
    ...
    pub fn save(&self) -> CliTypedResult<()> {
        let creditchain_folder = Self::creditchain_folder(ConfigSearchMode::CurrentDir)?;

        // Create if it doesn't exist
        let no_dir = !creditchain_folder.exists();
        create_dir_if_not_exist(creditchain_folder.as_path())?;

        // If the `.libra2/` doesn't exist, we'll add a .gitignore in it to ignore the config file
        // so people don't save their credentials...
        if no_dir {
            write_to_user_only_file(
                creditchain_folder.join(GIT_IGNORE).as_path(),
                GIT_IGNORE,
                CREDITCHAIN_FOLDER_GIT_IGNORE.as_bytes(),
            )?;
        }

        // Save over previous config file
        let config_file = creditchain_folder.join(CONFIG_FILE);
        ...
        let legacy_config_file = creditchain_folder.join(LEGACY_CONFIG_FILE);
    ...
    fn creditchain_folder(mode: ConfigSearchMode) -> CliTypedResult<PathBuf> {
...
impl RestOptions {
    ...
    pub fn client(&self, profile: &ProfileOptions) -> CliTypedResult<Client> {
        let mut client = Client::builder(CreditChainBaseUrl::Custom(self.url(profile)?))
...
pub struct FaucetOptions {
    /// URL for the faucet endpoint e.g. `https://faucet.devnet.creditchain.io`
    #[clap(long)]
...
impl TransactionOptions {
        ...
        ) -> CliTypedResult<(VMStatus, VMOutput)>,
    {
        ...
        let debugger = CreditChainDebugger::rest_client(client).unwrap();
...
pub fn get_mint_site_url(address: Option<AccountAddress>) -> String {
    ...
    format!("https://creditchain.io/network/faucet{}", params)
}
