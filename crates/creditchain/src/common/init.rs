...
use creditchain_crypto::{ed25519::Ed25519PrivateKey, PrivateKey, ValidCryptoMaterialStringExt};
use creditchain_ledger;
...
pub struct InitTool {
    ...
    ///
    /// Make sure that you have your Ledger device connected and unlocked, with the CreditChain app installed and opened.
...
impl CliCommand<()> for InitTool {
    ...
    async fn execute(self) -> CliTypedResult<()> {
        ...
        let mut profile_config = if let Some(profile_config) = config.remove_profile(profile_name) {
            prompt_yes_with_override(&format!("CreditChain already initialized for profile {}, do you want to overwrite the existing config?", profile_name), self.prompt_options)?;
            profile_config
        } else {
        ...
        match network {
            Network::Mainnet => {
                profile_config.rest_url =
                    Some("https://fullnode.mainnet.creditchain.io".to_string());
                profile_config.faucet_url = None;
            },
            Network::Testnet => {
                profile_config.rest_url =
                    Some("https://fullnode.testnet.creditchain.io".to_string());
            ...
            Network::Devnet => {
                profile_config.rest_url = Some("https://fullnode.devnet.creditchain.io".to_string());
                profile_config.faucet_url = Some("https://faucet.devnet.creditchain.io".to_string());
            },
            Network::Local => {
            ...
            self.hardware_wallet_options.extract_derivation_path()?
        {
            ...
            let account_map = creditchain_ledger::fetch_batch_accounts(Some(0..5))?;
            eprintln!(
            ...
            let path = creditchain_ledger::DERIVATION_PATH.replace("{index}", input_index);

            // Validate the path
            if !creditchain_ledger::validate_derivation_path(&path) {
        ...
        let public_key = if self.is_hardware_wallet() {
            let pub_key = match creditchain_ledger::get_public_key(
                derivation_path
                    .ok_or_else(|| {
        ...
        let client = creditchain_rest_client::Client::new(rest_url);
        ...
        let funded = matches!(client
            .get_account_balance(address, "0x1::CreditChainCoin::CreditChainCoin")
        ...
        eprintln!(
            "\n---\nCreditChain CLI is now set up for account {} as profile {}!\n---\n",
