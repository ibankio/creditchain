...
use creditchain_logger::{debug, Level};
use creditchain_rest_client::{creditchain_api_types::HashValue, Account, Client, FaucetClient, State};
use creditchain_telemetry::service::telemetry_is_disabled;
...
async fn send_telemetry_event(command: &str, latency: Duration, error: Option<&str>) {
    ...
    // Send the event
    creditchain_telemetry::cli_metrics::send_cli_telemetry_event(
        build_information,
        command.into(),
        latency,
        error.is_none(),
...
pub fn explorer_account_link(hash: AccountAddress, network: Option<Network>) -> String {
    ...
    if let Some(network) = network {
        format!(
            "https://explorer.creditchain.io/account/{}?network={}",
            hash, network
        )
    } else {
        format!("https://explorer.creditchain.io/account/{}", hash)
    }
}
...
pub fn explorer_transaction_link(
    ...
    if let Some(network) = network {
        format!(
            "https://explorer.creditchain.io/txn/{}?network={}",
            hash.to_hex_literal(),
            network
        )
    } else {
        format!(
            "https://explorer.creditchain.io/txn/{}",
            hash.to_hex_literal()
        )
    }
}
