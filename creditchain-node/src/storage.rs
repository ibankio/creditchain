...
use creditchain_config::{config::NodeConfig, utils::get_genesis_txn};
use creditchain_db::{fast_sync_storage_wrapper::FastSyncStorageWrapper, CreditChainDB};
use creditchain_db_indexer::db_indexer::InternalIndexerDB;
use creditchain_executor::db_bootstrapper::maybe_bootstrap;
...
pub(crate) fn maybe_apply_genesis(
    ...
    if let Some(genesis) = get_genesis_txn(node_config) {
        let ledger_info_opt =
            maybe_bootstrap::<CreditChainVMBlockExecutor>(db_rw, genesis, genesis_waypoint)
...
pub(crate) fn bootstrap_db(
    ...
    let (creditchain_db_reader, db_rw, backup_service) = match FastSyncStorageWrapper::initialize_dbs(
        node_config,
...
/// In consensus-only mode, return a in-memory based [FakeCreditChainDB] and
/// do not run the backup service.
#[cfg(feature = "consensus-only-perf-test")]
pub(crate) fn bootstrap_db(
    node_config: &NodeConfig,
) -> Result<(Arc<dyn DbReader>, DbReaderWriter, Option<Runtime>)> {
    use creditchain_db::db::fake_creditchaindb::FakeCreditChainDB;
    ...
    let (creditchain_db, db_rw) = DbReaderWriter::wrap(FakeCreditChainDB::new(creditchain_db));
    maybe_apply_genesis(&db_rw, node_config)?;
    Ok((creditchain_db, db_rw, None))
}
...
fn create_rocksdb_checkpoint_and_change_working_dir(
    ...
    CreditChainDB::create_checkpoint(
        &source_dir,
        &checkpoint_dir,
        node_config.storage.rocksdb_configs.enable_storage_sharding,
    )
    .expect("CreditChainDB checkpoint creation failed.");

    // Create a consensus db checkpoint
    creditchain_consensus::create_checkpoint(&source_dir, &checkpoint_dir)
        .expect("ConsensusDB checkpoint creation failed.");

    // Create a state sync db checkpoint
    let state_sync_db =
        creditchain_state_sync_driver::metadata_storage::PersistentMetadataStorage::new(&source_dir);
    state_sync_db
...
pub fn initialize_database_and_checkpoints(
    ...
    let (_creditchain_db, db_rw, backup_service, indexer_db_opt, update_receiver) =
