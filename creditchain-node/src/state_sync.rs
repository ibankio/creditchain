...
use creditchain_config::config::{NodeConfig, StateSyncConfig};
use creditchain_consensus_notifications::ConsensusNotifier;
use creditchain_data_client::{client::CreditChainDataClient, poller};
...
use creditchain_state_sync_driver::{
    driver_factory::{DriverFactory, StateSyncRuntimes},
    metadata_storage::PersistentMetadataStorage,
};
use creditchain_storage_interface::{DbReader, DbReaderWriter};
...
use creditchain_storage_service_server::{
...
pub fn start_state_sync_and_get_notification_handles(
    ...
    let (creditchain_data_client, creditchain_data_client_runtime) =
        setup_creditchain_data_client(node_config, network_client, db_rw.reader.clone())?;

    // Start the data streaming service
    let state_sync_config = node_config.state_sync;
    let (streaming_service_client, streaming_service_runtime) =
        setup_data_streaming_service(state_sync_config, creditchain_data_client.clone())?;
    ...
    let (mempool_notifier, mempool_listener) =
        creditchain_mempool_notifications::new_mempool_notifier_listener_pair(
            state_sync_config
                .state_sync_driver
                .max_pending_mempool_notifications,
        );
    let (consensus_notifier, consensus_listener) =
        creditchain_consensus_notifications::new_consensus_notifier_listener_pair(
            state_sync_config
                .state_sync_driver
    ...
    let state_sync_runtimes = StateSyncRuntimes::new(
        creditchain_data_client_runtime,
        state_sync,
...
fn setup_data_streaming_service(
    ...
    creditchain_data_client: CreditChainDataClient,
    ...
    let data_streaming_service = DataStreamingService::new(
        state_sync_config.creditchain_data_client,
    ...
    // Start the data streaming service
    let streaming_service_runtime = creditchain_runtimes::spawn_named_runtime("stream-serv".into(), None);
...
fn setup_creditchain_data_client(
    node_config: &NodeConfig,
    network_client: NetworkClient<StorageServiceMessage>,
    storage: Arc<dyn DbReader>,
) -> anyhow::Result<(CreditChainDataClient, Runtime)> {
    ...
    let creditchain_data_client_runtime = creditchain_runtimes::spawn_named_runtime("data-client".into(), None);

    // Create the data client and spawn the data poller
    let (creditchain_data_client, data_summary_poller) = CreditChainDataClient::new(
        node_config.state_sync.creditchain_data_client,
        node_config.base.clone(),
        TimeService::real(),
        storage,
        storage_service_client,
        Some(creditchain_data_client_runtime.handle().clone()),
    );
    creditchain_data_client_runtime.spawn(poller::start_poller(data_summary_poller));

    Ok((creditchain_data_client, creditchain_data_client_runtime))
}
...
fn setup_state_sync_storage_service(
    ...
    let storage_service_runtime = creditchain_runtimes::spawn_named_runtime("stor-server".into(), None);
