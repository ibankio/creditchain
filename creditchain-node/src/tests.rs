...
use creditchain_config::config::{NodeConfig, WaypointConfig};
use creditchain_event_notifications::EventSubscriptionService;
use creditchain_infallible::RwLock;
use creditchain_storage_interface::{DbReader, DbReaderWriter, DbWriter};
use creditchain_temppath::TempPath;
use creditchain_types::{chain_id::ChainId, waypoint::Waypoint};
...
#[test]
fn test_creditchain_vm_does_not_have_test_natives() {
    creditchain_vm::natives::assert_no_test_natives(crate::utils::ERROR_MSG_BAD_FEATURE_FLAGS)
}
...
fn test_create_single_node_test_config() {
    ...
    let test_dir = creditchain_temppath::TempPath::new().as_ref().to_path_buf();
    fs::DirBuilder::new()
        .recursive(true)
        .create(&test_dir)
