...
use creditchain_admin_service::AdminService;
use creditchain_api::bootstrap as bootstrap_api;
use creditchain_build_info::build_information;
use creditchain_config::config::{merge_node_config, NodeConfig, PersistableConfig};
use creditchain_framework::ReleaseBundle;
use creditchain_genesis::builder::GenesisConfiguration;
use creditchain_logger::{prelude::*, telemetry_log_writer::TelemetryLog, Level, LoggerFilterUpdater};
use creditchain_state_sync_driver::driver_factory::StateSyncRuntimes;
...
/// Runs an CreditChain validator or fullnode
#[derive(Clone, Debug, Parser)]
#[clap(name = "CreditChain Node", author, version)]
pub struct CreditChainNodeArgs {
    ...
    /// Paths to the CreditChain framework release package to be used for genesis.
    #[clap(long, requires("test"))]
...
impl CreditChainNodeArgs {
    /// Runs an CreditChain node based on the given command line arguments and config flags
    pub fn run(self) {
        #[cfg(target_os = "linux")]
...
pub fn start_and_report_ports(
    ...
    // Setup panic handler
    creditchain_crash_handler::setup_panic_handler();

    // Create global rayon thread pool
    utils::create_global_rayon_pool(create_global_rayon_pool);

    // Initialize the global creditchain-node-identity
    creditchain_node_identity::init(config.get_peer_id())?;
...
pub fn start_test_environment_node(
    ...
    let creditchain_root_key_path = test_dir.join("mint.key");
    ...
    println!("\tCreditChain root key path: {:?}", creditchain_root_key_path);
    ...
    println!(
        "\tCreditChainnet fullnode network endpoint: {}",
    ...
    println!("\nCreditChain is running, press ctrl-c to exit\n");
    ...
    R: rand::RngCore + rand::CryptoRng,
{
    ...
    let test_dir =
        test_dir.unwrap_or_else(|| creditchain_temppath::TempPath::new().as_ref().to_path_buf());
    ...
    R: rand::RngCore + rand::CryptoRng,
{
    ...
    let builder = creditchain_genesis::builder::Builder::new(test_dir, framework.clone())?
        ...
        .with_init_genesis_config(Some(Arc::new(|genesis_config| {
            ...
            match env::var("ENABLE_KEYLESS_DEFAULT") {
                Ok(val) if val.as_str() == "1" => {
                    let response = ureq::get("https://api.devnet.creditchain.io/v1/accounts/0x1/resource/0x1::keyless_account::Groth16VerificationKey").call();
    ...
    let mut key_file = fs::File::create(creditchain_root_key_path)?;
    ...
    creditchain_config::config::sanitize_node_config(validators[0].config.override_config_mut())?;
...
pub fn setup_environment_and_start_node(
    ...
    api_port_tx: Option<oneshot::Sender<u16>>,
    indexer_grpc_port_tx: Option<oneshot::Sender<u16>>,
) -> anyhow::Result<CreditChainHandle> {
    ...
    admin_service.set_creditchain_db(db_rw.clone().into());

    // Set the CreditChain VM configurations
    utils::set_creditchain_vm_configurations(&node_config);

    // Obtain the chain_id from the DB
    let chain_id = utils::fetch_chain_id(&db_rw)?;

    // Set the chain_id in global CreditChainNodeIdentity
    creditchain_node_identity::set_chain_id(chain_id)?;
    ...
    let (creditchain_data_client, state_sync_runtimes, mempool_listener, consensus_notifier) =
...
fn verify_tool() {
    ...
    CreditChainNodeArgs::command().debug_assert()
}
