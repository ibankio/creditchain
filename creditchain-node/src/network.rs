...
use creditchain_network_benchmark::NetbenchMessage;
use creditchain_network_builder::builder::NetworkBuilder;
use creditchain_peer_monitoring_service_types::PeerMonitoringServiceMessage;
use creditchain_storage_service_types::StorageServiceMessage;
...
pub fn consensus_network_configuration(node_config: &NodeConfig) -> NetworkApplicationConfig {
    let direct_send_protocols: Vec<ProtocolId> =
        creditchain_consensus::network_interface::DIRECT_SEND.into();
    let rpc_protocols: Vec<ProtocolId> = creditchain_consensus::network_interface::RPC.into();
    ...
    let network_service_config = NetworkServiceConfig::new(
        ...
        rpc_protocols,
        creditchain_channel::Config::new(node_config.consensus.max_network_channel_size)
            .queue_style(QueueStyle::FIFO)
            .counters(&creditchain_consensus::counters::PENDING_CONSENSUS_NETWORK_EVENTS),
    );
...
pub fn dkg_network_configuration(node_config: &NodeConfig) -> NetworkApplicationConfig {
    let direct_send_protocols: Vec<ProtocolId> =
        creditchain_dkg_runtime::network_interface::DIRECT_SEND.into();
    let rpc_protocols: Vec<ProtocolId> = creditchain_dkg_runtime::network_interface::RPC.into();
    ...
    let network_service_config = NetworkServiceConfig::new(
        direct_send_protocols,
        rpc_protocols,
        creditchain_channel::Config::new(node_config.dkg.max_network_channel_size)
            .queue_style(QueueStyle::FIFO),
    );
    NetworkApplicationConfig::new(network_client_config, network_service_config)
}
...
pub fn jwk_consensus_network_configuration(node_config: &NodeConfig) -> NetworkApplicationConfig {
    let direct_send_protocols: Vec<ProtocolId> =
        creditchain_jwk_consensus::network_interface::DIRECT_SEND.into();
    let rpc_protocols: Vec<ProtocolId> = creditchain_jwk_consensus::network_interface::RPC.into();
    ...
    let network_service_config = NetworkServiceConfig::new(
        direct_send_protocols,
        rpc_protocols,
        creditchain_channel::Config::new(node_config.jwk_consensus.max_network_channel_size)
            .queue_style(QueueStyle::FIFO),
    );
    NetworkApplicationConfig::new(network_client_config, network_service_config)
}
...
pub fn mempool_network_configuration(node_config: &NodeConfig) -> NetworkApplicationConfig {
    ...
    let network_service_config = NetworkServiceConfig::new(
        direct_send_protocols,
        rpc_protocols,
        creditchain_channel::Config::new(node_config.mempool.max_network_channel_size)
            .queue_style(QueueStyle::KLAST) // TODO: why is this not FIFO?
            .counters(&creditchain_mempool::counters::PENDING_MEMPOOL_NETWORK_EVENTS),
    );
    NetworkApplicationConfig::new(network_client_config, network_service_config)
}
...
pub fn peer_monitoring_network_configuration(node_config: &NodeConfig) -> NetworkApplicationConfig {
    ...
    let network_service_config = NetworkServiceConfig::new(
        direct_send_protocols,
        rpc_protocols,
        creditchain_channel::Config::new(max_network_channel_size)
            .queue_style(QueueStyle::FIFO)
            .counters(
                &creditchain_peer_monitoring_service_server::metrics::PENDING_PEER_MONITORING_SERVER_NETWORK_EVENTS,
            ),
    );
    NetworkApplicationConfig::new(network_client_config, network_service_config)
}
...
pub fn storage_service_network_configuration(node_config: &NodeConfig) -> NetworkApplicationConfig {
    ...
    let network_service_config = NetworkServiceConfig::new(
        ...
        creditchain_channel::Config::new(max_network_channel_size)
            .queue_style(QueueStyle::FIFO)
            .counters(
                &creditchain_storage_service_server::metrics::PENDING_STORAGE_SERVER_NETWORK_EVENTS,
            ),
    );
    NetworkApplicationConfig::new(network_client_config, network_service_config)
}
...
pub fn netbench_network_configuration(
    ...
    let network_service_config = NetworkServiceConfig::new(
        ...
        creditchain_channel::Config::new(max_network_channel_size)
            .queue_style(QueueStyle::FIFO)
            .counters(&creditchain_network_benchmark::PENDING_NETBENCH_NETWORK_EVENTS),
    );
...
pub fn setup_networks_and_get_interfaces(
    ...
    if !netbench_handles.is_empty() {
        ...
        let netbench_runtime =
            creditchain_runtimes::spawn_named_runtime("benchmark".into(), netbench_service_threads);
...
fn create_network_runtime(network_config: &NetworkConfig) -> Runtime {
    ...
    creditchain_runtimes::spawn_named_runtime(thread_name, network_config.runtime_threads)
}
