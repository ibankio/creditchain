...
use anyhow::anyhow;
use creditchain_config::config::{NodeConfig, DEFAULT_EXECUTION_CONCURRENCY_LEVEL};
#[cfg(unix)]
...
creditchain-node was compiled with feature flags that shouldn't be enabled.
...
PLEASE RECOMPILE CREDITCHAIN-NODE SEPARATELY using the following command:
    cargo build --package creditchain-node
...
pub fn fetch_chain_id(db: &DbReaderWriter) -> anyhow::Result<ChainId> {
    let db_state_view = db
        ...
        .map_err(|err| anyhow!("[creditchain-node] failed to create db state view {}", err))?;
    Ok(ChainIdResource::fetch_config(&db_state_view)
        .expect("[creditchain-node] missing chain ID resource")
        .chain_id())
}

/// Sets the CreditChain VM configuration based on the node configurations
pub fn set_creditchain_vm_configurations(node_config: &NodeConfig) {
    ...
    CreditChainVM::set_concurrency_level_once(effective_concurrency_level as usize);
    CreditChainVM::set_discard_failed_blocks(node_config.execution.discard_failed_blocks);
    CreditChainVM::set_num_proof_reading_threads_once(
