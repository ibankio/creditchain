...
# So here it relies on the same firestore-related parameters as `creditchain-keyless-pepper-service` does.
export FIRESTORE_EMULATOR_HOST=localhost:8081
export GOOGLE_APPLICATION_CREDENTIALS="<path-to-your-service-account-credential>"
export PROJECT_ID=$(gcloud config get-value project)
export DATABASE_ID='(default)' # the default name of a local firestore emulator
...
cargo run -p creditchain-keyless-pepper-example-client-rust
```
