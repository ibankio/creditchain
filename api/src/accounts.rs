...
use creditchain_api_types::{
    AccountData, Address, CreditChainErrorCode, AsConverter, AssetType, LedgerInfo, MoveModuleBytecode,
    MoveModuleId, MoveResource, MoveStructTag, StateKeyWrapper, U64,
};
use creditchain_sdk::types::{get_paired_fa_metadata_address, get_paired_fa_primary_store_address};
...
impl AccountsApi {
    ...
    async fn get_account_resources(
        ...
        /// then use the cursor returned in the X-CreditChain-Cursor header in the
        /// response.
    ...
    pub fn account(self, accept_type: &AcceptType) -> BasicResultWith404<AccountData> {
        ...
        let account_resource = if let Some(state_value) = &state_value_opt {
            ...
            let stateless_account_enabled = self
                .context
                .feature_enabled(
                    creditchain_types::on_chain_config::FeatureFlag::DEFAULT_ACCOUNT_RESOURCE,
                )
