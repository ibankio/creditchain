...
use creditchain_api_types::{
    verify_field_identifier, Address, CreditChainErrorCode, AsConverter, IdentifierWrapper, LedgerInfo,
...
impl EventsApi {
    ...
    async fn get_events_by_creation_number(
        &self,
        accept_type: AcceptType,
        /// Hex-encoded 32 byte CreditChain account, with or without a `0x` prefix, for
