...
use creditchain_sdk::{transaction_builder::creditchain_stdlib::creditchain_token_stdlib, types::LocalAccount};
...
async fn test_merkle_leaves_with_nft_transfer(
    ...
    let transfer_to_owner_txn = creator.sign_multi_agent_with_transaction_builder(
        ...
        ctx.transaction_factory()
            .payload(creditchain_token_stdlib::token_direct_transfer_script(
                creator.address(),
                collection_name.clone(),
                token_name.clone(),
                0,
