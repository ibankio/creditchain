# Coding Guidelines for CreditChain Core

This document describes the coding guidelines for the [CreditChain Core](https://github.com/ibankio/creditchain) Rust codebase. For Move language guidance, see the [Move Coding Conventions](https://github.com/ibankio/creditchain/blob/main/third_party/move/documentation/book/src/coding-conventions.md).
Secure coding guidance is provided in the [CreditChain Rust Secure Coding Guidelines](./RUST_SECURE_CODING.md).
...
The CreditChain codebase uses inclusive terminology (similar to other projects such as [the Linux kernel](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/commit/?id=49decddd39e5f6132ccd7d9fdc3d7c470b0061bb)). The terms below are recommended when appropriate.
...
Each major component of CreditChain Core needs to have a `README.md` file. Major components are:

- top-level directories (e.g. `creditchain/network`, `creditchain/language`)
...
Refer to the CreditChain Project contributing guide [LINK].

## License

Refer to the CreditChain Project License [LINK].
```

A good example of README.md is `creditchain/network/README.md` that describes the networking crate.
...
- `duration_since_epoch()` - to obtain the Unix time, call the function provided by `creditchain-infallible`.
- `RwLock` and `Mutex` - Instead of calling `unwrap()` on the standard library implementations of these functions, use the infallible equivalent types that we provide in `creditchain-infallible`.
...
CreditChain contains [property-based tests](https://hypothesis.works/articles/what-is-property-based-testing/) written in Rust using the [`proptest` framework](https://github.com/AltSysrq/proptest). Property-based tests generate random test cases and assert that invariants, also called _properties_, hold for the code under test.

Some examples of properties tested in CreditChain:
...
Production crates are defined as the set of crates that create externally published artifacts, e.g. the CreditChain validator,
...
feature activation. In the CreditChain codebase, we therefore recommend that
