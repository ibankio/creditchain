...
/// From bytecode version 7 onwards, CreditChain distinguishes from
/// other Move flavors by masking the version with the highest
/// byte 0xA.
pub const CREDITCHAIN_BYTECODE_VERSION_MASK: u32 = 0x0A000000;
    ...
    impl<'a> VersionedBinary<'a> {
        fn new(
            ...
            let version = match read_u32(&mut cursor) {
                Ok(v) => v & !CREDITCHAIN_BYTECODE_VERSION_MASK,
                Err(_) => {
                    return Err(PartialVMError::new(StatusCode::MALFORMED)
                        .with_message("Bad binary header".to_string()));
                },
