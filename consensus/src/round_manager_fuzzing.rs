...
use creditchain_consensus_types::{proposal_msg::ProposalMsg, utils::PayloadTxnsSize};
...
use creditchain_safety_rules::{test_utils, SafetyRules, TSafetyRules};
...
fn create_round_state() -> RoundState {
    ...
    let (round_timeout_sender, _) = creditchain_channels::new_test(1_024);
...
fn create_node_for_fuzzing() -> RoundManager {
    ...
    let (round_manager_tx, _) = creditchain_channel::new(QueueStyle::LIFO, 1, None);

    let (opt_proposal_loopback_tx, _) =
        creditchain_channels::new_unbounded(&counters::OP_COUNTERS.gauge("opt_proposal_loopback_queue"));
