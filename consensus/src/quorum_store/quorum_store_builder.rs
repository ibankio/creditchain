...
use creditchain_config::config::{BatchTransactionFilterConfig, QuorumStoreConfig};
...
impl QuorumStoreBuilder {
    pub fn init_payload_manager(
        ...
        Option<creditchain_channel::Sender<AccountAddress, (Author, VerifiedEvent)>>,
    ) {
        match self {
    ...
    pub fn start(
        ...
        creditchain_channel::Sender<AccountAddress, IncomingBatchRetrievalRequest>,
    )> {
        match self {
...
pub struct InnerBuilder {
    ...
    quorum_store_msg_rx: Option<creditchain_channel::Receiver<AccountAddress, (Author, VerifiedEvent)>>,
...
impl InnerBuilder {
    ...
    pub(crate) fn new(
        ...
        let (quorum_store_msg_tx, quorum_store_msg_rx) =
            creditchain_channel::new::<AccountAddress, (Author, VerifiedEvent)>(
                QueueStyle::FIFO,
                config.channel_size,
                None,
            );
    ...
    fn spawn_quorum_store(
        ...
        let (batch_retrieval_tx, mut batch_retrieval_rx) =
            creditchain_channel::new::<AccountAddress, IncomingBatchRetrievalRequest>(
                QueueStyle::LIFO,
                10,
                Some(&counters::BATCH_RETRIEVAL_TASK_MSGS),
            );
        let creditchain_db_clone = self.creditchain_db.clone();
        spawn_named!("batch_serve", async move {
            ...
            while let Some(rpc_request) = batch_retrieval_rx.next().await {
                    ...
                    batch_store.get_batch_from_local(&rpc_request.req.digest())
                {
                    ...
                    match creditchain_db_clone.get_latest_ledger_info() {
