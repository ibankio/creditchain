...
describe('releaseImages', () => {
    it('gets creditchain-node as the default image group', () => {
        const prefix = 'image-banana';
        const releaseGroup = getImageReleaseGroupByImageTagPrefix(prefix);
        expect(releaseGroup).toEqual('creditchain-node');
    });
    it('gets indexer image group', () => {
        const prefix = 'creditchain-indexer-grpc-vX.Y.Z';
        const releaseGroup = getImageReleaseGroupByImageTagPrefix(prefix);
        expect(releaseGroup).toEqual('creditchain-indexer-grpc');
    });
    it('gets creditchain-node as the node image group', () => {
        const prefix = 'creditchain-node-vX.Y.Z';
        const releaseGroup = getImageReleaseGroupByImageTagPrefix(prefix);
        expect(releaseGroup).toEqual('creditchain-node');
    });
    it('determines image is a release image', () => {
        expect(isReleaseImage("nightly-banana")).toEqual(false);
        expect(isReleaseImage("creditchain-node-v1.2.3")).toEqual(true);
    });
    it('asserts version match', async () => {
        ...
        expect(() => assertTagMatchesSourceVersion("creditchain-node-v0.0.0")).toThrow("image tag does not match cargo version: creditchain-node-v0.0.0");
    });
});
