...
pub static CREDITCHAIN_CONNECTIONS: Lazy<IntGaugeVec> = Lazy::new(|| {
    register_int_gauge_vec!(
        "creditchain_connections",
...
pub fn connections(network_context: &NetworkContext, origin: ConnectionOrigin) -> IntGauge {
    CREDITCHAIN_CONNECTIONS.with_label_values(&[
...
pub static CREDITCHAIN_CONNECTIONS_REJECTED: Lazy<IntCounterVec> = Lazy::new(|| {
    register_int_counter_vec!(
        "creditchain_connections_rejected",
...
pub fn connections_rejected(
    ...
    CREDITCHAIN_CONNECTIONS_REJECTED.with_label_values(&[
        network_context.role().as_str(),
        network_context.network_id().as_str(),
        network_context.peer_id().short_str().as_str(),
        origin.as_str(),
...
pub static CREDITCHAIN_NETWORK_PEER_CONNECTED: Lazy<IntGaugeVec> = Lazy::new(|| {
    register_int_gauge_vec!(
        "creditchain_network_peer_connected",
...
pub static CREDITCHAIN_NETWORK_PENDING_CONNECTION_UPGRADES: Lazy<IntGaugeVec> = Lazy::new(|| {
    register_int_gauge_vec!(
        "creditchain_network_pending_connection_upgrades",
...
pub fn pending_connection_upgrades(
    ...
    CREDITCHAIN_NETWORK_PENDING_CONNECTION_UPGRADES.with_label_values(&[
        network_context.role().as_str(),
        network_context.network_id().as_str(),
        network_context.peer_id().short_str().as_str(),
        direction.as_str(),
...
pub static CREDITCHAIN_NETWORK_CONNECTION_OPERATIONS: Lazy<IntCounterVec> = Lazy::new(|| {
    register_int_counter_vec!(
        "creditchain_network_connection_operations",
...
pub static CREDITCHAIN_NETWORK_CONNECTION_UPGRADE_TIME: Lazy<HistogramVec> = Lazy::new(|| {
    register_histogram_vec!(
        "creditchain_network_connection_upgrade_time_seconds",
...
pub fn connection_upgrade_time(
    ...
    CREDITCHAIN_NETWORK_CONNECTION_UPGRADE_TIME.with_label_values(&[
        network_context.role().as_str(),
        network_context.network_id().as_str(),
        network_context.peer_id().short_str().as_str(),
        direction.as_str(),
...
pub static CREDITCHAIN_NETWORK_DISCOVERY_NOTES: Lazy<IntGaugeVec> = Lazy::new(|| {
    register_int_gauge_vec!(
        "creditchain_network_discovery_notes",
        "CreditChain network discovery notes",
        &["role_type"]
    )
    .unwrap()
});

pub static CREDITCHAIN_NETWORK_RPC_MESSAGES: Lazy<IntCounterVec> = Lazy::new(|| {
    register_int_counter_vec!("creditchain_network_rpc_messages", "Number of RPC messages", &[
        "role_type",
        "network_id",
        "peer_id",
        "message_type",
...
pub fn rpc_messages(
    ...
    CREDITCHAIN_NETWORK_RPC_MESSAGES.with_label_values(&[
        network_context.role().as_str(),
        network_context.network_id().as_str(),
        network_context.peer_id().short_str().as_str(),
        message_type_label,
...
pub static CREDITCHAIN_NETWORK_RPC_BYTES: Lazy<IntCounterVec> = Lazy::new(|| {
    register_int_counter_vec!(
        "creditchain_network_rpc_bytes",
...
pub fn rpc_bytes(
    ...
    CREDITCHAIN_NETWORK_RPC_BYTES.with_label_values(&[
        network_context.role().as_str(),
        network_context.network_id().as_str(),
        network_context.peer_id().short_str().as_str(),
        message_type_label,
...
pub static INVALID_NETWORK_MESSAGES: Lazy<IntCounterVec> = Lazy::new(|| {
    register_int_counter_vec!(
        "creditchain_network_invalid_messages",
...
pub static PEER_SEND_FAILURES: Lazy<IntCounterVec> = Lazy::new(|| {
    register_int_counter_vec!(
        "creditchain_network_peer_send_failures",
...
pub static CREDITCHAIN_NETWORK_OUTBOUND_RPC_REQUEST_LATENCY: Lazy<HistogramVec> = Lazy::new(|| {
    register_histogram_vec!(
        "creditchain_network_outbound_rpc_request_latency_seconds",
...
pub fn outbound_rpc_request_latency(
    ...
    CREDITCHAIN_NETWORK_OUTBOUND_RPC_REQUEST_LATENCY.with_label_values(&[
        network_context.role().as_str(),
        network_context.network_id().as_str(),
        network_context.peer_id().short_str().as_str(),
        protocol_id.as_str(),
...
pub static CREDITCHAIN_NETWORK_INBOUND_RPC_HANDLER_LATENCY: Lazy<HistogramVec> = Lazy::new(|| {
    register_histogram_vec!(
        "creditchain_network_inbound_rpc_handler_latency_seconds",
...
pub fn inbound_rpc_handler_latency(
    ...
    CREDITCHAIN_NETWORK_INBOUND_RPC_HANDLER_LATENCY.with_label_values(&[
        network_context.role().as_str(),
        network_context.network_id().as_str(),
        network_context.peer_id().short_str().as_str(),
        protocol_id.as_str(),
...
pub static CREDITCHAIN_NETWORK_DIRECT_SEND_MESSAGES: Lazy<IntCounterVec> = Lazy::new(|| {
    register_int_counter_vec!(
        "creditchain_network_direct_send_messages",
...
pub fn direct_send_messages(
    ...
    CREDITCHAIN_NETWORK_DIRECT_SEND_MESSAGES.with_label_values(&[
        network_context.role().as_str(),
        network_context.network_id().as_str(),
        network_context.peer_id().short_str().as_str(),
        state_label,
...
pub static CREDITCHAIN_NETWORK_DIRECT_SEND_BYTES: Lazy<IntCounterVec> = Lazy::new(|| {
    register_int_counter_vec!(
        "creditchain_network_direct_send_bytes",
...
pub fn direct_send_bytes(
    ...
    CREDITCHAIN_NETWORK_DIRECT_SEND_BYTES.with_label_values(&[
        network_context.role().as_str(),
        network_context.network_id().as_str(),
        network_context.peer_id().short_str().as_str(),
        state_label,
...
pub static PENDING_NETWORK_NOTIFICATIONS: Lazy<IntCounterVec> = Lazy::new(|| {
    register_int_counter_vec!(
        "creditchain_network_pending_network_notifications",
...
pub static PENDING_NETWORK_REQUESTS: Lazy<IntCounterVec> = Lazy::new(|| {
    register_int_counter_vec!(
        "creditchain_network_pending_requests",
...
pub static PENDING_HEALTH_CHECKER_NETWORK_EVENTS: Lazy<IntCounterVec> = Lazy::new(|| {
    register_int_counter_vec!(
        "creditchain_network_pending_health_check_events",
...
pub static PENDING_DISCOVERY_NETWORK_EVENTS: Lazy<IntCounterVec> = Lazy::new(|| {
    register_int_counter_vec!(
        "creditchain_network_pending_discovery_events",
...
pub static PENDING_PEER_MANAGER_REQUESTS: Lazy<IntCounterVec> = Lazy::new(|| {
    register_int_counter_vec!(
        "creditchain_network_pending_peer_manager_requests",
...
pub static PENDING_CONNECTIVITY_MANAGER_REQUESTS: Lazy<IntGauge> = Lazy::new(|| {
    register_int_gauge!(
        "creditchain_network_pending_connectivity_manager_requests",
...
pub static PENDING_CONNECTION_HANDLER_NOTIFICATIONS: Lazy<IntGauge> = Lazy::new(|| {
    register_int_gauge!(
        "creditchain_network_pending_connection_handler_notifications",
...
pub static PENDING_PEER_MANAGER_DIAL_REQUESTS: Lazy<IntGauge> = Lazy::new(|| {
    register_int_gauge!(
        "creditchain_network_pending_peer_manager_dial_requests",
...
pub static PENDING_WIRE_MESSAGES: Lazy<IntCounterVec> = Lazy::new(|| {
    register_int_counter_vec!(
        "creditchain_network_pending_wire_messages",
...
pub static PENDING_MULTIPLEX_MESSAGE: Lazy<IntGauge> = Lazy::new(|| {
    register_int_gauge!(
        "creditchain_network_pending_multiplex_messages",
...
pub static PENDING_MULTIPLEX_STREAM: Lazy<IntGauge> = Lazy::new(|| {
    register_int_gauge!(
        "creditchain_network_pending_multiplex_stream",
...
pub static PENDING_DIRECT_SEND_REQUESTS: Lazy<IntGauge> = Lazy::new(|| {
    register_int_gauge!(
        "creditchain_network_pending_direct_send_requests",
...
pub static PENDING_DIRECT_SEND_NOTIFICATIONS: Lazy<IntGauge> = Lazy::new(|| {
    register_int_gauge!(
        "creditchain_network_pending_direct_send_notifications",
...
pub static PENDING_RPC_REQUESTS: Lazy<IntGauge> = Lazy::new(|| {
    register_int_gauge!(
        "creditchain_network_pending_rpc_requests",
...
pub static PENDING_RPC_NOTIFICATIONS: Lazy<IntGauge> = Lazy::new(|| {
    register_int_gauge!(
        "creditchain_network_pending_rpc_notifications",
...
pub static PENDING_PEER_REQUESTS: Lazy<IntGauge> = Lazy::new(|| {
    register_int_gauge!(
        "creditchain_network_pending_peer_requests",
...
pub static PENDING_PEER_RPC_NOTIFICATIONS: Lazy<IntGauge> = Lazy::new(|| {
    register_int_gauge!(
        "creditchain_network_pending_peer_rpc_notifications",
...
pub static PENDING_PEER_DIRECT_SEND_NOTIFICATIONS: Lazy<IntGauge> = Lazy::new(|| {
    register_int_gauge!(
        "creditchain_network_pending_peer_direct_send_notifications",
...
pub static PENDING_PEER_NETWORK_NOTIFICATIONS: Lazy<IntGauge> = Lazy::new(|| {
    register_int_gauge!(
        "creditchain_network_pending_peer_network_notifications",
...
pub static NETWORK_RATE_LIMIT_METRICS: Lazy<HistogramVec> = Lazy::new(|| {
    register_histogram_vec!(
        "creditchain_network_rate_limit",
...
pub static NETWORK_APPLICATION_INBOUND_METRIC: Lazy<HistogramVec> = Lazy::new(|| {
    register_histogram_vec!(
        "creditchain_network_app_inbound_traffic",
...
pub static NETWORK_APPLICATION_OUTBOUND_METRIC: Lazy<HistogramVec> = Lazy::new(|| {
    register_histogram_vec!(
        "creditchain_network_app_outbound_traffic",
...
pub static NETWORK_APPLICATION_SERIALIZATION_METRIC: Lazy<HistogramVec> = Lazy::new(|| {
    register_histogram_vec!(
        "creditchain_network_serialization_metric",
        "Time it takes to perform message serialization and deserialization",
...
pub static NETWORK_PEER_PING_TIMES: Lazy<HistogramVec> = Lazy::new(|| {
    register_histogram_vec!(
        "creditchain_network_peer_ping_times",
        "Counters related to peer ping times (before and after dialing)",
        &["network_id", "label"],
    )
    .unwrap()
...
pub static INBOUND_QUEUE_DELAY: Lazy<HistogramVec> = Lazy::new(|| {
    register_histogram_vec!(
        "creditchain_network_inbound_queue_time",
