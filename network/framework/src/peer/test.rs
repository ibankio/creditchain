...
use creditchain_config::{config::PeerRole, network_id::NetworkContext};
...
fn build_test_peer(
    ...
    PeerHandle,
    MemorySocket,
    creditchain_channels::Receiver<TransportNotification<MemorySocket>>,
) {
    ...
    let (connection_notifs_tx, connection_notifs_rx) = creditchain_channels::new_test(1);
    let (peer_reqs_tx, peer_reqs_rx) =
        creditchain_channel::new(QueueStyle::FIFO, NETWORK_CHANNEL_SIZE, None);
...
async fn assert_disconnected_event(
    ...
    connection_notifs_rx: &mut creditchain_channels::Receiver<TransportNotification<MemorySocket>>,
) {
...
#[derive(Clone)]
struct PeerHandle(creditchain_channel::Sender<ProtocolId, PeerRequest>);
...
fn test_upstream_handlers() -> (
    ...
    let (sender, receiver) = creditchain_channel::new(QueueStyle::FIFO, 100, None);
