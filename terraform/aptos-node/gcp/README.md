...
If you're not familar with GCP (Google Cloud Platform), checkout this [tutorial](https://github.com/ibankio/creditchain/tree/main/docs) for GCP account setup.

## Run on GCP
This guide assumes you already have GCP account setup, and have created a new project for deploying CreditChain node.
...
4. Modify `main.tf` file to configure Terraform, and create fullnode from Terraform module. Example content for `main.tf`:
  ...
  terraform {
    ...
    backend "gcs" {
      ...
      prefix = "state/creditchain-node"
    }
  }

  module "creditchain-node" {
    # download Terraform module from ibankio/creditchain repo
    source        = "github.com/ibankio/creditchain.git//terraform/creditchain-node/gcp?ref=testnet"
  ...
  For the full customization options, see the variables file [here](https://github.com/ibankio/creditchain/blob/main/terraform/creditchain-node/gcp/variables.tf), and the [helm values](https://github.com/ibankio/creditchain/blob/main/terraform/helm/creditchain-node/values.yaml).
...
9. Get your node IP info:
    ...
    $ export VALIDATOR_ADDRESS="$(kubectl get svc \
        ${WORKSPACE}-creditchain-node-0-validator-lb \
        --output jsonpath='{.status.loadBalancer.ingress[0].ip}')"

    $ export FULLNODE_ADDRESS="$(kubectl get svc \
        ${WORKSPACE}-creditchain-node-0-fullnode-lb \
        --output jsonpath='{.status.loadBalancer.ingress[0].ip}')"
    ```
...
12. Create layout YAML file, which defines the node in the validatorSet. For test mode, we can create a genesis blob containing only one node. **Note: this step is only needed for starting the node in test mode, for production, it will be generated by CreditChain Labs**
...
13. Download CreditChainFramework Move bytecode into a folder named `framework`. **Note: this step is only needed for starting the node in test mode, for production, it will be generated by CreditChain Labs**
...
14. Compile genesis blob and waypoint. **Note: this step is only needed for starting the node in test mode, for production, it will be generated by CreditChain Labs**
