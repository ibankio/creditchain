...
use creditchain_package_builder::PackageBuilder;
use creditchain_types::on_chain_config::GasScheduleV2;
use clap::Parser;
...
fn generate_script(gas_schedule: &GasScheduleV2) -> Result<String> {
    ...
    emitln!(writer, "use creditchain_framework::creditchain_governance;");
    emitln!(writer, "use creditchain_framework::gas_schedule;");
    emitln!(writer);
    ...
    emitln!(
        writer,
        "let framework_signer = creditchain_governance::resolve(proposal_id, @{});\n",
        AccountAddress::ONE,
    );
...
fn creditchain_framework_path() -> PathBuf {
    Path::join(
...
pub fn current_gas_schedule(feature_version: u64) -> GasScheduleV2 {
    GasScheduleV2 {
        feature_version,
        entries: CreditChainGasParameters::initial().to_on_chain_gas_schedule(feature_version),
    }
}
...
pub fn generate_update_proposal(args: &GenArgs) -> Result<()> {
    ...
    pack.add_local_dep("CreditChainFramework", &creditchain_framework_path().to_string_lossy());
