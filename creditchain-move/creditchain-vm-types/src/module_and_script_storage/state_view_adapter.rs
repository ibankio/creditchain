...
use crate::{
    module_and_script_storage::module_storage::CreditChainModuleStorage,
...
/// directly by clients - only via [AsCreditChainCodeStorage] trait. Can be used to resolve both modules
/// and cached scripts.
#[derive(Delegate)]
#[delegate(
    WithRuntimeEnvironment,
...
pub struct CreditChainCodeStorageAdapter<'ctx, S, E> {
...
impl<S, E> CreditChainCodeStorageAdapter<'_, S, E>
where
    S: StateView,
    E: WithRuntimeEnvironment,
{
    ...
    pub fn into_verified_module_code_iter(
        ...
        impl Iterator<
            Item = (
                ModuleId,
                Arc<ModuleCode<CompiledModule, Module, CreditChainModuleExtension>>,
            ),
        >,
        PanicError,
    > {
            ...
            .map(|(key, verified_code)| {
                ...
                let extension = state_view
                    ...
                    .map_or_else(
                        ...
                        |state_value| Ok(CreditChainModuleExtension::new(state_value)),
                    )?;
...
impl<S: StateView, E: WithRuntimeEnvironment> CreditChainModuleStorage
    for CreditChainCodeStorageAdapter<'_, S, E>
{
...
/// block executor, e.g., for single transaction simulation, in CreditChain debugger, etc.
pub trait AsCreditChainCodeStorage<'ctx, S, E> {
    fn as_creditchain_code_storage(
        &'ctx self,
        environment: &'ctx E,
    ) -> CreditChainCodeStorageAdapter<'ctx, S, E>;
}

impl<'ctx, S, E> AsCreditChainCodeStorage<'ctx, S, E> for S
where
    S: StateView,
    E: WithRuntimeEnvironment,
{
    fn as_creditchain_code_storage(
        ...
        CreditChainCodeStorageAdapter { storage }
    }
}
