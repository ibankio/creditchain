...
use crate::{CreditChainValidatorInterface, FilterCondition};
use anyhow::{anyhow, Result};
use creditchain_api_types::{CreditChainError, CreditChainErrorCode};
use creditchain_framework::{
    natives::code::{PackageMetadata, PackageRegistry},
    CREDITCHAIN_PACKAGES,
};
use creditchain_rest_client::{
    error::{CreditChainErrorResponse, RestError},
    Client,
};
...
async fn check_and_obtain_source_code(
    ...
    if let Some(target_package) = target_package_opt {
        ...
        if CREDITCHAIN_PACKAGES.contains(&target_package.name.as_str()) {
...
impl CreditChainValidatorInterface for RestDebuggerInterface {
    async fn get_state_value_by_version(
        ...
        match self.0.get_raw_state_value(state_key, version).await {
            ...
            Err(err) => match err {
                RestError::Api(CreditChainErrorResponse {
                    error:
                        CreditChainError {
                            error_code:
                                CreditChainErrorCode::StateValueNotFound | CreditChainErrorCode::TableItemNotFound, /* bug in pre 1.9 nodes */
                            ..
                        },
                    ..
                }) => Ok(None),
