...
use creditchain_crypto::{ed25519::Ed25519PrivateKey, ValidCryptoMaterialStringExt};
use creditchain_framework::natives::code::PackageRegistry;
use creditchain_gas_schedule::LATEST_GAS_FEATURE_VERSION;
use creditchain_release_builder::{
    components::fetch_config,
    initialize_creditchain_core_path,
...
pub struct Argument {
    ...
    cmd: Commands,
    #[clap(long)]
    creditchain_core_path: Option<PathBuf>,
}
...
pub enum Commands {
    ...
    PrintConfigs {
        /// Url endpoint for the desired network. e.g: https://fullnode.mainnet.creditchain.io/v1.
        #[clap(short, long)]
        endpoint: url::Url,
...
async fn main() -> anyhow::Result<()> {
    ...
    initialize_creditchain_core_path(args.creditchain_core_path.clone());
    ...
    match args.cmd {
        Commands::GenerateProposals {
            ...
            creditchain_release_builder::ReleaseConfig::load_config(release_config.as_path())
        ...
        Commands::WriteDefault { output_path } => {
            creditchain_release_builder::ReleaseConfig::default().save_config(output_path.as_path())
        },
        Commands::ValidateProposals {
            ...
            let root_key_path = creditchain_temppath::TempPath::new();
            root_key_path.create_as_file()?;

            let mut network_config = match input_option {
                InputOptions::FromDirectory { test_dir } => {
                    creditchain_release_builder::validate::NetworkConfig::new_from_dir(
                        endpoint.clone(),
                        test_dir.as_path(),
                    )?
                },
                InputOptions::FromArgs {
                    ...
                    creditchain_release_builder::validate::NetworkConfig {
                        root_key_path,
                        validator_account,
                        validator_key,
            ...
            if mint_to_validator {
                let mut client = Client::builder(CreditChainBaseUrl::Custom(endpoint));
            ...
            network_config
                ...
                .await?;
            creditchain_release_builder::validate::validate_config_and_generate_release(
                config,
        ...
        Commands::GenerateGasSchedule {
            ...
            let gas_schedule = creditchain_gas_schedule_updator::current_gas_schedule(version);
        ...
        Commands::PrintConfigs {
            ...
            print_configs!(OnChainConsensusConfig, OnChainExecutionConfig, CreditChainVersion);
            ...
            println!(
                ...
                serde_yaml::to_string(
                    &creditchain_release_builder::components::feature_flags::Features::from(&features)
                )?
            );
