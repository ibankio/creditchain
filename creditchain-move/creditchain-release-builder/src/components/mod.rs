...
use crate::{
    creditchain_core_path, creditchain_framework_path,
    components::{
...
use creditchain::governance::GenerateExecutionHash;
...
use creditchain_types::{
    ...
    on_chain_config::{
        CreditChainVersion, ExecutionConfigV1, FeatureFlag as CreditChainFeatureFlag, GasScheduleV2,
...
impl GasScheduleLocator {
    async fn fetch_gas_schedule(&self) -> Result<GasScheduleV2> {
        ...
        match self {
            ...
            GasScheduleLocator::Current => Ok(creditchain_gas_schedule_updator::current_gas_schedule(
    ...
    pub async fn generate_release_script(
        ...
        match self {
            ...
            ReleaseEntry::Version(version) => {
                if !fetch_and_equals::<CreditChainVersion>(client, version)? {
            ...
            ReleaseEntry::FeatureFlag(feature_flags) => {
                ...
                if let Some(client) = client {
                    let features = block_on(async {
                        client
                            .get_account_resource_bcs::<creditchain_types::on_chain_config::Features>(
                                CORE_CODE_ADDRESS,
            ...
            ReleaseEntry::RawScript(script_path) => {
                let base_path = creditchain_core_path().join(script_path.as_path());
...
pub fn get_execution_hash(result: &[(String, String)]) -> Option<HashValue> {
    if result.is_empty() {
        ...
        let (_, hash) = GenerateExecutionHash {
            ...
            framework_local_dir: Some(creditchain_framework_path()),
        }
        .generate_hash()
        .unwrap();
        Some(hash)
...
impl Default for ProposalMetadata {
    fn default() -> Self {
        ProposalMetadata {
            ...
            // CreditChain CLI need a valid url for the two fields.
