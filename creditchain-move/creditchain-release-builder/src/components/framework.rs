...
use crate::{creditchain_core_path, components::get_execution_hash};
use anyhow::Result;
use creditchain_crypto::HashValue;
use creditchain_framework::{BuildOptions, BuiltPackage, ReleasePackage};
...
pub fn generate_upgrade_proposals(
    ...
    const CREDITCHAIN_GIT_PATH: &str = "https://github.com/libra2org/libra2-core.git";

    // NOTE: This is skipping 0x7 (creditchain-experimental package) which is only meant to be released
    // to devnet (or local testnet) via the genesis process and never released/upgraded in testnet
    // or mainnet.
    let mut package_path_list = [
        ("0x1", "creditchain-move/framework/move-stdlib"),
        ("0x1", "creditchain-move/framework/creditchain-stdlib"),
        ("0x1", "creditchain-move/framework/creditchain-framework"),
        ("0x3", "creditchain-move/framework/creditchain-token"),
        ("0x4", "creditchain-move/framework/creditchain-token-objects"),
    ];
    ...
    let commit_info = if let Some(revision) = &config.git_hash {
        ...
        let repository = Repository::clone(CREDITCHAIN_GIT_PATH, temp_root_path.path())?;
        ...
        creditchain_build_info::get_git_hash()
    };
    ...
    for (publish_addr, relative_package_path) in package_path_list.iter() {
        ...
        // 0-move-stdlib.move	2-creditchain-framework.move	4-gas-schedule.move	6-features.move
        // 1-creditchain-stdlib.move	3-creditchain-token.move	5-version.move		7-consensus-config.move
        // The first framework file being generated is 3-creditchain-token.move. It's using the next_execution_hash being passed in (so in this case, the hash of 4-gas-schedule.move being passed in mod.rs).
        // The second framework file being generated would be 2-creditchain-framework.move, and it's using the hash of 3-creditchain-token.move (which would be result.last()).
