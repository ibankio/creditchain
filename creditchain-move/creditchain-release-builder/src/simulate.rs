...
use creditchain_transaction_simulation::{DeltaStateStore, SimulationStateStore};
...
use creditchain_vm::{data_cache::AsMoveResolver, move_vm_ext::SessionId, CreditChainVM};
...
use creditchain_vm_types::{
    module_and_script_storage::AsCreditChainCodeStorage, module_write_set::ModuleWriteSet,
 ...
 *
 **************************************************************************************************/
static MODULE_ID_CREDITCHAIN_GOVERNANCE: Lazy<ModuleId> = Lazy::new(|| {
    ModuleId::new(
        AccountAddress::ONE,
        Identifier::new("creditchain_governance").unwrap(),
    )
});
...
/// Patches `creditchain_framework::creditchain_governance::resolve_multi_step_proposal` so that
/// it returns the requested signer directly, skipping the governance process altogether.
fn patch_creditchain_governance(
    state_view: &impl SimulationStateStore,
    forbid_next_execution_hash: bool,
) -> Result<()> {
    use Bytecode::*;

    patch_module(state_view, &MODULE_ID_CREDITCHAIN_GOVERNANCE, |m| {
...
fn force_end_epoch(state_view: &impl SimulationStateStore) -> Result<()> {
    ...
    sess.execute_function_bypass_visibility(
        &MODULE_ID_CREDITCHAIN_GOVERNANCE,
...
pub async fn simulate_multistep_proposal(
    ...
    for path in proposal_scripts {
        let framework_package_args = FrameworkPackageArgs::try_parse_from([
            ...
            "--framework-local-dir",
            &creditchain_framework_path().to_string_lossy(),
    ...
    let mut client_builder = Client::builder(CreditChainBaseUrl::Custom(remote_url));
    ...
    let debugger =
        CreditChainDebugger::rest_client(client.clone()).context("failed to create CreditChainDebugger")?;
    ...
    std::io::stdout().flush()?;
    let mut rng = creditchain_keygen::KeyGen::from_seed([0; 32]);
        ...
        proposal_scripts.iter().zip(compiled_scripts).enumerate()
    {
        ...
        patch_creditchain_governance(&state_view, forbid_next_execution_hash)
