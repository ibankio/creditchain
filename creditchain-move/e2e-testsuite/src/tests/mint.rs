...
fn mint_to_new_account() {
    ...
    let mut root = Account::new_creditchain_root();
    let (private_key, public_key) = creditchain_vm_genesis::GENESIS_KEYPAIR.clone();
    ...
    let txn = root
        .transaction()
        .payload(creditchain_stdlib::creditchain_coin_mint(
            *new_account.address(),
            mint_amount,
        ))
        .gas_unit_price(0)
