...
module bonding_curve_launchpad::test_bonding_curve_launchpad {
    use creditchain_std::string;
    use creditchain_std::signer;
    ...
    fun test_setup_accounts(
        creditchain_framework: &signer, _swap_dex_signer: &signer, bonding_curve_creator: &signer
    ) {
        ...
        coin::register<CreditChainCoin>(bonding_curve_creator);
    ...
    #[test(deployer = @bonding_curve_launchpad)]
    #[expected_failure(abort_code = 393218, location = creditchain_framework::object)]
    ...
    fun test_e2e_bonding_curve_creation(
        ...
        test_setup_accounts(creditchain_framework, swap_dex_signer, bonding_curve_creator);
        ...
        let starting_apt_balance = coin::balance<CreditChainCoin>(user_address);
        let name = string::utf8(b"SheepyCoin");
        let symbol = string::utf8(b"SHEEP");
        ...
        assert!(coin::balance<CreditChainCoin>(user_address) == starting_apt_balance, EUSER_LBT_BALANCE_INCORRECT);
    ...
    fun test_e2e_bonding_curve_creation_with_initial_liquidity(
        ...
        assert!(coin::balance<CreditChainCoin>(user_address) == starting_apt_balance - 1000, EUSER_LBT_BALANCE_INCORRECT);
    ...
    fun test_e2e_directional_swaps(
        ...
        test_e2e_bonding_curve_creation(creditchain_framework, swap_dex_signer, bcl_owner_signer, bonding_curve_creator);
        let user_address = signer::address_of(bonding_curve_creator);
        let name = string::utf8(b"SheepyCoin");
        let symbol = string::utf8(b"SHEEP");
        let starting_apt_balance = coin::balance<CreditChainCoin>(user_address);
        ...
        assert!(
            coin::balance<CreditChainCoin>(user_address) == starting_apt_balance - 100_000_000,
        ...
        assert!(
            coin::balance<CreditChainCoin>(user_address) == starting_apt_balance - 26,
            EUSER_LBT_BALANCE_INCORRECT
    ...
    fun test_e2e_swap_after_graduation(
        ...
        test_e2e_graduation(creditchain_framework, swap_dex_signer, bcl_owner_signer, bonding_curve_creator);
        let fa_obj_metadata =
