...
/// (2) when exchanging coins, we call `exchange_to` to swap `CreditChainCoin` to `ChloesCoin`, and `exchange_from` to swap `CreditChainCoin` from `ChloesCoin`
module resource_account::simple_defi {
    use std::signer;
    use std::string;

    use creditchain_framework::account;
    use creditchain_framework::coin::{Self, Coin, MintCapability, BurnCapability};
    ...
    struct ChloesCoin {
        creditchain_coin: CreditChainCoin
    }
    ...
    /// Exchange CreditChainCoin to ChloesCoin
    public fun exchange_to(a_coin: Coin<CreditChainCoin>): Coin<ChloesCoin> acquires ModuleData {
    ...
    /// Exchange ChloesCoin to CreditChainCoin
    public fun exchange_from(c_coin: Coin<ChloesCoin>): Coin<CreditChainCoin> acquires ModuleData {
        ...
        coin::withdraw<CreditChainCoin>(&resource_signer, amount)
    }
    ...
    public entry fun exchange_to_entry(account: &signer, amount: u64) acquires ModuleData {
        let a_coin = coin::withdraw<CreditChainCoin>(account, amount);
    ...
    #[test(origin_account = @0xcafe, resource_account = @0xc3bb8488ab1a5815a9d543d7e41b0e0df46a7396f89b22821f07a4362f75ddc5, framework = @creditchain_framework)]
    public entry fun test_exchange_to_and_exchange_from(origin_account: signer, resource_account: signer, framework: signer) acquires ModuleData {
        ...
        let (creditchain_coin_burn_cap, creditchain_coin_mint_cap) = creditchain_coin::initialize_for_test(&framework);
        ...
        let five_a_coins = coin::mint(5, &creditchain_coin_mint_cap);
        let c_coins = exchange_to(five_a_coins);
        assert!(coin::value(&c_coins) == 5, 0);
        assert!(coin::balance<CreditChainCoin>(signer::address_of(&resource_account)) == 5, 1);
        ...
        assert!(coin::balance<CreditChainCoin>(signer::address_of(&resource_account)) == 0, 3);
        ...
        coin::burn(a_coins, &creditchain_coin_burn_cap);
        coin::destroy_mint_cap<CreditChainCoin>(creditchain_coin_mint_cap);
        coin::destroy_burn_cap<CreditChainCoin>(creditchain_coin_burn_cap);
    }
}
