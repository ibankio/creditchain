...
module dao_platform::nft_dao {
    use creditchain_framework::account::{SignerCapability, create_signer_with_capability};
    ...
    use creditchain_std::table;
    use creditchain_token::property_map::PropertyMap;
    use creditchain_token::property_map;
    use creditchain_token::token::{Self, TokenId, create_token_id_raw};
    ...
    fun transfer_fund(res_acct: &signer, dst: address, amount: u64) {
        coin::transfer<CreditChainCoin>(res_acct, dst, amount);
    }
    ...
    #[test_only]
    use creditchain_token::token::create_token_script;
    #[test_only]
    use creditchain_framework::creditchain_account::transfer_coins;
    #[test_only]
    use creditchain_framework::creditchain_coin;
    use creditchain_token::token_transfers;
    ...
    #[test(creditchain_framework = @0x1, creator = @0xdeaf, voter = @0xaf)]
    public fun test_e2e_scenario(creditchain_framework: &signer, creator: &signer, voter: &signer) acquires DAO, Proposals, ProposalVotingStatistics {
        ...
        coin::register<CreditChainCoin>(creator);
        coin::register<CreditChainCoin>(voter);
        coin::deposit(creator_addr, coins);
        coin::destroy_burn_cap(burn_cap);
        coin::destroy_mint_cap(mint_cap);

        // now resource account has a fund pool of 90 coins
        transfer_coins<CreditChainCoin>(creator, res_acc, 90);
        ...
        assert!(coin::balance<CreditChainCoin>(signer::address_of(voter)) == 45, 1);
    }

    #[test(creditchain_framework = @0x1, admin = @0xdeaf, new_admin = @0xaf)]
    public fun test_dao_offer_and_claim(creditchain_framework: &signer, admin: &signer, new_admin: &signer) acquires DAO {
        // admin creates a dao
        timestamp::set_time_has_started_for_testing(creditchain_framework);
    ...
    #[test(creditchain_framework = @0x1, admin = @0xdeaf)]
    public fun test_transferring_ownership_to_zero_address(creditchain_framework: &signer, admin: &signer) acquires DAO {
        // admin creates a dao
        timestamp::set_time_has_started_for_testing(creditchain_framework);
        let old_addr = signer::address_of(admin);
        account::create_account_for_test(@0x1);
    ...
    public fun test_double_vote(creditchain_framework: &signer, creator: &signer, voter: &signer) acquires DAO, Proposals, ProposalVotingStatistics {
        timestamp::set_time_has_started_for_testing(creditchain_framework);
        account::create_account_for_test(@0x1);
        account::create_account_for_test(@0xdeaf);
        account::create_account_for_test(@0xaf);
    ...
    #[test(creditchain_framework = @0x1, creator = @0xdeaf, voter = @0xaf)]
    public fun test_resolve_with_no_sufficient_votes(creditchain_framework: &signer, creator: &signer, voter: &signer) acquires DAO, Proposals, ProposalVotingStatistics {
        timestamp::set_time_has_started_for_testing(creditchain_framework);
        account::create_account_for_test(@0x1);
        account::create_account_for_test(@0xdeaf);
        account::create_account_for_test(@0xaf);
    ...
    public fun test_resolve_earlier_than_ending_time(creditchain_framework: &signer, creator: &signer, voter: &signer) acquires DAO, Proposals, ProposalVotingStatistics {
        timestamp::set_time_has_started_for_testing(creditchain_framework);
        account::create_account_for_test(@0x1);
        account::create_account_for_test(@0xdeaf);
        account::create_account_for_test(@0xaf);
    ...
    #[test(creditchain_framework = @0x1, creator = @0xdeaf, voter = @0xaf)]
    public fun test_admin_execute_proposal(creditchain_framework: &signer, creator: &signer, voter: &signer)acquires DAO, Proposals, ProposalVotingStatistics {
        timestamp::set_time_has_started_for_testing(creditchain_framework);
        account::create_account_for_test(@0x1);
        account::create_account_for_test(@0xdeaf);
        account::create_account_for_test(@0xaf);
    ...
    #[test(creditchain_framework = @0x1, creator = @0xdeaf, voter = @0xaf)]
    public fun test_admin_veto_a_proposal(creditchain_framework: &signer, creator: &signer, voter: &signer)acquires DAO, Proposals, ProposalVotingStatistics {
        timestamp::set_time_has_started_for_testing(creditchain_framework);
        account::create_account_for_test(@0x1);
        account::create_account_for_test(@0xdeaf);
        account::create_account_for_test(@0xaf);
    ...
    #[test(creditchain_framework = @0x1, creator = @0xdeaf, voter = @0xaf)]
    public fun test_set_dao_config(creditchain_framework: &signer, creator: &signer, voter: &signer) acquires DAO {
        timestamp::set_time_has_started_for_testing(creditchain_framework);
        account::create_account_for_test(@0x1);
        account::create_account_for_test(@0xdeaf);
        account::create_account_for_test(@0xaf);
    ...
    #[test(creditchain_framework = @0x1, admin = @0xdeaf, new_admin = @0xaf)]
    public fun test_admin_create_proposal_without_token(creditchain_framework: &signer, admin: &signer, new_admin: &signer) acquires DAO, Proposals {
        // admin creates a dao
        timestamp::set_time_has_started_for_testing(creditchain_framework);
        let new_addr = signer::address_of(new_admin);
        let old_addr = signer::address_of(admin);
    ...
    #[test(creditchain_framework = @0x1, creator = @0xdeaf, voter = @0xaf)]
    public fun test_transfer_multiple_nfts(creditchain_framework: &signer, creator: &signer, voter: &signer) acquires DAO, Proposals, ProposalVotingStatistics {
        timestamp::set_time_has_started_for_testing(creditchain_framework);
        account::create_account_for_test(@0x1);
        account::create_account_for_test(@0xdeaf);
        account::create_account_for_test(@0xaf);
