...
module staking::commission_tests {
    ...
    fun set_up() {
        timestamp::set_time_has_started_for_testing(&account::create_signer_for_test(@creditchain_framework));
    ...
    fun test_distribution_with_no_debt(manager: &signer) {
        ...
        commission::distribute_commission(manager);
        assert!(commission::commission_owed() == 0);
        assert!(coin::balance<CreditChainCoin>(signer::address_of(manager)) == expected_apt_amount);
        assert!(coin::balance<CreditChainCoin>(OPERATOR) == expected_apt_amount);
    }
    ...
    fun test_distribution_with_debt(manager: &signer) {
        ...
        assert!(coin::balance<CreditChainCoin>(signer::address_of(manager)) == 0);
        assert!(coin::balance<CreditChainCoin>(OPERATOR) == expected_apt_amount / 2);
    }
    ...
    fun mint_apt(amount: u64) {
        let (burn_cap, mint_cap) = creditchain_coin::initialize_for_test(
            &account::create_signer_for_test(@creditchain_framework));
        creditchain_account::deposit_coins(@staking, coin::mint(amount, &mint_cap));
