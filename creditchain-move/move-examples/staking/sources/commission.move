...
module staking::commission {
    ...
    public entry fun distribute_commission(account: &signer) acquires CommissionConfig {
        ...
        let balance = coin::balance<CreditChainCoin>(@staking);
        ...
        if (balance <= commission_in_apt) {
            ...
            creditchain_account::transfer(commission_signer, config.manager, surplus_balance);
        };

        let remaining_balance = coin::balance<CreditChainCoin>(@staking);
        creditchain_account::transfer(commission_signer, config.operator, remaining_balance);
