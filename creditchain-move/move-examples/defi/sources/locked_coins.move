...
module defi::locked_coins {
    ...
    #[test_only]
    use creditchain_framework::coin::BurnCapability;
    #[test_only]
    ...
    #[test_only]
    fun setup(creditchain_framework: &signer, sponsor: &signer): BurnCapability<CreditChainCoin> {
        ...
        coin::register<CreditChainCoin>(sponsor);
        let coins = coin::mint<CreditChainCoin>(2000, &mint_cap);
    ...
    #[test(creditchain_framework = @0x1, sponsor = @0x123, recipient = @0x234)]
    public entry fun test_recipient_can_claim_coins(
        creditchain_framework: &signer, sponsor: &signer, recipient: &signer) acquires Locks {
        let burn_cap = setup(creditchain_framework, sponsor);
        let recipient_addr = signer::address_of(recipient);
        creditchain_account::create_account(recipient_addr);
        let sponsor_address = signer::address_of(sponsor);
        initialize_sponsor<CreditChainCoin>(sponsor, sponsor_address);
        add_locked_coins<CreditChainCoin>(sponsor, recipient_addr, 1000, 1000);
        assert!(total_locks<CreditChainCoin>(sponsor_address) == 1, 0);
        timestamp::fast_forward_seconds(1000);
        claim<CreditChainCoin>(recipient, sponsor_address);
        assert!(total_locks<CreditChainCoin>(sponsor_address) == 0, 1);
        assert!(coin::balance<CreditChainCoin>(recipient_addr) == 1000, 0);
    ...
    public entry fun test_sponsor_can_update_lockup(
        ...
        add_locked_coins<CreditChainCoin>(sponsor, recipient_addr, 1000, 1000);
        assert!(total_locks<CreditChainCoin>(sponsor_address) == 1, 0);
        assert!(claim_time_secs<CreditChainCoin>(sponsor_address, recipient_addr) == 1000, 0);
        // Extend lockup.
        update_lockup<CreditChainCoin>(sponsor, recipient_addr, 2000);
        assert!(claim_time_secs<CreditChainCoin>(sponsor_address, recipient_addr) == 2000, 1);
        // Reduce lockup.
        update_lockup<CreditChainCoin>(sponsor, recipient_addr, 1500);
        assert!(claim_time_secs<CreditChainCoin>(sponsor_address, recipient_addr) == 1500, 2);
        assert!(total_locks<CreditChainCoin>(sponsor_address) == 1, 1);

        coin::destroy_burn_cap(burn_cap);
    }

    #[test(creditchain_framework = @0x1, sponsor = @0x123, recipient_1 = @0x234, recipient_2 = @0x345)]
    public entry fun test_sponsor_can_batch_update_lockup(
        creditchain_framework: &signer, sponsor: &signer, recipient_1: &signer, recipient_2: &signer) acquires Locks {
        let burn_cap = setup(creditchain_framework, sponsor);
        ...
        creditchain_account::create_account(recipient_1_addr);
        creditchain_account::create_account(recipient_2_addr);
        let sponsor_address = signer::address_of(sponsor);
        initialize_sponsor<CreditChainCoin>(sponsor, sponsor_address);
        batch_add_locked_coins<CreditChainCoin>(
            sponsor,
            vector[recipient_1_addr, recipient_2_addr],
            vector[1000, 1000],
            1000
        );
        assert!(claim_time_secs<CreditChainCoin>(sponsor_addr, recipient_1_addr) == 1000, 0);
        assert!(claim_time_secs<CreditChainCoin>(sponsor_addr, recipient_2_addr) == 1000, 0);
        // Extend lockup.
        batch_update_lockup<CreditChainCoin>(sponsor, vector[recipient_1_addr, recipient_2_addr], 2000);
        assert!(claim_time_secs<CreditChainCoin>(sponsor_addr, recipient_1_addr) == 2000, 1);
        assert!(claim_time_secs<CreditChainCoin>(sponsor_addr, recipient_2_addr) == 2000, 1);
        // Reduce lockup.
        batch_update_lockup<CreditChainCoin>(sponsor, vector[recipient_1_addr, recipient_2_addr], 1500);
        assert!(claim_time_secs<CreditChainCoin>(sponsor_addr, recipient_1_addr) == 1500, 2);
        assert!(claim_time_secs<CreditChainCoin>(sponsor_addr, recipient_2_addr) == 1500, 2);

        coin::destroy_burn_cap(burn_cap);
    }

    #[test(creditchain_framework = @0x1, sponsor = @0x123, recipient = @0x234, withdrawal = @0x345)]
    public entry fun test_sponsor_can_cancel_lockup(
        creditchain_framework: &signer, sponsor: &signer, recipient: &signer, withdrawal: &signer) acquires Locks {
        let burn_cap = setup(creditchain_framework, sponsor);
        let recipient_addr = signer::address_of(recipient);
        let withdrawal_addr = signer::address_of(withdrawal);
        creditchain_account::create_account(withdrawal_addr);
        creditchain_account::create_account(recipient_addr);
        let sponsor_address = signer::address_of(sponsor);
        initialize_sponsor<CreditChainCoin>(sponsor, withdrawal_addr);
        add_locked_coins<CreditChainCoin>(sponsor, recipient_addr, 1000, 1000);
        assert!(total_locks<CreditChainCoin>(sponsor_address) == 1, 0);
        assert!(coin::balance<CreditChainCoin>(withdrawal_addr) == 0, 0);
        cancel_lockup<CreditChainCoin>(sponsor, recipient_addr);
        assert!(total_locks<CreditChainCoin>(sponsor_address) == 0, 0);
        let locks = borrow_global_mut<Locks<CreditChainCoin>>(sponsor_address);
        assert!(!table::contains(&locks.locks, recipient_addr), 0);

        // Funds from canceled locks should be sent to the withdrawal address.
        assert!(coin::balance<CreditChainCoin>(withdrawal_addr) == 1000, 0);

        coin::destroy_burn_cap(burn_cap);
    }

    #[test(creditchain_framework = @0x1, sponsor = @0x123, recipient_1 = @0x234, recipient_2 = @0x345, withdrawal = @0x456)]
    public entry fun test_sponsor_can_batch_cancel_lockup(
        ...
        batch_cancel_lockup<CreditChainCoin>(sponsor, vector[recipient_1_addr, recipient_2_addr]);
        let locks = borrow_global_mut<Locks<CreditChainCoin>>(sponsor_address);
        assert!(!table::contains(&locks.locks, recipient_1_addr), 0);
        assert!(!table::contains(&locks.locks, recipient_2_addr), 0);
        // Funds from canceled locks should be sent to the withdrawal address.
        assert!(coin::balance<CreditChainCoin>(withdrawal_addr) == 2000, 0);
        coin::destroy_burn_cap(burn_cap);
    }

    #[test(creditchain_framework = @0x1, sponsor = @0x123, recipient = @0x234, withdrawal = @0x456)]
    ...
    public entry fun test_cannot_change_withdrawal_address_if_active_locks_exist(
        ...
        initialize_sponsor<CreditChainCoin>(sponsor, withdrawal_addr);
        add_locked_coins<CreditChainCoin>(sponsor, recipient_addr, 1000, 1000);
        update_withdrawal_address<CreditChainCoin>(sponsor, sponsor_address);
        coin::destroy_burn_cap(burn_cap);
    }
    ...
    public entry fun test_can_change_withdrawal_address_if_no_active_locks_exist(
        ...
        let sponsor_address = signer::address_of(sponsor);
        initialize_sponsor<CreditChainCoin>(sponsor, withdrawal_addr);
        assert!(withdrawal_address<CreditChainCoin>(sponsor_address) == withdrawal_addr, 0);
        add_locked_coins<CreditChainCoin>(sponsor, recipient_addr, 1000, 1000);
        cancel_lockup<CreditChainCoin>(sponsor, recipient_addr);
        update_withdrawal_address<CreditChainCoin>(sponsor, sponsor_address);
        assert!(withdrawal_address<CreditChainCoin>(sponsor_address) == sponsor_address, 0);
        coin::destroy_burn_cap(burn_cap);
    }
}
