module raffle::raffle_test {
    ...
    #[test_only]
    use creditchain_framework::coin::MintCapability;
    ...
    #[test_only]
    use creditchain_std::crypto_algebra::enable_cryptography_algebra_natives;
    #[test_only]
    use creditchain_framework::randomness;

    #[test_only]
    fun give_coins(mint_cap: &MintCapability<CreditChainCoin>, to: &signer) {
        let to_addr = signer::address_of(to);
        if (!account::exists_at(to_addr)) {
            account::create_account_for_test(to_addr);
        };
        coin::register<CreditChainCoin>(to);
    ...
    fun test_raffle(
        ...
        let (burn_cap, mint_cap) = creditchain_coin::initialize_for_test(&fx);

        // Create fake coins for users participating in raffle & initialize creditchain_framework
        ...
        while (i < num_players) {
            ...
            if (player == winner) {
                assert!(coin::balance<CreditChainCoin>(player) == raffle::get_ticket_price() * num_players, 1);
            } else {
                assert!(coin::balance<CreditChainCoin>(player) == 0, 1);
            };
