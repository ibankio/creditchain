...
module collection_offer {
    ...
    use creditchain_framework::object::{Self, DeleteRef, Object};
    ...
    use creditchain_token_objects::token::{Self as tokenv2, Token as TokenV2};
    ...
    inline fun init_coin_holder<CoinType>(
        ...
        creditchain_account::transfer_coins<CoinType>(purchaser, fee_address, fee);
    ...
    inline fun settle_payments<CoinType>(
        ...
        creditchain_account::deposit_coins(royalty_payee, royalties);
    ...
    inline fun cleanup<CoinType>(
        ...
        creditchain_account::deposit_coins(object::owner(collection_offer), coins);
...
module collection_offer_tests {
    ...
    use creditchain_token_objects::collection as collectionv2;
    ...
    fun test_token_v2(
        ...
        let collection_offer = collection_offer::init_for_tokenv2<CreditChainCoin>(
            purchaser,
            collection,
        ...
        assert!(coin::balance<CreditChainCoin>(purchaser_addr) == 8999, 0);
        assert!(coin::balance<CreditChainCoin>(seller_addr) == 10000, 0);

        collection_offer::sell_tokenv2<CreditChainCoin>(seller, collection_offer, token);
        assert!(coin::balance<CreditChainCoin>(marketplace_addr) == 6, 0);
        assert!(coin::balance<CreditChainCoin>(purchaser_addr) == 8999, 0);
        assert!(coin::balance<CreditChainCoin>(seller_addr) == 10495, 0);
        assert!(object::is_owner(token, purchaser_addr), 0);
        assert!(collection_offer::remaining(collection_offer) == 1, 0);

        collection_offer::sell_tokenv2<CreditChainCoin>(purchaser, collection_offer, token);
        assert!(coin::balance<CreditChainCoin>(marketplace_addr) == 11, 0);
        assert!(coin::balance<CreditChainCoin>(purchaser_addr) == 9489, 0);
        assert!(coin::balance<CreditChainCoin>(seller_addr) == 10500, 0);
        assert!(object::is_owner(token, purchaser_addr), 0);
    ...
    fun test_token_v1_direct_deposit(
        ...
        let collection_offer = collection_offer::init_for_tokenv1<CreditChainCoin>(
            purchaser,
            creator_addr,
            collection_name,
            test_utils::fee_schedule(marketplace),
        ...
        collection_offer::sell_tokenv1<CreditChainCoin>(seller, collection_offer, token_name, property_version);
        assert!(coin::balance<CreditChainCoin>(marketplace_addr) == 6, 0);
        assert!(coin::balance<CreditChainCoin>(purchaser_addr) == 8999, 0);
        assert!(coin::balance<CreditChainCoin>(seller_addr) == 10495, 0);
        ...
        collection_offer::sell_tokenv1<CreditChainCoin>(purchaser, collection_offer, token_name, property_version);
        assert!(coin::balance<CreditChainCoin>(marketplace_addr) == 11, 0);
        assert!(coin::balance<CreditChainCoin>(purchaser_addr) == 9489, 0);
        assert!(coin::balance<CreditChainCoin>(seller_addr) == 10500, 0);
        assert!(tokenv1::balance_of(purchaser_addr, token_id) == 1, 0);
    ...
    fun test_token_v1_indirect(
        ...
        let token_container = collection_offer::sell_tokenv1<CreditChainCoin>(
            seller,
            collection_offer,
            token_name,
            property_version,
    ...
    #[test(creditchain_framework = @0x1, marketplace = @0x111, seller = @0x222, purchaser = @0x333)]
    #[expected_failure(abort_code = 0x50004, location = creditchain_framework::object)]
    fun test_token_v2_has_none(
        ...
        collection_offer::sell_tokenv2<CreditChainCoin>(marketplace, collection_offer, token);
    }

    #[test(creditchain_framework = @0x1, marketplace = @0x111, seller = @0x222, purchaser = @0x333)]
    #[expected_failure(abort_code = 0x10005, location = creditchain_token::token)]
    fun test_token_v1_has_none(
        ...
        collection_offer::sell_tokenv1<CreditChainCoin>(
            marketplace,
            collection_offer,
            token_name,
            property_version,
