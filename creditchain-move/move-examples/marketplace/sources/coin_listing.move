...
module coin_listing {
    ...
    use creditchain_framework::object::{Self, ConstructorRef, Object, ObjectCore};
    ...
    inline fun init<CoinType>(
        ...
        creditchain_account::transfer_coins<CoinType>(
            seller,
    ...
    public entry fun purchase<CoinType>(
        ...
        let (price, type) = if (exists<AuctionListing<CoinType>>(listing_addr)) {
            ...
            if (option::is_some(&current_bid)) {
                ...
                creditchain_account::deposit_coins(bidder, coins);
            } else {
    ...
    inline fun complete_purchase<CoinType>(
        ...
        if (royalty_charge != 0) {
            ...
            creditchain_account::deposit_coins(royalty_addr, royalty);
        };
        ...
        creditchain_account::deposit_coins(fee_schedule::fee_address(fee_schedule), commission);

        // Seller gets what is left
        creditchain_account::deposit_coins(seller, coins);
...
module listing_tests {
    ...
    fun test_fixed_price(
        ...
        let (marketplace_addr, seller_addr, purchaser_addr) =
            test_utils::setup(creditchain_framework, marketplace, seller, purchaser);

        let (token, fee_schedule, listing) = fixed_price_listing(marketplace, seller);

        assert!(coin::balance<CreditChainCoin>(marketplace_addr) == 1, 0);
        assert!(coin::balance<CreditChainCoin>(seller_addr) == 9999, 0);
        assert!(listing::listed_object(listing) == object::convert(token), 0);
        assert!(listing::fee_schedule(listing) == fee_schedule, 0);
        assert!(coin_listing::price<CreditChainCoin>(listing) == option::some(500), 0);
        assert!(!coin_listing::is_auction<CreditChainCoin>(listing), 0);

        coin_listing::purchase<CreditChainCoin>(purchaser, listing);

        assert!(object::owner(token) == purchaser_addr, 0);
        assert!(coin::balance<CreditChainCoin>(marketplace_addr) == 6, 0);
        assert!(coin::balance<CreditChainCoin>(seller_addr) == 10494, 0);
        assert!(coin::balance<CreditChainCoin>(purchaser_addr) == 9500, 0);
    }

    #[test(creditchain_framework = @0x1, marketplace = @0x111, seller = @0x222, purchaser = @0x333)]
    fun test_fixed_price_high_royalty(
        ...
        assert!(coin::balance<CreditChainCoin>(marketplace_addr) == 1, 0);
        assert!(coin::balance<CreditChainCoin>(seller_addr) == 10499, 0);
        assert!(coin::balance<CreditChainCoin>(purchaser_addr) == 9500, 0);
    }
    ...
    fun test_fixed_price_end(
        ...
        assert!(coin::balance<CreditChainCoin>(marketplace_addr) == 1, 0);
        coin_listing::end_fixed_price<CreditChainCoin>(seller, listing);
        assert!(coin::balance<CreditChainCoin>(marketplace_addr) == 1, 0);
        assert!(coin::balance<CreditChainCoin>(seller_addr) == 9999, 0);
    ...
    fun test_auction_purchase(
        ...
        assert!(coin_listing::price<CreditChainCoin>(listing) == option::some(500), 0);
        assert!(coin_listing::is_auction<CreditChainCoin>(listing), 0);
        assert!(coin_listing::starting_bid<CreditChainCoin>(listing) == 100, 0);
        assert!(coin_listing::bid_increment<CreditChainCoin>(listing) == 50, 0);
        assert!(coin_listing::auction_end_time<CreditChainCoin>(listing) == timestamp::now_seconds() + 200, 0);
        assert!(coin_listing::minimum_bid_time_before_end<CreditChainCoin>(listing) == 150, 0);
        assert!(coin_listing::current_amount<CreditChainCoin>(listing) == option::none(), 0);
        assert!(coin_listing::current_bidder<CreditChainCoin>(listing) == option::none(), 0);
    ...
    fun test_auction_bid_then_purchase(
        ...
        coin_listing::bid<CreditChainCoin>(seller, listing, 100);
        assert!(coin_listing::current_amount<CreditChainCoin>(listing) == option::some(100), 0);
        assert!(coin_listing::current_bidder<CreditChainCoin>(listing) == option::some(seller_addr), 0);
        assert!(coin::balance<CreditChainCoin>(marketplace_addr) == 3, 0);
        assert!(coin::balance<CreditChainCoin>(seller_addr) == 9897, 0);

        // Return the bid and insert a new bid
        coin_listing::bid<CreditChainCoin>(purchaser, listing, 150);
        assert!(coin_listing::current_amount<CreditChainCoin>(listing) == option::some(150), 0);
        assert!(coin_listing::current_bidder<CreditChainCoin>(listing) == option::some(purchaser_addr), 0);
        assert!(coin::balance<CreditChainCoin>(marketplace_addr) == 5, 0);
        assert!(coin::balance<CreditChainCoin>(seller_addr) == 9997, 0);
        assert!(coin::balance<CreditChainCoin>(purchaser_addr) == 9848, 0);

        // Return the bid and replace with a purchase
        coin_listing::purchase<CreditChainCoin>(purchaser, listing);
        assert!(object::owner(token) == purchaser_addr, 0);
        assert!(coin::balance<CreditChainCoin>(marketplace_addr) == 10, 0);
        assert!(coin::balance<CreditChainCoin>(purchaser_addr) == 9498, 0);
    }
    ...
    fun test_auction_bidding(
        ...
        assert!(coin_listing::auction_end_time<CreditChainCoin>(listing) == end_time, 0);
        ...
        test_utils::increment_timestamp(150);
        coin_listing::complete_auction<CreditChainCoin>(creditchain_framework, listing);
        assert!(object::owner(token) == purchaser_addr, 0);
        assert!(coin::balance<CreditChainCoin>(marketplace_addr) == 6, 0);
        assert!(coin::balance<CreditChainCoin>(seller_addr) == 10146, 0);
    }
    ...
    fun test_not_started_fixed_price(
        ...
        let listing = coin_listing::init_fixed_price_internal<CreditChainCoin>(
            seller,
    ...
    fun test_not_started_auction(
        ...
        let token = test_utils::mint_tokenv2(seller);
        let fee_schedule = test_utils::fee_schedule(marketplace);
        let listing = coin_listing::init_auction_internal<CreditChainCoin>(
            seller,
            object::convert(token),
            fee_schedule,
            timestamp::now_seconds() + 1,
        ...
        coin_listing::bid<CreditChainCoin>(purchaser, listing, 1000);
    }
    ...
    #[test(creditchain_framework = @0x1, marketplace = @0x111, seller = @0x222, purchaser = @0x333)]
    #[expected_failure(abort_code = 0x10004, location = creditchain_framework::fungible_asset)]
    ...
    fun test_not_enough_coin_auction_bid(
        ...
        let (_token, _fee_schedule, listing) = auction_listing(marketplace, seller);
        coin_listing::bid<CreditChainCoin>(purchaser, listing, 100000);
    }
    ...
    fun test_bid_too_low(
        ...
        let (_token, _fee_schedule, listing) = auction_listing(marketplace, seller);
        coin_listing::bid<CreditChainCoin>(purchaser, listing, 100);
        coin_listing::bid<CreditChainCoin>(purchaser, listing, 125);
    }
    ...
    fun test_auction_view_on_fixed_price(
        ...
        coin_listing::auction_end_time<CreditChainCoin>(listing);
    }
    ...
    fun test_bad_fixed_price_end(
        ...
        let (_token, _fee_schedule, listing) = fixed_price_listing(marketplace, seller);
        coin_listing::end_fixed_price<CreditChainCoin>(purchaser, listing);
    }
    ...
    inline fun fixed_price_listing_for_tokenv1_with_token(
        ...
        let fee_schedule = test_utils::fee_schedule(marketplace);
        let listing = coin_listing::init_fixed_price_for_tokenv1_internal<CreditChainCoin>(
            seller,
            creator_addr,
            collection_name,
            token_name,
    ...
    inline fun auction_listing_for_tokenv1(
        ...
        let (creator_addr, collection_name, token_name, property_version) =
            tokenv1::get_token_id_fields(&token_id);
        let fee_schedule = test_utils::fee_schedule(marketplace);
        let listing = coin_listing::init_auction_for_tokenv1_internal<CreditChainCoin>(
            seller,
            creator_addr,
            collection_name,
            token_name,
