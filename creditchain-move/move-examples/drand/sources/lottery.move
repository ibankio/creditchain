...
///
/// WARNING #2: This code makes a STRONG assumption that the CreditChain clock and the drand clock are synchronized.
/// In practice, the CreditChain clock could be lagging behind. As an example, even though the current time is Friday, July
...
/// contracts account for any drift between the CreditChain clock and the drand clock. In this example, this can be done by
...
module drand::lottery {
    ...
    fun init_module(deployer: &signer) {
        ...
        // Initialize an CreditChainCoin coin store there, which is where the lottery bounty will be kept
        coin::register<CreditChainCoin>(&rsrc_acc_signer);
    ...
    public entry fun buy_a_ticket(user: &signer) acquires Lottery {
        ...
        coin::transfer<CreditChainCoin>(user, rsrc_acc_addr, TICKET_PRICE);
    ...
    public entry fun close_lottery(drand_signed_bytes: vector<u8>) acquires Lottery {
        ...
        let balance = coin::balance<CreditChainCoin>(rsrc_acc_addr);
        let winner_addr = *vector::borrow(&lottery.tickets, winner_idx);

        coin::transfer<CreditChainCoin>(
            &rsrc_acc_signer,
            winner_addr,
            balance);
