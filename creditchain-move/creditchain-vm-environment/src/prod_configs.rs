...
/// Returns [TypeBuilder] used by the CreditChain blockchain in production.
pub fn creditchain_prod_ty_builder(
    gas_feature_version: u64,
    gas_params: &CreditChainGasParameters,
) -> TypeBuilder {
    if gas_feature_version >= RELEASE_V1_15 {
        ...
        creditchain_default_ty_builder()
    }
}
...
pub fn creditchain_default_ty_builder() -> TypeBuilder {
    TypeBuilder::with_limits(128, 20)
}

/// Returns [DeserializerConfig] used by the CreditChain blockchain in production.
pub fn creditchain_prod_deserializer_config(features: &Features) -> DeserializerConfig {
...
/// Returns [VerifierConfig] used by the CreditChain blockchain in production.
pub fn creditchain_prod_verifier_config(gas_feature_version: u64, features: &Features) -> VerifierConfig {
...
/// Returns [VMConfig] used by the CreditChain blockchain in production, based on the set of feature
/// flags.
pub fn creditchain_prod_vm_config(
    gas_feature_version: u64,
    features: &Features,
    ...
    let deserializer_config = creditchain_prod_deserializer_config(features);
    let verifier_config = creditchain_prod_verifier_config(gas_feature_version, features);
