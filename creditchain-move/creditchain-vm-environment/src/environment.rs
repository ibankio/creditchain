...
use crate::{
    ...
    natives::creditchain_natives_with_builder,
    prod_configs::{
        creditchain_default_ty_builder, creditchain_prod_ty_builder, creditchain_prod_vm_config,
        get_timed_feature_override,
    },
};
use creditchain_gas_algebra::DynamicExpression;
use creditchain_gas_schedule::{CreditChainGasParameters, MiscGasParameters, NativeGasParameters};
use creditchain_native_interface::SafeNativeBuilder;
...
use creditchain_vm_types::storage::StorageGasParameters;
...
pub struct CreditChainEnvironment(Arc<Environment>);

impl CreditChainEnvironment {
    ...
    /// on-chain.
    #[inline]
    pub fn gas_params(&self) -> &Result<CreditChainGasParameters, String> {
        &self.0.gas_params
    }
...
impl Clone for CreditChainEnvironment {
    fn clone(&self) -> Self {
        Self(self.0.clone())
    }
}

impl PartialEq for CreditChainEnvironment {
    fn eq(&self, other: &Self) -> bool {
        self.0.hash == other.0.hash
    }
}

impl Eq for CreditChainEnvironment {}

impl WithRuntimeEnvironment for CreditChainEnvironment {
...
struct Environment {
    ...
    /// on-chain.
    gas_params: Result<CreditChainGasParameters, String>,
...
impl Environment {
    fn new(
        ...
        let (native_gas_params, misc_gas_params, ty_builder) = match &gas_params {
            Ok(gas_params) => {
                let ty_builder = creditchain_prod_ty_builder(gas_feature_version, gas_params);
                (
            ...
            Err(_) => {
                let ty_builder = creditchain_default_ty_builder();
                (
        ...
        let natives = creditchain_natives_with_builder(&mut builder, inject_create_signer_for_gov_sim);
        let vm_config =
            creditchain_prod_vm_config(gas_feature_version, &features, &timed_features, ty_builder);
        let verifier_bytes =
...
pub mod tests {
    ...
    fn test_environment_eq() {
        let state_view = MockStateView::empty();
        let environment_1 = CreditChainEnvironment::new(&state_view);
        let environment_2 = CreditChainEnvironment::new(&state_view);
    ...
    fn test_environment_ne() {
        ...
        for i in 0..state_views.len() {
            for j in 0..state_views.len() {
                if i != j {
                    let environment_1 = CreditChainEnvironment::new(&state_views[i]);
                    let environment_2 = CreditChainEnvironment::new(&state_views[j]);
                    assert!(environment_1 != environment_2);
                }
            }
        }
    ...
    fn test_environment_with_injected_create_signer_for_gov_sim() {
        ...
        let not_injected_envs = [
            CreditChainEnvironment::new(&state_view),
            CreditChainEnvironment::new_with_gas_hook(&state_view, Arc::new(|_| {})),
        ...
        // Injected.
        let env = CreditChainEnvironment::new_with_injected_create_signer_for_gov_sim(&state_view);
        #[allow(deprecated)]
