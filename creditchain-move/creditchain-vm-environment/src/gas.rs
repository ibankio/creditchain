...
use creditchain_vm_types::storage::{io_pricing::IoPricing, StorageGasParameters};
...
fn get_gas_config_from_storage(
...
) -> (Result<CreditChainGasParameters, String>, u64) {
    match GasScheduleV2::fetch_config_and_bytes(state_view) {
        Some((gas_schedule, bytes)) => {
            ...
            (
                CreditChainGasParameters::from_on_chain_gas_schedule(&map, feature_version),
                feature_version,
            )
        },
        None => match GasSchedule::fetch_config_and_bytes(state_view) {
            Some((gas_schedule, bytes)) => {
                sha3_256.update(&bytes);
                let map = gas_schedule.into_btree_map();
                (CreditChainGasParameters::from_on_chain_gas_schedule(&map, 0), 0)
            },
...
pub(crate) fn get_gas_parameters(
    ...
    Result<CreditChainGasParameters, String>,
