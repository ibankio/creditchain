...
///
/// # Arguments
/// * `guard` - The CreditChainModuleCacheManagerGuard to populate with empty modules
...
pub(crate) fn populate_guard_with_modules(
    guard: &mut CreditChainModuleCacheManagerGuard<'_>,
    module_ids: &[ModuleId],
) {
    for module_id in module_ids {
        ...
        let module_code = Arc::new(ModuleCode::from_arced_verified(
            Arc::new(module),
            Arc::new(CreditChainModuleExtension::new(StateValue::new_legacy(
...
{
    let mut guard = CreditChainModuleCacheManagerGuard::none();
