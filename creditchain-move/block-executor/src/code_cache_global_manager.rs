...
use creditchain_gas_schedule::gas_feature_versions::RELEASE_V1_34;
...
use creditchain_vm_logging::alert;
...
macro_rules! alert_or_println {
    ($($arg:tt)*) => {
        if cfg!(any(test, feature = "testing")) {
            ...
            use creditchain_vm_logging::{alert, prelude::CRITICAL_ERRORS};
...
pub struct ModuleCacheManager<K, D, V, E> {
    ...
    environment: Option<CreditChainEnvironment>,
    ...
    E: WithSize,
{
    ...
    fn check_ready(
        &mut self,
        storage_environment: CreditChainEnvironment,
        config: &BlockExecutorModuleCacheLocalConfig,
        transaction_slice_metadata: TransactionSliceMetadata,
    ) -> Result<(), VMStatus> {
...
/// Module cache manager used by CreditChain block executor. Ensures that only one thread has exclusive
/// access to it at a time.
pub struct CreditChainModuleCacheManager {
    inner: Mutex<ModuleCacheManager<ModuleId, CompiledModule, Module, CreditChainModuleExtension>>,
}

impl CreditChainModuleCacheManager {
    ...
    /// is ready for execution and updates states. If fails, [CreditChainModuleCacheManagerGuard::None]
    ...
    fn try_lock_inner(
        ...
        config: &BlockExecutorModuleCacheLocalConfig,
        transaction_slice_metadata: TransactionSliceMetadata,
    ) -> Result<CreditChainModuleCacheManagerGuard<'_>, VMStatus> {
        // Get the current environment from storage.
        let storage_environment =
            CreditChainEnvironment::new_with_delayed_field_optimization_enabled(&state_view);

        Ok(match self.inner.try_lock() {
            Some(mut guard) => {
                guard.check_ready(storage_environment, config, transaction_slice_metadata)?;
                CreditChainModuleCacheManagerGuard::Guard { guard }
            },
            None => {
                ...
                CreditChainModuleCacheManagerGuard::None {
    ...
    /// Tries to lock the manager using [CreditChainModuleCacheManager::try_lock_inner]. Additionally, if
    /// the module cache is empty, can prefetch CreditChain framework into it.
    pub fn try_lock(
        &self,
        state_view: &impl StateView,
        config: &BlockExecutorModuleCacheLocalConfig,
        ...
        if guard.module_cache().num_modules() == 0 && config.prefetch_framework_code {
            prefetch_creditchain_framework(state_view, &mut guard).map_err(|err| {
                alert_or_println!("Failed to load CreditChain framework to module cache: {:?}", err);
...
/// A guard that can be acquired from [CreditChainModuleCacheManager]. Variants represent successful and
/// no-successful lock acquisition.
pub enum CreditChainModuleCacheManagerGuard<'a> {
    /// Holds the guard to the [CreditChainModuleCacheManager], and has exclusive access to it.
    Guard {
        guard: MutexGuard<
            'a,
            ModuleCacheManager<ModuleId, CompiledModule, Module, CreditChainModuleExtension>,
        >,
    },
    /// Either there is no [CreditChainModuleCacheManager], or acquiring the lock for it failed.
    None {
        environment: CreditChainEnvironment,
        module_cache: GlobalModuleCache<ModuleId, CompiledModule, Module, CreditChainModuleExtension>,
    },
}

impl CreditChainModuleCacheManagerGuard<'_> {
    /// Returns the references to the environment. If environment is not set, panics.
    pub fn environment(&self) -> &CreditChainEnvironment {
        use CreditChainModuleCacheManagerGuard::*;
        match self {
            Guard { guard } => guard
                .environment
                .as_ref()
    ...
    pub fn module_cache(
        &self,
    ) -> &GlobalModuleCache<ModuleId, CompiledModule, Module, CreditChainModuleExtension> {
        use CreditChainModuleCacheManagerGuard::*;
        match self {
    ...
    pub fn module_cache_mut(
        &mut self,
    ) -> &mut GlobalModuleCache<ModuleId, CompiledModule, Module, CreditChainModuleExtension> {
        use CreditChainModuleCacheManagerGuard::*;
        match self {
    ...
    /// A guard in [CreditChainModuleCacheManagerGuard::None] state with empty module cache and default
    /// environment. Use for testing only.
    #[cfg(test)]
    pub(crate) fn none() -> Self {
        use creditchain_types::state_store::MockStateView;
        Self::none_for_state_view(&MockStateView::empty())
    }

    /// A guard in [CreditChainModuleCacheManagerGuard::None] state with empty module cache and the
    ...
    pub(crate) fn none_for_state_view(state_view: &impl StateView) -> Self {
        CreditChainModuleCacheManagerGuard::None {
            environment: CreditChainEnvironment::new(state_view),
            module_cache: GlobalModuleCache::empty(),
        }
    }
}

/// If CreditChain framework exists, loads "transaction_validation.move" and all its transitive
/// dependencies from storage into provided module cache. If loading fails for any reason, a panic
/// error is returned.
fn prefetch_creditchain_framework(
    state_view: &impl StateView,
    guard: &mut CreditChainModuleCacheManagerGuard,
) -> Result<(), PanicError> {
    let code_storage = state_view.as_creditchain_code_storage(guard.environment());
    ...
    cfg_if! {
        if #[cfg(fuzzing)] {
            let maybe_loaded = code_storage
                ...
                .map_err(|err| {
                    PanicError::CodeInvariantError(format!("Unable to fetch CreditChain framework: {:?}", err))
                })?;
        } else {
            let maybe_loaded = code_storage
...
mod test {
    ...
    #[test]
    fn test_prefetch_existing_creditchain_framework() {
        let state_view = InMemoryStateStore::from_head_genesis();

        let mut guard = CreditChainModuleCacheManagerGuard::none_for_state_view(&state_view);
        assert_eq!(guard.module_cache().num_modules(), 0);

        let result = prefetch_creditchain_framework(&state_view, &mut guard);
        assert!(result.is_ok());
        assert!(guard.module_cache().num_modules() > 0);
    }

    #[test]
    fn test_prefetch_non_existing_creditchain_framework() {
    ...
    fn test_check_ready() {
        ...
        assert_ok!(manager.check_ready(CreditChainEnvironment::new(&state_view), &config, metadata_1));
        ...
        assert_ok!(manager.check_ready(CreditChainEnvironment::new(&state_view), &config, metadata_2));
        ...
        assert_ok!(manager.check_ready(CreditChainEnvironment::new(&state_view), &config, metadata_3));
        ...
        assert_ok!(manager.check_ready(CreditChainEnvironment::new(&state_view), &config, metadata_4));
        ...
        assert_ok!(manager.check_ready(CreditChainEnvironment::new(&state_view), &config, metadata_5));
        ...
        assert_ok!(manager.check_ready(CreditChainEnvironment::new(&state_view), &config, metadata_6));
    ...
    fn test_try_lock_inner_single_thread() {
        let manager = CreditChainModuleCacheManager::new();
        ...
        assert!(matches!(guard, CreditChainModuleCacheManagerGuard::Guard { .. }));
    }

    #[test]
    fn test_try_lock_inner_multiple_threads() {
        let manager = Arc::new(CreditChainModuleCacheManager::new());
        ...
        for _ in 0..num_threads {
            let handle = std::thread::spawn({
                ...
                move || {
                    ...
                    if matches!(guard, CreditChainModuleCacheManagerGuard::Guard { .. }) {
                        1
                    } else {
                        0
                    }
