...
//!
//! '''bash
//! cargo run -p creditchain-sdk-builder -- --help
//! '''
...
#[derive(Debug, Parser)]
#[clap(name = "CreditChain SDK Builder", about = "Generate boilerplate CreditChain SDKs")]
struct Options {
    ...
    #[clap(long)]
    with_creditchain_types: Option<PathBuf>,
    ...
    /// and `creditchain_types` is assumed to be in "go_module/path/creditchain_types".
    #[clap(long)]
    ...
    /// Optional version number for the `creditchain_types` module (useful in Rust).
    /// If `--with-creditchain-types` is passed, this will be the version of the generated `creditchain_types` module.
    #[clap(long, default_value = "0.1.0")]
    creditchain_version_number: String,

    /// Optional package name (Python) or module path (Go) of the `creditchain_types` dependency.
    #[clap(long)]
...
fn main() {
    ...
    let abis = creditchain_sdk_builder::read_abis(&options.abi_directories)
    ...
    let install_dir = match options.target_source_dir {
        None => {
            ...
            match options.language {
                Language::Rust => {
                    creditchain_sdk_builder::rust::output(&mut out, &abis, /* local types */ true)
                        .unwrap()
                },
                Language::Go => {
                    creditchain_sdk_builder::golang::output(
                        &mut out,
    ...
    // CreditChain types
    if let Some(registry_file) = options.with_creditchain_types {
        ...
        if let Language::Rust = options.language {
            creditchain_sdk_builder::rust::replace_keywords(&mut registry)
        }

        let (package_name, _package_path) = match options.language {
            Language::Rust => (
                if options.creditchain_version_number == "0.1.0" {
                    "creditchain-types".to_string()
                } else {
                    format!("creditchain-types:{}", options.creditchain_version_number)
                },
                vec!["creditchain-types"],
            ),
    ...
    let installer: Box<dyn creditchain_sdk_builder::SourceInstaller<Error = Box<dyn std::error::Error>>> =
        match options.language {
            Language::Rust => Box::new(creditchain_sdk_builder::rust::Installer::new(
                install_dir,
                options.creditchain_version_number,
            )),
            Language::Go => Box::new(creditchain_sdk_builder::golang::Installer::new(
                install_dir,
