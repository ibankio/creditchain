...
/// If `local_types` is true, we generate a file suitable for the CreditChain codebase itself
...
struct RustEmitter<T> {
    ...
    /// Whether we are targeting the CreditChain repository itself (as opposed to generated CreditChain types).
    local_types: bool,
}
    ...
    fn get_external_definitions(local_types: bool) -> serde_generate::ExternalDefinitions {
        let definitions = if local_types {
            vec![
                ...
                ("creditchain_types::transaction", vec![
                    "TransactionPayload",
                    "EntryFunction",
                ]),
                ("creditchain_types::account_address", vec!["AccountAddress"]),
            ]
        } else {
            vec![("creditchain_types", vec![
                "AccountAddress",
                "TypeTag",
                "Script",
                "EntryFunction",
...
pub fn decode(payload: &TransactionPayload) -> Option<EntryFunctionCall> {{
    ...
    fn output_transaction_script_name_method(
        ...
        writeln!(
            self.out,
            r#"
/// Return the name of an CreditChain `Script` from a structured object `ScriptCall`.
...
pub struct Installer {
    install_dir: PathBuf,
    creditchain_types_version: String,
}

impl Installer {
    pub fn new(install_dir: PathBuf, creditchain_types_version: String) -> Self {
        Installer {
            install_dir,
            creditchain_types_version,
        }
    }
}
...
serde_bytes = "0.11.6"
creditchain-types = {{ path = "../creditchain-types", version = "{}" }}
"#,
            name, version, self.creditchain_types_version,
        )?;
