...
pub fn output(
    ...
    creditchain_module_path: Option<String>,
    package_name: String,
    abis: &[EntryABI],
) -> Result<()> {
        ...
        .filter(|abi| {
            if let EntryABI::EntryFunction(sf) = abi {
                sf.module_name().name().as_str() != "code"
                    ...
                    && sf.module_name().name().as_str() != "creditchain_account"
...
struct GoEmitter<T> {
    ...
    serde_module_path: Option<String>,
    /// Go module path for CreditChain types.
    ...
    T: Write,
{
    fn output_script_call_enum_with_imports(&mut self, abis: &[EntryABI]) -> Result<()> {
        let creditchain_types_package = match &self.creditchain_module_path {
            Some(path) => format!("{}/aptostypes", path),
            None => "aptostypes".into(),
        };
        let mut external_definitions =
            crate::common::get_external_definitions(&creditchain_types_package);
    ...
    fn output_encode_method(&mut self, abis: &[EntryABI]) -> Result<()> {
        ...
        if !transaction_script_abis.is_empty() {
            writeln!(
                self.out,
                r#"
// Build an CreditChain `Script` from a structured object `ScriptCall`.
func EncodeScript(call ScriptCall) aptostypes.Script {{"#
        ...
        if !entry_fun_abis.is_empty() {
            writeln!(
                self.out,
                r#"
// Build an CreditChain `TransactionPayload` from a structured object `EntryFunctionCall`.
func EncodeEntryFunction(call EntryFunctionCall) aptostypes.TransactionPayload {{"#
    ...
    fn output_transaction_script_decode_method(&mut self) -> Result<()> {
        writeln!(
            self.out,
            r#"
// Try to recognize an CreditChain `Script` and convert it into a structured object `ScriptCall`.
func DecodeScript(script *aptostypes.Script) (ScriptCall, error) {{
    ...
    fn output_entry_function_decode_method(&mut self) -> Result<()> {
        writeln!(
            self.out,
            r#"
// Try to recognize an CreditChain `TransactionPayload` and convert it into a structured object `EntryFunctionCall`.
...
impl crate::SourceInstaller for Installer {
    ...
    fn install_transaction_builders(
        ...
        output(
            ...
            self.creditchain_module_path.clone(),
            name.to_string(),
            abis,
        )?;
        Ok(())
