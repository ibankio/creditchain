...
use creditchain_sdk_builder as buildgen;
use creditchain_types::transaction::EntryABI;
...
fn get_creditchain_registry() -> Registry {
...
fn test_rust(abis: &[EntryABI], demo_file: &str, expected_output: &str) {
    let mut registry = get_creditchain_registry();
    buildgen::rust::replace_keywords(&mut registry);
    let dir = tempdir().unwrap();

    let installer = serdegen::rust::Installer::new(dir.path().to_path_buf());
    let config = serdegen::CodeGeneratorConfig::new("creditchain-types".to_string());
...
[dependencies]
creditchain-types = {{ path = "../creditchain-types", version = "0.1.0" }}
serde_bytes = "0.11.6"
...
#[test]
// Ignored because transactions require minting/transferring Coin<CreditChainCoin>, which the
...
fn test_that_rust_entry_fun_code_compiles() {
    ...
    let abis = vec![];
    test_rust(
        &abis, // &creditchain_cached_packages::head_release_bundle().abis(),
