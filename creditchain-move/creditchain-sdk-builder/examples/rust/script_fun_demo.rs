...
use creditchain_framework::{creditchain_coin_transfer, EntryFunctionCall};
use creditchain_types::AccountAddress;
...
fn demo_p2p_entry_function() {
    ...
    let payload = creditchain_coin_transfer(payee.clone(), amount);
    let function_call = EntryFunctionCall::decode(&payload);
    match function_call {
        Some(EntryFunctionCall::CreditChainCoinTransfer { amount: a, to: p }) => {
            assert_eq!(a, amount);
