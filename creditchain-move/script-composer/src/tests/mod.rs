...
fn simple_builder() {
    ...
    load_module(&mut builder, &h, "0x1::creditchain_account");
    builder
        .add_batched_call(
            "0x1::creditchain_account".to_string(),
            "transfer".to_string(),
            vec![],
            vec![
    ...
    assert_eq!(h.read_creditchain_balance(bob.address()), 1_000_000_000_000_010);
...
fn chained_deposit() {
    ...
    load_module(&mut builder, &h, "0x1::creditchain_coin");
    load_module(&mut builder, &h, "0x1::primary_fungible_store");
    let mut returns_1 = builder
        .add_batched_call(
            "0x1::coin".to_string(),
            "withdraw".to_string(),
            vec!["0x1::creditchain_coin::CreditChainCoin".to_string()],
            vec![
                CallArgument::new_signer(0),
                CallArgument::new_bytes(MoveValue::U64(10).simple_serialize().unwrap()),
            ],
