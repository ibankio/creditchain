...
use creditchain_transaction_workloads_lib::{EntryPoints, LoopType, MapType, OrderBookState};
...
fn get_parsed_calibration_values() -> HashMap<String, CalibrationInfo> {
    let calibration_values =
        fs::read_to_string("creditchain-move/e2e-benchmark/data/calibration_values.tsv")
...
fn main() {
    ...
    for (index, (flow, entry_point)) in entry_points.into_iter().enumerate() {
        ...
        json_lines.push(json!({
            "grep": "grep_json_creditchain_move_vm_perf",
    ...
    assert_eq!(
        0,
        creditchain_logger::ERROR_LOG_COUNT.get(),
