...
use creditchain_validator_interface::{
    CreditChainValidatorInterface, DBDebuggerInterface, DebuggerStateView, RestDebuggerInterface,
};
use creditchain_vm::{
    creditchain_vm::CreditChainVMBlockExecutor, data_cache::AsMoveResolver, CreditChainVM, VMBlockExecutor,
};
use creditchain_vm_environment::environment::CreditChainEnvironment;
use creditchain_vm_logging::log_schema::AdapterLogSchema;
use creditchain_vm_types::{module_and_script_storage::AsCreditChainCodeStorage, output::VMOutput};
use itertools::Itertools;
use std::{path::Path, sync::Arc, time::Instant};

pub struct CreditChainDebugger {
    debugger: Arc<dyn CreditChainValidatorInterface + Send>,
}

impl CreditChainDebugger {
    pub fn new(debugger: Arc<dyn CreditChainValidatorInterface + Send>) -> Self {
        Self { debugger }
    }
...
fn execute_block_no_limit(
    ...
    let executor = CreditChainVMBlockExecutor::new();
    executor
