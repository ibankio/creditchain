...
use crate::{creditchain_debugger::CreditChainDebugger, common::Opts};
use anyhow::Result;
...
impl Command {
    pub async fn run(self) -> Result<()> {
        let debugger = if let Some(rest_endpoint) = self.opts.target.rest_endpoint {
            CreditChainDebugger::rest_client(Client::new(Url::parse(&rest_endpoint)?))?
        } else if let Some(db_path) = self.opts.target.db_path {
            CreditChainDebugger::db(db_path)?
        } else {
        ...
        let user_txns = if let Some(block_rest_endpoint) = self.block_rest_endpoint {
            ...
            let cmd = creditchain_consensus::util::db_tool::Command {
