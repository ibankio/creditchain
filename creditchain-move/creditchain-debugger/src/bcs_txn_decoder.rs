...
use crate::creditchain_debugger::CreditChainDebugger;
use anyhow::Result;
...
impl Command {
    pub async fn run(self) -> Result<()> {
        ...
        let endpoint = format!("https://{}.creditchain.io/v1", network);
        let debugger = CreditChainDebugger::rest_client(Client::new(Url::parse(&endpoint)?))?;
        ...
        println!(
            "Overview: https://explorer.creditchain.io/txn/{:?}/userTxnOverview?network={}",
            version, network
        );
        println!(
            "Payload: https://explorer.creditchain.io/txn/{:?}/payload?network={}",
            version, network
        );
