...
fn mint_nft_e2e() {
    ...
    let result = h.run_transaction_payload(
        &acc,
        creditchain_cached_packages::creditchain_stdlib::resource_account_create_resource_account_and_publish_package(
            vec![],
...
/// in `creditchain-move/move-examples/mint_nft/4-Getting-Production-Ready/sources/create_nft_getting_production_ready.move`. åååååååå
#[test]
fn generate_nft_tutorial_part4_signature() {
    ...
    let mint_proof = MintProofChallenge {
        ...
        // you can find an account's sequence number by searching for the account's address on explorer.creditchain.io and going to the `Info` tab
        receiver_account_sequence_number: 0,
        receiver_account_address: *nft_receiver.address(),
        token_data_id,
    };
