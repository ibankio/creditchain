...
use creditchain_types::{
    ...
    vm::module_metadata::{KnownAttribute, RuntimeModuleMetadataV1, CREDITCHAIN_METADATA_KEY_V1},
};
...
fn test_bad_fun_attribute_in_compiled_module() {
    ...
    let metadata = Metadata {
        key: CREDITCHAIN_METADATA_KEY_V1.to_vec(),
...
fn test_bad_view_attribute_in_compiled_module() {
    ...
    let result = h.run_transaction_payload(
        &account,
        creditchain_stdlib::code_publish_package_txn(metadata, code),
    );
