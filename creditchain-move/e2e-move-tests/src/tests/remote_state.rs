...
//! of a remote CreditChain network.
//!
...
use creditchain_framework::BuildOptions;
...
const CREDITCHAIN_COIN_STRUCT_STRING: &str = "0x1::creditchain_coin::CreditChainCoin";
...
fn get_account_apt_balance(h: &mut MoveHarness, addr: AccountAddress) -> u64 {
    let bytes = h
        .execute_view_function(
            ...
            vec![TypeTag::from_str(CREDITCHAIN_COIN_STRUCT_STRING).unwrap()],
            vec![addr.to_vec()],
        )
        .values
        .unwrap()
...
async fn view_existing_account_balance() {
    ...
    let mut h = MoveHarness::new_with_remote_state(CreditChainBaseUrl::Testnet, TESTNET_TXN_VERSION);
