...
use creditchain_vm::testing::{testing_only::inject_error_once, InjectedError};
...
fn assert_result(
    ...
    let start_balance = h.read_creditchain_balance(account.address());
    ...
    let end_balance = h.read_creditchain_balance(account.address());
