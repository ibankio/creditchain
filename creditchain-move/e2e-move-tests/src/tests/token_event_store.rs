...
fn test_token_creation_with_token_events_store() {
    ...
    let acc = h.creditchain_framework_account();
    ...
    h.run_transaction_payload(
        &acc,
        creditchain_token_stdlib::token_create_collection_script(
    ...
    h.run_transaction_payload(
        &acc,
        creditchain_token_stdlib::token_create_token_script(
            collection_name,
            token_name,
            "collection description".to_owned().into_bytes(),
            10,
    ...
    let signed_txn =
        h.create_transaction_payload(&acc, creditchain_token_stdlib::token_opt_in_direct_transfer(true));
