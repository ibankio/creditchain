...
fn test_existing_account_with_fee_payer() {
    ...
    let alice_start = h.read_creditchain_balance(alice.address());
    let bob_start = h.read_creditchain_balance(bob.address());

    let payload = creditchain_stdlib::creditchain_coin_transfer(*alice.address(), 0);
    ...
    let alice_after = h.read_creditchain_balance(alice.address());
    let bob_after = h.read_creditchain_balance(bob.address());
...
fn test_existing_account_with_fee_payer_aborts() {
    ...
    let payload = creditchain_stdlib::creditchain_coin_transfer(*alice.address(), 1);
    let transaction = TransactionBuilder::new(alice.clone())
        .fee_payer(bob.clone())
        .payload(payload)
        .sequence_number(h.sequence_number(alice.address()))
...
fn test_account_not_exist_with_fee_payer() {
    ...
    let alice_start = h.read_resource::<CoinStoreResource<CreditChainCoinType>>(
        alice.address(),
    ...
    let payload = creditchain_stdlib::creditchain_account_set_allow_direct_coin_transfers(true);
    let transaction = TransactionBuilder::new(alice.clone())
        .fee_payer(bob.clone())
        .payload(payload)
        .sequence_number(0)
    ...
    let alice_after = h.read_resource::<CoinStoreResource<CreditChainCoinType>>(
        alice.address(),
        CoinStoreResource::<CreditChainCoinType>::struct_tag(),
    );
