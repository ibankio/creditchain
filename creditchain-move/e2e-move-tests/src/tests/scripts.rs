...
use creditchain_language_e2e_tests::account::TransactionBuilder;
...
fn test_script_with_type_parameter() {
    ...
    let package = BuiltPackage::build(
        ...
        creditchain_framework::BuildOptions::default(),
    )
    .expect("building package must succeed");
...
fn test_script_with_signer_parameter() {
    ...
    assert_eq!(
        ...
        TransactionStatus::Keep(
            creditchain_types::transaction::ExecutionStatus::MiscellaneousError(Some(
                creditchain_types::vm_status::StatusCode::INVALID_MAIN_FUNCTION_SIGNATURE
            ))
        )
    );
}
...
fn test_two_to_two_transfer() {
    ...
    let build_options = creditchain_framework::BuildOptions {
        with_srcs: false,
        with_abis: false,
...
fn read_coin(h: &MoveHarness, account: &AccountAddress) -> u64 {
    h.read_creditchain_balance(account)
}
