...
use creditchain_cached_packages::creditchain_stdlib::{creditchain_account_batch_transfer, creditchain_account_transfer};
...
fn test_basic_fungible_token() {
    ...
    let root = h.creditchain_framework_account();

    let mut build_options = creditchain_framework::BuildOptions::default();
    build_options
        .named_addresses
...
fn test_coin_to_fungible_asset_migration() {
    ...
    assert_success!(h.run_entry_function(
        ...
        .unwrap(),
        vec![TypeTag::from_str("0x1::creditchain_coin::CreditChainCoin").unwrap()],
        vec![],
    ));
    assert!(h
...
fn test_prologue_speculation() {
    ...
    let sink_txn = harness.create_transaction_payload(
        &independent_account,
        creditchain_account_batch_transfer(vec![AccountAddress::random(); 50], vec![10_000_000_000; 50]),
    );
    ...
    let fund_txn = harness.create_transaction_payload(
        &account,
        creditchain_account_batch_transfer(
    ...
    let transfer_1_txn =
        harness.create_transaction_payload(&dst_1, creditchain_account_transfer(*dst_2.address(), 1));
    let transfer_2_txn =
        harness.create_transaction_payload(&dst_2, creditchain_account_transfer(*dst_3.address(), 1));
    let transfer_3_txn =
        harness.create_transaction_payload(&dst_3, creditchain_account_transfer(*dst_1.address(), 1));
