...
use creditchain_crypto::{hash::CryptoHash, SigningKey};
use creditchain_framework::BuiltPackage;
use creditchain_language_e2e_tests::account::{Account, AccountPublicKey, TransactionBuilder};
...
/// Initializes an CreditChain VM and sets the keyless configuration via script.
...
fn spend_keyless_account(
    ...
    let payload = creditchain_stdlib::creditchain_coin_transfer(recipient, 1);
