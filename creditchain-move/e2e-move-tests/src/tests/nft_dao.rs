...
use creditchain_framework::BuiltPackage;
use creditchain_types::account_address::create_resource_address;
...
fn test_nft_dao_txn_arguments() {
    ...
    h.run_transaction_payload(
        &acc,
        creditchain_cached_packages::creditchain_token_sdk_builder::token_create_collection_script(
    ...
    for tok in &token_names {
        h.run_transaction_payload(
            &acc,
            creditchain_cached_packages::creditchain_token_sdk_builder::token_create_token_script(
                collection_name.clone(),
                tok.clone(),
                "".to_owned().into_bytes(),
                1,
    ...
    h.run_transaction_payload(
        &acc,
        creditchain_cached_packages::creditchain_token_sdk_builder::token_transfer_with_opt_in(
            *acc.address(),
            collection_name.clone(),
            token_names[0].clone(),
            0,
    ...
    h.run_transaction_payload(
        &voter,
        creditchain_cached_packages::creditchain_token_sdk_builder::token_opt_in_direct_transfer(true),
    );
    h.run_transaction_payload(
        &acc,
        creditchain_cached_packages::creditchain_token_sdk_builder::token_transfer_with_opt_in(
