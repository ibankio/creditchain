...
use crate::{assert_success, CreditChainPackageHooks};
use creditchain_cached_packages::creditchain_stdlib;
use creditchain_framework::{natives::code::PackageMetadata, BuildOptions, BuiltPackage};
use creditchain_gas_profiling::TransactionGasLog;
use creditchain_gas_schedule::{
    CreditChainGasParameters, FromOnChainGasSchedule, InitialGasSchedule, ToOnChainGasSchedule,
};
use creditchain_language_e2e_tests::{
    account::{Account, TransactionBuilder},
    executor::FakeExecutor,
};
use creditchain_rest_client::CreditChainBaseUrl;
use creditchain_transaction_simulation::SimulationStateStore;
...
impl MoveHarness {
    ...
    pub fn new() -> Self {
        register_package_hooks(Box::new(CreditChainPackageHooks {}));
        Self {
    ...
    fn new_with_remote_state_impl(
        ...
        let gas_params = CreditChainGasParameters::from_on_chain_gas_schedule(
    ...
    /// Creates a [`MoveHarness`] from a remote network state at the version specified by the
    /// transaction id.
    pub fn new_with_remote_state(network_url: CreditChainBaseUrl, txn_id: u64) -> Self {
    ...
    /// Gets the account where the CreditChain framework is installed (0x1).
    pub fn creditchain_framework_account(&mut self) -> Account {
    ...
    pub fn create_publish_built_package(
        ...
        self.create_transaction_payload(
            account,
            creditchain_stdlib::code_publish_package_txn(
    ...
    pub fn create_object_code_deployment_built_package(
        ...
        self.create_transaction_payload(
            account,
            creditchain_stdlib::object_code_deployment_publish(
                bcs::to_bytes(&metadata).expect("PackageMetadata has BCS"),
                code,
            ),
        )
    ...
    pub fn create_object_code_upgrade_built_package(
        ...
        self.create_transaction_payload(
            account,
            creditchain_stdlib::object_code_deployment_upgrade(
                bcs::to_bytes(&metadata).expect("PackageMetadata has BCS"),
                code,
                code_object,
            ),
    ...
    pub fn object_code_freeze_code_object(
        ...
        let txn = self.create_transaction_payload(
            account,
            creditchain_stdlib::object_code_deployment_freeze_code_object(code_object),
        );
        self.run(txn)
    }
    ...
    pub fn read_creditchain_balance(&self, addr: &AccountAddress) -> u64 {
        self.read_resource::<CoinStoreResource<CreditChainCoinType>>(
            addr,
            CoinStoreResource::<CreditChainCoinType>::struct_tag(),
        )
        .map(|c| c.coin())
        .unwrap_or(0)
            + self
                .read_resource_from_resource_group::<FungibleStoreResource>(
                    &creditchain_types::account_config::fungible_store::primary_apt_store(*addr),
    ...
    pub fn enable_features(&mut self, enabled: Vec<FeatureFlag>, disabled: Vec<FeatureFlag>) {
        let acc = self.creditchain_framework_account();
    ...
    fn override_one_gas_param(&mut self, param: &str, param_value: u64) {
        // TODO: The CreditChainGasParameters::zeros() schedule doesn't do what we want, so
        ...
        let entries = CreditChainGasParameters::initial()
        ...
        self.executor
            .exec("creditchain_governance", "force_end_epoch", vec![], vec![
                core_signer_arg,
            ]);
    }
    ...
    pub fn modify_gas_schedule(&mut self, modify: impl FnOnce(&mut CreditChainGasParameters)) {
    ...
    pub fn get_gas_params(&self) -> (u64, CreditChainGasParameters) {
        let gas_schedule: GasScheduleV2 = self.get_gas_schedule();
        let feature_version = gas_schedule.feature_version;
        let params = CreditChainGasParameters::from_on_chain_gas_schedule(
            &gas_schedule.into_btree_map(),
            feature_version,
        )
        .unwrap();
...
macro_rules! enable_golden {
    ($h:expr) => {
        $h.internal_set_golden(
            std::file!(),
            creditchain_language_e2e_tests::current_function_name!(),
        )
    };
}
...
macro_rules! assert_success {
    ($s:expr $(,)?) => {{
        assert_eq!($s, creditchain_types::transaction::TransactionStatus::Keep(
            creditchain_types::transaction::ExecutionStatus::Success))
    }};
    ($s:expr, $($arg:tt)+) => {{
        assert_eq!(
            $s,
            creditchain_types::transaction::TransactionStatus::Keep(
                creditchain_types::transaction::ExecutionStatus::Success),
            $($arg)+
        )
    }};
...
macro_rules! assert_out_of_gas {
    ($s:expr $(,)?) => {{
        assert_eq!($s, creditchain_types::transaction::TransactionStatus::Keep(
            creditchain_types::transaction::ExecutionStatus::OutOfGas))
    }};
    ($s:expr, $($arg:tt)+) => {{
        assert_eq!(
            $s,
...
macro_rules! assert_abort {
    ...
    ($s:expr, $c:ident $(,)?) => {{
        assert!(matches!(
            $s,
            creditchain_types::transaction::TransactionStatus::Keep(
                creditchain_types::transaction::ExecutionStatus::MoveAbort { code, .. }
            )
            if code == $c,
        ));
    }};
    ($s:expr, $c:pat $(,)?) => {{
        assert!(matches!(
            $s,
            creditchain_types::transaction::TransactionStatus::Keep(
                creditchain_types::transaction::ExecutionStatus::MoveAbort { code: $c, .. }
            ),
        ));
    }};
...
#[cfg(test)]
mod tests {
    use creditchain_types::transaction::{ExecutionStatus, TransactionStatus};
