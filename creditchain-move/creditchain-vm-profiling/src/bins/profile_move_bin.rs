...
use creditchain_gas_schedule::{
    MiscGasParameters, NativeGasParameters, LATEST_GAS_FEATURE_VERSION,
};
use creditchain_move_stdlib::natives::all_natives;
use creditchain_native_interface::SafeNativeBuilder;
use creditchain_table_natives::NativeTableContext;
use creditchain_types::on_chain_config::{Features, TimedFeaturesBuilder};
...
fn main() -> Result<()> {
    ...
    natives.extend(creditchain_table_natives::table_natives(
        stdlib_addr,
        &mut builder,
    ));
