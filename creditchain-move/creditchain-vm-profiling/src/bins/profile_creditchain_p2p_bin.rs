...
use creditchain_transaction_simulation::{AccountData, InMemoryStateStore, SimulationStateStore};
...
fn main() -> Result<()> {
        ...
        .map(|seq_num| {
            Transaction::UserTransaction(
                alice
                    ...
                    .transaction()
                    .gas_unit_price(100)
                    .payload(creditchain_cached_packages::creditchain_stdlib::creditchain_coin_transfer(
                        *bob.account().address(),
                        1000,
                    ))
    ...
    let outputs =
        CreditChainVMBlockExecutor::new().execute_block_no_limit(&txn_provider, &state_store)?;
    for i in 0..NUM_TXNS {
