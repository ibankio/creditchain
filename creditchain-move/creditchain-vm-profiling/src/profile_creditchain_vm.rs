...
const RUN_CREDITCHAIN_P2P: &str = "run-creditchain-p2p";

static PATH_BIN_RUN_CREDITCHAIN_P2P: Lazy<PathBuf> = Lazy::new(|| {
    PATH_CRATE_ROOT
        .parent()
        .unwrap()
        .parent()
        ...
        .join("target")
        .join(BUILD_PROFILE)
        .join(RUN_CREDITCHAIN_P2P)
});

fn run_creditchain_p2p() -> Result<()> {
    println!("Profiling CreditChain VM...");
...
fn build_binaries() -> Result<()> {
    let status = Command::new("cargo")
        ...
        .arg(BUILD_PROFILE)
        .arg("-p")
        .arg("creditchain-vm-profiling")
        .arg("--bin")
        .arg(RUN_CREDITCHAIN_P2P)
        .status()?;

    if !status.success() {
        bail!("Failed to compile {}", RUN_CREDITCHAIN_P2P);
    }
