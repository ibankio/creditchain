...
use creditchain_api_types::AsConverter;
...
use creditchain_gas_schedule::{InitialGasSchedule, TransactionGasParameters};
...
use creditchain_vm_environment::{environment::CreditChainEnvironment, prod_configs::set_paranoid_type_checks};
use creditchain_vm_genesis::GENESIS_KEYPAIR;
...
/// The CreditChain transaction test adapter.
///
...
///   - It executes transactions through CreditChainVM, instead of MoveVM directly
struct CreditChainTestAdapter<'a> {
...
/// CreditChain-specific arguments for the publish command.
#[derive(Parser, Debug)]
struct CreditChainPublishArgs {
...
/// CreditChain-specifc arguments for the run command.
#[derive(Parser, Debug)]
struct CreditChainRunArgs {
    #[clap(long = "private-key", value_parser = RawPrivateKey::parse)]
    private_key: Option<RawPrivateKey>,
...
/// CreditChain-specifc arguments for the init command.
#[derive(Parser, Debug)]
struct CreditChainInitArgs {
...
impl creditchain_storage_interface::DbReader for FakeDbReader {
...
/// Default private key mappings for special CreditChain accounts.
fn creditchain_framework_private_key_mapping() -> Vec<(String, Ed25519PrivateKey)> {
...
fn compile_framework_sources(
    ...
    let named_address_mapping_strings: Vec<String> = creditchain_framework::named_addresses()
        .iter()
    ...
    let options = move_compiler_v2::Options {
        ...
        known_attributes: creditchain_framework::extended_checks::get_all_attribute_names().clone(),
        language_version,
...
// creditchain-experimental can be using latest language features, while others cannot.
// So we need to compile it twice, once without creditchain-experimental, and once with.

static PRECOMPILED_CREDITCHAIN_FRAMEWORK_V2: Lazy<PrecompiledFilesModules> = Lazy::new(|| {
    compile_framework_sources(
        creditchain_cached_packages::head_release_bundle()
            .files()
            .unwrap()
            .iter()
            .filter(|f| !f.contains("creditchain-move/framework/creditchain-experimental/sources"))
            .cloned()
            .collect::<Vec<_>>(),
        None,
    )
...
static PRECOMPILED_CREDITCHAIN_FRAMEWORK_V2_WITH_EXPERIMENTAL: Lazy<PrecompiledFilesModules> =
    Lazy::new(|| {
        let named_address_mapping_strings: Vec<String> = creditchain_framework::named_addresses()
            .iter()
            .map(|(string, num_addr)| format!("{}={}", string, num_addr))
        ...
        // creditchain-experimental can be using latest language features, while others cannot.
        // So we need to compile it twice, once without creditchain-experimental, and once with.
        // (in the second pass, we need to provide both, in case creditchain-experimental depends on other modules)

        let all_sources = creditchain_cached_packages::head_release_bundle()
            .files()
            .unwrap();
...
impl CreditChainTestAdapter<'_> {
    ...
    fn fetch_account_resource(&self, signer_addr: &AccountAddress) -> AccountResource {
        self.storage
            ...
            .unwrap_or(AccountResource::new(
                0,
                signer_addr.to_vec(),
                creditchain_types::event::EventHandle::new(
                    creditchain_types::event::EventKey::new(0, *signer_addr),
                    0,
                ),
                creditchain_types::event::EventHandle::new(
                    creditchain_types::event::EventKey::new(1, *signer_addr),
                    0,
                ),
            ))
    }

    /// Obtain the CreditChainCoin amount under address `signer_addr`
    ...
    fn run_transaction(&mut self, txn: Transaction) -> Result<TransactionOutput> {
        ...
        let mut outputs = CreditChainVMBlockExecutor::new()
    ...
    fn create_and_fund_account(&mut self, account_addr: AccountAddress, amount: u64) {
        let parameters = self
            .fetch_transaction_parameters(&creditchain_test_root_address(), None, None, None, None)
            .unwrap();

        let txn = RawTransaction::new(
            creditchain_test_root_address(),
            parameters.sequence_number,
            creditchain_cached_packages::creditchain_stdlib::creditchain_account_create_account(account_addr),
        ...
        let txn = RawTransaction::new(
            ...
            creditchain_cached_packages::creditchain_stdlib::creditchain_coin_mint(account_addr, amount),
            parameters.max_gas_amount,
            parameters.gas_unit_price,
            parameters.expiration_timestamp_secs,
            ChainId::test(),
...
impl<'a> MoveTestAdapter<'a> for CreditChainTestAdapter<'a> {
    type ExtraInitArgs = CreditChainInitArgs;
    type ExtraPublishArgs = CreditChainPublishArgs;
    type ExtraRunArgs = CreditChainRunArgs;
    type ExtraValueArgs = ();
    type Subcommand = CreditChainSubCommand;
    ...
    fn known_attributes(&self) -> &BTreeSet<String> {
        creditchain_framework::extended_checks::get_all_attribute_names()
    }
    ...
    fn init(
        ...
        let mut named_address_mapping = creditchain_framework::named_addresses().clone();
        ...
        for (name, private_key) in creditchain_framework_private_key_mapping() {
        ...
        let warnings_opt = match (syntax, opt_model) {
            ...
            (_, Some(model)) => {
                let _runtime_metadata =
                    creditchain_framework::extended_checks::run_extended_checks(&model);
    ...
    fn view_data(
        ...
        match self.storage.get_state_value_bytes(&state_key).unwrap() {
            ...
            Some(data) => {
                let annotated =
                    CreditChainValueAnnotator::new(&self.storage).view_resource(&struct_tag, &data)?;
                Ok(format!("{}", annotated))
            },
        }
    }
    ...
    fn handle_subcommand(&mut self, input: TaskInput<Self::Subcommand>) -> Result<Option<String>> {
        match input.command {
            CreditChainSubCommand::BlockCommand(block_cmd) => {
            ...
            CreditChainSubCommand::ViewTableCommand(view_table_cmd) => {
    ...
    fn deserialize(&self, bytes: &[u8], layout: &MoveTypeLayout) -> Option<Value> {
        let environment = CreditChainEnvironment::new(&self.storage);
        let code_storage = self.storage.as_creditchain_code_storage(&environment);
...
fn precompiled_v2_framework() -> &'static PrecompiledFilesModules {
    &PRECOMPILED_CREDITCHAIN_FRAMEWORK_V2
}

fn precompiled_v2_framework_with_experimental() -> &'static PrecompiledFilesModules {
    &PRECOMPILED_CREDITCHAIN_FRAMEWORK_V2_WITH_EXPERIMENTAL
}

pub fn run_creditchain_test(path: &Path) -> Result<(), Box<dyn std::error::Error>> {
    run_creditchain_test_with_config(
        path,
...
pub fn run_creditchain_test_with_config(
    path: &Path,
    ...
    set_paranoid_type_checks(true);
    run_test_impl::<CreditChainTestAdapter>(config, path, v2_lib, &suffix)
}
