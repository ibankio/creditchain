...
//! Test infrastructure for modeling CreditChain accounts.
//! Ported from the old `e2e-tests` crate.

use creditchain_crypto::ed25519::*;
 ...
 *
 **************************************************************************************************/
/// Details about a CreditChain account.
///
...
impl Account {
    ...
    /// not automatically get added to the CreditChain store. To add an account to the store, use
    ...
    ///
    /// The address will be [`creditchain_test_root_address`][account_config::creditchain_test_root_address], and
    /// the account will use [`GENESIS_KEYPAIR`][static@GENESIS_KEYPAIR] as its keypair.
    pub fn new_creditchain_root() -> Self {
        Self::new_genesis_account(account_config::creditchain_test_root_address())
    }
...
impl CoinStore {
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        let coin_store = CoinStoreResource::<CreditChainCoinType>::new(
            self.coin,
            self.frozen,
...
impl AccountData {
    ...
    pub fn to_writeset(&self) -> WriteSet {
        ...
        if let Some(coin_store) = &self.coin_store {
            write_set.push((
                StateKey::resource_typed::<CoinStoreResource<CreditChainCoinType>>(self.address())
                    .unwrap(),
