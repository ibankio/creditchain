...
use creditchain_gas_algebra::{FeePerGasUnit, Gas, GasExpression};
use creditchain_gas_schedule::{CreditChainGasParameters, InitialGasSchedule, LATEST_GAS_FEATURE_VERSION};
...
impl AUTransactionGen for InsufficientBalanceGen {
    fn apply(
        ...
        let gas_params = CreditChainGasParameters::initial();
...
pub struct InvalidAuthkeyGen {
    sender: Index,
    #[proptest(
        strategy = "creditchain_crypto::test_utils::uniform_keypair_strategy_with_perturbation(1)"
    )]
