...
use creditchain_abstract_gas_usage::CalibrationAlgebra;
use creditchain_bitvec::BitVec;
use creditchain_block_executor::{
    code_cache_global_manager::CreditChainModuleCacheManager, txn_commit_hook::NoOpTransactionCommitHook,
    txn_provider::default::DefaultTxnProvider,
};
use creditchain_crypto::HashValue;
use creditchain_framework::ReleaseBundle;
use creditchain_gas_algebra::DynamicExpression;
use creditchain_gas_meter::{CreditChainGasMeter, GasAlgebra, StandardGasAlgebra, StandardGasMeter};
use creditchain_gas_profiling::{GasProfiler, TransactionGasLog};
use creditchain_keygen::KeyGen;
use creditchain_rest_client::CreditChainBaseUrl;
...
use creditchain_types::{
    ...
    move_utils::MemberId,
    on_chain_config::{
        CreditChainVersion, CurrentTimeMicroseconds, FeatureFlag, Features, OnChainConfig, ValidatorSet,
    },
...
use creditchain_validator_interface::{DebuggerStateView, RestDebuggerInterface};
use creditchain_vm::{
    block_executor::{CreditChainTransactionOutput, CreditChainVMBlockExecutorWrapper},
    data_cache::AsMoveResolver,
    gas::make_prod_gas_meter,
    move_vm_ext::{CreditChainMoveResolver, MoveVmExt, SessionExt, SessionId},
    CreditChainVM, VMValidator,
};
use creditchain_vm_environment::environment::CreditChainEnvironment;
use creditchain_vm_genesis::{generate_genesis_change_set_for_testing_with_count, GenesisOptions};
use creditchain_vm_logging::log_schema::AdapterLogSchema;
use creditchain_vm_types::{
    module_and_script_storage::{module_storage::CreditChainModuleStorage, AsCreditChainCodeStorage},
...
///
/// This struct is a mock in-memory implementation of the CreditChain executor.
...
impl FakeExecutor {
    ...
    fn from_remote_state_impl(
        network_url: CreditChainBaseUrl,
        txn_id: u64,
        api_key: Option<&str>,
    ) -> Self {
        let mut builder = creditchain_rest_client::Client::builder(network_url);
    ...
    ///
    /// If you hit rate limits, you can create a free CreditChain Build account and generate an API key:
    /// - https://build.creditchain.io/docs/start#api-quick-start
    ...
    /// Creates a [`FakeExecutor`] from a remote network state at the version specified by the
    /// transaction id.
    pub fn from_remote_state(network_url: CreditChainBaseUrl, txn_id: u64) -> Self {
    ...
    fn set_tracing(&mut self, test_name: &str, file_name: String) {
        ...
        if let Some(env_trace_dir) = env::var_os(ENV_TRACE_DIR) {
            let creditchain_version =
                CreditChainVersion::fetch_config(&self.state_store).map_or(0, |v| v.major);
            ...
            let mut name_file = OpenOptions::new()
                ...
                .unwrap();
            write!(name_file, "{}::{}", test_name, creditchain_version).unwrap();
            for sub_dir in &[
                TRACE_DIR_META,
                TRACE_DIR_DATA,
                TRACE_DIR_INPUT,
    ...
    pub fn stdlib_only_genesis() -> Self {
        ...
        for (bytes, module) in
            creditchain_cached_packages::head_release_bundle().code_and_compiled_modules()
        {
    ...
    pub fn custom_genesis(framework: &ReleaseBundle, validator_accounts: Option<usize>) -> Self {
        let genesis = creditchain_vm_genesis::generate_test_genesis(framework, validator_accounts);
    ...
    pub fn new_account_data_at(&mut self, addr: AccountAddress) -> AccountData {
        ...
        // Mint the account 10M CreditChain coins (with 8 decimals).
    ...
    pub fn read_apt_coin_info_resource(&self) -> Option<CoinInfoResource<CreditChainCoinType>> {
        self.read_resource(&CreditChainCoinType::coin_info_address())
    }
    ...
    fn execute_transaction_block_impl_with_state_view(
        ...
        CreditChainVMBlockExecutorWrapper::execute_block_on_thread_pool::<
            _,
        ...
        >(
            ...
            &CreditChainModuleCacheManager::new(),
            config,
    ...
    pub fn execute_transaction_with_gas_profiler(
        ...
        let env = CreditChainEnvironment::new(&self.state_store);
        let vm = CreditChainVM::new(&env, self.get_state_view());

        let resolver = self.state_store.as_move_resolver();
        let code_storage = self.get_state_view().as_creditchain_code_storage(&env);
    ...
    pub fn validate_transaction(&self, txn: SignedTransaction) -> VMValidatorResult {
        ...
        vm.validate_transaction(
            txn,
            &self.state_store,
            &self.state_store.as_creditchain_code_storage(&env),
        )
    }
    ...
    pub fn exec_func_record_running_time(
        ...
        let module_storage = self.state_store.as_creditchain_code_storage(&env);
    ...
    pub fn exec_abstract_usage(
        ...
        let (write_set, _events) = {
            let env = CreditChainEnvironment::new_with_gas_hook(
                &self.state_store,
    ...
    pub fn execute_view_function(
        ...
        CreditChainVM::execute_view_function(
            self.get_state_view(),
            fun.module_id,
            fun.member_id,
            type_args,
...
/// responsibility of the adapter, i.e., [CreditChainVM]).
