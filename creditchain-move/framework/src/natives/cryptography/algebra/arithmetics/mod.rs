...
macro_rules! ark_binary_op_internal {
    ($context:expr, $args:ident, $ark_typ:ty, $ark_func:ident, $gas:expr) => {{
        let handle_2 = creditchain_native_interface::safely_pop_arg!($args, u64) as usize;
        let handle_1 = creditchain_native_interface::safely_pop_arg!($args, u64) as usize;
...
macro_rules! ark_unary_op_internal {
    ($context:expr, $args:ident, $ark_typ:ty, $ark_func:ident, $gas:expr) => {{
        let handle = creditchain_native_interface::safely_pop_arg!($args, u64) as usize;
