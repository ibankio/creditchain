...
module creditchain_experimental::veiled_coin_tests {
    #[test_only]
    use std::features;
    #[test_only]
    use std::signer;
    ...
    #[test_only]
    use creditchain_std::ristretto255_bulletproofs as bulletproofs;
    #[test_only]
    use creditchain_std::debug::print;
    #[test_only]
    use creditchain_std::ristretto255_elgamal as elgamal;
    #[test_only]
    use creditchain_std::ristretto255;
    #[test_only]
    use creditchain_std::ristretto255_pedersen as pedersen;
    ...
    #[test_only]
    use creditchain_experimental::veiled_coin;
    #[test_only]
    use creditchain_experimental::helpers::generate_elgamal_keypair;
    #[test_only]
    use creditchain_experimental::sigma_protos::{serialize_withdrawal_subproof, prove_withdrawal};
    #[test_only]
    use creditchain_experimental::sigma_protos;
    ...
    fun set_up_for_veiled_coin_test(
        ...
        assert!(signer::address_of(&creditchain_fx) != signer::address_of(sender), 1);
        assert!(signer::address_of(&creditchain_fx) != signer::address_of(recipient), 2);
        ...
        account::create_account_for_test(signer::address_of(&creditchain_fx)); // needed in `coin::create_fake_money`
        ...
        // Create `amount` of `FakeCoin` coins at the CreditChain 0x1 address (must do) and register a `FakeCoin` coin
        ...
        println(b"Created fake money inside @creditchain_framework");
    ...
    #[
        test(
            veiled_coin = @creditchain_experimental,
            creditchain_fx = @creditchain_framework,
            sender = @0xc0ffee,
            recipient = @0x1337
        )
    ]
    fun veil_test(
        ...
        print(&@creditchain_experimental);
        println(b"@creditchain_framework:");
        print(&@creditchain_framework);
    ...
    #[test(
        veiled_coin = @creditchain_experimental, creditchain_fx = @creditchain_framework, sender = @0x1337
    )]
    fun unveil_test(
        veiled_coin: signer, creditchain_fx: signer, sender: signer
    ) {
        ...
        set_up_for_veiled_coin_test(&veiled_coin, creditchain_fx, &sender, &sender, 500, 0);
