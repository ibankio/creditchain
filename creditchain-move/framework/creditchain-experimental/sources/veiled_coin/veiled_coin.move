...
///
/// This module allows users to "register" a veiled account for any pre-existing `creditchain_framework::Coin` type `T` via
...
/// CreditChain framework. This is because coin amounts must be encrypted with an *efficient*, additively-homomorphic encryption
...
/// ```
///
/// Recall that: A coin has a *decimal precision* $d$ (e.g., for `CreditChainCoin`, $d = 8$; see `initialize` in
/// `creditchain_coin.move`). This precision $d$ is used when displaying a `u64` amount, by dividing the amount by $10^d$.
...
module creditchain_experimental::veiled_coin {
    use std::error;
    ...
    use creditchain_std::ristretto255_bulletproofs::RangeProof;
    ...
    use creditchain_experimental::helpers;
    ...
    const VEILED_COIN_BULLETPROOFS_DST: vector<u8> = b"CreditChainVeiledCoin/BulletproofRangeProof";
    ...
    /// represent normal `creditchain_framework::coin::Coin` values.
    ...
    fun get_resource_account_signer(): signer acquires VeiledCoinMinter {
        account::create_signer_with_capability(
            &borrow_global<VeiledCoinMinter>(@creditchain_experimental).signer_cap
        )
    }
