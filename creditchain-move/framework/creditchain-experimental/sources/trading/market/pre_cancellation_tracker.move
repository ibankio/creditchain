...
module creditchain_experimental::pre_cancellation_tracker {
    use std::signer;
    use creditchain_std::big_ordered_map;
    use creditchain_std::big_ordered_map::BigOrderedMap;
    ...
    public(package) fun pre_cancel_order_for_tracker(
        ...
        let current_time = creditchain_std::timestamp::now_microseconds();
    ...
    public(package) fun is_pre_cancelled(
        ...
        if (expiration_time_option.is_some()) {
            let current_time = creditchain_std::timestamp::now_seconds();
    ...
    #[test(account = @0x456, creditchain_framework = @0x1)]
    public fun test_order_id_tracking_flow(
        account: &signer, creditchain_framework: &signer
    ) {
