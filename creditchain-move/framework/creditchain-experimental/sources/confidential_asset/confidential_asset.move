...
module creditchain_experimental::confidential_asset {
    use std::bcs;
    use std::error;
    ...
    use creditchain_std::ristretto255_bulletproofs::Self as bulletproofs;
    ...
    use creditchain_framework::dispatchable_fungible_asset;
    use creditchain_framework::fungible_asset::{Metadata};
    use creditchain_framework::object::{Self, ExtendRef, Object};
    use creditchain_framework::primary_fungible_store;
    use creditchain_framework::system_addresses;

    use creditchain_experimental::confidential_balance;
    use creditchain_experimental::confidential_proof::{
        Self,
        NormalizationProof,
        RotationProof,
        TransferProof,
    ...
    use creditchain_experimental::ristretto255_twisted_elgamal as twisted_elgamal;

    #[test_only]
    use creditchain_std::ristretto255::Scalar;
    ...
    public fun enable_allow_list(creditchain_framework: &signer) acquires FAController {
        system_addresses::assert_creditchain_framework(creditchain_framework);

        let fa_controller = borrow_global_mut<FAController>(@creditchain_experimental);
    ...
    public fun disable_allow_list(creditchain_framework: &signer) acquires FAController {
        system_addresses::assert_creditchain_framework(creditchain_framework);
    ...
    public fun enable_token(
        creditchain_framework: &signer, token: Object<Metadata>
    ...
    public fun set_auditor(
        creditchain_framework: &signer, token: Object<Metadata>, new_auditor_ek: vector<u8>
    ) acquires FAConfig, FAController {
        system_addresses::assert_creditchain_framework(creditchain_framework);
    ...
    public fun confidential_asset_controller_exists(): bool {
        exists<FAController>(@creditchain_experimental)
    }
    ...
    public fun is_allow_list_enabled(): bool acquires FAController {
        ...
        borrow_global<FAController>(@creditchain_experimental).allow_list_enabled
    }
    ...
    fun get_fa_store_signer(): signer acquires FAController {
        object::generate_signer_for_extending(
            &borrow_global<FAController>(@creditchain_experimental).extend_ref
        )
    }
    ...
    fun get_fa_config_signer(token: Object<Metadata>): signer acquires FAController {
        let fa_ext = &borrow_global<FAController>(@creditchain_experimental).extend_ref;
    ...
    entry fun init_module_for_genesis(deployer: &signer) {
        assert!(
            signer::address_of(deployer) == @creditchain_experimental,
