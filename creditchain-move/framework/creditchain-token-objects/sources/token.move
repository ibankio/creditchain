...
///
module creditchain_token_objects::token {
    use std::error;
    use std::features;
    use std::option::{Self, Option};
    use std::string::{Self, String};
    use std::signer;
    use creditchain_framework::aggregator_v2::{Self, AggregatorSnapshot, DerivedStringSnapshot};
    use creditchain_framework::event;
    use creditchain_framework::object::{Self, ConstructorRef, Object};
    use creditchain_token_objects::collection::{Self, Collection};
    use creditchain_token_objects::royalty::{Self, Royalty};

    #[test_only]
    use creditchain_framework::object::ExtendRef;
    ...
    #[test(creator = @0x123, trader = @0x456, creditchain_framework = @creditchain_framework)]
    fun test_create_and_transfer_token_as_collection_owner(creator: &signer, trader: &signer, creditchain_framework: &signer) acquires Token {
        features::change_feature_flags_for_testing(creditchain_framework, vector[features::get_collection_owner_feature()], vector[]);
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123, trader = @0x456)]
    #[expected_failure(abort_code = 0x40002, location = creditchain_token_objects::token)]
    ...
    #[test(creator = @0x123, trader = @0x456, creditchain_framework = @creditchain_framework)]
    #[expected_failure(abort_code = 0x40008, location = creditchain_token_objects::token)]
    fun test_create_token_non_collection_owner(creator: &signer, trader: &signer, creditchain_framework: &signer) {
        features::change_feature_flags_for_testing(creditchain_framework, vector[features::get_collection_owner_feature()], vector[]);
    ...
    #[test(creator = @0x123, trader = @0x456, creditchain_framework = @creditchain_framework)]
    #[expected_failure(abort_code = 0x40008, location = creditchain_token_objects::token)]
    fun test_create_named_token_non_collection_owner(creator: &signer, trader: &signer, creditchain_framework: &signer) {
        features::change_feature_flags_for_testing(creditchain_framework, vector[features::get_collection_owner_feature()], vector[]);
        let constructor_ref = &create_fixed_collection_as_collection_owner(creator, string::utf8(b"collection name"), 5);
        let collection = get_collection_from_ref(&object::generate_extend_ref(constructor_ref));
    ...
    #[test(creator = @0x123, trader = @0x456, creditchain_framework = @creditchain_framework)]
    #[expected_failure(abort_code = 0x40008, location = creditchain_token_objects::token)]
    fun test_create_named_token_from_seed_non_collection_owner(creator: &signer, trader: &signer, creditchain_framework: &signer) {
        features::change_feature_flags_for_testing(creditchain_framework, vector[features::get_collection_owner_feature()], vector[]);
        let constructor_ref = &create_fixed_collection_as_collection_owner(creator, string::utf8(b"collection name"), 5);
        let collection = get_collection_from_ref(&object::generate_extend_ref(constructor_ref));
    ...
    #[test(creator = @0x123, trader = @0x456, creditchain_framework = @creditchain_framework)]
    #[expected_failure(abort_code = 0x40008, location = creditchain_token_objects::token)]
    fun test_create_token_after_transferring_collection(creator: &signer, trader: &signer, creditchain_framework: &signer) {
        features::change_feature_flags_for_testing(creditchain_framework, vector[features::get_collection_owner_feature()], vector[]);
        let constructor_ref = &create_fixed_collection_as_collection_owner(creator, string::utf8(b"collection name"), 5);
        let collection = get_collection_from_ref(&object::generate_extend_ref(constructor_ref));
        create_token_as_collection_owner(
    ...
    #[test(creator = @0x123, trader = @0x456, creditchain_framework = @creditchain_framework)]
    fun create_token_works_with_new_collection_owner(creator: &signer, trader: &signer, creditchain_framework: &signer) {
        features::change_feature_flags_for_testing(creditchain_framework, vector[features::get_collection_owner_feature()], vector[]);
        let constructor_ref = &create_fixed_collection_as_collection_owner(creator, string::utf8(b"collection name"), 5);
        let collection = get_collection_from_ref(&object::generate_extend_ref(constructor_ref));
        create_token_as_collection_owner(
    ...
    #[test(creator = @0x123)]
    #[expected_failure(abort_code = 0x20002, location = creditchain_token_objects::collection)]
    ...
    #[test(creator = @0x123)]
    #[expected_failure(abort_code = 0x80001, location = creditchain_framework::object)]
