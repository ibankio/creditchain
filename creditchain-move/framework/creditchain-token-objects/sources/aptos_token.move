...
module creditchain_token_objects::creditchain_token {
    use std::error;
    use std::option::{Self, Option};
    use std::string::String;
    use std::signer;
    use creditchain_framework::object::{Self, ConstructorRef, Object};
    use creditchain_token_objects::collection;
    use creditchain_token_objects::property_map;
    use creditchain_token_objects::royalty;
    ...
    struct CreditChainCollection has key {
    ...
    struct CreditChainToken has key {
        /// Used to burn.
    ...
    public fun create_collection_object(
        ...
        let creditchain_collection = CreditChainCollection {
            mutator_ref,
            royalty_mutator_ref,
            mutable_description,
            mutable_uri,
        ...
        move_to(&object_signer, creditchain_collection);
    ...
    ) acquires CreditChainCollection, CreditChainToken {
    ...
    public fun mint_token_object(
        ...
        property_types: vector<String>,
        property_values: vector<vector<u8>>,
    ): Object<CreditChainToken> acquires CreditChainCollection, CreditChainToken {
        ...
        if (freezable_by_creator) {
            let creditchain_token_addr = object::address_from_constructor_ref(&constructor_ref);
            let creditchain_token = &mut CreditChainToken[creditchain_token_addr];
            let transfer_ref = object::generate_transfer_ref(&constructor_ref);
            creditchain_token.transfer_ref.fill(transfer_ref);
        };
    ...
    ) acquires CreditChainCollection {
    ...
    public fun mint_soul_bound_token_object(
        ...
        property_values: vector<vector<u8>>,
        soul_bound_to: address,
    ): Object<CreditChainToken> acquires CreditChainCollection {
        let constructor_ref = mint_internal(
            creator,
            collection,
            description,
    ...
    fun mint_internal(
        ...
        property_types: vector<String>,
        property_values: vector<vector<u8>>,
    ): ConstructorRef acquires CreditChainCollection {
        ...
        let creditchain_token = CreditChainToken {
            burn_ref,
            transfer_ref: option::none(),
            mutator_ref,
            property_mutator_ref: property_map::generate_mutator_ref(&constructor_ref),
        };
        move_to(&object_signer, creditchain_token);
    ...
    inline fun borrow<T: key>(token: &Object<T>): &CreditChainToken {
        let token_address = object::object_address(token);
        assert!(
            exists<CreditChainToken>(token_address),
            error::not_found(ETOKEN_DOES_NOT_EXIST),
        );
        &CreditChainToken[token_address]
    }

    #[view]
    public fun are_properties_mutable<T: key>(token: Object<T>): bool acquires CreditChainCollection {
        let collection = token::collection_object(token);
        borrow_collection(&collection).mutable_token_properties
    }

    #[view]
    public fun is_burnable<T: key>(token: Object<T>): bool acquires CreditChainToken {
        borrow(&token).burn_ref.is_some()
    }

    #[view]
    public fun is_freezable_by_creator<T: key>(token: Object<T>): bool acquires CreditChainCollection {
        are_collection_tokens_freezable(token::collection_object(token))
    }

    #[view]
    public fun is_mutable_description<T: key>(token: Object<T>): bool acquires CreditChainCollection {
        is_mutable_collection_token_description(token::collection_object(token))
    }

    #[view]
    public fun is_mutable_name<T: key>(token: Object<T>): bool acquires CreditChainCollection {
        is_mutable_collection_token_name(token::collection_object(token))
    }

    #[view]
    public fun is_mutable_uri<T: key>(token: Object<T>): bool acquires CreditChainCollection {
        is_mutable_collection_token_uri(token::collection_object(token))
    }

    // Token mutators

    inline fun authorized_borrow<T: key>(token: &Object<T>, creator: &signer): &CreditChainToken {
        let token_address = object::object_address(token);
        assert!(
            exists<CreditChainToken>(token_address),
            error::not_found(ETOKEN_DOES_NOT_EXIST),
    ...
    public entry fun burn<T: key>(creator: &signer, token: Object<T>) acquires CreditChainToken {
        let creditchain_token = authorized_borrow(&token, creator);
        assert!(
            creditchain_token.burn_ref.is_some(),
            error::permission_denied(ETOKEN_NOT_BURNABLE),
        );
        move creditchain_token;
        let creditchain_token = move_from<CreditChainToken>(object::object_address(&token));
        let CreditChainToken {
            burn_ref,
            transfer_ref: _,
            mutator_ref: _,
    ...
    public entry fun freeze_transfer<T: key>(creator: &signer, token: Object<T>) acquires CreditChainCollection, CreditChainToken {
        let creditchain_token = authorized_borrow(&token, creator);
        assert!(
            are_collection_tokens_freezable(token::collection_object(token))
                && creditchain_token.transfer_ref.is_some(),
            error::permission_denied(EFIELD_NOT_MUTABLE),
        );
        object::disable_ungated_transfer(creditchain_token.transfer_ref.borrow());
    }
    ...
    public entry fun unfreeze_transfer<T: key>(
        ...
        object::enable_ungated_transfer(creditchain_token.transfer_ref.borrow());
    }
    ...
    public entry fun set_description<T: key>(
        ...
        let creditchain_token = authorized_borrow(&token, creator);
        token::set_description(creditchain_token.mutator_ref.borrow(), description);
    }
    ...
    public entry fun set_name<T: key>(
        ...
        let creditchain_token = authorized_borrow(&token, creator);
        token::set_name(creditchain_token.mutator_ref.borrow(), name);
    }
    ...
    public entry fun set_uri<T: key>(
        ...
        let creditchain_token = authorized_borrow(&token, creator);
        token::set_uri(creditchain_token.mutator_ref.borrow(), uri);
    }
    ...
    public entry fun add_property<T: key>(
        ...
        property_map::add(&creditchain_token.property_mutator_ref, key, type, value);
    }
    ...
    public entry fun add_typed_property<T: key, V: drop>(
        ...
        property_map::add_typed(&creditchain_token.property_mutator_ref, key, value);
    }
    ...
    public entry fun remove_property<T: key>(
        ...
        property_map::remove(&creditchain_token.property_mutator_ref, &key);
    }
    ...
    public entry fun update_property<T: key>(
        ...
        property_map::update(&creditchain_token.property_mutator_ref, &key, type, value);
    }
    ...
    public entry fun update_typed_property<T: key, V: drop>(
        ...
        property_map::update_typed(&creditchain_token.property_mutator_ref, &key, value);
    }

    // Collection accessors

    inline fun collection_object(creator: &signer, name: &String): Object<CreditChainCollection> {
        let collection_addr = collection::create_collection_address(&signer::address_of(creator), name);
        object::address_to_object<CreditChainCollection>(collection_addr)
    }

    inline fun borrow_collection<T: key>(token: &Object<T>): &CreditChainCollection {
        let collection_address = object::object_address(token);
        assert!(
            exists<CreditChainCollection>(collection_address),
            error::not_found(ECOLLECTION_DOES_NOT_EXIST),
        );
        &CreditChainCollection[collection_address]
    }

    public fun is_mutable_collection_description<T: key>(
        collection: Object<T>,
    ): bool acquires CreditChainCollection {
    ...
    inline fun authorized_borrow_collection<T: key>(collection: &Object<T>, creator: &signer): &CreditChainCollection {
    ...
    public entry fun set_collection_description<T: key>(
        ...
        let creditchain_collection = authorized_borrow_collection(&collection, creator);
        assert!(
            creditchain_collection.mutable_description,
            error::permission_denied(EFIELD_NOT_MUTABLE),
        );
        collection::set_description(creditchain_collection.mutator_ref.borrow(), description);
    }
    ...
    public fun set_collection_royalties<T: key>(
        ...
        royalty::update(creditchain_collection.royalty_mutator_ref.borrow(), royalty);
    }
    ...
    public entry fun set_collection_uri<T: key>(
        ...
        collection::set_uri(creditchain_collection.mutator_ref.borrow(), uri);
    }
    ...
    #[test(creator = @0x123)]
    fun test_create_and_transfer(creator: &signer) acquires CreditChainCollection, CreditChainToken {
    ...
    fun test_mint_soul_bound(creator: &signer, bob: &signer) acquires CreditChainCollection {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123)]
    #[expected_failure(abort_code = 0x50003, location = object)]
    fun test_frozen_transfer(creator: &signer) acquires CreditChainCollection, CreditChainToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123)]
    fun test_unfrozen_transfer(creator: &signer) acquires CreditChainCollection, CreditChainToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    fun test_noncreator_freeze(creator: &signer, another: &signer) acquires CreditChainCollection, CreditChainToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123, another = @0x456)]
    #[expected_failure(abort_code = 0x50003, location = Self)]
    fun test_noncreator_unfreeze(creator: &signer, another: &signer) acquires CreditChainCollection, CreditChainToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123)]
    fun test_set_description(creator: &signer) acquires CreditChainCollection, CreditChainToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    fun test_set_immutable_description(creator: &signer) acquires CreditChainCollection, CreditChainToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123)]
    fun test_set_name(creator: &signer) acquires CreditChainCollection, CreditChainToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123)]
    #[expected_failure(abort_code = 0x50004, location = Self)]
    fun test_set_immutable_name(creator: &signer) acquires CreditChainCollection, CreditChainToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123)]
    fun test_set_uri(creator: &signer) acquires CreditChainCollection, CreditChainToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123)]
    #[expected_failure(abort_code = 0x50004, location = Self)]
    fun test_set_immutable_uri(creator: &signer) acquires CreditChainCollection, CreditChainToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123)]
    fun test_burnable(creator: &signer) acquires CreditChainCollection, CreditChainToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
        ...
        assert!(exists<CreditChainToken>(token_addr), 0);
        burn(creator, token);
        assert!(!exists<CreditChainToken>(token_addr), 1);
    }

    #[test(creator = @0x123)]
    #[expected_failure(abort_code = 0x50005, location = Self)]
    fun test_not_burnable(creator: &signer) acquires CreditChainCollection, CreditChainToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123)]
    fun test_set_collection_description(creator: &signer) acquires CreditChainCollection {
        let collection_name = string::utf8(b"collection name");
    ...
    #[test(creator = @0x123)]
    #[expected_failure(abort_code = 0x50004, location = Self)]
    fun test_set_immutable_collection_description(creator: &signer) acquires CreditChainCollection {
        let collection_name = string::utf8(b"collection name");
    ...
    #[test(creator = @0x123)]
    fun test_set_collection_uri(creator: &signer) acquires CreditChainCollection {
        let collection_name = string::utf8(b"collection name");
        let collection = create_collection_helper(creator, collection_name, true);
        let value = string::utf8(b"not");
    ...
    #[test(creator = @0x123)]
    #[expected_failure(abort_code = 0x50004, location = Self)]
    fun test_set_immutable_collection_uri(creator: &signer) acquires CreditChainCollection {
        let collection_name = string::utf8(b"collection name");
        let collection = create_collection_helper(creator, collection_name, false);
    ...
    #[test(creator = @0x123)]
    fun test_property_add(creator: &signer) acquires CreditChainCollection, CreditChainToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123)]
    fun test_property_typed_add(creator: &signer) acquires CreditChainCollection, CreditChainToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
        let property_name = string::utf8(b"u8");
        ...
        create_collection_helper(creator, collection_name, true);
        let token = mint_helper(creator, collection_name, token_name);
        add_typed_property<CreditChainToken, u8>(creator, token, property_name, 0x8);

        assert!(property_map::read_u8(&token, &property_name) == 0x8, 0);
    }

    #[test(creator = @0x123)]
    fun test_property_update(creator: &signer) acquires CreditChainCollection, CreditChainToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
    ...
    #[test(creator = @0x123)]
    fun test_property_update_typed(creator: &signer) acquires CreditChainCollection, CreditChainToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
        let property_name = string::utf8(b"bool");
        ...
        create_collection_helper(creator, collection_name, true);
        let token = mint_helper(creator, collection_name, token_name);
        update_typed_property<CreditChainToken, bool>(creator, token, property_name, false);

        assert!(!property_map::read_bool(&token, &property_name), 0);
    }

    #[test(creator = @0x123)]
    fun test_property_remove(creator: &signer) acquires CreditChainCollection, CreditChainToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
        let property_name = string::utf8(b"bool");
    ...
    #[test(creator = @0x123)]
    fun test_royalties(creator: &signer) acquires CreditChainCollection, CreditChainToken {
        let collection_name = string::utf8(b"collection name");
        let token_name = string::utf8(b"token name");
