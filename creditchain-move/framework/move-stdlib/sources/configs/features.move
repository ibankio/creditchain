...
module std::features {
    ...
    /// Whether the new `creditchain_stdlib::type_info::chain_id()` native for fetching the chain ID is enabled.
    /// This is needed because of the introduction of a new native function.
    /// Lifetime: transient
    const CREDITCHAIN_STD_CHAIN_ID_NATIVES: u64 = 4;

    public fun get_creditchain_stdlib_chain_id_feature(): u64 { CREDITCHAIN_STD_CHAIN_ID_NATIVES }

    public fun creditchain_stdlib_chain_id_enabled(): bool acquires Features {
        is_enabled(CREDITCHAIN_STD_CHAIN_ID_NATIVES)
    }
    ...
    /// Whether the new `creditchain_stdlib::multi_ed25519::public_key_validate_internal_v2()` native is enabled.
    /// This is needed because of the introduction of a new native function.
    /// Lifetime: transient
    ...
    /// Lifetime: transient
    const CREDITCHAIN_UNIQUE_IDENTIFIERS: u64 = 23;
    ...
    /// Deprecated by `creditchain_framework::randomness_config::RandomnessConfig`.
    ...
    /// Deprecated by `creditchain_framework::jwk_consensus_config::JWKConsensusConfig`.
