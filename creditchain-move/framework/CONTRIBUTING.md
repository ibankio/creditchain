...
Every state change in the CreditChain blockchain occurs via executing a Move *entry function* or a *script* embedded in a [SignedTransaction](../../types/src/transaction/mod.rs). Entry functions and scripts invoke procedures of Move *modules* that update published *resources*. The Move standard library consists of [modules](modules/) initially published in the genesis transaction.

## Environment Setup

Start by following the general CreditChain setup advice [here](../../CONTRIBUTING.md). Nothing else is strictly required, but you may want to consider a Move syntax highlighter for your editor (asking it to interpret `.move` files as Rust source is a decent start).
...
### Naming
- **Module names**: are camel case e.g., `CreditChainAccount`, `CreditChain`
...
- Generic types should be descriptive, or anti-descriptive where appropriate (e.g. `T` for the Vector generic type parameter, `CreditChainAccount` for the core `CreditChainAccount` resource, `deposit<CoinType>(t: CoinType)` for depositing a token in the `CreditChain` module). Most of the time the "main" type in a module should be the same name as the module e.g., `CreditChain::CreditChain`, `CreditChainAccount::CreditChainAccount`.
- **Module file names**: are the same as the module name e.g., `CreditChainAccount.move`
