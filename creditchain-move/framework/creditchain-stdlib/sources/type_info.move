module creditchain_std::type_info {
    use std::bcs;
    use std::features;
    ...
    /// Returns the current chain ID, mirroring what `creditchain_framework::chain_id::get()` would return, except in `#[test]`
    /// functions, where this will always return `4u8` as the chain ID, whereas `creditchain_framework::chain_id::get()` will
    /// return whichever ID was passed to `creditchain_framework::chain_id::initialize_for_test()`.
    public fun chain_id(): u8 {
        if (!features::creditchain_stdlib_chain_id_enabled()) {
    ...
    fun test_type_of() {
        ...
        assert!(type_info.account_address() == @creditchain_std, 0);
    ...
    fun test_chain_id(fx: signer) {
        ...
        features::change_feature_flags_for_testing(&fx, vector[features::get_creditchain_stdlib_chain_id_feature()], vector[]);
    ...
    fun verify_type_of() {
        ...
        spec {
            assert account_address == @creditchain_std;
