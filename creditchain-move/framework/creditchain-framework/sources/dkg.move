...
module creditchain_framework::dkg {
    use std::error;
    use std::option;
    use std::option::Option;
    use creditchain_framework::event::emit;
    use creditchain_framework::randomness_config::RandomnessConfig;
    ...
    public(friend) fun start(
        ...
        let dkg_state = borrow_global_mut<DKGState>(@creditchain_framework);
    ...
    public fun incomplete_session(): Option<DKGSessionState> acquires DKGState {
        if (exists<DKGState>(@creditchain_framework)) {
            borrow_global<DKGState>(@creditchain_framework).in_progress
        } else {
            option::none()
        }
    }
