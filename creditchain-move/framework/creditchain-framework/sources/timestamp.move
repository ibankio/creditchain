...
module creditchain_framework::timestamp {
    ...
    public(friend) fun set_time_has_started(creditchain_framework: &signer) {
        system_addresses::assert_creditchain_framework(creditchain_framework);
        let timer = CurrentTimeMicroseconds { microseconds: 0 };
        move_to(creditchain_framework, timer);
    }
    ...
    public fun update_global_time(
        ...
        // Can only be invoked by CreditChainVM signer.
        system_addresses::assert_vm(account);

        let global_timer = borrow_global_mut<CurrentTimeMicroseconds>(@creditchain_framework);
    ...
    public fun set_time_has_started_for_testing(account: &signer) {
        if (!exists<CurrentTimeMicroseconds>(@creditchain_framework)) {
    ...
    public fun now_microseconds(): u64 acquires CurrentTimeMicroseconds {
        borrow_global<CurrentTimeMicroseconds>(@creditchain_framework).microseconds
    }
