module creditchain_framework::state_storage {
    ...
    friend creditchain_framework::storage_gas;
    ...
    public(friend) fun initialize(creditchain_framework: &signer) {
        ...
        assert!(
            !exists<StateStorageUsage>(@creditchain_framework),
            error::already_exists(ESTATE_STORAGE_USAGE)
        );
        move_to(creditchain_framework, StateStorageUsage {
            epoch: 0,
            usage: Usage {
                items: 0,
                bytes: 0,
    ...
    public(friend) fun on_new_block(epoch: u64) acquires StateStorageUsage {
        ...
        let usage = borrow_global_mut<StateStorageUsage>(@creditchain_framework);
    ...
    public(friend) fun current_items_and_bytes(): (u64, u64) acquires StateStorageUsage {
        ...
        let usage = borrow_global<StateStorageUsage>(@creditchain_framework);
