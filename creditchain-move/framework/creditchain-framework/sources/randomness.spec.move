spec creditchain_framework::randomness {

    spec module {
        use creditchain_framework::chain_status;
        pragma verify = true;
        invariant [suspendable] chain_status::is_operating() ==> exists<PerBlockRandomness>(@creditchain_framework);
    ...
    spec initialize(framework: &signer) {
        ...
        aborts_if framework_addr != @creditchain_framework;
    }

    spec on_new_block(vm: &signer, epoch: u64, round: u64, seed_for_new_block: Option<vector<u8>>) {
        use std::signer;
        aborts_if signer::address_of(vm) != @vm;
        ensures exists<PerBlockRandomness>(@creditchain_framework) ==> global<PerBlockRandomness>(@creditchain_framework).seed == seed_for_new_block;
        ensures exists<PerBlockRandomness>(@creditchain_framework) ==> global<PerBlockRandomness>(@creditchain_framework).epoch == epoch;
        ensures exists<PerBlockRandomness>(@creditchain_framework) ==> global<PerBlockRandomness>(@creditchain_framework).round == round;
    }
    ...
    spec next_32_bytes(): vector<u8> {
        ...
        let randomness = global<PerBlockRandomness>(@creditchain_framework);
    ...
    spec schema NextBlobAbortsIf {
        ...
        aborts_if !exists<PerBlockRandomness>(@creditchain_framework);
    }
    ...
    spec permutation(n: u64): vector<u64> {
        ...
        // aborts_if n > 1 && !exists<PerBlockRandomness>(@creditchain_framework);
    }
