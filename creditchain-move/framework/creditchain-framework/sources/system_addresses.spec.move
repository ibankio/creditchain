spec creditchain_framework::system_addresses {
    /// <high-level-req>
    /// No.: 1
    ...
    ///
    /// No.: 2
    /// Requirement: Asserting that a provided address corresponds to the CreditChain Framework Resources address should always
    /// yield a true result when matched.
    /// Criticality: High
    /// Implementation: The assert_creditchain_framework function ensures that the provided signer belongs to the
    /// @creditchain_framework account.
    /// Enforcement: Formally verified via [high-level-req-2](AbortsIfNotCreditChainFramework).
    ///
    /// No.: 3
    ...
    spec assert_creditchain_framework(account: &signer) {
        pragma opaque;
    ...
    spec schema AbortsIfNotCreditChainFramework {
        account: signer;
        /// [high-level-req-2]
        aborts_if signer::address_of(account) != @creditchain_framework with error::PERMISSION_DENIED;
    }
