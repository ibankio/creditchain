...
module creditchain_framework::storage_gas {
    ...
    public(friend) fun set_config(creditchain_framework: &signer, config: StorageGasConfig) acquires StorageGasConfig {
        system_addresses::assert_creditchain_framework(creditchain_framework);
        *borrow_global_mut<StorageGasConfig>(@creditchain_framework) = config;
    }
    ...
    public fun initialize(creditchain_framework: &signer) {
        system_addresses::assert_creditchain_framework(creditchain_framework);
        assert!(
            !exists<StorageGasConfig>(@creditchain_framework),
        ...
        move_to(creditchain_framework, StorageGasConfig {
            item_config,
            byte_config,
        });

        assert!(
            !exists<StorageGas>(@creditchain_framework),
            error::already_exists(ESTORAGE_GAS)
        );
        move_to(creditchain_framework, StorageGas {
            per_item_read: 300 * k,
            per_item_create: 5 * m,
    ...
    public(friend) fun on_reconfig() acquires StorageGas, StorageGasConfig {
        ...
        let gas_config = borrow_global<StorageGasConfig>(@creditchain_framework);
        let gas = borrow_global_mut<StorageGas>(@creditchain_framework);
    ...
    fun test_initialize_and_reconfig(framework: signer) acquires StorageGas, StorageGasConfig {
        ...
        on_reconfig();
        let gas_parameter = borrow_global<StorageGas>(@creditchain_framework);
