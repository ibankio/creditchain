module creditchain_framework::nonce_validation {
    use creditchain_std::table::{Self, Table};
    use creditchain_std::timestamp;
    use creditchain_std::big_ordered_map::{Self, BigOrderedMap};
    use creditchain_std::creditchain_hash::sip_hash_from_value;
    use creditchain_std::error;
    ...
    public(friend) fun initialize(creditchain_framework: &signer) {
        initialize_nonce_table(creditchain_framework);
    }

    public entry fun initialize_nonce_table(creditchain_framework: &signer) {
        system_addresses::assert_creditchain_framework(creditchain_framework);
        if (!exists<NonceHistory>(@creditchain_framework)) {
            let table = table::new();
            let nonce_history = NonceHistory {
                nonce_table: table,
                next_key: 0,
            };
            move_to<NonceHistory>(creditchain_framework, nonce_history);
        };
    }
    ...
    public entry fun add_nonce_buckets(count: u64) acquires NonceHistory {
        assert!(exists<NonceHistory>(@creditchain_framework), error::invalid_state(E_NONCE_HISTORY_DOES_NOT_EXIST));
        let nonce_history = &mut NonceHistory[@creditchain_framework];
        for (i in 0..count) {
    ...
    fun check_if_nonce_exists_in_history(
        ...
        let bucket_index = sip_hash_from_value(&nonce_key) % NUM_BUCKETS;
        let nonce_history = &NonceHistory[@creditchain_framework];
        if (nonce_history.nonce_table.contains(bucket_index)) {
