...
module creditchain_framework::reconfiguration {
    use std::error;
    use std::features;
    use std::signer;
    ...
    friend creditchain_framework::consensus_config;
    friend creditchain_framework::execution_config;
    friend creditchain_framework::gas_schedule;
    friend creditchain_framework::genesis;
    friend creditchain_framework::version;
    ...
    public(friend) fun initialize(creditchain_framework: &signer) {
        ...
        assert!(account::get_guid_next_creation_num(signer::address_of(creditchain_framework)) == 2, error::invalid_state(EINVALID_GUID_FOR_EVENT));
        move_to<Configuration>(
            creditchain_framework,
            Configuration {
                epoch: 0,
                last_reconfiguration_time: 0,
                events: account::new_event_handle<NewEpochEvent>(creditchain_framework),
            }
        );
    }
    ...
    fun disable_reconfiguration(creditchain_framework: &signer) {
        system_addresses::assert_creditchain_framework(creditchain_framework);
        assert!(reconfiguration_enabled(), error::invalid_state(ECONFIGURATION));
        move_to(creditchain_framework, DisableReconfiguration {})
    }

    /// Private function to resume reconfiguration.
    /// This function should only be used for offline WriteSet generation purpose and should never be invoked on chain.
    fun enable_reconfiguration(creditchain_framework: &signer) acquires DisableReconfiguration {
        system_addresses::assert_creditchain_framework(creditchain_framework);

        assert!(!reconfiguration_enabled(), error::invalid_state(ECONFIGURATION));
        DisableReconfiguration {} = move_from<DisableReconfiguration>(signer::address_of(creditchain_framework));
    }

    fun reconfiguration_enabled(): bool {
        !exists<DisableReconfiguration>(@creditchain_framework)
    }
    ...
    public(friend) fun reconfigure() acquires Configuration {
        ...
        let config_ref = borrow_global_mut<Configuration>(@creditchain_framework);
    ...
    public fun last_reconfiguration_time(): u64 acquires Configuration {
        borrow_global<Configuration>(@creditchain_framework).last_reconfiguration_time
    }

    public fun current_epoch(): u64 acquires Configuration {
        borrow_global<Configuration>(@creditchain_framework).epoch
    }
