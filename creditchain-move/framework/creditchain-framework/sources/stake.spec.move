spec creditchain_framework::stake {
    /// <high-level-req>
    /// No.: 1
    ...
    /// Criticality: Low
    /// Implementation: The total staked value (CreditChainCoin) of a stake pool is grouped by: active, inactive,
    ...
    spec module {
        ...
        invariant [suspendable] exists<ValidatorSet>(@creditchain_framework) ==> validator_set_is_valid();
        // After genesis, `CreditChainCoinCapabilities`, `ValidatorPerformance` and `ValidatorSet` exist.
        invariant [suspendable] chain_status::is_operating() ==> exists<CreditChainCoinCapabilities>(@creditchain_framework);
        invariant [suspendable] chain_status::is_operating() ==> exists<ValidatorPerformance>(@creditchain_framework);
        invariant [suspendable] chain_status::is_operating() ==> exists<ValidatorSet>(@creditchain_framework);
    ...
    spec schema ValidatorNotChangeDuringReconfig {
        ensures (reconfiguration_state::spec_is_in_progress() && old(exists<ValidatorSet>(@creditchain_framework))) ==>
            old(global<ValidatorSet>(@creditchain_framework)) == global<ValidatorSet>(@creditchain_framework);
    }
    ...
    spec fun validator_set_is_valid(): bool {
        let validator_set = global<ValidatorSet>(@creditchain_framework);
    ...
    spec initialize_validator(
        ...
        let allowed = global<AllowedValidators>(@creditchain_framework);
        aborts_if exists<ValidatorConfig>(addr);
        aborts_if exists<AllowedValidators>(@creditchain_framework) && !vector::spec_contains(allowed.accounts, addr);
    ...
    spec initialize(creditchain_framework: &signer) {
        ...
        let creditchain_addr = signer::address_of(creditchain_framework);
        aborts_if !system_addresses::is_creditchain_framework_address(creditchain_addr);
        aborts_if exists<ValidatorSet>(creditchain_addr);
        aborts_if exists<ValidatorPerformance>(creditchain_addr);
        ensures exists<ValidatorSet>(creditchain_addr);
        ensures global<ValidatorSet>(creditchain_addr).consensus_scheme == 0;
        ensures exists<ValidatorPerformance>(creditchain_addr);
    }
    ...
    )
    {
        ...
        aborts_if !exists<StakingConfig>(@creditchain_framework);
        aborts_if !exists<ValidatorSet>(@creditchain_framework);
        aborts_if reconfiguration_state::spec_is_in_progress();

        let stake_pool = global<StakePool>(pool_address);
        let validator_set = global<ValidatorSet>(@creditchain_framework);
        let post p_validator_set = global<ValidatorSet>(@creditchain_framework);
    ...
    )
    {
        ...
        let bool_find_validator = !option::spec_is_some(spec_find_validator(validator_set.active_validators, pool_address)) &&
                    !option::spec_is_some(spec_find_validator(validator_set.pending_inactive, pool_address)) &&
                        !option::spec_is_some(spec_find_validator(validator_set.pending_active, pool_address));
        aborts_if bool_find_validator && !exists<timestamp::CurrentTimeMicroseconds>(@creditchain_framework);
        ...
        aborts_if !exists<coin::CoinStore<CreditChainCoin>>(addr);
        include coin::DepositAbortsIf<CreditChainCoin>{account_addr: addr};

        let coin_store = global<coin::CoinStore<CreditChainCoin>>(addr);
        let post p_coin_store = global<coin::CoinStore<CreditChainCoin>>(addr);
        ensures bool_find_validator && timestamp::now_seconds() > stake_pool.locked_until_secs
                    && exists<account::Account>(addr) && exists<coin::CoinStore<CreditChainCoin>>(addr) ==>
                        coin_store.coin.value + new_withdraw_amount_1 == p_coin_store.coin.value;
        ensures !(bool_find_validator && exists<timestamp::CurrentTimeMicroseconds>(@creditchain_framework))
                    && exists<account::Account>(addr) && exists<coin::CoinStore<CreditChainCoin>>(addr) ==>
    ...
    spec leave_validator_set(
        ...
        let post post_validator_set = global<ValidatorSet>(@creditchain_framework);
    ...
    spec unlock_with_cap(amount: u64, owner_cap: &OwnerCapability) {
        ...
        let min_amount = creditchain_std::math64::min(amount,pre_stake_pool.active.value);
    ...
    spec increase_lockup_with_cap(owner_cap: &OwnerCapability) {
        let config = global<staking_config::StakingConfig>(@creditchain_framework);
        let pool_address = owner_cap.pool_address;
        let pre_stake_pool = global<StakePool>(pool_address);
        let post stake_pool = global<StakePool>(pool_address);
    ...
    spec reactivate_stake_with_cap(owner_cap: &OwnerCapability, amount: u64) {
        ...
        let post stake_pool = global<StakePool>(pool_address);
        modifies global<StakePool>(pool_address);
        let min_amount = creditchain_std::math64::min(amount, pre_stake_pool.pending_inactive.value);
    ...
    spec on_new_epoch {
        ...
        include creditchain_framework::creditchain_coin::ExistsCreditChainCoin;
    ...
    spec update_performance_statistics {
        ...
        let validator_perf = global<ValidatorPerformance>(@creditchain_framework);
        let post post_validator_perf = global<ValidatorPerformance>(@creditchain_framework);
    ...
    spec schema DistributeRewardsAbortsIf {
        ...
        let addr = type_info::type_of<CreditChainCoin>().account_address;
        aborts_if (rewards_amount > 0) && !exists<coin::CoinInfo<CreditChainCoin>>(addr);
        modifies global<coin::CoinInfo<CreditChainCoin>>(addr);
        include (rewards_amount > 0) ==> coin::CoinAddAbortsIf<CreditChainCoin> { amount: amount };
    }
    ...
    spec schema GetReconfigStartTimeRequirement {
        requires exists<timestamp::CurrentTimeMicroseconds>(@creditchain_framework);
        include reconfiguration_state::StartTimeSecsRequirement;
    }

    spec fun spec_get_reconfig_start_time_secs(): u64 {
        if (exists<reconfiguration_state::State>(@creditchain_framework)) {
    ...
    spec update_voting_power_increase(increase_amount: u64) {
        ...
        let libra2 = @creditchain_framework;
    ...
    spec configure_allowed_validators(creditchain_framework: &signer, accounts: vector<address>) {
        let creditchain_framework_address = signer::address_of(creditchain_framework);
        aborts_if !system_addresses::is_creditchain_framework_address(creditchain_framework_address);
        let post allowed = global<AllowedValidators>(creditchain_framework_address);
    ...
    spec fun spec_validator_index_upper_bound(): u64 {
        len(global<ValidatorPerformance>(@creditchain_framework).validators)
    }
    ...
    spec schema ResourceRequirement {
        requires exists<CreditChainCoinCapabilities>(@creditchain_framework);
        requires exists<ValidatorPerformance>(@creditchain_framework);
        requires exists<ValidatorSet>(@creditchain_framework);
        requires exists<StakingConfig>(@creditchain_framework);
        requires exists<StakingRewardsConfig>(@creditchain_framework) || !features::spec_periodical_reward_rate_decrease_enabled();
        requires exists<timestamp::CurrentTimeMicroseconds>(@creditchain_framework);
    }
    ...
    spec fun spec_get_reward_rate_1(config: StakingConfig): num {
        if (features::spec_periodical_reward_rate_decrease_enabled()) {
            let epoch_rewards_rate = global<staking_config::StakingRewardsConfig>(@creditchain_framework).rewards_rate;
            if (epoch_rewards_rate.value == 0) {
                0
            } else {
                let denominator_0 = creditchain_std::fixed_point64::spec_divide_u128(staking_config::MAX_REWARDS_RATE, epoch_rewards_rate);
                let denominator = if (denominator_0 > MAX_U64) {
                    MAX_U64
                } else {
                    denominator_0
                };
                let nominator = creditchain_std::fixed_point64::spec_multiply_u128(denominator, epoch_rewards_rate);
                nominator
            }
        } else {
                config.rewards_rate
