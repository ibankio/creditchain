...
module creditchain_framework::keyless_account {
    ...
    // The `creditchain_framework::reconfiguration_with_dkg` module needs to be able to call `on_new_epoch`.
    ...
    #[resource_group_member(group = creditchain_framework::keyless_account::Group)]
    ...
    public(friend) fun on_new_epoch(fx: &signer) acquires Groth16VerificationKey, Configuration {
        ...
        if (config_buffer::does_exist<Groth16VerificationKey>()) {
            ...
            if (exists<Groth16VerificationKey>(@creditchain_framework)) {
                *borrow_global_mut<Groth16VerificationKey>(@creditchain_framework) = vk;
            } else {
                move_to(fx, vk);
            }
        };
        ...
        if (config_buffer::does_exist<Configuration>()) {
            ...
            if (exists<Configuration>(@creditchain_framework)) {
                *borrow_global_mut<Configuration>(@creditchain_framework) = config;
            } else {
                move_to(fx, config);
            }
        };
