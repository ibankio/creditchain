spec creditchain_framework::storage_gas {
    // -----------------
    // Struct invariants
    // -----------------
    ...
    spec module {
        ...
        invariant [suspendable] chain_status::is_operating() ==> exists<StorageGasConfig>(@creditchain_framework);
        invariant [suspendable] chain_status::is_operating() ==> exists<StorageGas>(@creditchain_framework);
    }
    ...
    spec set_config(creditchain_framework: &signer, config: StorageGasConfig) {
    ...
    spec initialize(creditchain_framework: &signer) {
        ...
        aborts_if exists<StorageGasConfig>(@creditchain_framework);
        aborts_if exists<StorageGas>(@creditchain_framework);

        /// [high-level-req-1]
        ensures exists<StorageGasConfig>(@creditchain_framework);
        ensures exists<StorageGas>(@creditchain_framework);
    }
    ...
    spec on_reconfig {
        ...
        aborts_if !exists<StorageGasConfig>(@creditchain_framework);
        aborts_if !exists<StorageGas>(@creditchain_framework);
        aborts_if !exists<state_storage::StateStorageUsage>(@creditchain_framework);
    }
