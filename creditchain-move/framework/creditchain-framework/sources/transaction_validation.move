module creditchain_framework::transaction_validation {
    use std::error;
    use std::features;
    use std::option;
    ...
    use creditchain_framework::account_abstraction;
    ...
    public(friend) fun initialize(
        ...
        move_to(creditchain_framework, TransactionValidation {
    ...
    fun prologue_common(
        ...
        if (!skip_gas_payment(
            ...
            if (features::operations_default_to_fa_apt_store_enabled()) {
                assert!(
                    creditchain_account::is_fungible_balance_at_least(gas_payer_address, max_transaction_fee),
                    error::invalid_argument(PROLOGUE_ECANT_PAY_GAS_DEPOSIT)
                );
            } else {
                assert!(
                    coin::is_balance_at_least<CreditChainCoin>(gas_payer_address, max_transaction_fee),
                    error::invalid_argument(PROLOGUE_ECANT_PAY_GAS_DEPOSIT)
                );
            }
        };
    ...
    // Once the transaction_simulation_enhancement feature is enabled, the CreditChain VM will invoke this function instead.
    ...
    fun epilogue_gas_payer_extended(
        ...
        if (!skip_gas_payment(is_simulation, gas_payer)) {
            if (features::operations_default_to_fa_apt_store_enabled()) {
                assert!(
                    creditchain_account::is_fungible_balance_at_least(gas_payer, transaction_fee_amount),
                    error::out_of_range(PROLOGUE_ECANT_PAY_GAS_DEPOSIT),
                );
            } else {
                assert!(
                    coin::is_balance_at_least<CreditChainCoin>(gas_payer, transaction_fee_amount),
                    error::out_of_range(PROLOGUE_ECANT_PAY_GAS_DEPOSIT),
                );
            };
    ...
    fun unified_epilogue_v2(
        ...
        if (!skip_gas_payment(
            ...
            if (features::operations_default_to_fa_apt_store_enabled()) {
                assert!(
                    creditchain_account::is_fungible_balance_at_least(gas_payer_address, transaction_fee_amount),
                    error::out_of_range(PROLOGUE_ECANT_PAY_GAS_DEPOSIT),
                );
            } else {
                assert!(
                    coin::is_balance_at_least<CreditChainCoin>(gas_payer_address, transaction_fee_amount),
                    error::out_of_range(PROLOGUE_ECANT_PAY_GAS_DEPOSIT),
                );
            };
