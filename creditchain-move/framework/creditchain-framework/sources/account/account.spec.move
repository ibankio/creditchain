spec creditchain_framework::account {
    /// <high-level-req>
    /// No.: 1
    ...
    /// to the OriginatingAddress under the creditchain_framework account.
    ...
    /// Only the address `@creditchain_framework` can call.
    /// OriginatingAddress does not exist under `@creditchain_framework` before the call.
    spec initialize(creditchain_framework: &signer) {
        ...
        aborts_if exists<OriginatingAddress>(creditchain_addr);
        ensures exists<OriginatingAddress>(creditchain_addr);
    }
    ...
    spec create_account(new_address: address): signer {
        ...
        aborts_if new_address == @vm_reserved || new_address == @creditchain_framework || new_address == @creditchain_token;
    ...
    spec rotate_authentication_key(
        ...
        let address_map = global<OriginatingAddress>(@creditchain_framework).address_map;
        let new_auth_key = from_bcs::deserialize<address>(new_auth_key_vector);

        aborts_if !exists<OriginatingAddress>(@creditchain_framework);
        aborts_if !from_bcs::deserializable<address>(account_resource.authentication_key);
    ...
    spec offer_rotation_capability(
        ...
        let proof_challenge = RotationCapabilityOfferProofChallengeV2 {
            chain_id: global<chain_id::ChainId>(@creditchain_framework).id,
            sequence_number: account_resource.sequence_number,
            source_address,
            recipient_address,
        };
        aborts_if !exists<chain_id::ChainId>(@creditchain_framework);
    ...
    spec update_auth_key_and_originating_address_table(
        ...
        modifies global<OriginatingAddress>(@creditchain_framework);
        include UpdateAuthKeyAndOriginatingAddressTableAbortsIf;
    }
    spec schema UpdateAuthKeyAndOriginatingAddressTableAbortsIf {
        ...
        ensures table::spec_contains(global<OriginatingAddress>(@creditchain_framework).address_map, from_bcs::deserialize<address>(new_auth_key_vector));
    }
