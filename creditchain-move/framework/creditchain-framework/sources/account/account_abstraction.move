module creditchain_framework::account_abstraction {
    use std::bcs;
    use std::hash;
    ...
    use creditchain_std::ordered_map::{Self, OrderedMap};
    ...
    use creditchain_framework::features;
    use creditchain_framework::function_info::{Self, FunctionInfo};
    use creditchain_framework::object;
    use creditchain_framework::auth_data::AbstractionAuthData;
    use creditchain_framework::system_addresses;
    use creditchain_framework::permissioned_signer::is_permissioned_signer;
    #[test_only]
    use creditchain_framework::account::create_account_for_test;
    #[test_only]
    use creditchain_framework::auth_data;

    friend creditchain_framework::transaction_validation;
    #[test_only]
    friend creditchain_framework::account_abstraction_tests;
    ...
    public entry fun register_derivable_authentication_function(
        ...
        DerivableDispatchableAuthenticator[@creditchain_framework].auth_functions.add(
            function_info::new_function_info_from_address(module_address, module_name, function_name),
            DerivableRegisterValue::Empty,
        );
    }

    public entry fun initialize(creditchain_framework: &signer) {
        system_addresses::assert_creditchain_framework(creditchain_framework);
        move_to(
    ...
    inline fun resource_addr(source: address): address {
        object::create_user_derived_object_address(source, @creditchain_fungible_asset)
    }
    ...
    inline fun dispatchable_derivable_authenticator_internal(): &BigOrderedMap<FunctionInfo, DerivableRegisterValue> {
        assert!(exists<DerivableDispatchableAuthenticator>(@creditchain_framework), error::not_found(EDERIVABLE_AA_NOT_INITIALIZED));
        &DerivableDispatchableAuthenticator[@creditchain_framework].auth_functions
   }
    ...
    fun authenticate(
        ...
        // Returned signer MUST represent the same account address. Otherwise, it may break the invariant of CreditChain blockchain!
        assert!(
