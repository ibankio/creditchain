...
module creditchain_framework::managed_coin {
    use std::string;
    use std::error;
    use std::signer;

    use creditchain_framework::coin::{Self, BurnCapability, FreezeCapability, MintCapability, destroy_burn_cap,
    ...
    /// Initialize new coin `CoinType` in CreditChain Blockchain.
    ...
    #[test(framework = @creditchain_framework, source = @0xa11ce, destination = @0xb0b, mod_account = @0x1)]
    public entry fun test_end_to_end(
        ...
        creditchain_framework::account::create_account_for_test(source_addr);
        creditchain_framework::account::create_account_for_test(destination_addr);
        creditchain_framework::account::create_account_for_test(signer::address_of(&mod_account));
        aggregator_factory::initialize_aggregator_factory_for_test(&mod_account);
        creditchain_framework::coin::create_coin_conversion_map(&framework);
    ...
    public entry fun fail_mint(
        ...
        creditchain_framework::account::create_account_for_test(source_addr);
        creditchain_framework::account::create_account_for_test(signer::address_of(&destination));
        creditchain_framework::account::create_account_for_test(signer::address_of(&mod_account));
        aggregator_factory::initialize_aggregator_factory_for_test(&mod_account);
        creditchain_framework::coin::create_coin_conversion_map(&framework);
