...
module creditchain_framework::vesting {
    use std::bcs;
    use std::error;
    ...
    use creditchain_std::pool_u64::{Self, Pool};
    use creditchain_std::simple_map::{Self, SimpleMap};

    use creditchain_framework::account::{Self, SignerCapability, new_event_handle};
    use creditchain_framework::creditchain_account::{Self, assert_account_is_registered_for_apt};
    use creditchain_framework::creditchain_coin::CreditChainCoin;
    use creditchain_framework::coin::{Self, Coin};
    use creditchain_framework::event::{EventHandle, emit, emit_event};
    ...
    const VESTING_POOL_SALT: vector<u8> = b"creditchain_framework::vesting";
    ...
    public fun create_vesting_contract(
        ...
        buy_ins: SimpleMap<address, Coin<CreditChainCoin>>,
        vesting_schedule: VestingSchedule,
        withdrawal_address: address,
        operator: address,
        voter: address,
        ...
        let grant = coin::zero<CreditChainCoin>();
        let grant_amount = 0;
    ...
    public entry fun distribute(contract_address: address) acquires VestingContract {
        ...
        vector::for_each_ref(shareholders, |shareholder| {
            ...
            creditchain_account::deposit_coins(recipient_address, share_of_coins);
        });

        // Send any remaining "dust" (leftover due to rounding error) to the withdrawal address.
        if (coin::value(&coins) > 0) {
            creditchain_account::deposit_coins(vesting_contract.withdrawal_address, coins);
        } else {
            coin::destroy_zero(coins);
        };
    ...
    fun create_vesting_contract_account(
        ...
        coin::register<CreditChainCoin>(&account_signer);
    ...
    fun withdraw_stake(vesting_contract: &VestingContract, contract_address: address): Coin<CreditChainCoin> {
        ...
        let withdrawn_coins = coin::balance<CreditChainCoin>(contract_address);
        let contract_signer = &get_vesting_account_signer_internal(vesting_contract);
        coin::withdraw<CreditChainCoin>(contract_signer, withdrawn_coins)
    }
    ...
    #[test_only]
    use creditchain_framework::stake::with_rewards;
    ...
    #[test_only]
    public fun setup(creditchain_framework: &signer, accounts: &vector<address>) {
        use creditchain_framework::creditchain_account::create_account;
        ...
        std::features::change_feature_flags_for_testing(creditchain_framework, vector[], vector[std::features::get_periodical_reward_rate_decrease_feature()]);
    }
    ...
    public fun setup_vesting_contract_with_schedule(
        ...
        let admin_address = signer::address_of(admin);
        let buy_ins = simple_map::create<address, Coin<CreditChainCoin>>();
    ...
    #[test(creditchain_framework = @0x1, admin = @0x123, shareholder_1 = @0x234, shareholder_2 = @0x345, withdrawal = @111)]
    public entry fun test_end_to_end(
        ...
        setup(
            creditchain_framework, &vector[admin_address, withdrawal_address, shareholder_1_address, shareholder_2_address]);
        ...
        distribute(contract_address);
        let shareholder_1_bal = coin::balance<CreditChainCoin>(shareholder_1_address);
        let shareholder_2_bal = coin::balance<CreditChainCoin>(shareholder_2_address);
        ...
        assert!(coin::balance<CreditChainCoin>(shareholder_1_address) == shareholder_1_bal + pending_distribution / 4, 0);
        assert!(coin::balance<CreditChainCoin>(shareholder_2_address) == shareholder_2_bal + pending_distribution * 3 / 4, 1);
        // Withdrawal address receives the left-over dust of 1 coin due to rounding error.
        assert!(coin::balance<CreditChainCoin>(withdrawal_address) == 1, 0);
        ...
        let previous_bal = coin::balance<CreditChainCoin>(withdrawal_address);
        admin_withdraw(admin, contract_address);
        assert!(coin::balance<CreditChainCoin>(withdrawal_address) == previous_bal + withdrawn_amount, 0);
    }

    #[test(creditchain_framework = @0x1, admin = @0x123)]
    ...
    public entry fun test_create_vesting_contract_with_zero_grant_should_fail(
        ...
        let admin_address = signer::address_of(admin);
        setup(creditchain_framework, &vector[admin_address]);
    ...
    public entry fun test_create_empty_vesting_schedule_should_fail(creditchain_framework: &signer) {
        setup(creditchain_framework, &vector[]);
        create_vesting_schedule(vector[], 1, 1);
    }

    #[test(creditchain_framework = @0x1)]
    #[expected_failure(abort_code = 0x10003, location = Self)]
    public entry fun test_create_vesting_schedule_with_zero_period_duration_should_fail(creditchain_framework: &signer) {
        setup(creditchain_framework, &vector[]);
    ...
    public entry fun test_create_vesting_schedule_with_invalid_vesting_start_should_fail(creditchain_framework: &signer) {
        setup(creditchain_framework, &vector[]);
    ...
    #[test(creditchain_framework = @0x1, admin = @0x123, shareholder = @0x234)]
    public entry fun test_vest_twice_should_not_double_count(
        ...
        setup(creditchain_framework, &vector[admin_address, shareholder_address]);
    ...
    #[test(creditchain_framework = @0x1, admin = @0x123, shareholder = @0x234, operator = @0x345)]
    public entry fun test_unlock_rewards_should_pay_commission_first(
        ...
        let shareholder_address = signer::address_of(shareholder);
        setup(creditchain_framework, &vector[admin_address, shareholder_address, operator_address]);
        let contract_address = setup_vesting_contract(
            admin, &vector[shareholder_address], &vector[GRANT_AMOUNT], admin_address, 0);
        ...
        assert!(coin::balance<CreditChainCoin>(shareholder_address) == staker_rewards + 1, 0);
        assert!(coin::balance<CreditChainCoin>(operator_address) == commission - 1, 1);
    }
    ...
    #[test(creditchain_framework = @0x1, admin = @0x123, operator = @0x345)]
    public entry fun test_update_operator_with_same_commission(
        ...
        let admin_address = signer::address_of(admin);
        let operator_address = signer::address_of(operator);
        setup(creditchain_framework, &vector[admin_address, @11, operator_address]);
        let contract_address = setup_vesting_contract(
    ...
    public entry fun test_commission_percentage_change(
        ...
        assert!(coin::balance<CreditChainCoin>(operator_address) == expected_commission, 1);
    }
    ...
    public entry fun test_set_beneficiary_for_operator(
        ...
        setup(creditchain_framework, &vector[admin_address, shareholder_address, operator_address1, beneficiary_address]);
        let contract_address = setup_vesting_contract(
            admin, &vector[shareholder_address], &vector[GRANT_AMOUNT], admin_address, 0);
        assert!(operator_commission_percentage(contract_address) == 0, 0);
        let stake_pool_address = stake_pool_address(contract_address);
        ...
        assert!(coin::balance<CreditChainCoin>(operator_address1) == 0, 1);
        assert!(coin::balance<CreditChainCoin>(beneficiary_address) == expected_commission, 1);
        let old_beneficiay_balance = coin::balance<CreditChainCoin>(beneficiary_address);
        ...
        assert!(coin::balance<CreditChainCoin>(operator_address2) >= expected_commission, 1);
        assert!(coin::balance<CreditChainCoin>(beneficiary_address) == old_beneficiay_balance, 1);
    }
    ...
    public entry fun test_set_beneficiary_with_unregistered_account(
        ...
        std::features::change_feature_flags_for_testing(creditchain_framework, vector[], vector[fa_feature]);
        let admin_address = signer::address_of(admin);
        setup(creditchain_framework, &vector[admin_address]);
        let contract_address = setup_vesting_contract(
            admin, &vector[@1, @2], &vector[GRANT_AMOUNT, GRANT_AMOUNT], admin_address, 0);
    ...
    public entry fun test_set_beneficiary_should_send_distribution(
        ...
        let admin_address = signer::address_of(admin);
        setup(creditchain_framework, &vector[admin_address, @11]);
        let contract_address = setup_vesting_contract(
        ...
        let vested_amount = fraction(GRANT_AMOUNT, 3, 48);
        distribute(contract_address);
        let balance = coin::balance<CreditChainCoin>(@11);
    ...
    public entry fun test_reset_beneficiary(
        ...
        let admin_address = signer::address_of(admin);
        setup(creditchain_framework, &vector[admin_address, @11, @12]);
        let contract_address = setup_vesting_contract(
            admin, &vector[@11], &vector[GRANT_AMOUNT], admin_address, 0);
        ...
        let vested_amount = fraction(GRANT_AMOUNT, 3, 48);
        distribute(contract_address);
        assert!(coin::balance<CreditChainCoin>(@11) == vested_amount, 0);
        assert!(coin::balance<CreditChainCoin>(@12) == 0, 1);
    }

    #[test(creditchain_framework = @0x1, admin = @0x123, resetter = @0x234)]
    ...
    #[test(creditchain_framework = @0x1, admin = @0x123, resetter = @0x234, random = @0x345)]
