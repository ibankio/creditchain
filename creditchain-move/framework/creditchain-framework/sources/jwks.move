...
module creditchain_framework::jwks {
    use std::bcs;
    use std::error;
    use std::features;
    use std::option;
    ...
    use std::vector;
    use creditchain_std::comparator::{compare_u8_vector, is_greater_than, is_equal};
    use creditchain_std::copyable_any;
    use creditchain_std::copyable_any::Any;
    use creditchain_framework::chain_status;
    use creditchain_framework::config_buffer;
    use creditchain_framework::event::emit;
    ...
    const EINSTALL_FEDERATED_JWKS_AT_CREDITCHAIN_FRAMEWORK: u64 = 7;
    ...
    public fun patch_federated_jwks(jwk_owner: &signer, patches: vector<Patch>) acquires FederatedJWKs {
        // Prevents accidental calls in 0x1::jwks that install federated JWKs at the CreditChain framework address.
        assert!(!system_addresses::is_creditchain_framework_address(signer::address_of(jwk_owner)),
            error::invalid_argument(EINSTALL_FEDERATED_JWKS_AT_CREDITCHAIN_FRAMEWORK)
        );
    ...
    public fun try_get_patched_jwk(issuer: vector<u8>, jwk_id: vector<u8>): Option<JWK> acquires PatchedJWKs {
        let jwks = &borrow_global<PatchedJWKs>(@creditchain_framework).jwks;
    ...
    public fun upsert_oidc_provider(fx: &signer, name: vector<u8>, config_url: vector<u8>): Option<vector<u8>> acquires SupportedOIDCProviders {
        system_addresses::assert_creditchain_framework(fx);
        chain_status::assert_genesis();

        let provider_set = borrow_global_mut<SupportedOIDCProviders>(@creditchain_framework);
    ...
    public fun upsert_oidc_provider_for_next_epoch(fx: &signer, name: vector<u8>, config_url: vector<u8>): Option<vector<u8>> acquires SupportedOIDCProviders {
        ...
        let provider_set = if (config_buffer::does_exist<SupportedOIDCProviders>()) {
            ...
            *borrow_global<SupportedOIDCProviders>(@creditchain_framework)
        };
    ...
    public(friend) fun on_new_epoch(framework: &signer) acquires SupportedOIDCProviders {
        ...
        if (config_buffer::does_exist<SupportedOIDCProviders>()) {
            ...
            if (exists<SupportedOIDCProviders>(@creditchain_framework)) {
                *borrow_global_mut<SupportedOIDCProviders>(@creditchain_framework) = new_config;
            } else {
    ...
    public fun set_patches(fx: &signer, patches: vector<Patch>) acquires Patches, PatchedJWKs, ObservedJWKs {
        system_addresses::assert_creditchain_framework(fx);
        borrow_global_mut<Patches>(@creditchain_framework).patches = patches;
    ...
    public fun upsert_into_observed_jwks(fx: &signer, provider_jwks_vec: vector<ProviderJWKs>) acquires ObservedJWKs, PatchedJWKs, Patches {
        system_addresses::assert_creditchain_framework(fx);
        let observed_jwks = borrow_global_mut<ObservedJWKs>(@creditchain_framework);
    ...
    fun regenerate_patched_jwks() acquires PatchedJWKs, Patches, ObservedJWKs {
        let jwks = borrow_global<ObservedJWKs>(@creditchain_framework).jwks;
        let patches = borrow_global<Patches>(@creditchain_framework);
        vector::for_each_ref(&patches.patches, |obj|{
            let patch: &Patch = obj;
            apply_patch(&mut jwks, *patch);
        });
        *borrow_global_mut<PatchedJWKs>(@creditchain_framework) = PatchedJWKs { jwks };
    }
    ...
    #[test_only]
    fun initialize_for_test(creditchain_framework: &signer) {
        create_account_for_test(@creditchain_framework);
        reconfiguration::initialize_for_test(creditchain_framework);
        initialize(creditchain_framework);
    }

    #[test(fx = @creditchain_framework)]
    fun test_observed_jwks_operations(fx: &signer) acquires ObservedJWKs, PatchedJWKs, Patches {
        ...
        assert!(expected == borrow_global<ObservedJWKs>(@creditchain_framework).jwks, 1);
        ...
        assert!(expected == borrow_global<ObservedJWKs>(@creditchain_framework).jwks, 2);
        ...
        assert!(expected == borrow_global<ObservedJWKs>(@creditchain_framework).jwks, 3);

        remove_issuer_from_observed_jwks(fx, b"alice");
        let expected = AllProvidersJWKs { entries: vector[bob_jwks_v1] };
        assert!(expected == borrow_global<ObservedJWKs>(@creditchain_framework).jwks, 4);
    }
    ...
    fun test_observed_jwks_operations_per_key_mode(fx: &signer) acquires ObservedJWKs, PatchedJWKs, Patches {
        ...
        assert!(expected == borrow_global<PatchedJWKs>(@creditchain_framework).jwks, 999);
    ...
    fun test_patched_jwks(creditchain_framework: signer) acquires ObservedJWKs, PatchedJWKs, Patches {
        ...
        upsert_into_observed_jwks(&creditchain_framework, vector [
            ProviderJWKs {
                issuer: b"alice",
                version: 111,
                jwks: vector[jwk_0, jwk_1],
        ...
        set_patches(&creditchain_framework, vector[
