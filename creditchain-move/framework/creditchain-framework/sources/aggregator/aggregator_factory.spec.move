spec creditchain_framework::aggregator_factory {
    use creditchain_framework::aggregator;
    /// <high-level-req>
    /// No.: 1
    /// Requirement: During the module's initialization, it guarantees that the CreditChain framework is the caller and that the
    /// AggregatorFactory resource will move under the CreditChain framework account.
    /// Criticality: High
    ...
    /// resource transfers to the CreditChain framework account.
    ...
    /// under the CreditChain account.
    /// Criticality: High
    /// Implementation: The create_aggregator_internal function asserts that AggregatorFactory exists for the CreditChain
    /// account.
    /// Enforcement: Formally verified via [high-level-req-2](CreateAggregatorInternalAbortsIf).
    ///
    /// No.: 3
    /// Requirement: Only the CreditChain framework address may create an aggregator instance currently.
    /// Criticality: Low
    /// Implementation: The create_aggregator function ensures that the address calling it is the CreditChain framework
    /// address.
    ...
    spec initialize_aggregator_factory(creditchain_framework: &signer) {
        use std::signer;
    ...
    spec schema CreateAggregatorInternalAbortsIf {
        aborts_if !exists<AggregatorFactory>(@creditchain_framework);
    }
