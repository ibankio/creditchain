...
module creditchain_framework::aggregator_factory {
    use std::error;
    ...
    friend creditchain_framework::optional_aggregator;
    ...
    public(friend) fun initialize_aggregator_factory(creditchain_framework: &signer) {
        ...
        move_to(creditchain_framework, aggregator_factory);
    }
    ...
    public(friend) fun create_aggregator_internal(): Aggregator acquires AggregatorFactory {
        assert!(
            exists<AggregatorFactory>(@creditchain_framework),
            error::not_found(EAGGREGATOR_FACTORY_NOT_FOUND)
        );

        let aggregator_factory = borrow_global_mut<AggregatorFactory>(@creditchain_framework);
    ...
    public fun create_aggregator(account: &signer, limit: u128): Aggregator acquires AggregatorFactory {
        ...
        // Only CreditChain Framework (0x1) account can call this for now.
    ...
    #[test_only]
    public fun initialize_aggregator_factory_for_test(creditchain_framework: &signer) {
        initialize_aggregator_factory(creditchain_framework);
    }
