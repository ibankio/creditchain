...
module creditchain_framework::staking_contract {
    use std::bcs;
    use std::error;
    use std::features;
    use std::signer;
    ...
    use creditchain_framework::account::{Self, SignerCapability};
    ...
    use creditchain_framework::stake::{Self, OwnerCapability};
    use creditchain_framework::staking_config;

    const SALT: vector<u8> = b"creditchain_framework::staking_contract";
    ...
    #[resource_group_member(group = creditchain_framework::staking_contract::StakingGroupContainer)]
    ...
    public entry fun create_staking_contract(
        ...
        let staked_coins = coin::withdraw<CreditChainCoin>(staker, amount);
    ...
    fun distribute_internal(
        ...
        while (pool_u64::shareholders_count(distribution_pool) > 0) {
            ...
            creditchain_account::deposit_coins(recipient, coin::extract(&mut coins, amount_to_distribute));
        ...
        if (coin::value(&coins) > 0) {
            creditchain_account::deposit_coins(staker, coins);
    ...
    #[test_only]
    public fun setup(creditchain_framework: &signer, staker: &signer, operator: &signer, initial_balance: u64) {
    ...
    public fun setup_staking_contract(
        ...
        setup(creditchain_framework, staker, operator, amount);
        let operator_address = signer::address_of(operator);
        ...
        std::features::change_feature_flags_for_testing(creditchain_framework, vector[], vector[features::get_periodical_reward_rate_decrease_feature()]);
    }

    #[test(creditchain_framework = @0x1, staker = @0x123, operator = @0x234)]
    public entry fun test_end_to_end(
        creditchain_framework: &signer,
        staker: &signer,
        operator: &signer
    ) acquires Store, BeneficiaryForOperator, Staker {
        setup_staking_contract(creditchain_framework, staker, operator, INITIAL_BALANCE, 10);
        let staker_address = signer::address_of(staker);
        let operator_address = signer::address_of(operator);
        ...
        let operator_balance = coin::balance<CreditChainCoin>(operator_address);
        ...
        distribute(staker_address, operator_address);
        operator_balance = coin::balance<CreditChainCoin>(operator_address);
        ...
        let staker_balance = coin::balance<CreditChainCoin>(staker_address);
    ...
    public entry fun test_operator_cannot_request_same_commission_multiple_times(
        creditchain_framework: &signer, staker: &signer, operator: &signer) acquires Store, BeneficiaryForOperator {
        setup_staking_contract(creditchain_framework, staker, operator, INITIAL_BALANCE, 10);
        let staker_address = signer::address_of(staker);
        let operator_address = signer::address_of(operator);
        let pool_address = stake_pool_address(staker_address, operator_address);
    ...
    public entry fun test_staker_cannot_create_staking_contract_with_invalid_commission(
        ...
        setup_staking_contract(creditchain_framework, staker, operator, INITIAL_BALANCE, 101);
    }
    ...
    public entry fun test_staker_cannot_create_staking_contract_with_less_than_min_stake_required(
        ...
        setup_staking_contract(creditchain_framework, staker, operator, 50, 100);
    }
    ...
    #[test(creditchain_framework = @0x1, staker = @0x123, operator_1 = @0x234, operator_2 = @0x345)]
    public entry fun test_staker_can_switch_operator(
        ...
        setup_staking_contract(creditchain_framework, staker, operator_1, INITIAL_BALANCE, 10);
        ...
        let operator_1_balance = coin::balance<CreditChainCoin>(operator_1_address);
        ...
        let operator_2_balance = coin::balance<CreditChainCoin>(operator_2_address);
        new_balance = with_rewards(new_balance);
    ...
    #[test(creditchain_framework = @0x1, staker = @0x123, operator1 = @0x234, beneficiary = @0x345, operator2 = @0x456)]
    public entry fun test_operator_can_set_beneficiary(
        ...
        setup_staking_contract(creditchain_framework, staker, operator1, INITIAL_BALANCE, 10);
        let staker_address = signer::address_of(staker);
        ...
        creditchain_framework::creditchain_account::create_account(beneficiary_address);
        ...
        let operator_balance = coin::balance<CreditChainCoin>(operator1_address);
        let beneficiary_balance = coin::balance<CreditChainCoin>(beneficiary_address);
        ...
        assert!(coin::balance<CreditChainCoin>(operator2_address) >= expected_commission, 1);
