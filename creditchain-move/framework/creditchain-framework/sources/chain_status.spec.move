spec creditchain_framework::chain_status {
    /// <high-level-req>
    /// No.: 1
    /// Requirement: The end of genesis mark should persist throughout the entire life of the chain.
    /// Criticality: Medium
    /// Implementation: The CreditChain framework account should never drop the GenesisEndMarker resource.
    ...
    spec set_genesis_end(creditchain_framework: &signer) {
        use std::signer;
        pragma verify = true;
        ...
        /// [high-level-req-3]
        aborts_if exists<GenesisEndMarker>(@creditchain_framework);
        /// [high-level-req-1]
        ensures global<GenesisEndMarker>(@creditchain_framework) == GenesisEndMarker {};
    }
