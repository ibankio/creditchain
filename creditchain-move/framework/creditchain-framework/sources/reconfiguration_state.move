...
module creditchain_framework::reconfiguration_state {
    use std::error;
    use std::string;
    ...
    public fun is_initialized(): bool {
        exists<State>(@creditchain_framework)
    }
    ...
    public(friend) fun is_in_progress(): bool acquires State {
        ...
        let state = borrow_global<State>(@creditchain_framework);
    ...
    public(friend) fun on_reconfig_start() acquires State {
        if (exists<State>(@creditchain_framework)) {
            let state = borrow_global_mut<State>(@creditchain_framework);
            let variant_type_name = *string::bytes(copyable_any::type_name(&state.variant));
