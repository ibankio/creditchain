...
/// for blind signing.
module creditchain_framework::permissioned_signer {
    use std::features;
    use std::signer;
    use std::error;
    use std::vector;
    use std::option::{Option, Self};
    use creditchain_std::copyable_any::{Self, Any};
    use creditchain_framework::big_ordered_map::{Self, BigOrderedMap};
    use creditchain_framework::create_signer::create_signer;
    use creditchain_framework::transaction_context::generate_auid_address;
    ...
    fun signer_address_roundtrip(
        ...
        let creditchain_framework = create_signer(@0x1);
    ...
    #[test_only]
    use creditchain_std::bcs;

    #[test(creator = @0xcafe)]
    #[expected_failure(abort_code = 0x1C5, location = creditchain_std::bcs)]
