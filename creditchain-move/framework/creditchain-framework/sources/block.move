...
module creditchain_framework::block {
    use std::error;
    use std::vector;
    use std::option;
    use creditchain_std::table_with_length::{Self, TableWithLength};
    ...
    public(friend) fun initialize(creditchain_framework: &signer, epoch_interval_microsecs: u64) {
        ...
        move_to<CommitHistory>(creditchain_framework, CommitHistory {
            max_capacity: 2000,
            next_idx: 0,
        ...
        move_to<BlockResource>(
            ...
            BlockResource {
                ...
                new_block_events: account::new_event_handle<NewBlockEvent>(creditchain_framework),
                update_epoch_interval_events: account::new_event_handle<UpdateEpochIntervalEvent>(creditchain_framework),
            }
        );
    }
    ...
    public fun update_epoch_interval_microsecs(
        ...
        let block_resource = borrow_global_mut<BlockResource>(@creditchain_framework);
    ...
    public fun get_epoch_interval_secs(): u64 acquires BlockResource {
        borrow_global<BlockResource>(@creditchain_framework).epoch_interval / 1000000
    }
    ...
    fun block_prologue_common(
        ...
        let block_metadata_ref = borrow_global_mut<BlockResource>(@creditchain_framework);
    ...
    public fun get_current_block_height(): u64 acquires BlockResource {
        borrow_global<BlockResource>(@creditchain_framework).height
    }
    ...
    fun emit_new_block_event(
        ...
        if (exists<CommitHistory>(@creditchain_framework)) {
            let commit_history_ref = borrow_global_mut<CommitHistory>(@creditchain_framework);
    ...
    public entry fun test_update_epoch_interval(creditchain_framework: signer) acquires BlockResource {
        account::create_account_for_test(@creditchain_framework);
        initialize(&creditchain_framework, 1);
        assert!(borrow_global<BlockResource>(@creditchain_framework).epoch_interval == 1, 0);
        update_epoch_interval_microsecs(&creditchain_framework, 2);
        assert!(borrow_global<BlockResource>(@creditchain_framework).epoch_interval == 2, 1);
    }

    #[test(creditchain_framework = @creditchain_framework, account = @0x123)]
