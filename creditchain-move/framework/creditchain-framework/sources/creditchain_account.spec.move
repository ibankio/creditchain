spec creditchain_framework::creditchain_account {
    /// <high-level-req>
    /// No.: 1
    /// Requirement: During the creation of an CreditChain account the following rules should hold: (1) the authentication key
    /// should be 32 bytes in length, (2) an CreditChain account should not already exist for that authentication key, and (3)
    ...
    ///
    /// No.: 2
    /// Requirement: After creating an CreditChain account, the account should become registered to receive CreditChainCoin.
    /// Criticality: Critical
    ...
    /// exist; then, register CreditChainCoin for that account if a particular is unregistered before transferring the amount.
    /// Criticality: Critical
    /// Implementation: The transfer function checks if the recipient account exists. If the account does not exist,
    /// the function creates one and registers the account to CreditChainCoin if not registered.
    ...
    ///
    /// No.: 7
    /// Requirement: When performing a batch transfer of CreditChain Coin and/or a batch transfer of a custom coin type, it
    ...
    spec schema CreateAccountAbortsIf {
        ...
        aborts_if auth_key == @vm_reserved || auth_key == @creditchain_framework || auth_key == @creditchain_token;
    }
    ...
    spec transfer(source: &signer, to: address, amount: u64) {
        ...
        include GuidAbortsIf<CreditChainCoin>;
        include WithdrawAbortsIf<CreditChainCoin>{from: source};
        include TransferEnsures<CreditChainCoin>;

        aborts_if exists<coin::CoinStore<CreditChainCoin>>(to) && global<coin::CoinStore<CreditChainCoin>>(to).frozen;
        /// [high-level-req-5]
        ensures exists<creditchain_framework::account::Account>(to);
        ensures exists<coin::CoinStore<CreditChainCoin>>(to);
    }
    ...
    spec assert_account_is_registered_for_apt(addr: address) {
        ...
        // aborts_if !account::spec_exists_at(addr);
        // TODO(fa_migration)
        //aborts_if !coin::spec_is_account_registered<CreditChainCoin>(addr);
    }
    ...
    spec batch_transfer(source: &signer, recipients: vector<address>, amounts: vector<u64>) {
        ...
        pragma verify = false;
        let account_addr_source = signer::address_of(source);
        let coin_store_source = global<coin::CoinStore<CreditChainCoin>>(account_addr_source);
        ...
        aborts_if exists i in 0..len(recipients):
                !account::spec_exists_at(recipients[i]) && (recipients[i] == @vm_reserved || recipients[i] == @creditchain_framework || recipients[i] == @creditchain_token);
        ...
        aborts_if exists i in 0..len(recipients):
            !exists<coin::CoinStore<CreditChainCoin>>(account_addr_source);
        aborts_if exists i in 0..len(recipients):
            coin_store_source.frozen;
        aborts_if exists i in 0..len(recipients):
            global<coin::CoinStore<CreditChainCoin>>(account_addr_source).coin.value < amounts[i];

        // deposit properties
        aborts_if exists i in 0..len(recipients):
            exists<coin::CoinStore<CreditChainCoin>>(recipients[i]) && global<coin::CoinStore<CreditChainCoin>>(recipients[i]).frozen;

        // guid properties
        aborts_if exists i in 0..len(recipients):
            account::spec_exists_at(recipients[i]) && !exists<coin::CoinStore<CreditChainCoin>>(recipients[i]) && global<account::Account>(recipients[i]).guid_creation_num + 2 >= account::MAX_GUID_CREATION_NUM;
        aborts_if exists i in 0..len(recipients):
            account::spec_exists_at(recipients[i]) && !exists<coin::CoinStore<CreditChainCoin>>(recipients[i]) && global<account::Account>(recipients[i]).guid_creation_num + 2 > MAX_U64;
    }
    ...
    spec deposit_coins<CoinType>(to: address, coins: Coin<CoinType>) {
        ...
        /// [high-level-spec-6]
        ensures exists<creditchain_framework::account::Account>(to);
        ensures exists<creditchain_framework::coin::CoinStore<CoinType>>(to);
    ...
    spec schema CreateAccountTransferAbortsIf {
        ...
        aborts_if !account::spec_exists_at(to) && (to == @vm_reserved || to == @creditchain_framework || to == @creditchain_token);
    }
    ...
    spec schema RegistCoinAbortsIf<CoinType> {
        ...
        aborts_if exists<creditchain_framework::account::Account>(to);
        aborts_if type_info::type_of<CoinType>() != type_info::type_of<CreditChainCoin>();
    }
