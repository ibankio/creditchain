module creditchain_framework::creditchain_account {
    use creditchain_framework::account::{Self, new_event_handle};
    ...
    use creditchain_framework::event::{EventHandle, emit_event, emit};
    use creditchain_framework::fungible_asset::{Self, Metadata, BurnRef, FungibleAsset};
    ...
    use std::signer;
    use std::vector;
    use creditchain_framework::object::Object;

    friend creditchain_framework::genesis;
    friend creditchain_framework::resource_account;
    ...
    public entry fun transfer(source: &signer, to: address, amount: u64) {
        ...
        if (features::operations_default_to_fa_apt_store_enabled()) {
            ...
            if (!coin::is_account_registered<CreditChainCoin>(to)) {
                coin::register<CreditChainCoin>(&create_signer(to));
            };
            coin::transfer<CreditChainCoin>(source, to, amount)
        }
    }
    ...
    public fun deposit_coins<CoinType>(to: address, coins: Coin<CoinType>) acquires DirectTransferConfig {
        if (!account::exists_at(to)) {
            ...
            spec {
                // TODO(fa_migration)
                // assert coin::spec_is_account_registered<CreditChainCoin>(to);
                // assume creditchain_std::type_info::type_of<CoinType>() == creditchain_std::type_info::type_of<CreditChainCoin>() ==>
    ...
    public fun assert_account_is_registered_for_apt(addr: address) {
        ...
        assert!(coin::is_account_registered<CreditChainCoin>(addr), error::not_found(EACCOUNT_NOT_REGISTERED_FOR_LBT));
    }
    ...
    inline fun ensure_primary_fungible_store_exists(owner: address): address {
        ...
        if (fungible_asset::store_exists(store_addr)) {
            ...
            object::object_address(&primary_fungible_store::create_primary_store(owner, object::address_to_object<Metadata>(@creditchain_fungible_asset)))
        }
    }

    /// Address of LBT Primary Fungible Store
    inline fun primary_fungible_store_address(account: address): address {
        object::create_user_derived_object_address(account, @creditchain_fungible_asset)
    }
    ...
    public fun test_transfer(alice: &signer, core: &signer) {
        ...
        let (burn_cap, mint_cap) = creditchain_framework::creditchain_coin::initialize_for_test(core);
        create_account(signer::address_of(alice));
        coin::deposit(signer::address_of(alice), coin::mint(10000, &mint_cap));
        transfer(alice, bob, 500);
        assert!(coin::balance<CreditChainCoin>(bob) == 500, 0);
        transfer(alice, carol, 500);
        assert!(coin::balance<CreditChainCoin>(carol) == 500, 1);
        transfer(alice, carol, 1500);
        assert!(coin::balance<CreditChainCoin>(carol) == 2000, 2);
    ...
    public fun test_transfer_to_resource_account(alice: &signer, core: &signer) {
        ...
        let (burn_cap, mint_cap) = creditchain_framework::creditchain_coin::initialize_for_test(core);
        assert!(coin::is_account_registered<CreditChainCoin>(resource_acc_addr), 0);

        create_account(signer::address_of(alice));
        coin::deposit(signer::address_of(alice), coin::mint(10000, &mint_cap));
        transfer(alice, resource_acc_addr, 500);
        assert!(coin::balance<CreditChainCoin>(resource_acc_addr) == 500, 1);
    ...
    public fun test_batch_transfer(from: &signer, core: &signer, recipient_1: &signer, recipient_2: &signer) {
        ...
        assert!(coin::balance<CreditChainCoin>(recipient_1_addr) == 100, 0);
        assert!(coin::balance<CreditChainCoin>(recipient_2_addr) == 500, 1);
        coin::destroy_burn_cap(burn_cap);
        coin::destroy_mint_cap(mint_cap);
    }
