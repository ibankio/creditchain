...
/// }
/// ```
module creditchain_framework::resource_account {
    use std::error;
    use std::signer;
    use std::vector;
    ...
    /// used for resource accounts that need access to `Coin<CreditChainCoin>`.
    public entry fun create_resource_account_and_fund(
        ...
        let (resource, resource_signer_cap) = account::create_resource_account(origin, seed);
        coin::register<CreditChainCoin>(&resource);
        coin::transfer<CreditChainCoin>(origin, signer::address_of(&resource), fund_amount);
        rotate_account_authentication_key_and_store_capability(
            origin,
            resource,
            resource_signer_cap,
    ...
    public entry fun create_resource_account_and_publish_package(
        ...
        let (resource, resource_signer_cap) = account::create_resource_account(origin, seed);
        creditchain_framework::code::publish_package_txn(&resource, metadata_serialized, code);
        rotate_account_authentication_key_and_store_capability(
            origin,
            resource,
            resource_signer_cap,
    ...
    public entry fun test_create_account_and_retrieve_cap(user: signer) acquires Container {
        ...
        let resource_addr = creditchain_framework::account::create_resource_address(&user_addr, seed);
    ...
    #[test(user = @0x1111)]
    #[expected_failure(abort_code = 0x10002, location = creditchain_std::simple_map)]
    ...
    public entry fun with_coin(framework: signer, user: signer) acquires Container {
        let user_addr = signer::address_of(&user);
        let (burn, mint) = creditchain_framework::creditchain_coin::initialize_for_test(&framework);
        creditchain_framework::creditchain_account::create_account(copy user_addr);

        let coin = coin::mint<CreditChainCoin>(100, &mint);
        ...
        let resource_addr = creditchain_framework::account::create_resource_address(&user_addr, seed);
        coin::transfer<CreditChainCoin>(&user, resource_addr, 10);
    ...
    #[expected_failure(abort_code = 0x60005, location = creditchain_framework::coin)]
    public entry fun without_coin(framework: signer, user: signer) acquires Container {
        ...
        let user_addr = signer::address_of(&user);
        let (burn, mint) = creditchain_framework::creditchain_coin::initialize_for_test(&framework);
        creditchain_framework::creditchain_account::create_account(user_addr);
