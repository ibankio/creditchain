spec creditchain_framework::genesis {
    /// <high-level-req>
    /// No.: 1
    /// Requirement: All the core resources and modules should be created during genesis and owned by the CreditChain framework
    /// account.
    ...
    /// formally verified by a post condition to ensure that all the critical resources are owned by the CreditChain framework.
    ...
    ///
    /// No.: 3
    /// Requirement: The CreditChain coin should be initialized during genesis and only the CreditChain framework account should own
    /// the mint and burn capabilities for the LBT token.
    /// Criticality: Critical
    /// Implementation: Both mint and burn capabilities are wrapped inside the stake::CreditChainCoinCapabilities and
    /// transaction_fee::CreditChainCoinCapabilities resources which are stored under the libra2 framework account.
    /// Enforcement: Formally verified via [high-level-req-3](initialize_creditchain_coin).
    ///
    /// No.: 4
    ...
    spec initialize {
        ...
        // property 1: All the core resources and modules should be created during genesis and owned by the CreditChain framework account.
        /// [high-level-req-1]
        ensures exists<creditchain_governance::GovernanceResponsbility>(@creditchain_framework);
        ensures exists<consensus_config::ConsensusConfig>(@creditchain_framework);
        ensures exists<execution_config::ExecutionConfig>(@creditchain_framework);
        ensures exists<version::Version>(@creditchain_framework);
        ensures exists<stake::ValidatorSet>(@creditchain_framework);
        ensures exists<stake::ValidatorPerformance>(@creditchain_framework);
        ensures exists<storage_gas::StorageGasConfig>(@creditchain_framework);
        ensures exists<storage_gas::StorageGas>(@creditchain_framework);
        ensures exists<gas_schedule::GasScheduleV2>(@creditchain_framework);
        ensures exists<aggregator_factory::AggregatorFactory>(@creditchain_framework);
        ensures exists<coin::SupplyConfig>(@creditchain_framework);
        ensures exists<chain_id::ChainId>(@creditchain_framework);
        ensures exists<reconfiguration::Configuration>(@creditchain_framework);
        ensures exists<block::BlockResource>(@creditchain_framework);
        ensures exists<state_storage::StateStorageUsage>(@creditchain_framework);
        ensures exists<timestamp::CurrentTimeMicroseconds>(@creditchain_framework);
        ensures exists<account::Account>(@creditchain_framework);
        ensures exists<version::SetVersionCapability>(@creditchain_framework);
        ensures exists<staking_config::StakingConfig>(@creditchain_framework);
    }

    spec initialize_creditchain_coin {
        // property 3: The CreditChain coin should be initialized during genesis and only the CreditChain framework account should
        // own the mint and burn capabilities for the LBT token.
        /// [high-level-req-3]
        requires !exists<stake::CreditChainCoinCapabilities>(@creditchain_framework);
        ensures exists<stake::CreditChainCoinCapabilities>(@creditchain_framework);
        requires exists<transaction_fee::CreditChainCoinCapabilities>(@creditchain_framework);
        ensures exists<transaction_fee::CreditChainCoinCapabilities>(@creditchain_framework);
    }
    ...
    spec create_initialize_validators_with_commission {
        ...
        include creditchain_coin::ExistsCreditChainCoin;
    }
    ...
    spec set_genesis_end {
        ...
        /// [high-level-req-4]
        requires len(global<stake::ValidatorSet>(@creditchain_framework).active_validators) >= 1;
        // property 5: The end of genesis should be marked on chain.
        /// [high-level-req-5]
        let addr = std::signer::address_of(creditchain_framework);
        aborts_if addr != @creditchain_framework;
        aborts_if exists<chain_status::GenesisEndMarker>(@creditchain_framework);
        ensures global<chain_status::GenesisEndMarker>(@creditchain_framework) == chain_status::GenesisEndMarker {};
    }

    spec schema InitalizeRequires {
        execution_config: vector<u8>;
        requires !exists<account::Account>(@creditchain_framework);
    ...
    spec schema CompareTimeRequires {
        let staking_rewards_config = global<staking_config::StakingRewardsConfig>(@creditchain_framework);
