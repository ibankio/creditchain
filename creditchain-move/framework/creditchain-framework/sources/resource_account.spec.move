spec creditchain_framework::resource_account {
    /// <high-level-req>
    /// No.: 1
    ...
    spec create_resource_account_and_fund(
        ...
        let source_addr = signer::address_of(origin);
        let resource_addr = account::spec_create_resource_address(source_addr, seed);
        let coin_store_resource = global<coin::CoinStore<CreditChainCoin>>(resource_addr);

        include creditchain_account::WithdrawAbortsIf<CreditChainCoin>{from: origin, amount: fund_amount};
        include creditchain_account::GuidAbortsIf<CreditChainCoin>{to: resource_addr};
        include RotateAccountAuthenticationKeyAndStoreCapabilityAbortsIfWithoutAccountLimit;

        // TODO(fa_migration)
        //aborts_if !coin::spec_is_account_registered<CreditChainCoin>(resource_addr) && coin_store_resource.frozen;
        /// [high-level-req-3]
        ensures exists<creditchain_framework::coin::CoinStore<CreditChainCoin>>(resource_addr);
    }
    ...
    spec rotate_account_authentication_key_and_store_capability(
        ...
        ensures vector::length(optional_auth_key) != 0 ==>
            global<creditchain_framework::account::Account>(resource_addr).authentication_key == optional_auth_key;
    }
