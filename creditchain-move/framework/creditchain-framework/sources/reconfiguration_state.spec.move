spec creditchain_framework::reconfiguration_state {

    spec module {
        use creditchain_framework::chain_status;
        invariant [suspendable] chain_status::is_operating() ==> exists<State>(@creditchain_framework);
    }

    spec initialize(fx: &signer) {
        use std::signer;
        use creditchain_std::from_bcs;
        aborts_if signer::address_of(fx) != @creditchain_framework;
        let post post_state = global<State>(@creditchain_framework);
        ensures exists<State>(@creditchain_framework);
        ensures !exists<State>(@creditchain_framework) ==> from_bcs::deserializable<StateInactive>(post_state.variant.data);
    }
    ...
    spec fun spec_is_in_progress(): bool {
        if (!exists<State>(@creditchain_framework)) {
            false
        } else {
            copyable_any::type_name(global<State>(@creditchain_framework).variant).bytes == b"0x1::reconfiguration_state::StateActive"
        }
    }
    ...
    spec on_reconfig_start {
        ...
        let pre_state = global<State>(@creditchain_framework);
        let post post_state = global<State>(@creditchain_framework);
        ensures (exists<State>(@creditchain_framework) && copyable_any::type_name(pre_state.variant).bytes
    ...
    spec fun spec_start_time_secs(): u64 {
        use creditchain_std::from_bcs;
        let state = global<State>(@creditchain_framework);
        from_bcs::deserialize<StateActive>(state.variant.data).start_time_secs
    }

    spec schema StartTimeSecsRequirement {
        requires exists<State>(@creditchain_framework);
        requires copyable_any::type_name(global<State>(@creditchain_framework).variant).bytes
            == b"0x1::reconfiguration_state::StateActive";
        include UnpackRequiresStateActive {
            x:  global<State>(@creditchain_framework).variant
        };
    }
    ...
    spec schema StartTimeSecsAbortsIf {
        aborts_if !exists<State>(@creditchain_framework);
        include  copyable_any::type_name(global<State>(@creditchain_framework).variant).bytes
            == b"0x1::reconfiguration_state::StateActive" ==>
        copyable_any::UnpackAbortsIf<StateActive> {
            self: global<State>(@creditchain_framework).variant
        };
        aborts_if copyable_any::type_name(global<State>(@creditchain_framework).variant).bytes
            != b"0x1::reconfiguration_state::StateActive";
    }
