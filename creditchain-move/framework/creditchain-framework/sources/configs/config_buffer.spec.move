spec creditchain_framework::config_buffer {
    spec module {
        pragma verify = true;
    }
    ...
    spec fun spec_fun_does_exist<T: store>(type_name: String): bool {
        if (exists<PendingConfigs>(@creditchain_framework)) {
            let config = global<PendingConfigs>(@creditchain_framework);
    ...
    spec upsert<T: drop + store>(config: T) {
        aborts_if !exists<PendingConfigs>(@creditchain_framework);
    }
    ...
    spec schema ExtractAbortsIf<T> {
        let configs = global<PendingConfigs>(@creditchain_framework);
    ...
    spec schema SetForNextEpochAbortsIf {
        ...
        aborts_if account_addr != @creditchain_framework;
