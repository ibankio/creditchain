...
module creditchain_framework::randomness_config {
    use std::string;
    use creditchain_std::copyable_any;
    use creditchain_std::copyable_any::Any;
    use creditchain_std::fixed_point64::FixedPoint64;
    ...
    public fun initialize(framework: &signer, config: RandomnessConfig) {
        ...
        if (!exists<RandomnessConfig>(@creditchain_framework)) {
    ...
    public(friend) fun on_new_epoch(framework: &signer) acquires RandomnessConfig {
        ...
        if (config_buffer::does_exist<RandomnessConfig>()) {
            ...
            if (exists<RandomnessConfig>(@creditchain_framework)) {
                *borrow_global_mut<RandomnessConfig>(@creditchain_framework) = new_config;
            } else {
    ...
    public fun enabled(): bool acquires RandomnessConfig {
        if (exists<RandomnessConfig>(@creditchain_framework)) {
            let config = borrow_global<RandomnessConfig>(@creditchain_framework);
    ...
    public fun current(): RandomnessConfig acquires RandomnessConfig {
        if (exists<RandomnessConfig>(@creditchain_framework)) {
            *borrow_global<RandomnessConfig>(@creditchain_framework)
        } else {
            new_off()
        }
    }
