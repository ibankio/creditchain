spec creditchain_framework::execution_config {
    spec module {
        pragma verify = true;
    ...
    spec set(account: &signer, config: vector<u8>) {
        ...
        requires system_addresses::is_creditchain_framework_address(addr);
        requires timestamp::spec_now_microseconds() >= reconfiguration::last_reconfiguration_time();

        ensures exists<ExecutionConfig>(@creditchain_framework);
    }
