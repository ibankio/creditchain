...
module creditchain_framework::gas_schedule {
    use std::bcs;
    use std::error;
    ...
    use creditchain_framework::util::from_bytes;
    use creditchain_framework::storage_gas::StorageGasConfig;
    ...
    public(friend) fun initialize(creditchain_framework: &signer, gas_schedule_blob: vector<u8>) {
        system_addresses::assert_creditchain_framework(creditchain_framework);
        assert!(!vector::is_empty(&gas_schedule_blob), error::invalid_argument(EINVALID_GAS_SCHEDULE));

        // TODO(Gas): check if gas schedule is consistent
        let gas_schedule: GasScheduleV2 = from_bytes(gas_schedule_blob);
        move_to<GasScheduleV2>(creditchain_framework, gas_schedule);
    }
    ...
    public fun set_gas_schedule(creditchain_framework: &signer, gas_schedule_blob: vector<u8>) acquires GasSchedule, GasScheduleV2 {
        system_addresses::assert_creditchain_framework(creditchain_framework);
        assert!(!vector::is_empty(&gas_schedule_blob), error::invalid_argument(EINVALID_GAS_SCHEDULE));
        chain_status::assert_genesis();

        if (exists<GasScheduleV2>(@creditchain_framework)) {
            let gas_schedule = borrow_global_mut<GasScheduleV2>(@creditchain_framework);
            ...
            *gas_schedule = new_gas_schedule;
            ...
            if (exists<GasSchedule>(@creditchain_framework)) {
                _ = move_from<GasSchedule>(@creditchain_framework);
            };
            let new_gas_schedule: GasScheduleV2 = from_bytes(gas_schedule_blob);
            // TODO(Gas): check if gas schedule is consistent
            move_to<GasScheduleV2>(creditchain_framework, new_gas_schedule);
        };
    ...
    /// ```
    public fun set_for_next_epoch(creditchain_framework: &signer, gas_schedule_blob: vector<u8>) acquires GasScheduleV2 {
        system_addresses::assert_creditchain_framework(creditchain_framework);
        assert!(!vector::is_empty(&gas_schedule_blob), error::invalid_argument(EINVALID_GAS_SCHEDULE));
        let new_gas_schedule: GasScheduleV2 = from_bytes(gas_schedule_blob);
        if (exists<GasScheduleV2>(@creditchain_framework)) {
            let cur_gas_schedule = borrow_global<GasScheduleV2>(@creditchain_framework);
            assert!(
    ...
    public fun set_for_next_epoch_check_hash(
        ...
        if (exists<GasScheduleV2>(@creditchain_framework)) {
            ...
            let cur_gas_schedule_hash = creditchain_hash::sha3_512(cur_gas_schedule_bytes);
            assert!(
    ...
    public(friend) fun on_new_epoch(framework: &signer) acquires GasScheduleV2 {
        ...
        if (config_buffer::does_exist<GasScheduleV2>()) {
            ...
            if (exists<GasScheduleV2>(@creditchain_framework)) {
                *borrow_global_mut<GasScheduleV2>(@creditchain_framework) = new_gas_schedule;
            } else {
    ...
    public fun set_storage_gas_config(creditchain_framework: &signer, config: StorageGasConfig) {
        storage_gas::set_config(creditchain_framework, config);
        // Need to trigger reconfiguration so the VM is guaranteed to load the new gas fee starting from the next
        // transaction.
        reconfiguration::reconfigure();
    }

    public fun set_storage_gas_config_for_next_epoch(creditchain_framework: &signer, config: StorageGasConfig) {
        storage_gas::set_config(creditchain_framework, config);
    }
