...
module creditchain_framework::jwk_consensus_config {
    use std::error;
    use std::option;
    ...
    public fun initialize(framework: &signer, config: JWKConsensusConfig) {
        ...
        if (!exists<JWKConsensusConfig>(@creditchain_framework)) {
    ...
    public(friend) fun on_new_epoch(framework: &signer) acquires JWKConsensusConfig {
        ...
        if (config_buffer::does_exist<JWKConsensusConfig>()) {
            ...
            if (exists<JWKConsensusConfig>(@creditchain_framework)) {
                *borrow_global_mut<JWKConsensusConfig>(@creditchain_framework) = new_config;
            } else {
    ...
    fun enabled(): bool acquires JWKConsensusConfig {
        let variant= borrow_global<JWKConsensusConfig>(@creditchain_framework).variant;
