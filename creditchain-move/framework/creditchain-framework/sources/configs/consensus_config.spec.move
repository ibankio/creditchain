spec creditchain_framework::consensus_config {
    /// <high-level-req>
    /// No.: 1
    /// Requirement: During genesis, the CreditChain framework account should be assigned the consensus config resource.
    /// Criticality: Medium
    /// Implementation: The consensus_config::initialize function calls the assert_creditchain_framework function to ensure
    /// that the signer is the creditchain_framework and then assigns the ConsensusConfig resource to it.
    /// Enforcement: Formally verified via [high-level-req-1](initialize).
    ///
    /// No.: 2
    /// Requirement: Only libra2 framework account is allowed to update the consensus configuration.
    /// Criticality: Medium
    /// Implementation: The consensus_config::set function ensures that the signer is creditchain_framework.
    ...
    spec module {
        ...
        invariant [suspendable] chain_status::is_operating() ==> exists<ConsensusConfig>(@creditchain_framework);
    }

    /// Ensure caller is admin.
    /// Aborts if StateStorageUsage already exists.
    spec initialize(creditchain_framework: &signer, config: vector<u8>) {
        use std::signer;
        let addr = signer::address_of(creditchain_framework);
        /// [high-level-req-1]
        aborts_if !system_addresses::is_creditchain_framework_address(addr);
        aborts_if exists<ConsensusConfig>(@creditchain_framework);
        /// [high-level-req-3.1]
    ...
    spec set(account: &signer, config: vector<u8>) {
        ...
        ensures global<ConsensusConfig>(@creditchain_framework).config == config;
    }
    ...
    spec validator_txn_enabled(): bool {
        pragma opaque;
        aborts_if !exists<ConsensusConfig>(@creditchain_framework);
        ensures [abstract] result == spec_validator_txn_enabled_internal(global<ConsensusConfig>(@creditchain_framework).config);
    }
