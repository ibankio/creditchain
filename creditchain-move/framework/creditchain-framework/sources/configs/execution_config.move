...
module creditchain_framework::execution_config {
    ...
    public fun set(account: &signer, config: vector<u8>) acquires ExecutionConfig {
        ...
        if (exists<ExecutionConfig>(@creditchain_framework)) {
            let config_ref = &mut borrow_global_mut<ExecutionConfig>(@creditchain_framework).config;
            *config_ref = config;
        } else {
    ...
    public(friend) fun on_new_epoch(framework: &signer) acquires ExecutionConfig {
        ...
        if (config_buffer::does_exist<ExecutionConfig>()) {
            ...
            if (exists<ExecutionConfig>(@creditchain_framework)) {
                *borrow_global_mut<ExecutionConfig>(@creditchain_framework) = config;
            } else {
