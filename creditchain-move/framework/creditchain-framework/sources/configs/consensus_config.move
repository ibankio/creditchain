...
module creditchain_framework::consensus_config {
    use std::error;
    use std::vector;
    ...
    public(friend) fun initialize(creditchain_framework: &signer, config: vector<u8>) {
        system_addresses::assert_creditchain_framework(creditchain_framework);
        assert!(vector::length(&config) > 0, error::invalid_argument(EINVALID_CONFIG));
        move_to(creditchain_framework, ConsensusConfig { config });
    }
    ...
    public fun set(account: &signer, config: vector<u8>) acquires ConsensusConfig {
        ...
        let config_ref = &mut borrow_global_mut<ConsensusConfig>(@creditchain_framework).config;
        *config_ref = config;
    ...
    public(friend) fun on_new_epoch(framework: &signer) acquires ConsensusConfig {
        ...
        if (config_buffer::does_exist<ConsensusConfig>()) {
            ...
            if (exists<ConsensusConfig>(@creditchain_framework)) {
                *borrow_global_mut<ConsensusConfig>(@creditchain_framework) = new_config;
            } else {
    ...
    public fun validator_txn_enabled(): bool acquires ConsensusConfig {
        let config_bytes = borrow_global<ConsensusConfig>(@creditchain_framework).config;
