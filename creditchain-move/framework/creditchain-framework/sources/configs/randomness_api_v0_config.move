module creditchain_framework::randomness_api_v0_config {
    ...
    public fun on_new_epoch(framework: &signer) acquires RequiredGasDeposit, AllowCustomMaxGasFlag {
        ...
        if (config_buffer::does_exist<RequiredGasDeposit>()) {
            ...
            if (exists<RequiredGasDeposit>(@creditchain_framework)) {
                *borrow_global_mut<RequiredGasDeposit>(@creditchain_framework) = new_config;
            } else {
        ...
        if (config_buffer::does_exist<AllowCustomMaxGasFlag>()) {
            ...
            if (exists<AllowCustomMaxGasFlag>(@creditchain_framework)) {
                *borrow_global_mut<AllowCustomMaxGasFlag>(@creditchain_framework) = new_config;
            } else {
                move_to(framework, new_config);
            }
        }
