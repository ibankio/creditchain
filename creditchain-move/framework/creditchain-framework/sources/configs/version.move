...
module creditchain_framework::version {
    use std::error;
    use std::signer;
    ...
    public(friend) fun initialize(creditchain_framework: &signer, initial_version: u64) {
        system_addresses::assert_creditchain_framework(creditchain_framework);

        move_to(creditchain_framework, Version { major: initial_version });
        ...
        move_to(creditchain_framework, SetVersionCapability {});
    }
    ...
    public entry fun set_version(account: &signer, major: u64) acquires Version {
        ...
        let old_major = borrow_global<Version>(@creditchain_framework).major;
        assert!(old_major < major, error::invalid_argument(EINVALID_MAJOR_VERSION_NUMBER));

        let config = borrow_global_mut<Version>(@creditchain_framework);
        config.major = major;
    ...
    /// Example usage:
    /// - `creditchain_framework::version::set_for_next_epoch(&framework_signer, new_version);`
    ...
    public(friend) fun on_new_epoch(framework: &signer) acquires Version {
        ...
        if (config_buffer::does_exist<Version>()) {
            ...
            if (exists<Version>(@creditchain_framework)) {
                *borrow_global_mut<Version>(@creditchain_framework) = new_value;
            } else {
    ...
    public entry fun test_set_version(creditchain_framework: signer) acquires Version {
        initialize(&creditchain_framework, 1);
        assert!(borrow_global<Version>(@creditchain_framework).major == 1, 0);
        set_version(&creditchain_framework, 2);
        assert!(borrow_global<Version>(@creditchain_framework).major == 2, 1);
    }

    #[test(creditchain_framework = @creditchain_framework, core_resources = @core_resources)]
    ...
    #[test(creditchain_framework = @creditchain_framework, random_account = @0x123)]
