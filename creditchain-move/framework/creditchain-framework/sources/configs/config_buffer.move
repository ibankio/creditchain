...
module creditchain_framework::config_buffer {
    use std::error;
    use std::string::String;
    use creditchain_std::any;
    use creditchain_std::any::Any;
    use creditchain_std::simple_map;
    use creditchain_std::simple_map::SimpleMap;
    ...
    friend creditchain_framework::jwks;
    friend creditchain_framework::jwk_consensus_config;
    friend creditchain_framework::keyless_account;
    friend creditchain_framework::randomness_api_v0_config;
    friend creditchain_framework::randomness_config;
    friend creditchain_framework::randomness_config_seqnum;
    ...
    public fun initialize(creditchain_framework: &signer) {
        system_addresses::assert_creditchain_framework(creditchain_framework);
        if (!exists<PendingConfigs>(@creditchain_framework)) {
            move_to(creditchain_framework, PendingConfigs {
    ...
    public fun does_exist<T: store>(): bool acquires PendingConfigs {
        if (exists<PendingConfigs>(@creditchain_framework)) {
            let config = borrow_global<PendingConfigs>(@creditchain_framework);
    ...
    public(friend) fun upsert<T: drop + store>(config: T) acquires PendingConfigs {
        let configs = borrow_global_mut<PendingConfigs>(@creditchain_framework);
