...
module creditchain_framework::randomness_config_seqnum {
    ...
    public fun initialize(framework: &signer) {
        system_addresses::assert_creditchain_framework(framework);
        if (!exists<RandomnessConfigSeqNum>(@creditchain_framework)) {
    ...
    public(friend) fun on_new_epoch(framework: &signer) acquires RandomnessConfigSeqNum {
        ...
        if (config_buffer::does_exist<RandomnessConfigSeqNum>()) {
            ...
            if (exists<RandomnessConfigSeqNum>(@creditchain_framework)) {
                *borrow_global_mut<RandomnessConfigSeqNum>(@creditchain_framework) = new_config;
            } else {
