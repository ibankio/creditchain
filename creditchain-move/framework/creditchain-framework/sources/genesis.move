module creditchain_framework::genesis {
    use std::error;
    ...
    use creditchain_framework::chain_id;
    ...
    use creditchain_framework::create_signer::create_signer;
    use creditchain_framework::gas_schedule;
    use creditchain_framework::nonce_validation;
    use creditchain_framework::reconfiguration;
    use creditchain_framework::stake;
    use creditchain_framework::staking_contract;
    use creditchain_framework::staking_config;
    use creditchain_framework::state_storage;
    use creditchain_framework::storage_gas;
    use creditchain_framework::timestamp;
    use creditchain_framework::transaction_fee;
    use creditchain_framework::transaction_validation;
    use creditchain_framework::version;
    use creditchain_framework::vesting;
    ...
    fun initialize(
        ...
        // to use this account.
        let (creditchain_framework_account, creditchain_framework_signer_cap) = account::create_framework_reserved_account(@creditchain_framework);
        // Initialize account configs on libra2 framework account.
        account::initialize(&creditchain_framework_account);
        ...
        creditchain_governance::store_signer_cap(&creditchain_framework_account, @creditchain_framework, creditchain_framework_signer_cap);
        ...
        while (!vector::is_empty(&framework_reserved_addresses)) {
            ...
            creditchain_governance::store_signer_cap(&creditchain_framework_account, address, framework_signer_cap);
        };

        consensus_config::initialize(&creditchain_framework_account, consensus_config);
        execution_config::set(&creditchain_framework_account, execution_config);
        version::initialize(&creditchain_framework_account, initial_version);
        stake::initialize(&creditchain_framework_account);
        stake::initialize_pending_transaction_fee(&creditchain_framework_account);
        timestamp::set_time_has_started(&creditchain_framework_account);
        ...
        storage_gas::initialize(&creditchain_framework_account);
        gas_schedule::initialize(&creditchain_framework_account, gas_schedule);

        // Ensure we can create aggregators for supply, but not enable it for common use just yet.
        aggregator_factory::initialize_aggregator_factory(&creditchain_framework_account);

        chain_id::initialize(&creditchain_framework_account, chain_id);
        reconfiguration::initialize(&creditchain_framework_account);
        block::initialize(&creditchain_framework_account, epoch_interval_microsecs);
        state_storage::initialize(&creditchain_framework_account);
        nonce_validation::initialize(&creditchain_framework_account);
    }

    /// Genesis step 2: Initialize CreditChain coin.
    fun initialize_creditchain_coin(creditchain_framework: &signer) {
        let (burn_cap, mint_cap) = creditchain_coin::initialize(creditchain_framework);

        coin::create_coin_conversion_map(creditchain_framework);
        coin::create_pairing<CreditChainCoin>(creditchain_framework);

        // Give stake module MintCapability<CreditChainCoin> so it can mint rewards.
        stake::store_creditchain_coin_mint_cap(creditchain_framework, mint_cap);
        // Give transaction_fee module BurnCapability<CreditChainCoin> so it can burn gas.
        transaction_fee::store_creditchain_coin_burn_cap(creditchain_framework, burn_cap);
        // Give transaction_fee module MintCapability<CreditChainCoin> so it can mint refunds.
        transaction_fee::store_creditchain_coin_mint_cap(creditchain_framework, mint_cap);
    }

    /// Only called for testnets and e2e tests.
    fun initialize_core_resources_and_creditchain_coin(
        ...
        creditchain_account::register_apt(&core_resources); // registers LBT store
        creditchain_coin::configure_accounts_for_test(creditchain_framework, &core_resources, mint_cap);
    }

    fun create_accounts(creditchain_framework: &signer, accounts: vector<AccountMap>) {
    ...
    fun create_account(creditchain_framework: &signer, account_address: address, balance: u64): signer {
        ...
        if (coin::balance<CreditChainCoin>(account_address) == 0) {
            coin::register<CreditChainCoin>(&account);
            creditchain_coin::mint(creditchain_framework, account_address, balance);
        };
        account
    }
    ...
    fun create_employee_validators(
        ...
        vector::for_each_ref(&employees, |employee_group| {
            ...
            while (j < num_employees_in_group) {
                ...
                let total = coin::balance<CreditChainCoin>(*account);
                let coins = coin::withdraw<CreditChainCoin>(&employee, total);
    ...
    fun create_initialize_validators_with_commission(
        ...
        vector::for_each_ref(&validators, |validator| {
            ...
            create_initialize_validator(creditchain_framework, validator, use_staking_contract);
        });

        // Destroy the libra2 framework account's ability to mint coins now that we're done with setting up the initial
        // validators.
        creditchain_coin::destroy_mint_cap(creditchain_framework);
    ...
    fun create_initialize_validators(creditchain_framework: &signer, validators: vector<ValidatorConfiguration>) {
        ...
        create_initialize_validators_with_commission(creditchain_framework, false, validators_with_commission);
    }
    ...
    fun create_initialize_validator(
        ...
        let owner = &create_account(creditchain_framework, validator.owner_address, validator.stake_amount);
        create_account(creditchain_framework, validator.operator_address, 0);
        create_account(creditchain_framework, validator.voter_address, 0);
    ...
    fun set_genesis_end(creditchain_framework: &signer) {
        chain_status::set_genesis_end(creditchain_framework);
    }
    ...
    fun initialize_for_verification(
        ...
        features::change_feature_flags_for_verification(creditchain_framework, vector[1, 2], vector[]);
        initialize_creditchain_coin(creditchain_framework);
        creditchain_governance::initialize_for_verification(
            creditchain_framework,
            min_voting_threshold,
            required_proposer_stake,
            voting_duration_secs
        );
        create_accounts(creditchain_framework, accounts);
        create_employee_validators(employee_vesting_start, employee_vesting_period_duration, employees);
        create_initialize_validators_with_commission(creditchain_framework, true, validators);
        set_genesis_end(creditchain_framework);
    }
    ...
    fun test_setup() {
        setup();
        assert!(account::exists_at(@creditchain_framework), 1);
    ...
    #[test(creditchain_framework = @0x1)]
    fun test_create_account(creditchain_framework: &signer) {
        setup();
        initialize_creditchain_coin(creditchain_framework);

        let addr = @0x121341; // 01 -> 0a are taken
        let test_signer_before = create_account(creditchain_framework, addr, 15);
        let test_signer_after = create_account(creditchain_framework, addr, 500);
        assert!(test_signer_before == test_signer_after, 0);
        assert!(coin::balance<CreditChainCoin>(addr) == 15, 1);
    }

    #[test(creditchain_framework = @0x1)]
    fun test_create_accounts(creditchain_framework: &signer) {
        setup();
        initialize_creditchain_coin(creditchain_framework);
        ...
        create_accounts(creditchain_framework, accounts);
        assert!(coin::balance<CreditChainCoin>(addr0) == 12345, 0);
        assert!(coin::balance<CreditChainCoin>(addr1) == 67890, 1);

        create_account(creditchain_framework, addr0, 23456);
        assert!(coin::balance<CreditChainCoin>(addr0) == 12345, 2);
    }

    #[test(creditchain_framework = @0x1, root = @0xabcd)]
    fun test_create_root_account(creditchain_framework: &signer) {
        use creditchain_framework::aggregator_factory;
        ...
        features::change_feature_flags_for_testing(creditchain_framework, vector[feature], vector[]);

        aggregator_factory::initialize_aggregator_factory_for_test(creditchain_framework);

        let (burn_cap, mint_cap) = creditchain_coin::initialize(creditchain_framework);
        creditchain_coin::ensure_initialized_with_apt_fa_metadata_for_test();

        let core_resources = account::create_account(@core_resources);
        creditchain_account::register_apt(&core_resources); // registers LBT store

        let apt_metadata = object::address_to_object<Metadata>(@creditchain_fungible_asset);
