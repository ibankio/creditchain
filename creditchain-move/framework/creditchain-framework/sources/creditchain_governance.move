...
module creditchain_framework::creditchain_governance {
    use std::error;
    use std::option;
    use std::signer;
    ...
    use creditchain_std::simple_map::{Self, SimpleMap};
    use creditchain_std::smart_table::{Self, SmartTable};
    use creditchain_std::table::{Self, Table};

    use creditchain_framework::account::{Self, SignerCapability, create_signer_with_capability};
    use creditchain_framework::coin;
    use creditchain_framework::event::{Self, EventHandle};
    use creditchain_framework::governance_proposal::{Self, GovernanceProposal};
    ...
    use creditchain_framework::reconfiguration_with_dkg;
    use creditchain_framework::timestamp;
    use creditchain_framework::voting;
    ...
    public fun store_signer_cap(
        ...
        if (!exists<GovernanceResponsbility>(@creditchain_framework)) {
            move_to(
        ...
        let signer_caps = &mut borrow_global_mut<GovernanceResponsbility>(@creditchain_framework).signer_caps;
    ...
    fun initialize(
        ...
        voting::register<GovernanceProposal>(creditchain_framework);
        initialize_partial_voting(creditchain_framework);
        move_to(creditchain_framework, GovernanceConfig {
            voting_duration_secs,
            min_voting_threshold,
            required_proposer_stake,
        });
        move_to(creditchain_framework, GovernanceEvents {
            create_proposal_events: account::new_event_handle<CreateProposalEvent>(creditchain_framework),
            update_config_events: account::new_event_handle<UpdateConfigEvent>(creditchain_framework),
            vote_events: account::new_event_handle<VoteEvent>(creditchain_framework),
        });
        move_to(creditchain_framework, VotingRecords {
            votes: table::new(),
        });
        move_to(creditchain_framework, ApprovedExecutionHashes {
    ...
    public fun update_governance_config(
        ...
        let governance_config = borrow_global_mut<GovernanceConfig>(@creditchain_framework);
        ...
        if (std::features::module_event_migration_enabled()) {
            ...
            let events = borrow_global_mut<GovernanceEvents>(@creditchain_framework);
    ...
    public fun initialize_partial_voting(
        ...
        move_to(creditchain_framework, VotingRecordsV2 {
    ...
    public fun get_voting_duration_secs(): u64 acquires GovernanceConfig {
        borrow_global<GovernanceConfig>(@creditchain_framework).voting_duration_secs
    }

    #[view]
    public fun get_min_voting_threshold(): u128 acquires GovernanceConfig {
        borrow_global<GovernanceConfig>(@creditchain_framework).min_voting_threshold
    }

    #[view]
    public fun get_required_proposer_stake(): u64 acquires GovernanceConfig {
        borrow_global<GovernanceConfig>(@creditchain_framework).required_proposer_stake
    }
    ...
    public fun has_entirely_voted(stake_pool: address, proposal_id: u64): bool acquires VotingRecords {
        ...
        let voting_records = borrow_global<VotingRecords>(@creditchain_framework);
    ...
    public fun get_remaining_voting_power(
        ...
        let used_voting_power = *VotingRecordsV2[@creditchain_framework].votes.borrow_with_default(record_key, &0);
    ...
    public fun create_proposal_v2_impl(
        ...
        let governance_config = borrow_global<GovernanceConfig>(@creditchain_framework);
        ...
        let total_voting_token_supply = coin::supply<CreditChainCoin>();
    ...
    fun vote_internal(
        ...
        let used_voting_power = VotingRecordsV2[@creditchain_framework].votes.borrow_mut_with_default(record_key, 0);
        ...
        *used_voting_power += voting_power;
        ...
        if (std::features::module_event_migration_enabled()) {
            ...
            let events = &mut GovernanceEvents[@creditchain_framework];
            event::emit_event(
        ...
        let proposal_state = voting::get_proposal_state<GovernanceProposal>(@creditchain_framework, proposal_id);
    ...
    public fun add_approved_script_hash(proposal_id: u64) acquires ApprovedExecutionHashes {
        let approved_hashes = borrow_global_mut<ApprovedExecutionHashes>(@creditchain_framework);
        ...
        let execution_hash = voting::get_execution_hash<GovernanceProposal>(@creditchain_framework, proposal_id);
    ...
    public fun resolve(
        ...
        voting::resolve<GovernanceProposal>(@creditchain_framework, proposal_id);
    ...
    public fun resolve_multi_step_proposal(
        ...
        voting::resolve_proposal_v2<GovernanceProposal>(@creditchain_framework, proposal_id, next_execution_hash);
    ...
    public fun remove_approved_hash(proposal_id: u64) acquires ApprovedExecutionHashes {
        assert!(
            voting::is_resolved<GovernanceProposal>(@creditchain_framework, proposal_id),
            error::invalid_argument(EPROPOSAL_NOT_RESOLVED_YET),
        );

        let approved_hashes = &mut borrow_global_mut<ApprovedExecutionHashes>(@creditchain_framework).hashes;
    ...
    public entry fun reconfigure(creditchain_framework: &signer) {
        system_addresses::assert_creditchain_framework(creditchain_framework);
        if (consensus_config::validator_txn_enabled() && randomness_config::enabled()) {
            reconfiguration_with_dkg::try_start();
        } else {
            reconfiguration_with_dkg::finish(creditchain_framework);
        }
    }
    ...
    public entry fun force_end_epoch(creditchain_framework: &signer) {
        system_addresses::assert_creditchain_framework(creditchain_framework);
        reconfiguration_with_dkg::finish(creditchain_framework);
    }
    ...
    public entry fun force_end_epoch_test_only(creditchain_framework: &signer) acquires GovernanceResponsbility {
        let core_signer = get_signer_testnet_only(creditchain_framework, @0x1);
        system_addresses::assert_creditchain_framework(&core_signer);
        reconfiguration_with_dkg::finish(&core_signer);
    }

    /// Update feature flags and also trigger reconfiguration.
    public fun toggle_features(creditchain_framework: &signer, enable: vector<u64>, disable: vector<u64>) {
        system_addresses::assert_creditchain_framework(creditchain_framework);
        features::change_feature_flags_for_next_epoch(creditchain_framework, enable, disable);
        reconfigure(creditchain_framework);
    }
    ...
    public fun get_signer_testnet_only(
        ...
        assert!(creditchain_coin::has_mint_capability(core_resources), error::unauthenticated(EUNAUTHORIZED));
        get_signer(signer_address)
    }
    ...
    fun get_signer(signer_address: address): signer acquires GovernanceResponsbility {
        let governance_responsibility = borrow_global<GovernanceResponsbility>(@creditchain_framework);
    ...
    fun assert_voting_initialization() {
        assert!(exists<VotingRecordsV2>(@creditchain_framework), error::invalid_state(EPARTIAL_VOTING_NOT_INITIALIZED));
    }
    ...
    public entry fun test_voting_generic(
        ...
        setup_partial_voting(&creditchain_framework, &proposer, &yes_voter, &no_voter);
        ...
        test_resolving_proposal_generic(creditchain_framework, use_generic_resolve_function, execution_hash);
    }
    ...
    public entry fun test_resolving_proposal_generic(
        ...
        let proposal_state = voting::get_proposal_state<GovernanceProposal>(signer::address_of(&creditchain_framework), 0);
        assert!(proposal_state == PROPOSAL_STATE_SUCCEEDED, proposal_state);

        // Add approved script hash.
        add_approved_script_hash(0);
        let approved_hashes = borrow_global<ApprovedExecutionHashes>(@creditchain_framework).hashes;
        assert!(*simple_map::borrow(&approved_hashes, &0) == execution_hash, 0);

        // Resolve the proposal.
        let account = resolve_proposal_for_test(0, @creditchain_framework, use_generic_resolve_function, true);
        assert!(signer::address_of(&account) == @creditchain_framework, 1);
        assert!(voting::is_resolved<GovernanceProposal>(@creditchain_framework, 0), 2);
        let approved_hashes = borrow_global<ApprovedExecutionHashes>(@creditchain_framework).hashes;
        assert!(!simple_map::contains_key(&approved_hashes, &0), 3);
    }

    #[test(creditchain_framework = @creditchain_framework, proposer = @0x123, yes_voter = @0x234, no_voter = @345)]
    public entry fun test_voting(
        ...
        test_voting_generic(creditchain_framework, proposer, yes_voter, no_voter, false, false);
    }
    ...
    public entry fun test_voting_multi_step(
        ...
        test_voting_generic(creditchain_framework, proposer, yes_voter, no_voter, true, true);
    }

    #[test(creditchain_framework = @creditchain_framework, proposer = @0x123, yes_voter = @0x234, no_voter = @345)]
    #[expected_failure(abort_code = 0x5000a, location = creditchain_framework::voting)]
    public entry fun test_voting_multi_step_cannot_use_single_step_resolve(
        ...
        test_voting_generic(creditchain_framework, proposer, yes_voter, no_voter, true, false);
    }
    ...
    public entry fun test_voting_single_step_can_use_generic_resolve_function(
        ...
        test_voting_generic(creditchain_framework, proposer, yes_voter, no_voter, false, true);
    }
    ...
    public entry fun test_can_remove_approved_hash_if_executed_directly_via_voting_generic(
        ...
        if (multi_step) {
            ...
            vector::push_back(&mut execution_hash, 1);
            voting::resolve_proposal_v2<GovernanceProposal>(@creditchain_framework, 0, next_execution_hash);
            assert!(voting::is_resolved<GovernanceProposal>(@creditchain_framework, 0), 0);
            if (vector::length(&next_execution_hash) == 0) {
            ...
            voting::resolve<GovernanceProposal>(@creditchain_framework, 0);
            assert!(voting::is_resolved<GovernanceProposal>(@creditchain_framework, 0), 0);
            remove_approved_hash(0);
        };
        let approved_hashes = borrow_global<ApprovedExecutionHashes>(@creditchain_framework).hashes;
    ...
    #[test(creditchain_framework = @creditchain_framework, proposer = @0x123, voter_1 = @0x234, voter_2 = @345)]
    #[expected_failure(abort_code = 65541, location = creditchain_framework::creditchain_governance)]
    public entry fun test_cannot_double_vote(
        ...
        setup_voting(&creditchain_framework, &proposer, &voter_1, &voter_2);
    ...
    #[test(creditchain_framework = @creditchain_framework, proposer = @0x123, voter_1 = @0x234, voter_2 = @345)]
    #[expected_failure(abort_code = 65551, location = creditchain_framework::creditchain_governance)]
    public entry fun test_cannot_vote_for_expired_proposal(
        ...
        setup_partial_voting_with_initialized_stake(&creditchain_framework, &proposer, &voter_1, &voter_2);
    ...
    #[test(creditchain_framework = @creditchain_framework, proposer = @0x123, voter_1 = @0x234, voter_2 = @0x345)]
    #[expected_failure(abort_code = 65539, location = creditchain_framework::creditchain_governance)]
    ...
    public entry fun test_stake_pool_can_vote_on_partial_voting_proposal_many_times(
        ...
        setup_partial_voting(&creditchain_framework, &proposer, &voter_1, &voter_2);
        let execution_hash = vector[1];
        ...
        test_resolving_proposal_generic(creditchain_framework, true, execution_hash);
    }
    ...
    public entry fun test_batch_vote(
        ...
        features::change_feature_flags_for_testing(&creditchain_framework, vector[features::get_coin_to_fungible_asset_migration_feature()], vector[]);
        setup_partial_voting(&creditchain_framework, &proposer, &voter_1, &voter_2);
        let execution_hash = vector[1];
        let voter_1_addr = signer::address_of(&voter_1);
        let voter_2_addr = signer::address_of(&voter_2);
    ...
    public fun setup_voting(
        ...
        account::create_account_for_test(signer::address_of(creditchain_framework));
        ...
        staking_config::initialize_for_test(creditchain_framework, 0, 1000, 2000, true, 0, 1, 100);
        initialize(creditchain_framework, 10, 100, 1000);
        store_signer_cap(
            creditchain_framework,
            @creditchain_framework,
            account::create_test_signer_cap(@creditchain_framework),
        );
        ...
        stake::create_validator_set(creditchain_framework, active_validators, pks);
        ...
        coin::destroy_mint_cap<CreditChainCoin>(mint_cap);
    ...
    public fun setup_voting_with_initialized_stake(
        ...
        // Initialize the governance.
        stake::initialize_for_test_custom(creditchain_framework, 0, 1000, 2000, true, 0, 1, 1000);
        initialize(creditchain_framework, 10, 100, 1000);
        store_signer_cap(
            creditchain_framework,
            @creditchain_framework,
    ...
    public fun setup_partial_voting_with_initialized_stake(
        ...
        setup_voting_with_initialized_stake(creditchain_framework, proposer, yes_voter, no_voter);
    }
    ...
    #[test(creditchain_framework = @creditchain_framework)]
    public entry fun test_update_governance_config(
        creditchain_framework: signer,
    ) acquires GovernanceConfig, GovernanceEvents {
        account::create_account_for_test(signer::address_of(&creditchain_framework));
        initialize(&creditchain_framework, 1, 2, 3);
        update_governance_config(&creditchain_framework, 10, 20, 30);

        let config = borrow_global<GovernanceConfig>(@creditchain_framework);
    ...
    #[test(account = @0x123)]
    #[expected_failure(abort_code = 0x50003, location = creditchain_framework::system_addresses)]
    ...
    public fun initialize_for_test(
        ...
        initialize(creditchain_framework, min_voting_threshold, required_proposer_stake, voting_duration_secs);
    }
