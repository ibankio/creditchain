spec creditchain_framework::staking_contract {
    /// <high-level-req>
    /// No.: 1
    ...
    contract_creation_seed: vector<u8>,
        ...
        include WithdrawAbortsIf<CreditChainCoin> { account: staker };
    ...
    spec add_stake(staker: &signer, operator: address, amount: u64) {
        ...
        include WithdrawAbortsIf<CreditChainCoin> { account: staker };
        let balance = global<coin::CoinStore<CreditChainCoin>>(staker_address).coin.value;
        let post post_coin = global<coin::CoinStore<CreditChainCoin>>(staker_address).coin.value;
    ...
    spec schema PreconditionsInCreateContract {
        requires exists<stake::ValidatorPerformance>(@creditchain_framework);
        requires exists<stake::ValidatorSet>(@creditchain_framework);
        requires exists<staking_config::StakingRewardsConfig>(
            @creditchain_framework
        ) || !std::features::spec_periodical_reward_rate_decrease_enabled();
        requires exists<creditchain_framework::timestamp::CurrentTimeMicroseconds>(@creditchain_framework);
    ...
    spec schema CreateStakePoolAbortsIf {
        ...
        let allowed = global<stake::AllowedValidators>(@creditchain_framework);
        aborts_if exists<stake::AllowedValidators>(@creditchain_framework) && !contains(allowed.accounts, resource_addr);
