spec creditchain_framework::creditchain_governance {
    /// <high-level-req>
    /// No.: 1
    ...
    spec store_signer_cap(
        ...
        aborts_if !system_addresses::is_creditchain_framework_address(signer::address_of(creditchain_framework));
        aborts_if !system_addresses::is_framework_reserved_address(signer_address);

        let signer_caps = global<GovernanceResponsbility>(@creditchain_framework).signer_caps;
        aborts_if exists<GovernanceResponsbility>(@creditchain_framework) &&
            simple_map::spec_contains_key(signer_caps, signer_address);
        ensures exists<GovernanceResponsbility>(@creditchain_framework);
        let post post_signer_caps = global<GovernanceResponsbility>(@creditchain_framework).signer_caps;
    ...
    spec initialize(
        ...
        use creditchain_std::type_info::Self;
        pragma aborts_if_is_partial;
    ...
    spec initialize_partial_voting(
        ...
        aborts_if exists<VotingRecordsV2>(@creditchain_framework);
        ensures exists<VotingRecordsV2>(@creditchain_framework);
    }
    ...
    spec update_governance_config(
        ...
        let addr = signer::address_of(creditchain_framework);
        let governance_config = global<GovernanceConfig>(@creditchain_framework);

        let post new_governance_config = global<GovernanceConfig>(@creditchain_framework);
        aborts_if addr != @creditchain_framework;
        aborts_if !exists<GovernanceConfig>(@creditchain_framework);
    ...
    spec toggle_features(
        ...
        requires exists<staking_config::StakingRewardsConfig>(@creditchain_framework);
    ...
    spec has_entirely_voted(stake_pool: address, proposal_id: u64): bool {
        aborts_if !exists<VotingRecords>(@creditchain_framework);
    }
    ...
    spec schema CreateProposalAbortsIf {
        ...
        let staking_config = global<staking_config::StakingConfig>(@creditchain_framework);
        ...
        let addr = creditchain_std::type_info::type_of<CreditChainCoin>().account_address;
        aborts_if !exists<coin::CoinInfo<CreditChainCoin>>(addr);
        let maybe_supply = global<coin::CoinInfo<CreditChainCoin>>(addr).supply;
        let supply = option::spec_borrow(maybe_supply);
        let total_supply = creditchain_framework::optional_aggregator::optional_aggregator_value(supply);
        ...
        aborts_if !exists<voting::VotingForum<GovernanceProposal>>(@creditchain_framework);
        let voting_forum = global<voting::VotingForum<GovernanceProposal>>(@creditchain_framework);
        let proposal_id = voting_forum.next_proposal_id;
        aborts_if proposal_id + 1 > MAX_U64;
        let post post_voting_forum = global<voting::VotingForum<GovernanceProposal>>(@creditchain_framework);
        ...
        aborts_if !exists<GovernanceEvents>(@creditchain_framework);
    }
    ...
    spec schema VoteAbortIf {
        ...
        aborts_if spec_proposal_expiration <= locked_until && !exists<timestamp::CurrentTimeMicroseconds>(@creditchain_framework);
        let spec_proposal_expiration = voting::spec_get_proposal_expiration_secs<GovernanceProposal>(@creditchain_framework, proposal_id);
        ...
        aborts_if !remain_zero_1_cond && !exists<VotingRecords>(@creditchain_framework);
        ...
        let voting_records_v2 = borrow_global<VotingRecordsV2>(@creditchain_framework);
        ...
        aborts_if !exists<VotingRecords>(@creditchain_framework);
        let voting_records = global<VotingRecords>(@creditchain_framework);


        // verify get_voting_power(stake_pool)
        let allow_validator_set_change = global<staking_config::StakingConfig>(@creditchain_framework).allow_validator_set_change;
        let stake_pool_res = global<stake::StakePool>(stake_pool);
        ...
        let post post_approved_hashes = global<ApprovedExecutionHashes>(@creditchain_framework);

        // Due to the complexity of the success state, the validation of 'borrow_global_mut<ApprovedExecutionHashes>(@creditchain_framework);' is discussed in four cases.
        /// [high-level-req-3]
        aborts_if
            if (should_pass) {
                proposal_state_successed_0 && !exists<ApprovedExecutionHashes>(@creditchain_framework)
            } else {
                proposal_state_successed_1 && !exists<ApprovedExecutionHashes>(@creditchain_framework)
            };
        aborts_if
            if (should_pass) {
                proposal_state_successed_2 && !exists<ApprovedExecutionHashes>(@creditchain_framework)
            } else {
                proposal_state_successed_3 && !exists<ApprovedExecutionHashes>(@creditchain_framework)
            };
    ...
    spec schema AddApprovedScriptHash {
        proposal_id: u64;
        aborts_if !exists<ApprovedExecutionHashes>(@creditchain_framework);
    ...
    spec resolve(proposal_id: u64, signer_address: address): signer {
        ...
        let is_multi_step_proposal = creditchain_std::from_bcs::deserialize<bool>(simple_map::spec_get(proposal.metadata, multi_step_key));
        aborts_if has_multi_step_key && !creditchain_std::from_bcs::deserializable<bool>(simple_map::spec_get(proposal.metadata, multi_step_key));
        aborts_if !string::spec_internal_check_utf8(voting::IS_MULTI_STEP_PROPOSAL_KEY);
        ...
        aborts_if !exists<ApprovedExecutionHashes>(@creditchain_framework);
        let post post_approved_hashes = global<ApprovedExecutionHashes>(@creditchain_framework).hashes;
        ensures !simple_map::spec_contains_key(post_approved_hashes, proposal_id);

        // verify get_signer
        include GetSignerAbortsIf;
        let governance_responsibility = global<GovernanceResponsbility>(@creditchain_framework);
    ...
    spec remove_approved_hash(proposal_id: u64) {
        ...
        let proposal = table::spec_get(voting_forum.proposals, proposal_id);
        aborts_if !proposal.is_resolved;
        let post approved_hashes = global<ApprovedExecutionHashes>(@creditchain_framework).hashes;
        ensures !simple_map::spec_contains_key(approved_hashes, proposal_id);
    }

    spec reconfigure(creditchain_framework: &signer) {
        use creditchain_framework::chain_status;
        use creditchain_framework::coin::CoinInfo;
        use creditchain_framework::creditchain_coin::CreditChainCoin;
        pragma verify = false; // TODO: set because of timeout (property proved).
    ...
    spec get_signer_testnet_only(core_resources: &signer, signer_address: address): signer {
        ...
        aborts_if !exists<creditchain_coin::MintCapStore>(signer::address_of(core_resources));
        include GetSignerAbortsIf;
    }
    ...
    spec schema GetVotingPowerAbortsIf {
        ...
        let staking_config = global<staking_config::StakingConfig>(@creditchain_framework);
        aborts_if !exists<staking_config::StakingConfig>(@creditchain_framework);
        let allow_validator_set_change = staking_config.allow_validator_set_change;
        let stake_pool_res = global<stake::StakePool>(pool_address);
        ...
        aborts_if !allow_validator_set_change && !exists<stake::ValidatorSet>(@creditchain_framework);
    ...
    spec schema GetSignerAbortsIf {
        ...
        aborts_if !exists<GovernanceResponsbility>(@creditchain_framework);
        let cap_map = global<GovernanceResponsbility>(@creditchain_framework).signer_caps;
    ...
    spec resolve_multi_step_proposal(proposal_id: u64, signer_address: address, next_execution_hash: vector<u8>): signer {
        ...
        aborts_if simple_map::spec_contains_key(proposal.metadata, multi_step_key) &&
            !creditchain_std::from_bcs::deserializable<bool>(simple_map::spec_get(proposal.metadata, multi_step_key));
        let is_multi_step = simple_map::spec_contains_key(proposal.metadata, multi_step_key) &&
                            creditchain_std::from_bcs::deserialize<bool>(simple_map::spec_get(proposal.metadata, multi_step_key));
    ...
    spec schema VotingIsProposalResolvableAbortsif {
        ...
        let resolvable_time = creditchain_std::from_bcs::deserialize<u64>(simple_map::spec_get(proposal.metadata, utf8(voting::RESOLVABLE_TIME_METADATA_KEY)));
        aborts_if !creditchain_std::from_bcs::deserializable<u64>(simple_map::spec_get(proposal.metadata, utf8(voting::RESOLVABLE_TIME_METADATA_KEY)));
        aborts_if timestamp::now_seconds() <= resolvable_time;
        aborts_if creditchain_framework::transaction_context::spec_get_script_hash() != proposal.execution_hash;
    }
    ...
    spec assert_proposal_expiration(stake_pool: address, proposal_id: u64) {
        include VotingInitializationAbortIfs;
        include voting::AbortsIfNotContainProposalID<GovernanceProposal>{voting_forum_address: @creditchain_framework};
        let proposal_expiration = voting::spec_get_proposal_expiration_secs<GovernanceProposal>(@creditchain_framework, proposal_id);
    ...
    spec force_end_epoch(creditchain_framework: &signer) {
        use creditchain_framework::reconfiguration_with_dkg;
        use std::signer;
        pragma verify = false; // TODO: set because of timeout (property proved).
        let address = signer::address_of(creditchain_framework);
        include reconfiguration_with_dkg::FinishRequirement {
            framework: creditchain_framework
        };
    }
