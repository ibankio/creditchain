spec creditchain_framework::coin {
    /// <high-level-req>
    /// No.: 1
    ...
    /// Can only be updated by `@creditchain_framework`.
    spec allow_supply_upgrades(_creditchain_framework: &signer, _allowed: bool) {
        aborts_if true;
    }
    ...
    spec fun spec_paired_metadata<CoinType>(): Option<Object<Metadata>> {
        if (exists<CoinConversionMap>(@creditchain_framework)) {
            let map = global<CoinConversionMap>(@creditchain_framework).coin_to_fungible_asset_map;
    ...
    /// The creator of `CoinType` must be `@creditchain_framework`.
    ...
    // `account` must be `@creditchain_framework`.
    ...
    monitor_supply: bool,
        ...
        aborts_if monitor_supply && !exists<aggregator_factory::AggregatorFactory>(@creditchain_framework);
    ...
    parallelizable: bool,
        ...
        aborts_if monitor_supply && parallelizable
            && !exists<aggregator_factory::AggregatorFactory>(@creditchain_framework);
