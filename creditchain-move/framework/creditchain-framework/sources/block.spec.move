spec creditchain_framework::block {
    ...
    /// the module's context, and moved under the CreditChain framework account, (2) the block height is set to zero during
    /// initialization, and (3) the epoch interval is greater than zero.
    /// Enforcement: Formally Verified via [high-level-req-1](Initialize).
    ///
    /// No.: 2
    /// Requirement: Only the CreditChain framework address may execute the following functionalities: (1) initialize
    /// BlockResource, and (2) update the epoch interval.
    /// Criticality: Critical
    /// Implementation: The initialize and  update_epoch_interval_microsecs functions ensure that only creditchain_framework
    /// can call them.
    ...
    spec module {
        ...
        invariant [suspendable] chain_status::is_operating() ==> exists<BlockResource>(@creditchain_framework);
        // After genesis, `CommitHistory` exist.
        invariant [suspendable] chain_status::is_operating() ==> exists<CommitHistory>(@creditchain_framework);
    }
    ...
    spec emit_genesis_block_event {
        ...
        requires event::counter(global<BlockResource>(@creditchain_framework).new_block_events) == 0;
    ...
    spec initialize(creditchain_framework: &signer, epoch_interval_microsecs: u64) {
        use std::signer;
        /// [high-level-req-1]
        include Initialize;
    ...
    spec get_epoch_interval_secs(): u64 {
        aborts_if !exists<BlockResource>(@creditchain_framework);
    }
    ...
    spec schema EmitWritesetBlockEvent {
        ...
        aborts_if !exists<BlockResource>(@creditchain_framework);
        aborts_if !exists<reconfiguration::Configuration>(@creditchain_framework);
