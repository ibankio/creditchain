...
module creditchain_framework::randomness {
    use std::hash;
    use std::option;
    ...
    public fun initialize(framework: &signer) {
        ...
        if (!exists<PerBlockRandomness>(@creditchain_framework)) {
    ...
    public(friend) fun on_new_block(vm: &signer, epoch: u64, round: u64, seed_for_new_block: Option<vector<u8>>) acquires PerBlockRandomness {
        ...
        if (exists<PerBlockRandomness>(@creditchain_framework)) {
            let randomness = borrow_global_mut<PerBlockRandomness>(@creditchain_framework);
    ...
    fun next_32_bytes(): vector<u8> acquires PerBlockRandomness {
        ...
        let input = DST;
        let randomness = borrow_global<PerBlockRandomness>(@creditchain_framework);
