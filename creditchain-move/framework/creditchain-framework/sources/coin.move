...
module creditchain_framework::coin {
    use std::error;
    use std::features;
    ...
    use creditchain_framework::aggregator::Aggregator;
    use creditchain_framework::event::{Self, EventHandle};
    use creditchain_framework::guid;
    use creditchain_framework::optional_aggregator::{Self, OptionalAggregator};
    use creditchain_framework::permissioned_signer;
    use creditchain_framework::system_addresses;

    use creditchain_framework::fungible_asset::{Self, FungibleAsset, Metadata, MintRef, TransferRef, BurnRef};
    use creditchain_framework::object::{Self, Object, object_address};
    use creditchain_framework::primary_fungible_store;
    use creditchain_std::type_info::{Self, TypeInfo};
    use creditchain_framework::create_signer;

    friend creditchain_framework::creditchain_coin;
    ...
    public fun paired_metadata<CoinType>(): Option<Object<Metadata>> acquires CoinConversionMap {
        if (exists<CoinConversionMap>(@creditchain_framework) && features::coin_to_fungible_asset_migration_feature_enabled(
        )) {
            let map = &borrow_global<CoinConversionMap>(@creditchain_framework).coin_to_fungible_asset_map;
    ...
    public entry fun create_coin_conversion_map(creditchain_framework: &signer) {
        system_addresses::assert_creditchain_framework(creditchain_framework);
        if (!exists<CoinConversionMap>(@creditchain_framework)) {
            move_to(creditchain_framework, CoinConversionMap {
                coin_to_fungible_asset_map: table::new(),
            })
        };
    }

    /// Create LBT pairing by passing `CreditChainCoin`.
    ...
    inline fun is_apt<CoinType>(): bool {
        type_info::type_name<CoinType>() == string::utf8(b"0x1::creditchain_coin::CreditChainCoin")
    }
    ...
    inline fun create_and_return_paired_metadata_if_not_exist<CoinType>(allow_apt_creation: bool): Object<Metadata> {
        ...
        assert!(exists<CoinConversionMap>(@creditchain_framework), error::not_found(ECOIN_CONVERSION_MAP_NOT_FOUND));
        let map = borrow_global_mut<CoinConversionMap>(@creditchain_framework);
        let type = type_info::type_of<CoinType>();
        if (!table::contains(&map.coin_to_fungible_asset_map, type)) {
            ...
            let metadata_object_cref =
                if (is_apt) {
                    object::create_sticky_object_at_address(@creditchain_framework, @creditchain_fungible_asset)
                } else {
                    object::create_named_object(
                        &create_signer::create_signer(@creditchain_fungible_asset),
    ...
    public fun allow_supply_upgrades(_creditchain_framework: &signer, _allowed: bool) {
    ...
    public(friend) fun initialize_with_parallelizable_supply<CoinType>(
        ...
        system_addresses::assert_creditchain_framework(account);
    ...
    fun mint_internal<CoinType>(amount: u64): Coin<CoinType> acquires CoinInfo {
        ...
        if (option::is_some(maybe_supply)) {
            ...
            spec {
                ...
                use creditchain_framework::aggregator;
    ...
    #[test(source = @0x2, framework = @creditchain_framework)]
    ...
    #[test(account = @0x1)]
    #[expected_failure(abort_code = 0x50003, location = creditchain_framework::fungible_asset)]
    ...
    #[test(framework = @creditchain_framework, other = @0x123)]
    ...
    #[test(framework = @creditchain_framework)]
    ...
    #[test(framework = @creditchain_framework)]
    #[expected_failure(abort_code = 0x20001, location = creditchain_framework::aggregator)]
    ...
    #[test(account = @creditchain_framework)]
    ...
    #[test(account = @creditchain_framework, aaron = @0xcafe)]
    ...
    #[test(account = @creditchain_framework, aaron = @0xaa10, bob = @0xb0b)]
    ...
    #[test(account = @creditchain_framework, bob = @0xb0b)]
    ...
    #[test(account = @creditchain_framework)]
    #[expected_failure(abort_code = 0x50024, location = creditchain_framework::fungible_asset)]
