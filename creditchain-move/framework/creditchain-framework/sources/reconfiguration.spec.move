spec creditchain_framework::reconfiguration {
    /// <high-level-req>
    /// No.: 1
    /// Requirement: The Configuration resource is stored under the CreditChain framework account with initial values upon
    /// module's initialization.
    /// Criticality: Medium
    /// Implementation: The Configuration resource may only be initialized with specific values and published under the
    /// creditchain_framework account.
    ...
    spec module {
        ...
        invariant [suspendable] chain_status::is_operating() ==> exists<Configuration>(@creditchain_framework);
    ...
    spec schema AbortsIfNotCreditChainFramework {
    ...
    spec initialize(creditchain_framework: &signer) {
        ...
        include AbortsIfNotCreditChainFramework;
        let addr = signer::address_of(creditchain_framework);
        let post config = global<Configuration>(@creditchain_framework);
        requires exists<Account>(addr);
        aborts_if !(global<Account>(addr).guid_creation_num == 2);
        aborts_if exists<Configuration>(@creditchain_framework);
        // property 1: During the module's initialization, it guarantees that the Configuration resource will move under
        // the CreditChain framework account with initial values.
        /// [high-level-req-1]
        ensures exists<Configuration>(@creditchain_framework);
    ...
    spec current_epoch(): u64 {
        aborts_if !exists<Configuration>(@creditchain_framework);
        ensures result == global<Configuration>(@creditchain_framework).epoch;
    }

    spec disable_reconfiguration(creditchain_framework: &signer) {
        include AbortsIfNotCreditChainFramework;
        aborts_if exists<DisableReconfiguration>(@creditchain_framework);
        ensures exists<DisableReconfiguration>(@creditchain_framework);
    }

    /// Make sure the caller is admin and check the resource DisableReconfiguration.
    spec enable_reconfiguration(creditchain_framework: &signer) {
        use creditchain_framework::reconfiguration::{DisableReconfiguration};
        include AbortsIfNotCreditChainFramework;
        aborts_if !exists<DisableReconfiguration>(@creditchain_framework);
        ensures !exists<DisableReconfiguration>(@creditchain_framework);
    }
    ...
    spec emit_genesis_reconfiguration_event {
        use creditchain_framework::reconfiguration::{Configuration};

        aborts_if !exists<Configuration>(@creditchain_framework);
        let config_ref = global<Configuration>(@creditchain_framework);
        aborts_if !(config_ref.epoch == 0 && config_ref.last_reconfiguration_time == 0);
        ensures global<Configuration>(@creditchain_framework).epoch == 1;
    }

    spec last_reconfiguration_time {
        aborts_if !exists<Configuration>(@creditchain_framework);
        ensures result == global<Configuration>(@creditchain_framework).last_reconfiguration_time;
    }
    ...
    spec reconfigure {
        ...
        let success = !(chain_status::is_genesis() || timestamp::spec_now_microseconds() == 0 || !reconfiguration_enabled())
            && timestamp::spec_now_microseconds() != global<Configuration>(@creditchain_framework).last_reconfiguration_time;
        include features::spec_periodical_reward_rate_decrease_enabled() ==> staking_config::StakingRewardsConfigEnabledRequirement;
        include success ==> creditchain_coin::ExistsCreditChainCoin;
        aborts_if false;
        ...
        ensures success ==> global<Configuration>(@creditchain_framework).epoch == old(global<Configuration>(@creditchain_framework).epoch) + 1;
        ensures success ==> global<Configuration>(@creditchain_framework).last_reconfiguration_time == timestamp::spec_now_microseconds();
        ...
        /// [high-level-req-4]
        /// [high-level-req-5]
        ensures !success ==> global<Configuration>(@creditchain_framework).epoch == old(global<Configuration>(@creditchain_framework).epoch);
    }
    ...
    spec reconfiguration_enabled {
        ...
        /// [high-level-req-2]
        aborts_if false;
        ensures result == !exists<DisableReconfiguration>(@creditchain_framework);
    }
}
