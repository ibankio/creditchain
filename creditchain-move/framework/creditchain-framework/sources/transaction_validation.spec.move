spec creditchain_framework::transaction_validation {
    /// <high-level-req>
    /// No.: 1
    ...
    /// Ensure caller is `creditchain_framework`.
    ...
    spec schema PrologueCommonAbortsIf {
        use creditchain_framework::timestamp::{CurrentTimeMicroseconds};
        use creditchain_framework::chain_id::{ChainId};
        sender: &signer;
        gas_payer: &signer;
        ...
        aborts_if !exists<CurrentTimeMicroseconds>(@creditchain_framework);
        aborts_if !(timestamp::now_seconds() < txn_expiration_time);

        aborts_if !exists<ChainId>(@creditchain_framework);
    ...
    /// `CreditChainCoinCapabilities` and `CoinInfo` should exists.
    ...
    /// Abort according to the conditions.
    /// `CreditChainCoinCapabilities` and `CoinInfo` should exist.
    /// Skip transaction_fee::burn_fee verification.
    ...
    spec schema EpilogueGasPayerAbortsIf {
        ...
        use creditchain_framework::coin::{CoinStore, CoinInfo};
        use creditchain_framework::optional_aggregator;
        use creditchain_framework::transaction_fee::{CreditChainCoinCapabilities, CreditChainCoinMintCapability};
        ...
        // TODO(fa_migration)
        // let pre_balance = global<coin::CoinStore<CreditChainCoin>>(gas_payer).coin.value;
        // let post balance = global<coin::CoinStore<CreditChainCoin>>(gas_payer).coin.value;
        let pre_account = global<account::Account>(addr);
        let post account = global<account::Account>(addr);

        aborts_if !exists<CoinStore<CreditChainCoin>>(gas_payer);
        ...
        let apt_addr = type_info::type_of<CreditChainCoin>().account_address;
        let maybe_apt_supply = global<CoinInfo<CreditChainCoin>>(apt_addr).supply;
        ...
        let post post_maybe_apt_supply = global<CoinInfo<CreditChainCoin>>(apt_addr).supply;
        ...
        aborts_if amount_to_burn > 0 && !exists<CreditChainCoinCapabilities>(@creditchain_framework);
        aborts_if amount_to_burn > 0 && !exists<CoinInfo<CreditChainCoin>>(apt_addr);
        ...
        let total_supply = coin::supply<CreditChainCoin>;
        let post post_total_supply = coin::supply<CreditChainCoin>;

        aborts_if amount_to_mint > 0 && !exists<CoinStore<CreditChainCoin>>(addr);
        aborts_if amount_to_mint > 0 && !exists<CreditChainCoinMintCapability>(@creditchain_framework);
        ...
        aborts_if (amount_to_mint != 0) && !exists<coin::CoinInfo<CreditChainCoin>>(creditchain_addr);
        include coin::CoinAddAbortsIf<CreditChainCoin> { amount: amount_to_mint };
    }
}
