 ...
 */
module creditchain_framework::delegation_pool {
    use std::error;
    use std::features;
    use std::signer;
    use std::vector;

    use creditchain_std::math64;
    use creditchain_std::pool_u64_unbound::{Self as pool_u64, total_coins};
    ...
    use creditchain_framework::event::{Self, EventHandle, emit};
    use creditchain_framework::permissioned_signer;
    use creditchain_framework::stake;
    use creditchain_framework::stake::get_operator;
    use creditchain_framework::staking_config;
    use creditchain_framework::timestamp;

    const MODULE_SALT: vector<u8> = b"creditchain_framework::delegation_pool";
    ...
    public fun calculate_and_update_remaining_voting_power(
        ...
        if (creditchain_governance::get_remaining_voting_power(pool_address, proposal_id) == 0) {
            return 0
        };
    ...
    public entry fun initialize_delegation_pool(
        ...
        coin::register<CreditChainCoin>(&stake_pool_signer);
    ...
    public entry fun vote(
        ...
        creditchain_governance::assert_proposal_expiration(pool_address, proposal_id);
        ...
        *used_voting_power = *used_voting_power + voting_power;
        ...
        creditchain_governance::partial_vote(&pool_signer, pool_address, proposal_id, voting_power, should_pass);
    ...
    public entry fun create_proposal(
        ...
        assert!(
            total_voting_power >= creditchain_governance::get_required_proposer_stake(),
            error::invalid_argument(EINSUFFICIENT_PROPOSER_STAKE));
        let pool_signer = retrieve_stake_pool_owner(borrow_global<DelegationPool>(pool_address));
        let proposal_id = creditchain_governance::create_proposal_v2_impl(
            &pool_signer,
            pool_address,
            execution_hash,
            metadata_location,
    ...
    public entry fun add_stake(
        ...
        creditchain_account::transfer(delegator, pool_address, amount);
    ...
    fun withdraw_internal(
        ...
        creditchain_account::transfer(stake_pool_owner, delegator_address, amount);
    ...
    inline fun assert_and_update_proposal_used_voting_power(
        ...
        let stake_pool_remaining_voting_power = creditchain_governance::get_remaining_voting_power(pool_address, proposal_id);
        let stake_pool_used_voting_power = creditchain_governance::get_voting_power(
            pool_address
    ...
    #[test_only]
    use creditchain_framework::stake::fast_forward_to_unlock;
    #[test_only]
    use creditchain_framework::timestamp::fast_forward_seconds;
    ...
    #[test_only]
    public fun end_creditchain_epoch() {
    ...
    #[test_only]
    public fun initialize_for_test_no_reward(creditchain_framework: &signer) {
        initialize_for_test_custom(
            creditchain_framework,
            100 * ONE_LBT,
            10000000 * ONE_LBT,
    ...
    public fun initialize_for_test_custom(
        ...
        reconfiguration::initialize_for_test(creditchain_framework);
    ...
    #[test_only]
    public fun enable_delegation_pool_allowlisting_feature(creditchain_framework: &signer) {
        features::change_feature_flags_for_testing(
            creditchain_framework,
    ...
    #[test(creditchain_framework = @creditchain_framework, validator = @0x123, delegator1 = @0x010, delegator2 = @0x020)]
    ...
    #[test(creditchain_framework = @creditchain_framework, validator = @0x123, delegator = @0x010)]
    ...
    public entry fun test_add_stake_single(
        ...
        let balance = coin::balance<CreditChainCoin>(validator_address);
        add_stake(validator, pool_address, 250 * ONE_LBT);

        // check added stake have been transferred out of delegator account
        assert!(coin::balance<CreditChainCoin>(validator_address) == balance - 250 * ONE_LBT, 0);
    ...
    public entry fun test_unlock_single(
        ...
        end_creditchain_epoch(); // additionally forwards EPOCH_DURATION seconds
        ...
        end_creditchain_epoch(); // additionally forwards EPOCH_DURATION seconds and expires lockup cycle
        ...
        assert!(coin::balance<CreditChainCoin>(delegator_address) == 0, 0);

        // withdraw own unlocked stake
        withdraw(validator, pool_address, 15301499997);
        assert!(coin::balance<CreditChainCoin>(validator_address) == 15301499997, 0);
        ...
        assert!(coin::balance<CreditChainCoin>(validator_address) == balance + 5457545100, 0);
        ...
        balance = coin::balance<CreditChainCoin>(validator_address);
        withdraw(validator, pool_address, 10303010000);

        assert!(coin::balance<CreditChainCoin>(validator_address) == balance + 10303010000, 0);
        ...
        assert_pending_withdrawal(validator_address, pool_address, false, 0, false, 0);
        assert!(coin::balance<CreditChainCoin>(validator_address) == balance + 1000000000, 0);
    }
    ...
    public entry fun test_reactivate_stake_single(
        ...
        unlock(validator, pool_address, 100 * ONE_LBT);
        assert!(coin::balance<CreditChainCoin>(validator_address) == 15149999998, 0);
    ...
    public entry fun test_withdraw_many(
        ...
        let balance = coin::balance<CreditChainCoin>(validator_address);
        withdraw(validator, pool_address, MAX_U64);
        assert!(coin::balance<CreditChainCoin>(validator_address) == balance + 10100000000, 0);
        assert_pending_withdrawal(validator_address, pool_address, false, 0, false, 0);
        assert_inactive_shares_pool(pool_address, 0, false, 0);
    ...
    public entry fun test_pending_inactive_stake_rewards(
        ...
        end_creditchain_epoch(); // 81000000001 active stake * 1.01 + 19999999999 pending_inactive stake * 1.01
        end_creditchain_epoch(); // 81810000001 active stake * 1.01 + 20199999998 pending_inactive stake * 1.01

        timestamp::fast_forward_seconds(LOCKUP_CYCLE_SECONDS);
        end_creditchain_epoch(); // 82628100001 active stake * 1.01 + 20401999997 pending_inactive stake * 1.01
        end_creditchain_epoch(); // 83454381001 active stake * 1.01 + 20606019996 pending_inactive stake(now inactive)
        ...
        unlock(validator, pool_address, 200 * ONE_LBT);
        assert!(coin::balance<CreditChainCoin>(validator_address) == 20606019996, 0);
        ...
        end_creditchain_epoch(); // 64288924812 active stake * 1.01 + 19999999999 pending_inactive stake * 1.01
        end_creditchain_epoch(); // 64931814060 active stake * 1.01 + 20199999998 pending_inactive stake * 1.01
        end_creditchain_epoch(); // 65581132200 active stake * 1.01 + 20401999997 pending_inactive stake * 1.01
        end_creditchain_epoch(); // 66236943522 active stake * 1.01 + 20606019996 pending_inactive stake * 1.01
        assert_delegation(validator_address, pool_address, 66899312957, 0, 20812080195);

        timestamp::fast_forward_seconds(LOCKUP_CYCLE_SECONDS);
        end_creditchain_epoch(); // 66899312957 active stake * 1.01 + 20812080195 pending_inactive stake * 1.01
        end_creditchain_epoch(); // 67568306086 active stake * 1.01 + 21020200996 pending_inactive stake(now inactive)
        end_creditchain_epoch(); // 68243989147 active stake * 1.01 + 21020200996 inactive stake
    ...
    public entry fun test_out_of_order_redeem(
        ...
        assert!(coin::balance<CreditChainCoin>(delegator1_address) == 15149999998, 0);
        assert!(coin::balance<CreditChainCoin>(delegator2_address) == 5149999997, 0);
        ...
        assert!(coin::balance<CreditChainCoin>(delegator2_address) == 15149999998, 0);
        ...
        assert!(coin::balance<CreditChainCoin>(delegator1_address) == 15149999998 + 10099999998, 0);
        assert_pending_withdrawal(delegator1_address, pool_address, false, 0, false, 0);
    }
    ...
    public entry fun test_operator_fee(
        ...
        synchronize_delegation_pool(pool_address);
        assert!(coin::balance<CreditChainCoin>(validator_address) == 25536996, 0);
    ...
    #[test(creditchain_framework = @creditchain_framework, old_operator = @0x123, delegator = @0x010, new_operator = @0x020)]
    ...
    public entry fun test_set_beneficiary_for_operator(
        ...
        creditchain_account::create_account(operator1_address);

        let operator2_address = signer::address_of(operator2);
        creditchain_account::create_account(operator2_address);

        let beneficiary_address = signer::address_of(beneficiary);
        creditchain_account::create_account(beneficiary_address);
        ...
        assert!(coin::balance<CreditChainCoin>(operator1_address) == ONE_LBT - 1, 0);
        ...
        assert!(coin::balance<CreditChainCoin>(beneficiary_address) == ONE_LBT - 1, 0);
        assert!(coin::balance<CreditChainCoin>(operator1_address) == ONE_LBT - 1, 0);
        ...
        assert!(coin::balance<CreditChainCoin>(beneficiary_address) == ONE_LBT - 1, 0);
        assert!(coin::balance<CreditChainCoin>(operator2_address) == ONE_LBT - 1, 0);
    }

    #[test(creditchain_framework = @creditchain_framework, operator = @0x123, delegator = @0x010)]
    ...
    #[test(creditchain_framework = @creditchain_framework, validator = @0x123, delegator1 = @0x010)]
    ...
    public entry fun test_create_proposal_abort_if_inefficient_stake(
        ...
        initialize_for_test(creditchain_framework);
        creditchain_governance::initialize_for_test(
            creditchain_framework,
            (10 * ONE_LBT as u128),
            100 * ONE_LBT,
            1000,
    ...
    public entry fun test_voting_power_change(
        ...
        initialize_for_test_no_reward(creditchain_framework);
        creditchain_governance::initialize_for_test(
            creditchain_framework,
            (10 * ONE_LBT as u128),
            100 * ONE_LBT,
    ...
    #[test(creditchain_framework = @creditchain_framework, validator = @0x123, delegator_1 = @0x010)]
    #[expected_failure(abort_code = 0x30018, location = Self)]
    ...
    #[test(creditchain_framework = @creditchain_framework, validator = @0x123, delegator_1 = @0x010, delegator_2 = @0x020)]
    public entry fun test_allowlist_operations_only_e2e(
        ...
        initialize_for_test(creditchain_framework);
        initialize_test_validator(validator, 100 * ONE_LBT, true, true);
        enable_delegation_pool_allowlisting_feature(creditchain_framework);
