spec creditchain_framework::timestamp {
    /// <high-level-req>
    /// No.: 1
    ...
    ///
    /// No.: 2
    /// Requirement: The global wall clock resource should only be owned by the CreditChain framework.
    /// Criticality: High
    /// Implementation: The function set_time_has_started ensures that only the creditchain_framework account can possess the
    /// CurrentTimeMicroseconds resource using the assert_creditchain_framework function.
    ...
    spec module {
        ...
        /// [high-level-req-1]
        /// [high-level-req-2]
        invariant [suspendable] chain_status::is_operating() ==> exists<CurrentTimeMicroseconds>(@creditchain_framework);
    }
    ...
    spec fun spec_now_microseconds(): u64 {
        global<CurrentTimeMicroseconds>(@creditchain_framework).microseconds
    }
