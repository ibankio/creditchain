module creditchain_framework::system_addresses {
    use std::error;
    use std::signer;
    ...
    const ENOT_CREDITCHAIN_FRAMEWORK_ADDRESS: u64 = 3;
    ...
    public fun assert_creditchain_framework(account: &signer) {
        assert!(
            is_creditchain_framework_address(signer::address_of(account)),
            error::permission_denied(ENOT_CREDITCHAIN_FRAMEWORK_ADDRESS),
        )
    }
    ...
    public fun is_framework_reserved_address(addr: address): bool {
        is_creditchain_framework_address(addr) ||
            addr == @0x2 ||
            addr == @0x3 ||
            addr == @0x4 ||
            addr == @0x5 ||
    ...
    public fun is_creditchain_framework_address(addr: address): bool {
    ...
    /// Return true if `addr` is either the VM address or an CreditChain Framework address.
    public fun is_reserved_address(addr: address): bool {
        is_creditchain_framework_address(addr) || is_vm_address(addr)
    }
}
