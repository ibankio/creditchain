...
use crate::{
    check_creditchain_packages_availability, compile_creditchain_packages, compile_package,
    data_state_view::DataStateView, generate_compiled_blob, is_creditchain_package, CompilationCache,
    DataManager, IndexReader, PackageInfo, TxnIndex, CREDITCHAIN_COMMONS,
};
use anyhow::Result;
use creditchain_framework::CREDITCHAIN_PACKAGES;
use creditchain_language_e2e_tests::executor::FakeExecutor;
use creditchain_transaction_simulation::{InMemoryStateStore, SimulationStateStore};
...
use creditchain_validator_interface::CreditChainValidatorInterface;
use clap::ValueEnum;
...
fn add_creditchain_packages_to_state_store(
    state_store: &impl SimulationStateStore,
...
impl Execution {
    ...
    pub async fn execute_txns(&self, begin: Version, num_txns_to_execute: u64) -> Result<()> {
        let creditchain_commons_path = self.input_path.join(CREDITCHAIN_COMMONS);
        if !check_creditchain_packages_availability(creditchain_commons_path.clone()) {
        ...
        if self.execution_mode.is_v1_or_compare() {
            compile_creditchain_packages(
                &creditchain_commons_path,
    ...
    fn execute_one_txn(
        ...
        if let Some(txn_idx) = data_manager.get_txn_index(cur_version) {
            ...
            if txn_idx.package_info.is_compilable()
                && !is_creditchain_package(&txn_idx.package_info.package_name)
            {
    ...
    pub(crate) fn execute_and_compare(
        ...
        debugger: Option<Arc<dyn CreditChainValidatorInterface + Send>>,
    ) {
    ...
    fn execute_code(
        ...
        txn: &Transaction,
        compiled_package_cache: &HashMap<PackageInfo, HashMap<ModuleId, Vec<u8>>>,
        debugger_opt: Option<Arc<dyn CreditChainValidatorInterface + Send>>,
        v2_flag: bool,
    ) -> Result<(WriteSet, Vec<ContractEvent>), VMStatus> {
        // Always add CreditChain (0x1) packages.
        add_creditchain_packages_to_state_store(&state, compiled_package_cache);
