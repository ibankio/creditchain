...
use creditchain_framework::{
    natives::code::PackageMetadata, unzip_metadata_str, BuiltPackage, CREDITCHAIN_PACKAGES,
};
...
const CREDITCHAIN_PACKAGES_DIR_NAMES: [&str; 5] = [
    "creditchain-framework",
    "move-stdlib",
    "creditchain-stdlib",
    "creditchain-token",
    "creditchain-token-objects",
];
...
pub const CREDITCHAIN_COMMONS: &str = "aptos-commons";
...
fn is_creditchain_package(package_name: &str) -> bool {
    CREDITCHAIN_PACKAGES.contains(&package_name)
}

fn get_creditchain_dir(package_name: &str) -> Option<&str> {
    if is_creditchain_package(package_name) {
        for i in 0..CREDITCHAIN_PACKAGES.len() {
            if CREDITCHAIN_PACKAGES[i] == package_name {
                return Some(CREDITCHAIN_PACKAGES_DIR_NAMES[i]);
            }
        }
    }
    None
...
async fn download_creditchain_packages(path: &Path) -> anyhow::Result<()> {
    ...
    let source_framework_path = PathBuf::from(tmp_dir.path()).join("creditchain-move/framework");
    for package_name in CREDITCHAIN_PACKAGES {
        let source_framework_path =
            source_framework_path.join(get_creditchain_dir(package_name).unwrap());
        let target_framework_path = PathBuf::from(path).join(get_creditchain_dir(package_name).unwrap());
        Command::new("cp")
            .arg("-r")
...
fn check_creditchain_packages_availability(path: PathBuf) -> bool {
    if !path.exists() {
        return false;
    }
    for package in CREDITCHAIN_PACKAGES {
        if !path.join(get_creditchain_dir(package).unwrap()).exists() {
            return false;
        }
    }
    true
...
pub async fn prepare_creditchain_packages(path: PathBuf) {
    ...
    if success {
        ...
        download_creditchain_packages(&path).await.unwrap();
    }
}
...
fn compile_creditchain_packages(
    creditchain_commons_path: &Path,
    compiled_package_map: &mut HashMap<PackageInfo, HashMap<ModuleId, Vec<u8>>>,
    v2_flag: bool,
) -> anyhow::Result<()> {
    for package in CREDITCHAIN_PACKAGES {
        let root_package_dir = creditchain_commons_path.join(get_creditchain_dir(package).unwrap());
...
fn compile_package(
    ...
    let mut build_options = creditchain_framework::BuildOptions {
    ...
    for manifest_dep in manifest_deps {
        ...
        for pack_dep in &root_package_metadata.deps {
            ...
            if pack_dep_name == manifest_dep_name {
                if is_creditchain_package(&pack_dep_name) {
                    fix_manifest_dep(
                        dep,
                        &format!(
                            "{}/{}",
                            CREDITCHAIN_COMMONS,
                            get_creditchain_dir(&pack_dep_name).unwrap()
                        ),
                    );
                    break;
                }
