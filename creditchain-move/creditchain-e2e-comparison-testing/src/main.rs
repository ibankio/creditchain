...
use anyhow::Result;
use creditchain_comparison_testing::{
    prepare_creditchain_packages, DataCollection, Execution, ExecutionMode, OnlineExecutor, CREDITCHAIN_COMMONS,
};
...
pub enum Cmd {
    ...
    Dump {
        /// Endpoint url to obtain the txn data, e.g. `https://api.mainnet.creditchain.io/v1` for mainnet.
        /// To avoid rate limiting, users need to apply for API key from `https://developers.creditchain.io/`
    ...
    Online {
        ...
        /// e.g. `https://api.mainnet.creditchain.io/v1` for mainnet.
        /// To avoid rate limiting, users need to apply for API key from `https://developers.creditchain.io/`
        /// and set the env variable X_API_KEY using the obtained key
        endpoint: String,
        /// Path to the dumped data
...
async fn main() -> Result<()> {
    ...
    match args.cmd {
        Cmd::Dump {
            ...
            if !skip_source_code {
                prepare_creditchain_packages(output.join(CREDITCHAIN_COMMONS)).await;
            }
        ...
        Cmd::Execute {
            ...
            prepare_creditchain_packages(input.join(CREDITCHAIN_COMMONS)).await;
