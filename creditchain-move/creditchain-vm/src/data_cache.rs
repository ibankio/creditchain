...
use crate::move_vm_ext::{
    resource_state_key, CreditChainMoveResolver, AsExecutorView, AsResourceGroupView,
...
use creditchain_vm_environment::gas::get_gas_feature_version;
...
/// Adapter to convert a `ExecutorView` into a `CreditChainMoveResolver`.
///
...
impl<E: ExecutorView> CreditChainMoveResolver for StorageAdapter<'_, E> {}
...
/// Converts `StateView` into `CreditChainMoveResolver`.
...
pub(crate) mod tests {
    use super::*;
    use creditchain_vm_types::resource_group_adapter::GroupSizeKind;
