...
use crate::{
    block_executor::{CreditChainTransactionOutput, CreditChainVMBlockExecutorWrapper},
    counters::*,
    ...
    move_vm_ext::{
        ...
        CreditChainMoveResolver, MoveVmExt, SessionExt, SessionId, UserTransactionContext,
    },
...
use creditchain_block_executor::{
    code_cache_global_manager::CreditChainModuleCacheManager,
    txn_commit_hook::NoOpTransactionCommitHook,
    txn_provider::{default::DefaultTxnProvider, TxnProvider},
};
use creditchain_crypto::HashValue;
use creditchain_framework::natives::code::PublishRequest;
use creditchain_gas_algebra::{Gas, GasQuantity, NumBytes, Octa};
use creditchain_gas_meter::{CreditChainGasMeter, GasAlgebra};
use creditchain_gas_schedule::{
    gas_feature_versions,
    gas_feature_versions::{RELEASE_V1_10, RELEASE_V1_27},
    CreditChainGasParameters, VMGasParameters,
};
use creditchain_logger::{enabled, prelude::*, Level};
#[cfg(any(test, feature = "testing"))]
use creditchain_types::state_store::StateViewId;
...
use creditchain_vm_types::{
    ...
    module_and_script_storage::{
        code_storage::CreditChainCodeStorage, module_storage::CreditChainModuleStorage, AsCreditChainCodeStorage,
    },
...
pub(crate) fn get_system_transaction_output(
    session: SessionExt<impl CreditChainMoveResolver>,
...
impl CreditChainVM {
    ...
    pub fn new(env: &CreditChainEnvironment, state_view: &impl StateView) -> Self {
        let resolver = state_view.as_move_resolver();
        let module_storage = state_view.as_creditchain_code_storage(env);
    ...
    pub fn new_session<'r, R: CreditChainMoveResolver>(
        &self,
        resolver: &'r R,
    ...
    pub(crate) fn gas_params(
        &self,
        log_context: &AdapterLogSchema,
    ) -> Result<&CreditChainGasParameters, VMStatus> {
    ...
    #[inline(always)]
    pub fn environment(&self) -> CreditChainEnvironment {
    ...
    #[cfg(any(test, feature = "testing"))]
    pub fn gas_params_for_test(&self) -> Result<&CreditChainGasParameters, VMStatus> {
    ...
    fn fee_statement_from_gas_meter(
        ...
        gas_meter: &impl CreditChainGasMeter,
    ...
    pub(crate) fn failed_transaction_cleanup(
        ...
        gas_meter: &mut impl CreditChainGasMeter,
        txn_data: &TransactionMetadata,
        ...
        if self.gas_feature_version() >= 12 {
            ...
            if let Err(err) = gas_meter.algebra().check_consistency() {
                println!(
                    "[creditchain-vm][gas-meter][failure-epilogue] {}",
                    err.message()
    ...
    fn finish_aborted_transaction(
        ...
        let (previous_session_change_set, fee_statement) = if should_create_account_resource {
            ...
            let fee_statement =
                CreditChainVM::fee_statement_from_gas_meter(txn_data, gas_meter, ZERO_STORAGE_REFUND);
    ...
    fn success_transaction_cleanup(
        ...
        if self.gas_feature_version() >= 12 {
            ...
            if let Err(err) = gas_meter.algebra().check_consistency() {
                println!(
                    "[creditchain-vm][gas-meter][success-epilogue] {}",
                    err.message()
                        .unwrap_or("No message found -- this should not happen.")
                );
        ...
        let fee_statement = CreditChainVM::fee_statement_from_gas_meter(
            txn_data,
            gas_meter,
    ...
    fn validate_and_execute_script<'a>(
        &self,
        session: &mut SessionExt<impl CreditChainMoveResolver>,
        serialized_signers: &SerializedSigners,
        code_storage: &impl CreditChainCodeStorage,
        // Note: cannot use CreditChainGasMeter because it is not implemented for
        //       UnmeteredGasMeter.
    ...
    fn execute_script_or_entry_function<'a, 'r>(
        &self,
        resolver: &'r impl CreditChainMoveResolver,
        code_storage: &impl CreditChainCodeStorage,
        ...
        fail_point!("creditchain_vm::execute_script_or_entry_function", |_| {
            Err(VMStatus::Error {
    ...
    fn validate_signed_transaction(
        ...
        let keyless_authenticators = creditchain_types::keyless::get_authenticators(transaction)
    ...
    fn execute_user_transaction_impl(
        ...
        let _timer = VM_TIMER.timer_with_label("CreditChainVM::execute_user_transaction_impl");
        ...
        let should_create_account_resource_timer =
            VM_TIMER.timer_with_label("CreditChainVM::create_account_resource_lazily");
        ...
        let payload_timer =
            VM_TIMER.timer_with_label("CreditChainVM::execute_user_transaction_impl [payload]");
    ...
    pub fn execute_user_transaction_with_custom_gas_meter<'a, C, G, F>(
        &self,
        resolver: &'a impl CreditChainMoveResolver,
        code_storage: &'a C,
        txn: &SignedTransaction,
        log_context: &AdapterLogSchema,
        make_gas_meter: F,
    ...
    where
        C: CreditChainCodeStorage + BlockSynchronizationKillSwitch,
    ...
    pub fn execute_user_transaction_with_modified_gas_meter<'a, G, F>(
        &self,
        resolver: &'a impl CreditChainMoveResolver,
        code_storage: &'a (impl CreditChainCodeStorage + BlockSynchronizationKillSwitch),
        txn: &SignedTransaction,
        log_context: &AdapterLogSchema,
        modify_gas_meter: F,
    ) -> Result<(VMStatus, VMOutput, G), VMStatus>
    ...
    pub fn execute_user_transaction(
        &self,
        resolver: &impl CreditChainMoveResolver,
        code_storage: &(impl CreditChainCodeStorage + BlockSynchronizationKillSwitch),
        txn: &SignedTransaction,
        log_context: &AdapterLogSchema,
    ) -> (VMStatus, VMOutput) {
        ...
        match write_set_payload {
            WriteSetPayload::Direct(change_set) => {
                ...
                if !Self::should_restart_execution(change_set.events()) {
                    ...
                    error!(
                        "[creditchain_vm] direct write set finished without requiring should_restart_execution");
                }
    ...
    fn validate_waypoint_change_set(
        ...
        if has_new_block_event && has_new_epoch_event {
            ...
            error!(
                *log_context,
                "[creditchain_vm] waypoint txn needs to emit new epoch and block"
            );
    ...
    pub fn execute_view_function(
        ...
        let env = CreditChainEnvironment::new(state_view);
        let vm = CreditChainVM::new(&env, state_view);
    ...
    fn gas_used(max_gas_amount: Gas, gas_meter: &impl CreditChainGasMeter) -> u64 {
        max_gas_amount
            .checked_sub(gas_meter.balance())
    ...
    pub fn execute_single_transaction(
        ...
        Ok(match txn.expect_valid() {
            Transaction::BlockMetadata(block_metadata) => {
                fail_point!("creditchain_vm::execution::block_metadata");
            ...
            Transaction::BlockMetadataExt(block_metadata_ext) => {
                fail_point!("creditchain_vm::execution::block_metadata_ext");
            ...
            Transaction::UserTransaction(txn) => {
                fail_point!("creditchain_vm::execution::user_transaction");
                    ...
                    match vm_status.status_code() {
                        ...
                        | StatusCode::TYPE_RESOLUTION_FAILURE => {
                            speculative_error!(
                                log_context,
                                format!(
                                    "[creditchain_vm] Transaction breaking invariant violation. txn: {:?}, status: {:?}",
                        ...
                        if vm_status.sub_status()
                            == Some(move_core_types::vm_status::sub_status::unknown_invariant_violation::EPARANOID_FAILURE) =>
                            {
                                error!(
                                *log_context,
                                "[creditchain_vm] Transaction breaking paranoid mode. txn: {:?}, status: {:?}",
                                bcs::to_bytes::<SignedTransaction>(txn),
                                vm_status,
                            );
                            },
...
pub struct CreditChainVMBlockExecutor {
    ...
    module_cache_manager: CreditChainModuleCacheManager,
}

impl CreditChainVMBlockExecutor {
    ...
    pub fn execute_block_with_config(
        ...
        fail_point!("creditchain_vm_block_executor::execute_block_with_config", |_| {
            Err(VMStatus::error(
                StatusCode::UNKNOWN_INVARIANT_VIOLATION_ERROR,
                None,
            ))
        ...
        let result = CreditChainVMBlockExecutorWrapper::execute_block::<
            _,
            NoOpTransactionCommitHook<CreditChainTransactionOutput, VMStatus>,
...
impl VMBlockExecutor for CreditChainVMBlockExecutor {
    fn new() -> Self {
        Self {
            module_cache_manager: CreditChainModuleCacheManager::new(),
        }
    }
    ...
    fn execute_block(
        ...
        let config = BlockExecutorConfig {
            local: BlockExecutorLocalConfig {
                blockstm_v2: false,
                concurrency_level: CreditChainVM::get_concurrency_level(),
                allow_fallback: true,
                discard_failed_blocks: CreditChainVM::get_discard_failed_blocks(),
    ...
    fn execute_block_sharded<S: StateView + Sync + Send + 'static, C: ExecutorClient<S>>(
        ...
        let ret = sharded_block_executor.execute_block(
            state_view,
            transactions,
            CreditChainVM::get_concurrency_level(),
            onchain_config,
        );
        if ret.is_ok() {
            // Record the histogram count for transactions per block.
...
impl VMValidator for CreditChainVM {
    ...
    fn validate_transaction(
        ...
        {
            if let creditchain_types::transaction::authenticator::TransactionAuthenticator::SingleSender{ .. } = transaction.authenticator_ref() {
...
// Ensure encapsulation of CreditChainVM APIs by using a wrapper.
pub struct CreditChainSimulationVM;

impl CreditChainSimulationVM {
    ...
    pub fn create_vm_and_simulate_signed_transaction(
        ...
        let env = CreditChainEnvironment::new(state_view);
        let mut vm = CreditChainVM::new(&env, state_view);
...
#[cfg(test)]
mod tests {
    use crate::{move_vm_ext::MoveVmExt, CreditChainVM};
    use creditchain_types::{
        account_address::AccountAddress,
    ...
    fn vm_thread_safe() {
        ...
        assert_send::<CreditChainVM>();
        assert_sync::<CreditChainVM>();
    ...
    fn should_restart_execution_on_new_epoch() {
        ...
        assert!(CreditChainVM::should_restart_execution(&[(
            new_epoch_event,
            None
        )]));
        assert!(CreditChainVM::should_restart_execution(&[(
