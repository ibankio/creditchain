...
use crate::{
    creditchain_vm::SerializedSigners,
    errors::{convert_epilogue_error, convert_prologue_error, expect_only_successful_execution},
    move_vm_ext::{CreditChainMoveResolver, SessionExt},
    system_module_names::{
...
use creditchain_gas_algebra::Gas;
...
pub static CREDITCHAIN_TRANSACTION_VALIDATION: Lazy<TransactionValidation> =
        ...
        || features.is_derivable_account_abstraction_enabled()
    {
            ...
            if let (Some(_fee_payer), Some(fee_payer_auth_key)) = (
                ...
                (
                    if features.is_transaction_payload_v2_enabled() {
                        &CREDITCHAIN_TRANSACTION_VALIDATION.unified_prologue_fee_payer_v2_name
                    } else {
                        &CREDITCHAIN_TRANSACTION_VALIDATION.unified_prologue_fee_payer_name
                    },
                    serialized_args,
                )
            } else {
                ...
                (
                    if features.is_transaction_payload_v2_enabled() {
                        &CREDITCHAIN_TRANSACTION_VALIDATION.unified_prologue_v2_name
                    } else {
                        &CREDITCHAIN_TRANSACTION_VALIDATION.unified_prologue_name
                    },
                    serialized_args,
                )
            };
        session
            .execute_function_bypass_visibility(
                &CREDITCHAIN_TRANSACTION_VALIDATION.module_id(),
        ...
        let (prologue_function_name, args) = if let (Some(fee_payer), Some(fee_payer_auth_key)) = (
            ...
            if features.is_transaction_simulation_enhancement_enabled() {
                ...
                (
                    &CREDITCHAIN_TRANSACTION_VALIDATION.fee_payer_prologue_extended_name,
                    args,
                )
            } else {
                let args = vec![
                ...
                (&CREDITCHAIN_TRANSACTION_VALIDATION.fee_payer_prologue_name, args)
            }
            ...
            if features.is_transaction_simulation_enhancement_enabled() {
                ...
                (
                    &CREDITCHAIN_TRANSACTION_VALIDATION.multi_agent_prologue_extended_name,
                    args,
                )
            } else {
                let args = vec![
                ...
                (
                    &CREDITCHAIN_TRANSACTION_VALIDATION.multi_agent_prologue_name,
                    args,
                )
            }
        } else {
            ...
            if features.is_transaction_simulation_enhancement_enabled() {
                ...
                (
                    &CREDITCHAIN_TRANSACTION_VALIDATION.script_prologue_extended_name,
                    args,
                )
            } else {
                let args = vec![
                ...
                (&CREDITCHAIN_TRANSACTION_VALIDATION.script_prologue_name, args)
            }
        };
...
fn run_epilogue(
        ...
        || features.is_derivable_account_abstraction_enabled()
    {
        ...
        session.execute_function_bypass_visibility(
            &CREDITCHAIN_TRANSACTION_VALIDATION.module_id(),
            if features.is_transaction_payload_v2_enabled() {
                &CREDITCHAIN_TRANSACTION_VALIDATION.unified_epilogue_v2_name
            } else {
                &CREDITCHAIN_TRANSACTION_VALIDATION.unified_epilogue_name
            },
            vec![],
            serialize_args,
            &mut UnmeteredGasMeter,
        ...
        if let Some(fee_payer) = txn_data.fee_payer() {
            let (func_name, args) = {
                if features.is_transaction_simulation_enhancement_enabled() {
                    ...
                    (
                        &CREDITCHAIN_TRANSACTION_VALIDATION.user_epilogue_gas_payer_extended_name,
                        args,
                    )
                } else {
                    let args = vec![
                    ...
                    (
                        &CREDITCHAIN_TRANSACTION_VALIDATION.user_epilogue_gas_payer_name,
                        args,
                    )
                }
            };
            ...
            let (func_name, args) = {
                if features.is_transaction_simulation_enhancement_enabled() {
                    ...
                    (
                        &CREDITCHAIN_TRANSACTION_VALIDATION.user_epilogue_extended_name,
                        args,
                    )
                } else {
                    let args = vec![
                    ...
                    (&CREDITCHAIN_TRANSACTION_VALIDATION.user_epilogue_name, args)
                }
            };
            session.execute_function_bypass_visibility(
                &CREDITCHAIN_TRANSACTION_VALIDATION.module_id(),
...
pub(crate) fn run_failure_epilogue(
    ...
    .or_else(|err| {
        expect_only_successful_execution(
            err,
            CREDITCHAIN_TRANSACTION_VALIDATION.user_epilogue_name.as_str(),
            log_context,
        )
    })
}
