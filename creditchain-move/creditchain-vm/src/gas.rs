...
use crate::{move_vm_ext::CreditChainMoveResolver, transaction_metadata::TransactionMetadata};
use creditchain_gas_algebra::{Gas, GasExpression, InternalGas};
use creditchain_gas_meter::{StandardGasAlgebra, StandardGasMeter};
use creditchain_gas_schedule::{
    gas_feature_versions::RELEASE_V1_13, gas_params::txn::KEYLESS_BASE_COST, CreditChainGasParameters,
    VMGasParameters,
};
use creditchain_logger::{enabled, Level};
use creditchain_memory_usage_tracker::MemoryTrackedGasMeter;
use creditchain_types::on_chain_config::Features;
use creditchain_vm_logging::{log_schema::AdapterLogSchema, speculative_log, speculative_warn};
...
pub(crate) fn check_gas(
    ...
    if crate::creditchain_vm::should_create_account_resource(
        txn_metadata,
        features,
        resolver,
        module_storage,
