...
use crate::transaction_validation::CREDITCHAIN_TRANSACTION_VALIDATION;
...
pub fn convert_prologue_error(
    ...
    Err(match status {
        ...
        VMStatus::MoveAbort(location, code)
            if !CREDITCHAIN_TRANSACTION_VALIDATION.is_account_module_abort(&location) =>
        {
            let new_major_status = match error_split(code) {
                ...
                (category, reason) => {
                    let err_msg = format!("[creditchain_vm] Unexpected prologue Move abort: {:?}::{:?} (Category: {:?} Reason: {:?})",
        ...
        status @ VMStatus::ExecutionFailure { .. } | status @ VMStatus::Error { .. } => {
            speculative_error!(
                log_context,
                format!("[creditchain_vm] Unexpected prologue error: {:?}", status),
            );
            VMStatus::Error {
                status_code: StatusCode::UNEXPECTED_ERROR_FROM_KNOWN_MOVE_FUNCTION,
...
pub fn convert_epilogue_error(
    ...
    Err(match status {
            ...
            if !CREDITCHAIN_TRANSACTION_VALIDATION.is_account_module_abort(&location) =>
        {
            ...
            let err_msg = format!("[creditchain_vm] Unexpected success epilogue Move abort: {:?}::{:?} (Category: {:?} Reason: {:?})",
			location, code, category, reason);
            speculative_error!(log_context, err_msg.clone());
            VMStatus::error(
                StatusCode::UNEXPECTED_ERROR_FROM_KNOWN_MOVE_FUNCTION,
        ...
        status => {
            let err_msg = format!("[creditchain_vm] Unexpected success epilogue error: {:?}", status);
            speculative_error!(log_context, err_msg.clone());
            VMStatus::Error {
                status_code: StatusCode::UNEXPECTED_ERROR_FROM_KNOWN_MOVE_FUNCTION,
                sub_status: status.sub_status(),
...
pub fn expect_only_successful_execution(
    ...
    Err(match status {
        ...
        status => {
            ...
            let err_msg = format!(
                "[creditchain_vm] Unexpected error from known Move function, '{}'. Error: {:?}",
