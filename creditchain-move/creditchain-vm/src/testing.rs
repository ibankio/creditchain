...
use crate::creditchain_vm::{serialized_signer, SerializedSigners};
use crate::CreditChainVM;
#[cfg(any(test, feature = "testing"))]
use crate::{
...
#[cfg(any(test, feature = "testing"))]
use creditchain_types::{state_store::StateView, transaction::SignedTransaction};
#[cfg(any(test, feature = "testing"))]
use creditchain_vm_logging::log_schema::AdapterLogSchema;
#[cfg(any(test, feature = "testing"))]
use creditchain_vm_types::{
    module_and_script_storage::AsCreditChainCodeStorage, output::VMOutput,
...
impl CreditChainVM {
    ...
    pub fn test_failed_transaction_cleanup(
        ...
        let module_storage = state_view.as_creditchain_code_storage(self.runtime_environment());
