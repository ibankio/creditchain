...
CreditChainVM is used to execute a single CreditChain transaction.
...
2. **Execution:**
   ...
   When the code is verified and loaded, transaction arguments are checked and then the payload is executed via MoveVM that CreditChainVM wraps. 
   If execution is successful, a change set that can be (but is not yet) applied to the blockchain state is produced.
3. **Aborted execution:**
   If execution is not successful, CreditChainVM checks if an account needs to be created for this user transaction.
...
### CreditChainSimulationVM

Used to simulate transactions before executing them on a real network.
Implementation-wise, wraps CreditChainVM with some minor modifications for multi-signature transactions.

### CreditChainVMBlockExecutor
