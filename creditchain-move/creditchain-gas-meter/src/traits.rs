...
use creditchain_gas_algebra::{Fee, FeePerGasUnit, Gas, GasExpression, GasScalingFactor, Octa};
use creditchain_gas_schedule::{gas_feature_versions::RELEASE_V1_30, VMGasParameters};
...
/// Trait representing a gas meter used inside the CreditChain VM.
///
/// It extends Move VM's `GasMeter` trait with a few CreditChain-specific callbacks, along with
/// some primitives that would allow gas meters to be composable.
pub trait CreditChainGasMeter: MoveGasMeter {
