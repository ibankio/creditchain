...
use creditchain_framework::{ReleaseBundle, ReleasePackage};
use creditchain_gas_schedule::{
    CreditChainGasParameters, InitialGasSchedule, ToOnChainGasSchedule, LATEST_GAS_FEATURE_VERSION,
};
use creditchain_types::{
    account_config::{
        self, creditchain_test_root_address, events::NewEpochEvent, CORE_CODE_ADDRESS,
...
use creditchain_vm::{
    ...
    move_vm_ext::{
        convert_modules_into_write_ops, CreditChainMoveResolver, GenesisMoveVm, GenesisRuntimeBuilder,
        SessionExt,
    },
};
...
const GOVERNANCE_MODULE_NAME: &str = "creditchain_governance";
...
const CREDITCHAIN_COINS_BASE_WITH_DECIMALS: u64 = u64::pow(10, 8);
...
pub fn default_gas_schedule() -> GasScheduleV2 {
    GasScheduleV2 {
        ...
        entries: CreditChainGasParameters::initial().to_on_chain_gas_schedule(LATEST_GAS_FEATURE_VERSION),
    }
}

pub fn encode_creditchain_mainnet_genesis_transaction(
    ...
    let module_storage = state_view.as_creditchain_code_storage(&genesis_runtime_environment);
    ...
    initialize_creditchain_coin(&mut session, &module_storage, &mut traversal_context);
...
pub fn encode_genesis_transaction(
    creditchain_root_key: Ed25519PublicKey,
...
pub fn encode_genesis_change_set(
    ...
    if genesis_config.is_test {
        initialize_core_resources_and_creditchain_coin(
            &mut session,
            &module_storage,
            &mut traversal_context,
            core_resources_key,
...
fn initialize(
    ...
    exec_function(
        ...
        serialize_values(&vec![
            ...
            MoveValue::U64(CREDITCHAIN_MAX_KNOWN_VERSION.major),
...
fn initialize_creditchain_coin(
    session: &mut SessionExt<impl CreditChainMoveResolver>,
    module_storage: &impl CreditChainModuleStorage,
    traversal_context: &mut TraversalContext,
) {
...
fn initialize_core_resources_and_creditchain_coin(
    session: &mut SessionExt<impl CreditChainMoveResolver>,
    module_storage: &impl CreditChainModuleStorage,
    traversal_context: &mut TraversalContext,
    core_resources_key: &Ed25519PublicKey,
...
fn allow_core_resources_to_set_version(
    ...
    exec_function(
        ...
        "initialize_for_test",
        vec![],
        serialize_values(&vec![MoveValue::Signer(creditchain_test_root_address())]),
    );
}
...
fn code_to_writes_for_publishing(
    ...
    let module_storage = genesis_state_view.as_creditchain_code_storage(genesis_runtime_environment);
...
pub fn generate_genesis_change_set_for_testing_with_count(
    ...
    let framework = match genesis_options {
        GenesisOptions::Head => creditchain_cached_packages::head_release_bundle(),
        GenesisOptions::Testnet => creditchain_framework::testnet_release_bundle(),
        GenesisOptions::Mainnet => {
            // We don't yet have mainnet, so returning testnet here
            creditchain_framework::testnet_release_bundle()
        },
    };
...
pub fn generate_genesis_change_set_for_mainnet(genesis_options: GenesisOptions) -> ChangeSet {
    let framework = match genesis_options {
        ...
        GenesisOptions::Testnet => creditchain_framework::testnet_release_bundle(),
        // We don't yet have mainnet, so returning testnet here
        GenesisOptions::Mainnet => creditchain_framework::testnet_release_bundle(),
    };
...
pub fn test_genesis_change_set_and_validators(
    ...
    generate_test_genesis(creditchain_cached_packages::head_release_bundle(), count)
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Validator {
    /// The CreditChain account address of the validator or the admin in the case of a commissioned or
    /// vesting managed validator.
    pub owner_address: AccountAddress,
    /// The CreditChain account address of the validator's operator (same as `address` if the validator is
    /// its own operator).
...
pub fn generate_test_genesis(
    ...
    let genesis = encode_genesis_change_set(
        ...
        &GenesisConfiguration {
            ...
            // 1M CREDITCHAIN coins (with 8 decimals).
...
fn mainnet_genesis_config() -> GenesisConfiguration {
    ...
    GenesisConfiguration {
        ...
        is_test: false,
        min_stake: 1_000_000 * CREDITCHAIN_COINS_BASE_WITH_DECIMALS, // 1M LBT
        // 400M LBT
        min_voting_threshold: (400_000_000 * CREDITCHAIN_COINS_BASE_WITH_DECIMALS as u128),
        max_stake: 50_000_000 * CREDITCHAIN_COINS_BASE_WITH_DECIMALS, // 50M LBT.
        recurring_lockup_duration_secs: 30 * 24 * 3600,         // 1 month
        required_proposer_stake: 1_000_000 * CREDITCHAIN_COINS_BASE_WITH_DECIMALS, // 1M LBT
        rewards_apy_percentage: 10,
...
pub fn test_mainnet_end_to_end() {
    ...
    let balance = 10_000_000 * CREDITCHAIN_COINS_BASE_WITH_DECIMALS;
    let non_validator_balance = 10 * CREDITCHAIN_COINS_BASE_WITH_DECIMALS;
    ...
    let transaction = encode_creditchain_mainnet_genesis_transaction(
        &accounts,
        &employees,
        &validators,
        creditchain_cached_packages::head_release_bundle(),
