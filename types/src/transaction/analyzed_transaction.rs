...
pub fn creditchain_coin_info_location() -> StorageLocation {
    StorageLocation::Specific(
        StateKey::resource_typed::<CoinInfoResource<CreditChainCoinType>>(
...
pub fn rw_set_for_coin_transfer(
    ...
    let read_hints = vec![
        current_ts_location(),
        features_location(),
        creditchain_coin_info_location(),
        chain_id_location(),
         ...
         -> (Vec<StorageLocation>, Vec<StorageLocation>) {
            match (
                *func.module().address(),
                ...
                (AccountAddress::ONE, "creditchain_account", "transfer") => {
                    let receiver_address = bcs::from_bytes(&func.args()[0]).unwrap();
